    ...
    <Layout title={t.title} description={t.subtitle}>
      <main className={`container ${styles.page}`}>
        <header className={styles.hero}>
          <div>
            <span className={styles.eyebrow}>Community Stories</span>
            <h1>{t.title}</h1>
            <p>{t.subtitle}</p>
            <Link className="button button--primary" to={`/submit-experience${filters.program || filters.school ? `?scope=${encodeURIComponent(filters.program || filters.school)}` : ''}`}>{experienceText.title}</Link>
          </div>
        ...
        <nav className={styles.schools} aria-label={t.chooseSchool}>
          <Link to={location.pathname + schoolLink('')} className={`${styles.school} ${!filters.school ? styles.active : ''}`} aria-current={!filters.school ? 'true' : undefined}>
            <span>{t.allSchools}</span><strong>{catalog.entries.length}</strong>
          </Link>
          {catalog.schools.map((school) => (
            <Link key={school.id} to={location.pathname + schoolLink(school.id)} className={`${styles.school} ${filters.school === school.id ? styles.active : ''}`} aria-current={filters.school === school.id ? 'true' : undefined}>
              <span>{school.name}</span><strong>{count({school: school.id})}</strong>
            </Link>
          ))}
        </nav>

        <div className={`${styles.workspace} ${selectedSchool ? styles.withSidebar : ''}`}>
          {selectedSchool && (
            <aside className={styles.sidebar}>
              <nav aria-label={t.chooseProgram}>
                <h2>{selectedSchool.name}</h2>
                <Link to={location.pathname + programLink('')} className={`${styles.program} ${!filters.program ? styles.active : ''}`} aria-current={!filters.program ? 'true' : undefined}>
                  <span>{t.allPrograms}</span><small>{count({school: filters.school})}</small>
                </Link>
                {departments.map((department) => (
                  <details key={`${filters.school}/${department}`} className={styles.department} open>
                    <summary>{department || t.unspecifiedDepartment}</summary>
                    {schoolPrograms.filter((program) => program.department === department).map((program) => (
                      <Link key={program.id} to={location.pathname + programLink(program.id)} className={`${styles.program} ${filters.program === program.id ? styles.active : ''}`} aria-current={filters.program === program.id ? 'true' : undefined}>
                        <span>{scopeName(program)}</span><small>{count({program: program.id})}</small>
                      </Link>
                    ))}
                  </details>
                ))}
          ...
          <section className={styles.results} aria-labelledby="experience-results-heading">
            ...
            <div className={styles.filters}>
              ...
              <label><span>{t.yearLabel}</span><select value={filters.year} onChange={(event) => update({year: event.target.value})}>
                <option value="">{t.allYears}</option>
                {filters.year && !years.includes(Number(filters.year)) && <option value={filters.year}>{filters.year}</option>}
                {years.map((year) => <option key={year} value={year}>{year}</option>)}
              </select></label>
              <label><span>{t.source}</span><select value={filters.kind} onChange={(event) => update({kind: event.target.value})}>
            ...
            <ul className={styles.list} id="experience-results">
              {visible.map(({entry, placements}) => (
                <li key={entry.url} className={styles.card}>
                  ...
                  <h3>
                    {entry.kind === 'external'
                      ? <a href={entry.url}>{entry.title}<FiExternalLink aria-label={t.external} /></a>
