    ...
    legalSections: [
      ...
      },
      {
        ...
        items: [
          ...
          'ログイン、マイページ、クラウド同期、学習ランキング、投稿、開発者 API を利用する場合、メールアドレス、認証識別子、学習進捗、問題ノート、投稿履歴、API 利用申請、API Key のメタデータ、必要なアクセスログを処理する場合があります。',
          '学習ランキングには、メールアドレスではなく、アカウントの表示名または利用者が設定した表示名、期間内に学習した問題数、順位が公開されます。ランキング設定から、いつでも匿名の「Kai友」表示へ切り替えられます。',
    ...
    legalSections: [
      ...
      },
      {
        ...
        items: [
          ...
          'If you use login, the Personal Center, cloud sync, practice leaderboards, submissions, or the Developer API, we may process your account email, authentication identifiers, study progress, problem notes, submission history, API access applications, API key metadata, and necessary request logs.',
          'The practice leaderboard displays your account name or chosen leaderboard name, the number of problems practiced during the period, and your rank. It does not display your email address. You can switch to an anonymous “Kai Friend” name at any time in leaderboard settings.',
