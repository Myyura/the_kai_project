...
test('cross-shard navigation performs one cache-busted document replacement', async () => {
  ...
  const runtime = createWindow(
    'https://runjp.com/docs/institute-of-science-tokyo/exam?lang=zh#answer',
  );
  ...
  assert.equal(replacement.pathname, '/docs/institute-of-science-tokyo/exam');
...
test('a returned wrong shard stops after the URL-marked attempt', async () => {
  const {requestSchoolShardNavigation} = await import(moduleUrl);
  const runtime = createWindow(
    'https://runjp.com/docs/institute-of-science-tokyo/exam?__kai_shard_reload=1722510000000',
  );
...
test('session state still prevents a loop when a redirect strips the marker', async () => {
  const {requestSchoolShardNavigation} = await import(moduleUrl);
  const runtime = createWindow(
    'https://runjp.com/docs/institute-of-science-tokyo/exam',
    {target: '/docs/institute-of-science-tokyo/exam', at: 1722510000000},
  );
...
test('the session cooldown expires and does not block a different target', async () => {
  ...
  const expired = createWindow(
    'https://runjp.com/docs/institute-of-science-tokyo/exam',
    {
      target: '/docs/institute-of-science-tokyo/exam',
  ...
  const differentTarget = createWindow(
    ...
    {target: '/docs/institute-of-science-tokyo/exam', at: 1722509999000},
  );
...
test('the URL marker guards retries even when browser storage is blocked', async () => {
  const {requestSchoolShardNavigation} = await import(moduleUrl);
  const runtime = createWindow(
    'https://runjp.com/docs/institute-of-science-tokyo/exam?__kai_shard_reload=1',
    null,
    true,
  );
...
test('a successfully rendered destination clears the shard retry state', async () => {
  ...
  const runtime = createWindow(
    'https://runjp.com/docs/institute-of-science-tokyo/exam?lang=zh&__kai_shard_reload=1#answer',
    {target: '/docs/institute-of-science-tokyo/exam?lang=zh', at: 1722510000000},
  );
