...
- `yarn test`: run the automated tests under the repository-root `tests/` directory locally.
...
**External articles**: group records by source and admission year under [external/](src/data/experiences/external/), with a JSON array in each file. For example, 2026 admission stories use `zhuanlan.zhihu.com/2026.json` for Zhihu and `note.com/2026.json` for note. Use the original URL's full hostname (including `www.`). Append the following record to the array in `example.com/2026.json`:
...
Store articles covering multiple schools or years once, under the latest admission year resolved from all `placements`; every school and exam attempt remains searchable. Storage and the directory share the same year rules: explicit `admissionYear` first, then summer exam year plus one or winter exam year unchanged, then `publishedYear` as a fallback. If every year is unknown, use the source's `unknown.json`. Move the record when a year or hostname correction changes its group. Each source/admission-year pair shares a file; do not create a file per article. The build merges these files into one frontend directory, and the submission converter uses the same reader and grouping rules.

Run `yarn test` and `yarn docusaurus build`. The build rejects misplaced records, duplicate URLs across files, unknown programs, and public articles without classifications.
