...
**外部記事**：[external/](src/data/experiences/external/) 内で掲載元・入学年度別に分け、各ファイルに JSON 配列を保存します。例えば2026年度入学の知乎記事は `zhuanlan.zhihu.com/2026.json`、note 記事は `note.com/2026.json` に置きます。掲載元は原文 URL の完全なホスト名（`www.` を含む）を使います。次のレコードは `example.com/2026.json` の配列に追加してください。
...
複数大学・年度を扱う記事は、全 `placements` から求めた最新の入学年度に一度だけ保存し、すべての大学・受験歴を検索対象に残します。保存先と画面表示は年度の計算を共有します。明示された `admissionYear` を優先し、夏季入試は試験の翌年、冬季入試は当年、不明な場合は `publishedYear` を使います。すべて不明な場合は掲載元の `unknown.json` に置いてください。年度やホスト名の訂正でグループが変わる場合はレコードも移動します。掲載元と入学年度の組み合わせごとにファイルを共有し、記事ごとのファイルは作りません。ビルド時に一つのフロントエンド用一覧に統合し、投稿変換にも同じ読み込み・振り分け処理を使います。

`yarn test` と `yarn docusaurus build` で確認してください。配置先の誤り、ファイルをまたぐ URL の重複、不明な専攻、分類のない公開記事はビルド時にエラーになります。
