...
で定める。$t$ の単射性と全射性を判定する。以下は [公式原卷の PDF 8–9 ページ](https://www.uec.ac.jp/education/graduate/admission/pdf/kako_j_choice202108.pdf#page=8) にある穴埋めの手順を独立に要約したものであり、原文の逐語転載ではない。

1. 狭義単調増加な写像 $f$ について、$x_1\,[22]\,x_2$ なら $f(x_1)<f(x_2)$、$x_1\,[23]\,x_2$ なら $f(x_1)>f(x_2)$ となる。従って $x_1\,[24]\,x_2$ なら $f(x_1)\,[24]\,f(x_2)$ であり、$f$ は $[21]$ と結論できる。
2. $f,g$ がともに狭義単調増加なら、$x_1<x_2$ から $g(x_1)<g(x_2)$、さらに $[25]$ が従う。合成 $f\circ g$ も $[21]$ である。
3. $h(u)=(u+u^{-1})/2$ とおき、$t(x)=h([26])$ と分解する。$[26]$ の $[0,\infty)$ における像は $[[27],\infty)$ であり、この区間で $h$ も狭義単調増加である。
4. $t(x)=[28]$ を満たす $x\ge0$ が $[29]$ ので、$t$ は $[30]$ である。

記号・数値の候補は $=,>,<,\ne,\in,\notin,-1,0,1,\infty$ である。文章の候補は、単射である／ない、全射である／ない、全単射である、単射でも全射でもない、存在する／存在しない／複数存在する、である。数式の候補は次のとおりである。

$$
e^x,\quad x_1,\quad x_2,\quad e^x+e^{-x},\quad
f(x_1)<f(x_2),\quad f(x_2)<f(x_1),\quad
f(g(x_1))<f(g(x_2)),\quad f(g(x_2))<f(g(x_1)),\quad
g(f(x_1))<g(f(x_2)),\quad g(f(x_2))<g(f(x_1)).
$$
