...
**第 1 题**：某实验在温度 $t$ K 下的成功概率为 $p(t)$。以 $1$ 表示成功、$0$ 表示失败，令相互独立的重复试验结果为 $X_1,X_2,\ldots$。求前 $n$ 次结果之和 $S_n=\sum_{i=1}^nX_i$ 的期望；再令 $N_r$ 为累计获得 $r$ 次成功所需的总试验次数，求 $E[N_r]$。若

$$
p(t)=\frac{\exp(\alpha+\beta t)}{1+\exp(\alpha+\beta t)},\qquad\beta\ne0
$$

且 $\alpha,\beta$ 为已知常数，求满足 $E[N_r]=n$ 的温度 $t$。
...
求该分布的样本空间，以及随机变量 $X$ 的期望 $E[X]$ 和方差 $V[X]$。

**第 2 题 B**：随机变量序列 $X_i$（$i=0,1,2,\ldots$）满足

$$
X_{i+1}\mid X_i\sim N(\mu+X_i,\sigma^2).
$$

在给定 $X_1$ 时求 $E[X_3\mid X_1]$；求正态分布的矩母函数；令 $Y_i=e^{X_i}$，在给定 $Y_i$ 时求 $E[Y_{i+1}\mid Y_i]$；最后在 $Y_0=a$ 为常数时求 $E[Y_n]$。

**第 3 题**：企业生产三种产品 $P_i$（$i=1,2,3$），生产 1 kg 所需材料 $w_i$、设备时间 $h_i$、单位利润 $r_i$ 与销量上限 $U_i$ 如上表所示；可用材料总量为 $W=520$ kg，设备总工时上限为 $H=200$ hour。以 $x_i$ 表示产品 $P_i$ 的产量，先将总利润最大化问题写成线性规划，再求各产品的最优产量与最大总利润。
...
温度を $t\ge0$ K に制限する場合は、得られた値が非負であることも必要である。
...
と表すことは、掲載されている一段先の条件付分布だけからは導けない。以下の (1) では、さらに $X_i$ がこの推移確率をもつマルコフ過程であると仮定する。
...
一段先の条件だけでは不十分な例として、$\mu=0$, $\sigma^2=1$ とし、独立な標準正規変数 $Z_1,Z_2,Z_3$ から $X_0=0$, $X_1=Z_1$, $X_2=Z_1+Z_2$, $X_3=\frac32Z_1+\frac12Z_2+Z_3/\sqrt2$ を作る。このとき $X_3\mid X_2\sim N(X_2,1)$ であるが、$E[X_3\mid X_1]=\frac32X_1$ となる。以後は独立な標準正規増分を加えれば、全時刻で題干の一段先の条件を満たす。
...
この小問と (4) は、一段先の条件付分布だけで求められる。$Y_i=e^{X_i}$ であり、$Y_i$ は $X_i$ と一対一に対応する。正規分布のモーメント母関数に $s=1$ を代入して、

$$
\begin{aligned}
E[Y_{i+1}\mid Y_i]
&=E[e^{X_{i+1}}\mid X_i]\\
&=\exp(X_i+\mu+\sigma^2/2)\\
...
条件付期待値の全期待値を取ると $E[Y_{i+1}]=e^{\mu+\sigma^2/2}E[Y_i]$ となる。これを反復して、$Y_0=a>0$ より
