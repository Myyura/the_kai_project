...
1. 独立な $X_i\sim\operatorname{Poisson}(\lambda)$ の和
   ...
   $P(S_{20}>120)$ を求めよ。また、$S_n$ が初めて正の整数 $K$ を超える時刻 $N$ の確率関数を求めよ。
2. 密度をもつ正の確率変数 $X$ の平均を $\mu$ とする。$X>\mu+\delta$（$\delta>0$）なら $a>1$ 枚、そうでなければ $0$ 枚のメダルが返るゲームを考え、各回は独立とする。
   1. $P(X\ge\mu+\delta)$ の上限を求めよ。
   2. $\mu=\delta=2$ のとき、10 回で返るメダル枚数の期待値の上限を求めよ。
   3. さらに $V[X]=\sigma^2$ が既知の場合の確率の上限を求めよ。
   4. $\mu=\delta=2,\sigma=1$ のとき、10 回で返る枚数の期待値の上限を求めよ。
...
スラック変数を $x_4,x_5,x_6$ とし、初期実行基底解、0 回目・1 回目の表、2 回目の実行基底解を求め、最適になるまで計算を続けよ。
...
[原卷 PDF 5 ページ](https://www.uec.ac.jp/education/graduate/admission/pdf/kako_j_choice202208.pdf#page=5) 。$K$ は正の整数とする。$S_0=0$、$p_0(s)=\mathbf1_{\{s=0\}}$ と定めれば、$N=n$ は $S_{n-1}\le K<S_n$ と同値である。したがって $n\ge1$ に対し、
