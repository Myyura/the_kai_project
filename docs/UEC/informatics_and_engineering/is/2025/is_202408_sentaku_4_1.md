...
$\Sigma=\{a,b\}$ とする。`aa` または `bb` を連続部分文字列として含む語の言語を $L_1$、$a,b$ をともに一個以上含む語の言語を $L_2$ とする。各言語および $L_1\cap L_2$ を受理する DFA を構成せよ。
...
$r_A,r_B$ は「両記号を見たが同一記号の連続はなく、末尾がそれぞれ $a,b$」を表す。$r_F$ のみを受理状態として、次表で定めればよい。
