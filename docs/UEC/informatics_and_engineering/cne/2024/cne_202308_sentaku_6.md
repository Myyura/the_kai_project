...
5 ビットの 2 の補数表現 $b_4\cdots b_0$ を入力し、偶数なら $0$、奇数なら絶対値を符号なし 4 ビット $d_3\cdots d_0$ で出力する組合せ回路を XOR と AND のみで構成せよ。正の奇数の場合の $d_3,d_2,d_1$、負の奇数の場合の $b_4,b_0$、および $d_i=b_0F_i$（$F_i$ は $b_0$ を含まない）の $F_i$ を求めよ。

また、初期状態 $Q_1Q_2Q_3=111$ から各ビットが周期 7 の列

$$
Q_1:1001011,\qquad Q_2:1100101,\qquad Q_3:1110010
$$

を繰り返し、$000$ は自己遷移する同期式順序回路について、状態遷移図、次状態論理式、および D フリップフロップと XOR のみによる回路を示せ。
