...
二分木の再帰的な深さ優先走査について、中順走査の出力を（ア）左部分木の再帰呼出し前、（イ）左右の再帰呼出しの間、（ウ）右部分木の再帰呼出し後のどこで行うか答え、スタックを用いる非再帰版を書け。また、$10,15,3,2,7,4,12$ を順に挿入した二分探索木を描き、最悪時の計算量を説明せよ。
