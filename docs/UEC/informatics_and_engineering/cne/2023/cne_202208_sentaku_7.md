...
区間 $[0,\infty)$ で 2 回連続微分可能な関数 $f$ が $f'(x)>0$、$f''(x)>0$ を満たし、方程式 $f(x)=0$ はこの区間に唯一の解 $\alpha$ をもつとする。初期値 $x_0>\alpha$ から Newton 法を適用する。反復式を導き、反復列が $\alpha$ に単調収束すること、誤差が 2 次収束すること、および有効桁数の変化を示せ。
