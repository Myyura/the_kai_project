...
- **特征值与特征空间**：由含参数的 $3\times3$ 矩阵求全部特征值，并为最大特征值求出特征空间的基。
- **线性变换的核与像**：分析矩阵多项式 $(\lambda_2E-A)^2$ 所定义的变换，确定其核与像的维数。
- **矩阵对角化**：根据特征值的重数及特征向量情况判断参数 $a$ 在何种条件下使矩阵可对角化。
- **矩阵幂**：在可对角化条件下利用相似对角化计算一般的 $A^n$。
