---
sidebar_label: '2019年8月実施 数理科学'
tags:
  - Tokyo-University
  - Physics.Mechanics.Small-Angle-Pendulum
  - Physics.Mechanics.Two-Dimensional-Harmonic-Oscillator-Effective-Potential
  - Physics.Mechanics.Radial-Small-Oscillation-Frequency
---

# 東京大学 理学系研究科 化学専攻 2019年8月実施 数理科学

## **Author**
[Miyake](https://miyake.github.io/exams/index.html)

## **Description**

以下の問（1）～（4）に答えよ．以下において，質点の質量を $m$ とする．

### （1）

図１のように，天井の固定点 $P$ から糸で吊るされた質点を考える．鉛直線（重力の向き）と糸とのなす角度 $\theta$ が十分に小さい場合には，質点は２次元平面上を運動すると近似することができる．運動の平面は， $\theta=0$ における質点の位置（原点 $O$ とする）を通り，鉛直線に垂直である．以下の問に答えよ．

#### (a) 

$\sin \theta \approx \theta$ ，$\cos \theta \approx 1$ の近似が成り立つ範囲において，質点に働く合力の大きさが，原点 $O$ から質点までの距離 $r$ に比例することを示せ．また，その比例係数 $k$ を求めよ．ただし，重力加速度を $g$ ，糸の長さを $l$ とする．

<figure style="text-align:center;">
  <img src="https://raw.githubusercontent.com/Myyura/the_kai_project_assets/main/kakomonn/tokyo_university/science/astron_2023_astro_p1.png" width="300" alt=""/>
</figure>


図１. 糸に吊るされた質点の図．角度 $\theta$ が十分に小さい場合，平面上の運動として近似できる．

前問（１）のように，原点からの距離に比例した復元力（バネ定数$k$ ）が，質点の位置から原点へ向かう方向に働く中心力となる場合の運動は，２次元調和振動として記述される．

### （2）
質点の位置ベクトルを２次元直交座標で $r=(x,y)$ と表す．まず手始めに，運動が $x$ 軸上に限られる１次元調和振動（常に $y=0$）を考えよう．以下の問に答えよ．

#### (b) 
質点の運動方程式を $k$ を使って書け．

#### (c) 
その運動方程式の一般解を時間 $t$ の関数として求めよ．

#### (d) 
振動の周期 $T$ を求めよ．

### （3）
次に，$y$方向の運動も考えると，その運動は一般に楕円軌道を描くことが知られている．この楕円運動を，２次元極座標 $r=(r,\varphi)$ で考えよう．原点周りの角運動量 $L$ が保存することを利用すれば，動径の長さ（原点からの距離）$r$ のみの関数として，有効ポテンシャル $V_{\mathrm{eff}}(r)$ を以下の式 (1) のように書くことができる．

$$
V_{\mathrm{eff}}(r)=\frac{kr^{2}}{2}+\frac{L^{2}}{2mr^{2}} \tag{1}
$$

以下の問に答えよ．

#### (e)
式 (1) の右辺第１項 $\frac{kr^2}2$ は復元力のポテンシャルを表している．それでは右辺第２項 $\frac{L^2}{2mr^2}$ は何と呼ばれる力に起因する項か．力の名称を答えよ．

#### (f)
有効ポテンシャル $V_{\mathrm{eff}}(r)$ が極小となるときの距離 $r$（この値を $r_0$ とする），および $r=r_0$ での極小値  $V_{\mathrm{eff}}(r_0)$ を求めよ．

#### (g)
距離 $r$ が $r_0$ に近い値のとき，具体的には $|r- r_0| \ll r_0$ のとき，有効ポテンシャルはその極小値を基準として以下の式 (2) で表せる

$$
V_{\mathrm{eff}}\:(r)\:=\:V_{\mathrm{eff}}\:(r_{0})\:+\:a\:(r\:-\:r_{0})^{2}\:+O\:\left((r\:-\:r_{0})^{3}\right)\tag{2}
$$

ただし，$O( ( r - r_{0}) ^{3})$  は $(r - r_{0})$ に関する３次以上の高次項を表す．式 (2) の２次の展開係数 $a$ を Taylor 展開などの方法により求めよ．

#### (h)
距離 $r$ が $r_0$ の近傍で微小振動するとき，式 (2) の高次項を無視して考えることができる．距離 $r$ が伸び縮みする振動の周波数 $\nu_r(={\frac{\omega_{r}}{2\pi}})$ を $a$ を用いて表せ．

### （4）
この楕円運動を２次元直交座標で考えると，その $x$ 方向の振動の周波数 $\nu_{x}$は，問（２）で考えた１次元調和振動の場合と同じである．一方，前問 (h) では，極座標における振動の周波数 $\nu_{r}$ を求めた．以下の問に答えよ．

#### (i) 
両者の周波数を比較すると， $\nu_r$ は $\nu_{x}$ の何倍になっているか．

#### (j) 
周波数比 $\nu_r/\nu_x$ が $1$ とはならない理由を説明せよ．

### 题目描述

以下各问中质点质量均为 $m$。

（1）如图，质点由长度为 $l$ 的细线悬挂在天花板固定点 $P$。当细线与竖直方向夹角
$\theta$ 足够小时，可把质点运动近似为在通过最低点 $O$、垂直于竖直方向的二维平面内运动。

- （a）在 $\sin\theta\approx\theta$、$\cos\theta\approx1$ 的范围内，证明质点所受合力大小与其到
  $O$ 的距离 $r$ 成正比，并求比例系数（等效弹簧常数）$k$；重力加速度为 $g$。

此后考虑指向原点、大小为 $kr$ 的中心回复力，即二维简谐振动。

（2）用直角坐标
$\boldsymbol r=(x,y)$ 表示位置，先限制在 $x$ 轴上，即恒有 $y=0$。

- （b）用 $k$ 写出运动方程。
- （c）求关于时间 $t$ 的通解。
- （d）求振动周期 $T$。

（3）加入 $y$ 方向运动后，轨道一般为椭圆。用极坐标
$(r,\varphi)$ 描述，并利用关于原点的角动量 $L$ 守恒。径向有效势能为
$$
V_{\mathrm{eff}}(r)
=\frac{kr^2}{2}+\frac{L^2}{2mr^2}.
$$

- （e）第二项
  $L^2/(2mr^2)$ 源于哪一种力？写出名称。
- （f）求使 $V_{\mathrm{eff}}$ 最小的半径 $r_0$ 及最小值
  $V_{\mathrm{eff}}(r_0)$。
- （g）在 $|r-r_0|\ll r_0$ 时，
  $$
  V_{\mathrm{eff}}(r)
  =V_{\mathrm{eff}}(r_0)+a(r-r_0)^2
  +O((r-r_0)^3).
  $$
  用 Taylor 展开等方法求二次系数 $a$。
- （h）忽略高阶项，把 $r$ 在 $r_0$ 附近的运动视为微振动；用 $a$ 表示径向振动频率
  $\nu_r=\omega_r/(2\pi)$。

（4）在直角坐标中，椭圆运动的 $x$ 向频率 $\nu_x$ 与第（2）问的一维简谐振动相同。

- （i）求 $\nu_r$ 是 $\nu_x$ 的多少倍。
- （j）解释频率比 $\nu_r/\nu_x$ 不等于 $1$ 的原因。

## **Kai**
### (1)
#### (a)
小角近似では鉛直方向の加速度を高次として無視でき、張力を $F_T$ とすれば $F_T\cos\theta\simeq mg$ である。水平方向の合力の大きさは

$$
\begin{aligned}
F_T\sin\theta\simeq mg \tan \theta \approx mg \cdot \frac{r}{l}
\end{aligned}
$$

なので $r$ に比例し、比例係数は

$$
\begin{aligned}
k = \frac{mg}{l}
\end{aligned}
$$

である。

### (2)
#### (b)

$$
\begin{aligned}
m \frac{d^2x}{dt^2} = -kx
\end{aligned}
$$

#### (c)

$$
\begin{aligned}
x = A \sin \left( \sqrt{\frac{k}{m}} t \right)
+ B \cos \left( \sqrt{\frac{k}{m}} t \right)
\ \ \ \ \ \ \ \ (A,B \text{ は積分定数})
\end{aligned}
$$

#### (d)

$$
\begin{aligned}
T = 2 \pi \sqrt{\frac{m}{k}}
\end{aligned}
$$

### (3)
#### (e)
遠心力

#### (f)

$$
\begin{aligned}
\frac{dV_{\mathrm{eff}}(r)}{dr}
&= kr - \frac{L^2}{mr^3}
\\
&= \frac{mkr^4 - L^2}{mr^3}
\end{aligned}
$$

非退化な楕円運動として $L\ne0$ を考えると、$r_0>0$ なので

$$
\begin{aligned}
r_0 &= \left( \frac{L^2}{mk} \right)^\frac{1}{4}
,\\
V_\mathrm{eff}(r_0)
&= \frac{k}{2} \frac{|L|}{\sqrt{mk}} + \frac{L^2}{2m} \frac{\sqrt{mk}}{|L|}
\\
&= |L| \sqrt{\frac{k}{m}}
\end{aligned}
$$

である。

#### (g)

$$
\begin{aligned}
\frac{d^2V_{\mathrm{eff}}(r)}{dr^2}
&= k + \frac{3L^2}{mr^4}
\\
\therefore \ \ 
\frac{d^2V_{\mathrm{eff}}(r_0)}{dr^2}
&= k + \frac{3L^2}{mr_0^4}
\\
&= k + \frac{3L^2}{m} \frac{mk}{L^2}
\\
&= 4k
\\
\therefore \ \ 
a
&= \frac{1}{2!} \frac{d^2V_{\mathrm{eff}}(r_0)}{dr^2}
\\
&= 2k
\end{aligned}
$$

#### (h)
(2) と同様に考えると、次がわかる：

$$
\begin{aligned}
\nu_r
&= \frac{1}{2\pi} \sqrt{\frac{4k}{m}}
\\
&= \frac{1}{2\pi} \sqrt{\frac{2a}{m}}
\end{aligned}
$$

### (4)
#### (i)
2倍

#### (j)
$\omega=\sqrt{k/m}$ とし、楕円の主軸に沿って座標を選べば、時刻原点の選択により

$$
x=A\cos\omega t,\qquad y=B\sin\omega t
$$

と書ける。したがって

$$
r^2=\frac{A^2+B^2}{2}+\frac{A^2-B^2}{2}\cos2\omega t.
$$

非円形の楕円では、質点が軌道を1周する間に半径は2回伸び縮みするため、$\nu_r=2\nu_x$ となる。円軌道では半径そのものは一定であり、$\nu_r$ はそこからの微小な径方向の摂動の周波数を表す。

$L=0$ の場合は原点を通る直線運動に退化し、$r_0=0$ となるため、(g)、(h)の $|r-r_0|\ll r_0$ という近似は適用しない。

## **Reference**

- [東京大学 化学専攻 令和2年度 数理科学（公式PDF）](https://www.chem.s.u-tokyo.ac.jp/chem_graduate/pdf/kakomon/R02/R2%E6%95%B0%E7%90%86%E7%A7%91%E5%AD%A6.pdf)
