...
3. $P\in\mathbb R^{n\times n}$、$M=PP^T$、$N=P^TP$ とする。
   ...
   3. 同じ固有値 $\lambda$ に属する $M,N$ の単位固有ベクトル $u,v$ の関係を示せ。
   4. $P=U\operatorname{diag}(\sqrt{\lambda_1},\ldots,\sqrt{\lambda_n})V^T$ を示せ。
4. $512\times512$ 画像行列を上の分解で階数 $r$ に近似する。各実数を4バイトとするとき、$r=100,10$ のファイルサイズと元画像に対する割合を求めよ。
...
は零行列でないが、$M=N=\operatorname{diag}(1,0)$ である。$P$ が正則なら全固有値は正となる。
...
$\lambda_i>0$ では問(c)の対応を用い、$\lambda_i=0$ では $\ker P,\ker P^T$ の正規直交基底を選ぶ。このとき $Pv_i=\sqrt{\lambda_i}u_i$ だから
