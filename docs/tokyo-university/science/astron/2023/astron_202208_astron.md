...
$x\in\{0,1,\ldots\}$ 服从 Poisson 分布

$$
P(x)=\frac{\lambda^x}{x!}e^{-\lambda},
$$

其中 $\lambda=\mathbb E[x]$，且 $0!=1$。以下设平均每秒探测到
$n$ 个光子。

- （a）求 $t'$ 秒内一个光子也未探测到的概率 $p(t')$。
- （b）$p(t')$ 也可解释为相邻两次光子事件之间的等待时间超过
  $t'$ 的概率。若等待时间随机变量为 $t$、密度为 $g(t)$，满足

  $$
  p(t')=\int_{t'}^\infty g(t)\,\mathrm dt,
  $$
...
$50$ 年一次，最近一次为 1987 年的 SN1987A。设下一次发生在公元
$Y$ 年，求使 1987 年至 $Y$ 年间至少发生一次超新星的概率为
$0.5$ 的整数年份 $Y$。可用 $\ln2\approx0.693$。

### 问题 3

用积累光电效应电子的探测器观测稳定光源。曝光 $t$ 秒后积累的电子数为
$X_s$，其 Poisson 均值足够大，因此近似服从
$N(\lambda,\lambda)$。读出总电子数时产生测量误差
$X_r\sim N(0,\sigma_r^2)$，测量值为
$X_m=X_s+X_r$。可使用：独立正态变量之和的均值、方差分别相加；若
$X\sim N(0,1)$，则 $aX+b\sim N(b,a^2)$；以及中心极限定理。

假设平均每秒有 $m$ 个光子入射，且每个光子恰转换为一个电子。

- （a）用 $X_m,t,m,\sigma_r$ 写出 $X_m$ 的概率密度
  $h(X_m)$。
- （b）定义信噪比 S/N 为测量值相对于其标准差的倍数。取
  $m=40$、$\sigma_r=20$，求使 S/N 的期望为 $30$ 的曝光时间。

### 问题 4

考虑用计算机生成伪观测数据。准备 $k\gg1$ 组随机数，每组含
$j\gg1$ 个在 $(0,1)$ 上均匀分布的独立随机数
$q_1,\ldots,q_j$。已知该均匀分布均值为 $1/2$、方差为
$1/12$。说明如何结合这些随机数与中心极限定理，生成服从问题
3（a）中密度 $h(X_m)$ 的 $k$ 个数值
$w_1,\ldots,w_k$。

## **Kai**
### 問 1.
$n>0$、待ち時間 $t,t'\ge0$ とする。密度は $t<0$ では $g(t)=0$ である。
...
となる $t \ (\gt 0)$ を求めると $t=30$ を得る。求める露光時間は $30$ 秒である。

### 問 4.
各セット内およびセット間で乱数を独立に生成する。
...
とおくと、与えられた性質 (中心極限定理) より、$j$ が十分大きいとき、これは標準正規分布 $\mathrm{N}(0,1)$ に近似的に従う。
...
これは正規分布 $\mathrm{N}(mt, mt+\sigma_r^2)$ に近似的に従うことがわかる。
このようにして $w_1, w_2, \cdots, w_k$ を生成すればよい。

## **Reference**

- [東京大学 天文学専攻 令和5年度 専門科目（公式PDF）](https://www.astron.s.u-tokyo.ac.jp/tenmon/wp-content/uploads/2023/01/R5%E4%BF%AE%E5%A3%AB%E3%83%BB%E5%8D%9A%E5%A3%AB%E8%AA%B2%E7%A8%8B%E5%85%A5%E5%AD%A6%E8%A9%A6%E9%A8%93%E5%95%8F%E9%A1%8C_%E5%85%AC%E9%96%8B.pdf)
