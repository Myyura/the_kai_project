...
- **极坐标下的 Laplace 算子**：用多元链式法则逐项转换一、二阶偏导，合并为 $g_{rr}+r^{-1}g_r+r^{-2}g_{\theta\theta}$。
- **偏微分方程分离变量**：把角向因子 $\sin\theta$ 代入二维 Helmholtz 方程，得到一阶 Bessel 方程。
- **幂级数解法**：对径向级数逐项求导并按同次幂比较系数，求初始系数与隔项递推。
