...
- **多元函数微分**：对指数复合函数使用乘积法则、链式法则计算一阶与二阶偏导。
- **二元 Taylor 展开**：在指定点计算函数值、梯度与 Hessian，并保留到二次齐次项。
- **Hessian 极值判别**：联立梯度为零并检查二阶型的负定性，证明给定驻点为局部极大。
- **约束优化**：在第一象限单位圆弧上使用参数化或 Lagrange 乘子，并比较端点以确定全局极值。
