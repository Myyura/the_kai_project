...
- **离散分布碰撞概率**：把双方取值相同的胜率写成 $\sum_i p_iq_i$，并对均匀与截断几何型分布求和。
- **概率单纯形上的凸优化**：在 $p=q$ 时最小化 $\sum_i p_i^2$，再加入均值线性约束分析最小碰撞概率。
- **一次博弈的最佳响应**：对固定机器分布，胜率是玩家混合策略的线性函数，因此把全部概率放在最大 $q_i$ 对应的纯策略上最优。
