...
玩家与机器只进行一局游戏。玩家按概率分布

$$
P(\text{玩家出 }i)=p_i,\qquad
\sum_{i=1}^{n}p_i=1,
$$
...
$$
P(\text{机器出 }j)=q_j,\qquad
\sum_{j=1}^{n}q_j=1,
$$

各自从 $1,\ldots,n$ 中取一个自然数；两数相同则玩家获胜。回答：

1. 若 $p_i=1/n$（$i=1,\ldots,n$），求玩家胜率。
2. 若

   $$
   p_i=p_1\alpha^{i-1},\qquad
   q_j=q_1\beta^{j-1},
   $$

   求胜率，并只用 $\alpha,\beta,n$ 表示结果；归一化常数须由概率和为 1 确定。
3. 若 $p_i=q_i$：
   1. 求玩家胜率的最小值；
   2. 再加上机器输出的期望为 $(n+1)/2$，求胜率的最小值。
4. 把 $(p_1,\ldots,p_n)$ 称为玩家策略，并考虑
   ...
   已知 $q_n$ 是 $q_1,\ldots,q_n$ 中的最大值。
   1. 证明策略 $E$ 优于 $R$；“优于”指采用前者的胜率不低于后者。
   2. 证明 $E$ 在所有玩家策略中最优。

## **Kai**
The player and the machine choose their numbers independently.
...
この表示で分母が 0 になる場合は極限値を取る。すなわち
$S_n(t)=\sum_{r=0}^{n-1}t^r$ とおけば、すべての場合をまとめて

$$
P(\text{Player wins})=\frac{S_n(\alpha\beta)}{S_n(\alpha)S_n(\beta)}
$$
...
&= (0 - \Delta_1)q_1 + (0 - \Delta_2)q_2 + \cdots + (0 - \Delta_{n-1})q_{n-1} + \sum_{1}^{n-1}\Delta_{i}q_n \\
