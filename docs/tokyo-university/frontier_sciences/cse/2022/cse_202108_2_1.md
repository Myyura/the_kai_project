...
- **矩阵幂极限**：对二阶矩阵作特征分解，将绝对值小于 1 的特征分量消去并保留稳态投影。
- **Rayleigh 商与 Courant–Fischer 原理**：在全部空间及排除前 $m$ 个特征方向的正交补上分别求最小谱值。
- **特征多项式系数**：把三阶矩阵的二阶主子式和识别为两两特征值乘积之和。
- **矩阵指数的行列式**：利用矩阵指数的特征值为 $e^{\gamma_i}$，证明 $\det(e^C)=e^{\operatorname{tr}C}$。
