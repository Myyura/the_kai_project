...
For the induction step, convolution gives

$$
f_{S_{n+1}}(x)=\int_0^x
\frac{\lambda^n y^{n-1}e^{-\lambda y}}{(n-1)!}
\lambda e^{-\lambda(x-y)}\,\mathrm{d}y
=\frac{\lambda^{n+1}x^ne^{-\lambda x}}{n!}.
$$

Together with the case $n=1$, this proves
...
Thus the MLE is biased for $n\ge2$; for $n=1$, its expectation diverges.
