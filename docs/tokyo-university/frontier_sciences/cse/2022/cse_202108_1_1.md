...
- **线性常微分方程与指数外力**：由特征方程求齐次解，再为 $e^{2x}$ 选择待定系数特解。
- **一阶方程变量代换**：以 $u=x+y$ 把原方程化为可分离或关于 $x(u)$ 的线性方程。
- **指数—三角函数积分**：通过分部积分联立消元得到闭式。
- **椭圆切线与截距优化**：由参数化接点写切线，表示两轴截距并最小化线段长度。
- **椭圆区域变量替换**：用缩放极坐标把椭圆化为单位圆，结合 Jacobian 计算二重积分。
