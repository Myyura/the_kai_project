...
- **均匀分布次序统计量**：由最大值小于阈值等价于两个独立样本同时小于阈值，计算尾概率。
- **条件密度边缘化**：在 $y_1\in[0,1]$ 上积分 Laplace 型条件密度，并按 $y_2$ 相对区间的位置拆分绝对值。
- **均匀尺度参数的无偏估计**：用样本和的期望、独立性求估计量均值与方差。
- **独立变量卷积**：当 $n=2$ 时对两个均匀变量之和作卷积，再按线性缩放得到三角形密度。
