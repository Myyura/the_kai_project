...
- **参数矩阵的特征值、秩与核**：随 $a$ 分解特征多项式，判断奇异参数并求零空间。
- **离散线性动力系统收敛**：依据各特征值模长及单位特征值的 Jordan 结构，判定 $A^n\boldsymbol x_0$ 对所有初值的极限。
- **线性相关与张成平面**：检查三个向量的行列式并求其张成子空间的法向关系。
- **三直线共点或平行判据**：把三个一次方程存在公共射影解等价为系数矩阵行列式为零，展开得到 $f_i$。
