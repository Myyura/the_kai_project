...
#### (2)
If the three lines have a common point and are not all parallel, relabel them if necessary so that lines 1 and 2 are not parallel. Let $(x_0,y_0)$ be their intersection. Then
...
For the converse and the parallel case, let
...
The displayed condition is exactly $\det M=0$, so $M(X,Y,Z)^{\mathrm T}=0$ for some nonzero $(X,Y,Z)$. If $Z\ne0$, all three lines pass through $(X/Z,Y/Z)$. If $Z=0$, the nonzero vector $(X,Y)$ is orthogonal to every $(a_i,b_i)$, so the three lines are parallel. The reverse implications follow by taking $(X,Y,Z)=(x_0,y_0,1)$ or a common direction vector with $Z=0$.
