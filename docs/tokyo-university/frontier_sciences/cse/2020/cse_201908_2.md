...
tags:
  ...
  - Mathematics.Vector-Calculus.Hyperplane-to-Hypersphere-Distance
  - Mathematics.Linear-Algebra.Distance-from-Point-to-Linear-Subspace
---
...
(問2) $A \boldsymbol{x}$ の集合 $Q = \{A \boldsymbol{x} \mid \boldsymbol{x} \in \mathbb{R}^4\}$ は線形空間である.
$Q$ の次元と基底ベクトルを求めよ.
...
(問4) $\boldsymbol{n} = (1, 0, 0, 0)^{\top}$, $\boldsymbol{c} = (1, 1, 1, 1)^{\top}$, $a=1$ とし, $\boldsymbol{x}$ と $L=\{A \boldsymbol{p} \mid \boldsymbol{p} \in P\} \subset Q$ との距離 $d(\boldsymbol{x})$ を, $\boldsymbol{x}$ と $\boldsymbol{l} \in L$ の間の距離の最小値として以下のように定義する.
...
与 $\boldsymbol x=(x_1,x_2,x_3,x_4)^T\in\mathbb R^4$。另设单位向量 $\boldsymbol n\in\mathbb R^4$（$\|\boldsymbol n\|=1$），定义过原点、法向为 $\boldsymbol n$ 的超平面与以 $\boldsymbol c$ 为中心、半径 $a>0$ 的超球面：

$$
P=\{\boldsymbol x\in\mathbb R^4\mid\boldsymbol n^T\boldsymbol x=0\},
$$

$$
S=\{\boldsymbol x\in\mathbb R^4\mid\|\boldsymbol x-\boldsymbol c\|=a\},
\qquad
\|\boldsymbol x\|=\sqrt{x_1^2+x_2^2+x_3^2+x_4^2}.
$$

回答：

1. 求 $A$ 的四个按 $\lambda_1<\lambda_2<\lambda_3<\lambda_4$ 排列的特征值及对应特征向量 $\boldsymbol e_1,\ldots,\boldsymbol e_4$。
2. 对像空间

   $$
   Q=\{A\boldsymbol x\mid\boldsymbol x\in\mathbb R^4\},
   $$
...
3. 把超平面与超球面的距离定义为
   ...
   $$
   g=\min_{\boldsymbol p\in P,\boldsymbol s\in S}
   \|\boldsymbol p-\boldsymbol s\|,
   $$

   用 $\boldsymbol n,\boldsymbol c,a$ 表示 $g$，并处理相交时距离为零的情形。
4. 取

   $$
   \boldsymbol n=(1,0,0,0)^T,\quad
   \boldsymbol c=(1,1,1,1)^T,\quad a=1,
   $$

   并令

   $$
   L=\{A\boldsymbol p\mid\boldsymbol p\in P\}\subset Q,\qquad
   d(\boldsymbol x)=\min_{\boldsymbol l\in L}\|\boldsymbol x-\boldsymbol l\|.
   $$

   1. 用 $x_1,x_2,x_3,x_4$ 表示 $d(\boldsymbol x)$；
   2. 对 $\boldsymbol s\in S$，求 $\max_{\boldsymbol s\in S}d(\boldsymbol s)$。
...
When $\lambda = 0$, (v) gives $x_1 = 0, x_2 = x_3 = x_4 = 1$, and hence $d=0$.

When $\lambda \neq 0$, solving (i) ~ (v) gives the maximum point
