...
- **特征值与特征向量**：求具体四阶矩阵的谱，并利用零特征值与非零特征向量理解线性变换。
- **像空间、秩与基**：从列空间或行化简确定 $\operatorname{Im}A$ 的维数及生成基。
- **超平面到超球面的距离**：先求球心到单位法向超平面的距离，再扣除半径并截断于零。
- **点到线性子空间的距离**：刻画 $L=A(P)$ 的线性约束或正交补，把距离写成正交投影残差并在球面上最大化。
