...
以下 $x,y,z,t,k$ 均为实数，回答五个相互独立的问题：

1. 对
   $$
   f(x,y)=x^3+y^2-xy,
   $$
   求 $\partial f/\partial x$、$\partial f/\partial y$，并求曲面 $z=f(x,y)$ 在
   $$
   (x,y,z)=(1,2,f(1,2))
   $$
   处的切平面方程。
2. 把
   ...
   在 $x=0$ 附近展开到一次项。若
   $$
   a=\lim_{x\to0}\frac{1-h(x)}{x^k}
   $$
   存在且 $0<|a|<\infty$，求 $k,a$。
3. $\cos^{-1}$ 是定义域 $[-1,1]$、值域 $[0,\pi]$ 上的反余弦函数。画出
   $$
   y=\cos^{-1}(x+\tfrac12)
   $$
   的曲线，并求该曲线与 $x$ 轴、$y$ 轴围成区域的面积。
4. 求微分方程
   $$
   g''(t)+g'(t)+\sin t=0
   $$
   的一般解；再求满足 $g(0)=2$、$g'(0)=0$ 的特解。
5. 令
   $$
   D=\{(x,y)\mid0\le2x-y\le1,\ 0\le x+3y\le2\}.
   $$
   使用变量变换
   $$
   u=2x-y,\qquad v=x+3y
   ...
   计算
   $$
   \iint_D\frac{(2x-y)^3}{4+(x+3y)^2}\,dx\,dy.
   $$

#### 考点

- **切平面与偏导数**：由二元函数梯度在指定点写出曲面的一阶线性近似。
- **Taylor 展开与极限阶数**：提取复合指数函数的首个非零增量项，确定使商趋于非零有限值的幂次。
- **定积分与反三角函数图像**：根据反余弦的定义域、值域确定截取区域并计算面积。
- **二阶线性常微分方程**：求齐次通解和三角函数特解，再由初值确定常数。
- **变量替换与 Jacobian**：把斜平行四边形区域化为矩形，计算变换行列式并分离二重积分。
