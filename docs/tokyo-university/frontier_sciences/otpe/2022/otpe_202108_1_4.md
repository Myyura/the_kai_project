...
- **分部积分**：结合 $\sin x\cos x$ 的倍角化简，对指数—三角函数定积分求值。
- **叉积面积与数量三重积体积**：由位置向量叉积求三角形面积，并以标量三重积求四面体体积。
- **相关变化率**：由直角三角形 $\tan\theta=h/200$ 对时间求导，在指定高度代入上升速度。
- **周期直线覆盖**：分析光源相对等间距接收机的最不利水平位置，用勾股关系求覆盖圆半径允许的最大线间距。
