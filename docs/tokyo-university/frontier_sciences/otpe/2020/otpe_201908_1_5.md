...
tags:
  - Tokyo-University
  - Mathematics.Calculus.Inverse-Hyperbolic-Sine-Integral
  - Mathematics.Calculus.Hyperbolic-Functions-from-Power-Series
  - Mathematics.Linear-Algebra.Matrix-Inverse
  - Mathematics.Linear-Algebra.Determinant-of-Inverse-Matrix
...
1. 计算定积分
   $$
   I=\int_0^{\sqrt3}\frac{dx}{\sqrt{x^2+1}}.
   $$
2. 对任意实数 $\theta$，定义
   $$
   x(\theta)=\sum_{m=0}^{\infty}\frac{\theta^{2m}}{(2m)!},
...
3. 对
   ...
   2. 验证
      ...
      $$
      其中竖线表示行列式。
4. 上图给出坐标系 $Oxy$ 中三角形 $ABC$ 变换为坐标系 $Ox'y'$ 中三角形 $A'B'C'$ 的对应关系。依据图中各顶点坐标，求任意点向量 $\boldsymbol x$ 到 $\boldsymbol x'$ 的仿射变换；现有文字未另行列出图内的坐标数据。
5. 对
   $$
   ...
   1. 求特征值与特征向量；
   2. 对自然数 $n$ 求 $A^n$；
   3. 求 $\lim_{n\to\infty}A^n$。

#### 考点

- **反双曲正弦型积分**：用双曲代换或对数原函数计算 $\int dx/\sqrt{x^2+1}$ 的定积分。
- **双曲函数的幂级数**：逐项求导识别 $x'(\theta)=y(\theta)$、$y'(\theta)=x(\theta)$，并推出不变量。
- **逆矩阵与行列式**：用消元或伴随矩阵求三阶逆矩阵，并应用逆矩阵行列式的倒数性质。
- **仿射变换**：由三对不共线对应点确定二维线性部分与平移向量。
- **矩阵幂与极限**：对二阶对称矩阵作特征分解，按两个特征值的幂求闭式及长期极限。
