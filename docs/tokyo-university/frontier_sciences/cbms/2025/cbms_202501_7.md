...
对单位三维实列向量 $\boldsymbol u\in\mathbb R^3$，

$$
\|\boldsymbol u\|=\sqrt{\boldsymbol u^T\boldsymbol u}=1,
$$

定义 Householder 矩阵

$$
R_u=I-2\boldsymbol u\boldsymbol u^T\in\mathbb R^{3\times3}.
$$

要求给出数学推导并回答：

1. 对任意 $\boldsymbol x\in\mathbb R^3$，令 $\boldsymbol y=R_u\boldsymbol x$，证明存在 $a\in\mathbb R$ 使
...
3. 给定 $\boldsymbol x=(x_1,x_2,x_3)^T$，求所有单位向量 $\boldsymbol u$，使某个 $b\in\mathbb R$ 满足

   $$
   R_u\boldsymbol x=(b,0,0)^T;
   $$

   需涵盖输入向量的退化情形。
4. 对同一 $\boldsymbol x$，求所有单位向量 $\boldsymbol u$，使某个 $c\in\mathbb R$ 满足

   $$
   R_u\boldsymbol x=(x_1,c,0)^T.
   $$
...
5. 给定
   ...
   求一组单位向量 $(\boldsymbol u,\boldsymbol v)$，使

   $$
   B=R_uR_vA
   =\begin{pmatrix}d&e&f\\0&g&h\\0&0&i\end{pmatrix}
   $$
...
The candidate directions are also sufficient: whenever $\|x\|=\|y\|$ and $x\ne y$,

$$
(x-y)^Tx=\frac{\|x-y\|^2}{2},\qquad
\left(I-2\frac{(x-y)(x-y)^T}{\|x-y\|^2}\right)x=y.
$$

The same identity applies in (4).
...
である。$\boldsymbol{x}-\boldsymbol{y}\ne\boldsymbol0$ の場合に限り、これにより $u_1=0$ である。
...
If $\boldsymbol{x}-\boldsymbol{y}\ne\boldsymbol0$, this implies $u_1=0$.
