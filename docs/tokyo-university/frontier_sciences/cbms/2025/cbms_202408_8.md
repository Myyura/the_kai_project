---
sidebar_label: 2024年8月実施 問題8
tags:
  - Tokyo-University
  - Mathematics.Linear-Algebra.Singular-Value-Decomposition
  - Mathematics.Linear-Algebra.Low-Rank-Approximation
...
## **Description**
以下で $\mathbb{R}^{n \times m}$ は $n$ 行 $m$ 列実数値行列の集合を表すものとする。
...
ここで、$\hat{U} \in \mathbb{R}^{n \times r}, \hat{V} \in \mathbb{R}^{n \times r}$ は $U, V$ の最初の $r$ 列からなる行列であり、$\hat{\Sigma} = \text{diag}(\sigma_1, \dots, \sigma_r) \in \mathbb{R}^{r \times r}$ である。$\hat{A}$ は以下の最適化問題の一つの解 $X$ を与えることが知られている。
...
记 $\mathbb R^{n\times m}$ 为 $n$ 行 $m$ 列实矩阵集合。对 $n\ge2$ 的可逆矩阵 $A\in\mathbb R^{n\times n}$，给定奇异值分解
...
\qquad \sigma_1\ge\cdots\ge\sigma_n>0.
$$

对 $1\le r<n$，取 $U,V$ 的前 $r$ 列组成 $\hat U,\hat V$，并令

$$
\hat\Sigma=\operatorname{diag}(\sigma_1,\ldots,\sigma_r),
\qquad
\hat A=\hat U\hat\Sigma\hat V^T.
$$

已知 $\hat A$ 是秩恰为 $r$ 的最佳 Frobenius 范数近似之一：

$$
\min_{\substack{X\in\mathbb R^{n\times n}\\\operatorname{rank}(X)=r}}
\|A-X\|^2,
\qquad
\|M\|^2=\operatorname{trace}(M^TM)=\sum_{i,j}M_{ij}^2.
$$

要求给出推导并回答：

1. 求 $\hat A^T\hat A$ 与 $\hat A\hat A^T$ 的奇异值分解。
2. 定义

   $$
   \hat A^+=\hat V\hat\Sigma^{-1}\hat U^T,
   $$
...
3. 求优化问题
   ...
   $$
   \min_{\substack{X\in\mathbb R^{n\times r}\\X^TX=I_r}}
   \|A-XX^TA\|^2
   $$
...
4. 求优化问题
   ...
   $$
   \max_{\substack{X\in\mathbb R^{n\times r}\\X^TX=I_r}}
   \operatorname{trace}(X^TAA^TX)
   $$

   的解。
5. 若 $B,C\in\mathbb R^{n\times n}$ 且 $A=BC$，求
...
よって $X=\hat UQ$（$Q^TQ=I_r$）は解である。$\sigma_r>\sigma_{r+1}$ ならこれが全解である。$\sigma_r=\sigma_{r+1}$ の場合は、$\sigma_r$ より大きい特異値の左特異ベクトルを全て含み、$\sigma_r$ の左特異空間から残りの次元を任意に選んだ $r$ 次元部分空間の正規直交基も解となる。
...
Thus $X=\hat UQ$, where $Q^TQ=I_r$, is a solution. If $\sigma_r>\sigma_{r+1}$, these are all solutions. If $\sigma_r=\sigma_{r+1}$, any orthonormal basis of an $r$-dimensional subspace containing all left singular spaces with singular value greater than $\sigma_r$ and choosing the remaining dimensions inside the $\sigma_r$ singular space is also optimal.

### (4)
$P=XX^T$ は直交射影なので、次の恒等式により (3) と同じ最適化になる。Since $P=XX^T$ is an orthogonal projector, the identity below reduces this problem to (3):

$$
\|A-XX^TA\|^2=\operatorname{tr}(A^TA)-\operatorname{tr}(X^TAA^TX).
$$
...
したがって $X=\hat UQ$（$Q$ は任意の直交行列）は解であり、最大値は $\sum_{i=1}^r\sigma_i^2$ である。$\sigma_r>\sigma_{r+1}$ ならこれが全解であり、境界で重複する場合の全解は (3) と同じ主部分空間の任意性を持つ。
...
Therefore $X=\hat UQ$, with arbitrary orthogonal $Q$, is optimal and the maximum is $\sum_{i=1}^r\sigma_i^2$. If $\sigma_r>\sigma_{r+1}$ these are all solutions; with a tie at the boundary, the full solution set has the same freedom inside the tied principal subspace as in (3).
...
エッカート・ヤング・ミルスキーの定理より、この最適化問題の一つの解は $Y = \hat{A}$ である。
...
By the Eckart-Young-Mirsky Theorem, one solution to this optimization problem is $Y = \hat{A}$.
