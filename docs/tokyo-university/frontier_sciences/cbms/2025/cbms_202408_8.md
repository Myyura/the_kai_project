...
- **奇异值分解**：由截断 SVD 计算两个 Gram 矩阵的谱分解，并跟踪左右奇异向量与平方奇异值。
- **低秩近似**：利用 Eckart–Young 型结论把最优秩 $r$ 逼近转化为保留最大的 $r$ 个奇异方向。
- **Moore–Penrose 伪逆**：使用截断伪逆对应的左右正交投影验证矩阵恒等式。
- **迹优化与主子空间**：把投影残差最小化改写为 $\operatorname{trace}(X^TAA^TX)$ 最大化，识别最优列空间。
