...
tags:
  - Tokyo-University
  - Computer-Science.Algorithm-Design.Horner-Method
...
- (A) The multiplication of $2^{n+1}$-bit integers can be constructed from the multiplication of $2^n$-bit integers as follows: $ab = r_2 2^{2^{n+1}} + r_1 2^{2^n} + r_0$, where $r_2 = a_{hi} b_{hi}, r_1 = a_{hi} b_{lo} + a_{lo} b_{hi}, r_0 = a_{lo} b_{lo}$. We can recursively apply this reduction until every multiplication only involves integers small enough to fit in the machine word of the computer. Let $T(2^{n+1})$ be the number of machine-word multiplications required in the multiplication of two $2^{n+1}$-bit integers. Express $T(2^{n+1})$ in terms of $T(2^n)$.
...
- (A) $2^{n+1}$ 位整数的乘法可以通过 $2^n$ 位整数的乘法构造，如下所示：$ab = r_2 2^{2^{n+1}} + r_1 2^{2^n} + r_0$，其中 $r_2 = a_{hi} b_{hi}, r_1 = a_{hi} b_{lo} + a_{lo} b_{hi}, r_0 = a_{lo} b_{lo}$。我们可以递归地应用这种缩减，直到每次乘法仅涉及足够小的整数以适应计算机的机器字。设 $T(2^{n+1})$ 为两个 $2^{n+1}$ 位整数相乘所需的机器字乘法次数。用 $T(2^n)$ 表示 $T(2^{n+1})$。
...
1. 给定整数 $x$ 与多项式
   ...
   f=\sum_{i=0}^{n}a_ix^i,
   $$
   其中 $n$ 及 $a_i$（$0\le i\le n$）为自然数，并假设任意两个数的一次加法或乘法均耗费单位时间，与数值位数无关。
   1. 令 $f_i=a_ix^i$。先分别计算 $f_0,\ldots,f_n$，再求和得到 $f$，求该算法关于 $n$ 的时间复杂度。
   2. 定义
      $$
      g_i=
      \begin{cases}
      ...
      \end{cases}
      $$
      且 $f=g_0$。按 $g_n,g_{n-1},\ldots,g_0$ 的顺序求值，求该算法关于 $n$ 的时间复杂度。
2. 给定两个 $2^{n+1}$ 位整数 $a,b$，将各自高、低 $2^n$ 位记为 $a_{hi},a_{lo}$ 与 $b_{hi},b_{lo}$。设 $n$ 足够大，且相较于乘法可忽略加法和移位时间；递归分解直到每次乘法的操作数可装入一个机器字。
   1. 使用
      $$
      ab=r_2\,2^{2^{n+1}}+r_1\,2^{2^n}+r_0,
      $$
      其中
      $$
      r_2=a_{hi}b_{hi},\quad
      r_1=a_{hi}b_{lo}+a_{lo}b_{hi},\quad
      r_0=a_{lo}b_{lo},
      $$
      若 $T(2^{n+1})$ 表示相乘两个 $2^{n+1}$ 位整数所需的机器字乘法次数，用 $T(2^n)$ 表示它。
   2. 解所得递推，并用大 $O$ 表示 $T(2^n)$。
   3. 改用
      $$
      r_1=r_2+r_0-(a_{hi}-a_{lo})(b_{hi}-b_{lo})
      $$
      减少子乘法次数；求新的 $T(2^n)$ 渐近复杂度并证明。
...
- To compute each $f_i = a_i x^i$:
  - Computing $x^i$ separately by repeated multiplication takes $O(i)$ multiplications.
...
Thus, this direct repeated-multiplication implementation takes $\Theta(n^2)$ time. If each power is instead computed by binary exponentiation, the separate evaluations take $O(n\log n)$ time.
...
With a half-word length $h=2^n$, $a=a_{hi}2^h+a_{lo}$ and $b=b_{hi}2^h+b_{lo}$. Therefore:
...
- [東京大学 CBMS 公式過去問](https://www.cbms.k.u-tokyo.ac.jp/media/files/past/H29_senmon.pdf)
