...
(4) Design an algorithm that calculates the $k$-th smallest element in $\mathbf{M}$, where $1\le k\le n$, in $O(n)$ expected time, and prove this property.
...
设 $\mathbf M$ 是含 $n\ge1$ 个互异整数的数组。升序快速排序执行：

1. 从 $\mathbf M$ 中选出并移除枢轴 $x$；
2. 把其余元素分成 $\mathbf M_1,\mathbf M_2$，使每个 $y\in\mathbf M_1$ 满足 $y\le x$，每个 $z\in\mathbf M_2$ 满足 $x\le z$；
3. 递归升序排序两部分，并返回 $\mathbf M_1$、$x$、$\mathbf M_2$ 的拼接。

回答下列问题：

1. 说明如何实现划分步骤。
2. 若总取当前数组的第一个元素为枢轴，给出使运行时间达到 $O(n^2)$ 最坏情形的输入数组，并证明该复杂度。
3. 若在当前数组中均匀随机选择位置并以该元素为枢轴，证明对任意输入数组，期望运行时间为 $O(n\log n)$。
4. 设计利用类似随机划分、在期望 $O(n)$ 时间内求 $\mathbf M$ 第 $k$ 小元素的算法，并证明该期望界。
...
def partition(arr, low, high):
    ...
    for j in range(low, high):
        if arr[j] <= pivot:
            ...
            # Swap the current element to the end of the smaller half of arr
...
Since the pivot rank is uniform on $1,\ldots,n$, conditioning on it gives:
...
Let $c_n=E[C(n)]$, with $c_0=c_1=0$. Multiplying the recurrence by $n$ gives

$$
nc_n=2\sum_{i=0}^{n-1}c_i+n(n-1).
$$

Subtracting the corresponding equation for $n-1$ yields

$$
nc_n=(n+1)c_{n-1}+2(n-1).
$$

Therefore, for $d_n=c_n/(n+1)$,

$$
d_n=d_{n-1}+\frac{2(n-1)}{n(n+1)}.
$$

Telescoping gives $c_n=2(n+1)H_n-4n=\Theta(n\log n)$. Thus the expected running time is $O(n\log n)$.
...
Quickselect recurses into only one subarray. With probability at least $1/2$, the random pivot lies in the middle half, so the next subproblem has size at most $3n/4$. The expected number of partitions before such a reduction is at most $2$. Hence, the expected work is bounded by:

$$
T(n) \leq O(n)+O(3n/4)+O((3/4)^2n)+\cdots=O(n).
$$

Thus the expected time complexity is $O(n)$.
