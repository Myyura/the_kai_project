...
tags:
  - Tokyo-University
  - Computer-Science.String-Algorithms.Longest-Palindromic-Substring
---
...
给定字符串 $x=x_1\cdots x_m$，设计动态规划以求其最长回文子串。字符串 $y=y_1\cdots y_n$ 为回文串，当且仅当对每个 $1\le j\le n$ 都有 $y_j=y_{n+1-j}$；任意单字符串都是回文串，双字符 $y_1y_2$ 为回文串当且仅当 $y_1=y_2$。

1. 已知 $x_i\cdots x_j$ 是回文串，给出并证明 $x_{i-1}\cdots x_{j+1}$ 仍为回文串的充要条件。
2. 对 $1\le i,j\le m$，定义

   $$
   R(i,j)=
   \begin{cases}
   1,&x_i\cdots x_j\text{ 是回文串},\\
   0,&\text{否则},
   \end{cases}
   $$

   且 $\delta(a,b)$ 在 $a=b$ 时为 $1$，否则为 $0$。补全下式中的 A、B、C：

   $$
   R(i-1,\mathrm A)=\delta(\mathrm B,x_{j+1})R(\mathrm C,j).
   $$

3. 给出计算全部 $R(i,j)$ 所需的所有初值，并说明使用上述递推式时的迭代顺序。
4. 说明如何从全部 $R(i,j)$ 中取得 $x$ 的一个最长回文子串。

## **Kai**
### (1)

For $1<i\le j<m$, the extended substring is within the given string. Then:
...
Only entries with $i\le j$ represent nonempty substrings. Entries with $i>j$ are unused by this iteration and may be initialized to $0$.
...
This approach optimizes the search by starting with the longest possible length and stopping as soon as a palindrome is found, ensuring the longest palindrome is identified as quickly as possible.
...
- 动态规划只需填充 $i\le j$ 的上三角区域，并按子串长度递增计算。
