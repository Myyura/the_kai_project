...
tags:
  - Tokyo-University
  - Probability-Statistics.Stochastic-Processes.Autoregressive-Order-One-with-Bernoulli-Innovations
...
令 $V_1,V_2,\ldots$ 相互独立，且每个 $V_i$ 都以概率 $p$ 取 $1$、以概率 $1-p$ 取 $0$，其中 $0<p<1$。给定正实数 $a$，定义随机过程

$$
X_0=1,\qquad X_i=aX_{i-1}+V_i\quad(i\ge1).
$$

回答下列问题：

1. 求 $E[X_1]$。
2. 求 $\operatorname{Var}(X_1)=E[X_1^2]-E[X_1]^2$。
3. 对 $i\ge1$，把 $X_i$ 显式表示为 $a$ 与 $V_1,\ldots,V_i$ 的函数。
4. 求一般的 $E[X_i]$。
5. 按 $a$ 的不同取值讨论并求

   $$
   x_\infty=\lim_{i\to\infty}E[X_i],
   $$

   将结果表示为 $a,p$ 的函数；若极限不为有限值，也需据公式判定。
...
$$
\mathbb{E}(X_i) = \mathbb{E}\left(a^i+\sum_{j=1}^{i} a^{i-j}V_j\right)
$$

$$
\mathbb{E}(X_i) = a^i+\sum_{j=1}^{i} a^{i-j}\mathbb{E}(V_j)
$$

Since $\mathbb{E}(V_j) = p$ for all $j$:

$$
\mathbb{E}(X_i) = a^i+\sum_{j=1}^{i} a^{i-j}p
$$

$$
\mathbb{E}(X_i) = a^i+p\sum_{j=1}^{i} a^{i-j}
$$

For $a\ne1$, the sum is a geometric series:

$$
\sum_{j=1}^{i} a^{i-j} = \frac{a^i - 1}{a - 1}
$$

$$
\mathbb{E}(X_i) = a^i+p\left(\frac{a^i - 1}{a - 1}\right).
$$

For $a=1$, $\mathbb{E}(X_i)=1+ip$.
...
$$
\mathbb{E}(X_i) = \frac{p(a^i - 1) + a^i(a - 1)}{a - 1}
$$

Simplifying the numerator:

$$
\mathbb{E}(X_i) = \frac{p a^i - p + a^{i+1} - a^i}{a - 1}
$$

$$
\mathbb{E}(X_i) = a^i\left(1+\frac{p}{a-1}\right)-\frac{p}{a-1}
$$
...
When $a > 1$, $a^i$ grows exponentially and $1+p/(a-1)>0$. Thus:

$$
\lim_{i \to \infty} \mathbb{E}(X_i)
=\lim_{i\to\infty}\left[a^i\left(1+\frac{p}{a-1}\right)-\frac{p}{a-1}\right]
=\infty.
$$
...
$$
\lim_{i \to \infty} \mathbb{E}(X_i) = \lim_{i \to \infty} (1+ip) = \infty
$$
...
- For $a = 1$, $\lim_{i \to \infty} \mathbb{E}(X_i)$ does not exist as a finite value (diverges to infinity).
- For $0 < a < 1$, $\lim_{i \to \infty} \mathbb{E}(X_i) = \frac{p}{1-a}$, which is finite.
