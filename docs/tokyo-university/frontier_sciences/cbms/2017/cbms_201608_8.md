...
1. 记零向量为 $\mathbf o$，取非零二维向量 $\mathbf a$，并令 $T_{\mathbf a}(\mathbf x)$ 表示点 $\mathbf x$ 在 $\mathbf a$ 所张成直线上的正交投影。结合上图证明：
   1. 对任意二维点 $\mathbf x$，
      $$
      T_{\mathbf a}(T_{\mathbf a}(\mathbf x))=T_{\mathbf a}(\mathbf x);
      $$
   2. 对任意与 $\mathbf a$ 正交的非零二维向量 $\mathbf b$，
      $$
      T_{\mathbf b}(T_{\mathbf a}(\mathbf x))=\mathbf o.
      $$
2. 设实对称矩阵 $\mathbf P$ 满足 $\mathbf P^2=\mathbf P$，证明其特征值只能是 $0$ 或 $1$。
3. 在三维空间中，列向量 $\mathbf a_1,\mathbf a_2$ 构成某二维子空间的一组基。令
   $$
   \mathbf A=[\mathbf a_1,\mathbf a_2],
   $$
   用 $\mathbf A$ 写出到该子空间的正交投影矩阵。
...
The columns of $\mathbf A$ are a basis, so $\|\mathbf A\mathbf c\|^2>0$ for every nonzero $\mathbf c$. Thus $\mathbf A^T\mathbf A$ is positive definite and invertible, and:
