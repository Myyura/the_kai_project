...
When a vertex $v$ is processed, every predecessor has already been processed. Every path from $s$ to $v$ ends with exactly one incoming edge $(u,v)$, so $P(v)=\sum_{(u,v)\in E}P(u)$ counts every such path exactly once.
