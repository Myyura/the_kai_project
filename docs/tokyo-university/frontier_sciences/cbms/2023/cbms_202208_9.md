...
$$
E_2=\{(i,i+1)\mid i=1,\ldots,n-1\}
\cup\{(i,i+2)\mid i=1,\ldots,n-2\}.
$$

1. 对每个 $E_1,E_2$，求满足下列条件的双射 $f:V\to V$ 的总数并说明理由：

   $$
   (u,v)\in E\Longrightarrow f(u)<f(v).
   $$

2. 对每个 $E_1,E_2$，求从顶点 1 到顶点 $n$ 的不同路径数；也可给出计算该数的递推式，并说明理由。
3. 对任意 DAG $G=(V,E)$ 与任意 $s,t\in V$，设计在 $O(|V|+|E|)$ 时间内计算从 $s$ 到 $t$ 路径总数的算法，并证明其正确性与复杂度。
...
- **Ordering**: The remaining values $\{f(2), \ldots, f(n-1)\}$ may be assigned in any order between $f(1)$ and $f(n)$.
...
$$
  ...
  - Base cases: $P(1) = 1$ (the length-zero path from 1 to itself)
...
- Initialize every entry of an array $P$ of size $|V|$ to zero.
...
When a vertex $v$ is processed, every predecessor has already been processed. Every path from $s$ to $v$ ends with exactly one incoming edge $(u,v)$, so $P(v)=\mathbf{1}_{v=s}+\sum_{(u,v)\in E}P(u)$ counts every such path exactly once, including the length-zero path at $s$.

#### Time Complexity

- The topological sort can be done in $O(|V| + |E|)$ time.
- Updating the number of paths for each vertex takes $O(|E|)$ time.
- Thus, the algorithm uses $O(|V|+|E|)$ arithmetic operations and takes that much time when additions of path counts cost unit time.
