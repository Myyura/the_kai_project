...
Conditional probabilities below are used for suffixes with $P(X)>0$. Suffixes with zero probability are omitted from the weighted sum. If any of F, R, E or D has probability zero, the unconditional occurrence probability is zero.
### (1)
...
2. $n = 4$: Only the ending $X=\mathbf{FRED}$ contributes, so the probability is $P(\mathbf{FRED})Q(4,\mathbf{FRED})=p(F)p(R)p(E)p(D)$.

3. $n > 4$: The probability that $\mathbf{FRED}$ appears in a sequence of length $n$ is given by:

$$
\Pr\{\mathbf{FRED}\text{ appears}\}=\sum_{X \in A^4}P(X)Q(n,X)
$$
...
$$
\Pr\{\mathbf{FRED}\text{ appears in length }n\} =
\begin{cases}
0 & \text{if} \quad n < 4 \\
p(F) \cdot p(R) \cdot p(E) \cdot p(D) & \text{if} \quad n = 4 \\
\sum_{X \in A^4}P(X)Q(n,X) & \text{if} \quad n > 4
\end{cases}
$$
