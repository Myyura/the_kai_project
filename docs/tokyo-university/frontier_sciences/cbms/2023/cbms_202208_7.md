...
3. Leave out $\mathbf{v_s}$ of $P$, sort in decreasing order of the slope of the line from $\mathbf{v_s}$ to $\mathbf{v_i}$. Let the result be $P'$
4. for $\mathbf{v}$ in $P'$:
...
1. 上文给出的栈算法先取最左点中最低者 $\mathbf v_s$，按从 $\mathbf v_s$ 出发直线的斜率递减排序其余点，再逐点执行第 4—7 行：当栈顶两点与新点不构成顺时针转向时弹栈，随后压入新点。函数 $\operatorname{cw}(a,b,c)$ 仅在 $\overrightarrow{ab}\times\overrightarrow{bc}<0$ 时为真；实数运算视为无限精度且单位时间。求第 4—7 行的时间复杂度并说明原因。
...
Assume the $x_i$ are distinct. If repetitions are allowed, the hull vertices must also carry their multiplicities: the arrays $[0,0,1]$ and $[0,1,1]$ have the same geometric hull, so their sorted arrays cannot be distinguished from the hull alone.
...
For a terminating execution using $d$ independent fair bits, its exact bit string has probability $2^{-d}$. It produces one fixed permutation, whose total probability must be $1/n!$. Therefore

$$
2^{-d}\le\frac1{n!},\qquad d\ge\lceil\log_2(n!)\rceil.
$$

This argument also applies to algorithms with a variable number of calls. Exact uniform sampling can require rejection and an unbounded worst-case number of calls when $n!$ is not a power of two.

### (5) Convex hull complexity using comparison model

Sorting based on comparison between 2 numbers can be viewed as constructing a decision tree where each comparison narrows down the possible orderings. The height of this tree represents the number of comparisons needed, which is at least $\log_2 n!$ in the worst case.
...
2. Upper bound:
   - Sort points lexicographically in $O(n\log n)$ time, then construct the lower and upper chains by stack scans. In each scan, remove the last point while the last two points and the new point fail the required turn condition; then append the new point. Each point is pushed and popped at most once per chain. Joining the two chains, without duplicating their endpoints, gives the hull in $O(n)$ additional time.
