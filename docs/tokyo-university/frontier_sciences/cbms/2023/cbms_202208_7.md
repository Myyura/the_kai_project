...
- **Graham 扫描式凸包**：用栈维护顺时针转向不变式，并通过每点至多压栈、弹栈各一次证明扫描阶段为线性时间。
- **由排序归约到凸包**：把实数映射到抛物线 $(x,x^2)$，从凸包边界顺序在线性时间恢复原数值次序。
- **比较模型下界**：以 $n!$ 种排列所需的二进制信息量推出 $\Omega(n\log n)$，再结合排序加线性扫描的上界得到紧确界。
