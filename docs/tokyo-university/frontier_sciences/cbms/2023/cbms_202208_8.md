...
(3) For $n=1$, the condition is $|b|>0$ and the matrix $[b]$ is invertible. For $n\ge2$, consider an $n \times n$ symmetric matrix where all diagonal elements are $b$ and all non-diagonal elements are $a$. Show that this matrix is non-singular when $|b| > |(n - 1)a|$.
...
1. 对实参数 $\lambda$，求矩阵
   ...
   的全部特征值及相应特征向量。
2. 求使 $\mathbf A_\lambda$ 为半正定矩阵的 $\lambda$ 取值范围。
3. 设一个 $n\times n$ 对称矩阵的全部对角元均为 $b$，全部非对角元均为 $a$。证明当
...
The full eigenvector families are $c\mathbf v_1$ for $c\ne0$, and $c_2\mathbf v_2+c_3\mathbf v_3$ for $(c_2,c_3)\ne(0,0)$, respectively.
...
For $n=1$, the condition is $|b|>0$ and the matrix $[b]$ is invertible. For $n\ge2$, consider an $n \times n$ symmetric matrix $\mathbf{B}$ where all diagonal elements are $b$ and all non-diagonal elements are $a$.
...
$$
\mathbf{B} = (b-a)\mathbf{I} + a\mathbf{J}
$$
