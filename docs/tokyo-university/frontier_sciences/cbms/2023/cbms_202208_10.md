...
Therefore, the odd-indexed list $L=(P_1,P_3,\ldots,P_{2^n-1})$ and the even-indexed list $R=(P_2,P_4,\ldots,P_{2^n})$ are both already sorted by distance from $B$. Merge them as follows; `compare_B` uses one call to $f$ (reversing its sign when the arguments' original indices are reversed).
...
Merging two lists with a total of $2^n$ elements uses at most

$$
2^n-1
$$

calls to $f$. This is optimal in the worst case: if the two lists alternate in the final order, each of the $2^n-1$ adjacent cross-list pairs must be compared; otherwise that pair could be interchanged without violating either list's known internal order.
