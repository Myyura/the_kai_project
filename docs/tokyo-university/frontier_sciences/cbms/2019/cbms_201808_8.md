...
(5) Suppose the eigenvalues of positive definite matrix $\mathbf{A}$ are all different. Furthermore, suppose you know the largest eigenvalue $\lambda_1$ and its associated eigenvector $\mathbf{v}_1$. Explain how to compute the second largest eigenvalue $\lambda_2$ using $\lambda_1$ and $\mathbf{v}_1$ without computing the third largest or smaller eigenvalues.
...
(5) 假设正定矩阵 $\mathbf{A}$ 的特征值互异。此外，假设你知道最大特征值 $\lambda_1$ 及其关联的特征向量 $\mathbf{v}_1$。解释如何在不计算第三大或更小的特征值的情况下，使用 $\lambda_1$ 和 $\mathbf{v}_1$ 计算第二大特征值 $\lambda_2$。

### 题目描述

若 $n\times n$ 实对称矩阵 $\mathbf A$ 对任意非零实列向量 $\mathbf v$ 都满足

$$
\mathbf v^T\mathbf A\mathbf v>0,
$$

则称 $\mathbf A$ 为正定矩阵。回答下列问题：

1. 证明实正定矩阵的每个对角元 $A_{kk}$（$k=1,\ldots,n$）均为正。
2. 证明实对称矩阵的全部特征值均为实数。
3. 证明正定矩阵的全部特征值均为正数。
4. 令

   $$
   S=\{\mathbf v\in\mathbb R^n\mid \mathbf v^T\mathbf v=1\},
   $$

   证明

   $$
   \lambda_1=\max_{\mathbf v\in S}\mathbf v^T\mathbf A\mathbf v
   $$

   是 $\mathbf A$ 的最大特征值。
5. 设 $\mathbf A$ 的特征值互异，已知最大特征值 $\lambda_1$ 及其特征向量 $\mathbf v_1$，说明如何在不求第三大及更小特征值的情况下计算第二大特征值 $\lambda_2$。
...
For a real symmetric matrix $\mathbf{A}$, consider its eigenvalue equation over $\mathbb C$:

$$
\mathbf{A} \mathbf{x} = \lambda \mathbf{x},
$$

where $\mathbf{x}\ne0$ is an eigenvector. Since $\mathbf A^*=\mathbf A$:
...
$$
\overline{\mathbf{x}^*\mathbf A\mathbf{x}}
...
Let $\mathbf{A}$ be a positive definite matrix and $\lambda$ be an eigenvalue. By (2), $\lambda$ is real, so a corresponding nonzero real eigenvector $\mathbf{x}$ can be chosen:
...
First normalize $\mathbf v_1$ so that $\mathbf v_1^T\mathbf v_1=1$.
...
$$
   \lambda_2 = \max_{\substack{\mathbf{v} \in S_1\\\mathbf v\ne0}} \frac{\mathbf{v}^T \mathbf{A} \mathbf{v}}{\mathbf{v}^T \mathbf{v}}.
$$
