...
给定可能含负数的整数序列 $s_1,\ldots,s_n$，要求找到
...
最大。先定义以位置 $i$ 结尾的最佳非空连续区间和

$$
t_i=\max_{1\le j\le i}\left(\sum_{k=j}^{i}s_k\right).
$$

1. 写出 $t_1$。
2. 用 $t_i$ 与 $s_{i+1}$ 写出 $t_{i+1}$ 的递推式。
3. 设计一个在 $O(n)$ 时间内计算
   $$
   \max_{1\le x\le y\le n}\left(\sum_{k=x}^{y}s_k\right)
   $$
   且输出一对达到最大值的端点 $(x,y)$ 的算法。假设两个整数的加法与取最大值均耗费单位时间。
