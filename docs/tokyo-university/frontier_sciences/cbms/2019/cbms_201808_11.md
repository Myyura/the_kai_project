...
tags:
  ...
  - Probability-Statistics.Stochastic-Processes.Stationary-Pair-Frequencies
...
二进制序列 $\mathbf x=x_1x_2\cdots x_n$ 由一阶平稳 Markov 模型生成，初始概率与转移概率满足

$$
P(x_1=k)=\pi_k\quad(k\in\{0,1\}),
$$

$$
P(x_t=j\mid x_{t-1}=i)=a_{ij}
\quad(i,j\in\{0,1\},\ t=2,\ldots,n).
$$

其中 $P(X)$ 表示事件 $X$ 的概率，$P(X\mid Y)$ 表示在 $Y$ 成立条件下 $X$ 的条件概率。回答下列问题：

1. 当 $n=4$ 时，分别求连续子串 `101` 至少出现一次的概率，以及连续子串 `111` 至少出现一次的概率。
2. 当 $n=4$ 且已知 `101` 作为连续子串出现时，求 $\mathbf x$ 中数字 1 的条件期望个数。
3. 以下采用
   $$
   a_{00}=0.8,\quad a_{01}=0.2,\quad
   a_{10}=0.3,\quad a_{11}=0.7.
   $$
   求 $n\to\infty$ 时 $\mathbf x$ 中 1 的期望比例。
4. 令 $n=2m$，其中 $m$ 为正整数。把相邻两位 $(x_{2i-1},x_{2i})$ 依下列规则编码成 $y_i$：
   $$
   00\mapsto a,\qquad01\mapsto c,\qquad
   10\mapsto g,\qquad11\mapsto t,
   $$
   得到 $\mathbf y=y_1\cdots y_m$。求 $m\to\infty$ 时 $\mathbf y$ 中 $a,c,g,t$ 各自的期望比例。

#### 考点

- **Markov 序列中的模式概率**：枚举长度 4 序列中目标三字符模式的可能位置并处理重叠，按初始概率与转移概率求和。
- **条件概率与条件期望**：在包含 `101` 的事件下重新归一化各可行序列概率，并对其中 1 的个数加权。
- **平稳分布与二元组频率**：求二状态转移矩阵的平稳概率，再用平稳状态概率乘一步转移概率得到四种相邻位组合的长期比例。
