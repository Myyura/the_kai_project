...
(2) 写出以下图的邻接矩阵 $\mathbf{A}$，并证明节点 1 和 4 之间长度为 2 的游走数等于 $\mathbf{A}^2$ 的 $(1,4)$ 元素。
...
(3) 证明对于任何具有某个邻接矩阵 $\mathbf{A}$ 的简单图 $\mathbf{G}$，节点 $i, j$ 之间长度为 $k$ 的游走数等于 $\mathbf{A}^k$ 的 $(i,j)$ 元素。

(4) 假设一个简单连通图 $\mathbf{G}$ 有两个或更多节点。证明 $\mathbf{G}$ 至少有两个度数相同的节点。

### 题目描述

讨论无向图；简单图不含重边与自环，游走是首尾相接的边序列。

1. 证明任意图中奇数度顶点的数量总为偶数。
2. 对下列图写出邻接矩阵 $\mathbf A$，并验证从顶点 1 到顶点 4 的长度为 2 的游走数等于 $\mathbf A^2$ 的 $(1,4)$ 元素：
...
3. 对任意邻接矩阵为 $\mathbf A$ 的简单图 $\mathbf G$，证明从顶点 $i$ 到顶点 $j$ 的长度为 $k$ 的游走数等于 $\mathbf A^k$ 的 $(i,j)$ 元素。
4. 证明任意至少含两个顶点的简单连通图中，至少有两个顶点的度数相同。
...
Since the sum of the degrees is even, and the sum of all even-degree terms is even, the sum of the odd-degree terms must also be even. Therefore, the number of odd-degree vertices, $n_{odd}$, must be even.
...
The $(1,4)$-element of $\mathbf{A}^2$ is 2, which means there are 2 walks of length 2 between nodes 1 and 4. The walks are:
