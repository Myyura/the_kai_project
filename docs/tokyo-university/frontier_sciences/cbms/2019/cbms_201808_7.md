...
![Example decision tree for three distinct elements](https://raw.githubusercontent.com/Myyura/the_kai_project_assets/main/kakomonn/tokyo_university/frontier_sciences/cbms/2019/tokyo-cbms-201808-decision-tree.svg)
...
要确定 $n\ge3$ 个互异自然数 $a_1,\ldots,a_n$ 的全序，从根开始反复构造二叉树：若当前已有比较结果足以确定全部元素顺序，则把当前节点作为叶，并以该全序标记；否则选择一对尚未确定相对次序的 $a_i,a_j$（$i<j$）比较，若 $a_i<a_j$ 则进入左孩子，否则进入右孩子。这样的树称为排序的比较决策树。

1. 固定 $n$，在所有决策树的所有根到叶路径中，描述可能的最短路径与最长路径。
2. 证明在任意决策树中，每一种元素全序必出现在某一个叶节点，且不可能同时标记不同叶节点。
3. 令决策树高度为 $h$，证明存在常数 $c>0$ 使
...
To sort $n$ elements, we need to compare some pairs of elements. Each comparison has at most two outcomes. The shortest path from the root to a leaf in a decision tree corresponds to the minimum number of comparisons needed to determine the order of all elements. The longest path corresponds to the maximum number of comparisons needed.

#### Shortest Path

A path of length $n-1$ is obtained by comparing consecutive elements and following the outcomes:
...
These comparisons determine $a_1<a_2<\cdots<a_n$. Conversely, the graph whose edges are the compared pairs must be connected; otherwise elements in different components have no determined relative order. Hence every path has at least $n-1$ comparisons, so the shortest length is $n-1$.

#### Longest Path

No unordered pair can be compared twice, so a path has length at most $\binom n2$. This bound is attained by comparing the pairs in the following order and following the outcomes $a_1>a_2>\cdots>a_n$:
...
When $(a_i,a_j)$ is compared in this order, its relation has not yet been implied transitively. Thus all $\binom n2=n(n-1)/2$ comparisons are permitted, and this is the longest path.
...
2. **Deterministic Nature of Comparisons**: If two paths diverge, at their last common node the same pair is compared and the two branches require opposite outcomes. No single total order can satisfy both paths.
...
1. **Information-Theoretic Argument**: The height of a decision tree represents the maximum number of comparisons needed in the worst case. For $n$ elements, there are $n!$ possible permutations (orders). Each comparison splits the set of possible permutations, providing at most one bit of information.
2. **Lower Bound on Comparisons**: The number of comparisons needed to distinguish between $n!$ permutations is at least $\log_2(n!)$. For $n\ge4$, the largest $\lceil n/2\rceil$ factors in $n!$ are at least $n/2$, so:
...
$$
   ...
   for a positive constant $c$ (with a constant adjustment for $n<4$).
