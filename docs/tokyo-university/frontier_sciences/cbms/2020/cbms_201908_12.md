...
tags:
  - Tokyo-University
  - Computer-Science.Dynamic-Programming.Counting-Ordered-Pairings
...
在小鼠衰老实验中给雌雄小鼠配对，规则为：每只小鼠至多参加一次配对；每对必须一雄一雌；雌鼠只能与比它年轻的雄鼠配对。所有小鼠年龄互异，并将 $n$ 只小鼠按年龄递增编号为 $1,\ldots,n$。定义
...
$G[i]$ 为前 $i$ 只小鼠中的雄鼠数，$C[i,j]$ 为仅使用前 $i$ 只小鼠组成恰好 $j$ 对的方案数，并规定
...
1. 对 $j>0$，求 $C[1,j]$。
2. 已在前 $i$ 只中组成 $j$ 对，且 $F[i+1]=1$，求为第 $i+1$ 只雌鼠选择尚未配对、比它年轻的雄鼠共有多少种可能。
3. 用 $C[i,j+1]$ 与 $C[i,j]$ 写出 $C[i+1,j+1]$ 的递推式；公式须同时反映第 $i+1$ 只小鼠的性别以及选择不配对或新增一对的情形。

#### 考点

- **有序异性配对计数**：利用年龄顺序保证新出现的雌鼠只能选择此前雄鼠，并扣除已被 $j$ 对占用的雄鼠。
- **组合计数递推**：按第 $i+1$ 只小鼠是否参与新配对分类，把前缀方案数递推到更长前缀。
