...
- **受限相邻交换的可达计数**：根据最小元素能否跨越最大元素，分别建立“任意插入位置”与“由最大元素分隔左右子数组”的乘法分解。
- **递归算法与复杂度**：按极值关系递归删除最小值或围绕最大值分治，并计入扫描、复制子数组的成本。
- **组合概率下的平均分析**：在 $W=N$ 的随机排列模型中，利用最大元素位置等分布对递推运行时间取期望。
