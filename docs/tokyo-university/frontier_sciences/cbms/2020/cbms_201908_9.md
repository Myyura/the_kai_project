...
The structural isomers of pentane ($C_5H_{12}$) can be represented as graphs where each vertex represents a carbon atom, and each edge represents a bond between carbon atoms. The condensed representations below are followed by graphs showing every atom.
...
The carbon skeleton is a tree on five vertices with maximum degree four. Its possible degree sequences are $(2,2,2,1,1)$, $(3,2,1,1,1)$ and $(4,1,1,1,1)$; each determines one tree up to isomorphism. Thus the list is exhaustive.

![All atoms in the three pentane isomers](https://raw.githubusercontent.com/Myyura/the_kai_project_assets/main/kakomonn/tokyo_university/frontier_sciences/cbms/2020/tokyo-cbms-201908-pentane.svg)
...
$$
P(\text{Network functional}) = (1 - p)^3 + 3p(1 - p)^2 = 2p^3 - 3p^2 + 1
$$
