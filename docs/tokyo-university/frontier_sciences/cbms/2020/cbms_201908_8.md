...
设 $A=(a_{ij})$、$P=(p_{ij})$、$\mathbf\Lambda=(\lambda_{ij})$ 均为 $n\times n$ 方阵，$n\ge2$，且

$$
P^{-1}AP=\mathbf\Lambda.
$$

其中 $P$ 可逆，$\mathbf\Lambda$ 为对角矩阵：
...
1. 分别写出 $\mathbf\Lambda^{-1}$ 与 $A^{-1}$。
2. 对每个对角元 $\lambda_{ii}$，证明它是 $A$ 的一个特征值，并用 $P$ 的元素或列向量写出一个对应特征向量。
3. 对任意正整数 $k$，列出 $A^k$ 的全部“特征值—对应特征向量”配对。
...
Thus, each column $\mathbf y_i=\mathbf P\mathbf e_i$ is an eigenvector of $\mathbf A^k$ with eigenvalue $\lambda_{ii}^k$. More generally, for each distinct value $\mu$ among $\{\lambda_{11}^k,\ldots,\lambda_{nn}^k\}$, the full eigenspace is

$$
\ker(\mathbf A^k-\mu\mathbf I)
=\operatorname{span}\{\mathbf y_i:\lambda_{ii}^k=\mu\}.
$$

Hence every eigenvector of $\mathbf A^k$ is a nonzero vector in one of these spaces.
