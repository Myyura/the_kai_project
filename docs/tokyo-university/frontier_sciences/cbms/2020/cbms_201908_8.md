...
- **矩阵对角化**：由 $P^{-1}AP=\mathbf\Lambda$ 改写 $AP=P\mathbf\Lambda$，识别 $P$ 的各列为对应特征向量。
- **逆矩阵**：对相似分解逐项取逆，利用非零对角元构造 $\mathbf\Lambda^{-1}$ 与 $A^{-1}$。
- **矩阵幂的谱**：使用 $A^k=P\mathbf\Lambda^kP^{-1}$，说明特征向量保持不变而特征值变为 $\lambda_{ii}^k$。
