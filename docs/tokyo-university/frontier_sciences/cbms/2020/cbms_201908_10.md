...
设 $H[1,\ldots,n]$ 是含 $n$ 个实数的数组。若

$$
H\!\left[\left\lfloor\frac i2\right\rfloor\right]\ge H[i]
\qquad(2\le i\le n),
$$

其中 $\lfloor i/2\rfloor$ 为 $i$ 除以 2 的整数商，则称 $H$ 为最大堆。回答下列问题：

1. 给出在最坏 $O(n)$ 时间内把任意 $H[1,\ldots,n]$ 建成堆的算法。
2. 证明若 $H$ 是堆，则 $H[1]$ 是数组中的最大元素。
3. 利用堆设计对任意 $H[1,\ldots,n]$ 排序的算法，并证明其最坏时间为 $O(n\log n)$。
...
A node has height at least $h$ exactly when its leftmost descendant at depth $h$ exists. There are $\lfloor n/2^h\rfloor$ such nodes. Summing their heights bounds the total number of downward steps by

$$
\sum_{h=1}^{\lfloor\log_2 n\rfloor}\left\lfloor\frac{n}{2^h}\right\rfloor<n.
$$

Including the constant work per call, BUILD-HEAP runs in $O(n)$ time.
...
We can use the heap sort algorithm to sort the array. The algorithm involves building a heap from the array and then repeatedly extracting the maximum element from the heap and restoring the heap by sifting down from the root.
