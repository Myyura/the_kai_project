...
tags:
  - Tokyo-University
  - Probability-Statistics.Bayesian-Statistics.Bayesian-Model-Selection-for-Sequence-Sources
...
机器 A 独立产生长度为 $n$ 的字符序列 $x=x_1\cdots x_n$，对 $k\in\{a,c,g,t\}$ 与每个位置 $s$，
...
$$
P(x_s=j\mid x_{s-1}=i)=\lambda_{ij}
\quad(i,j\in\{a,c,g,t\},\ s=2,\ldots,n).
$$

机器 C 对输入中每一次出现的连续模式 `ac`，相互独立地以概率 $\epsilon$ 将其替换为 `gt`。记 $P_A(x),P_B(x)$ 分别为 A、B 输出具体序列 $x$ 的概率；第 2 至第 4 问允许直接保留这类记号。回答：

1. 当 $n=5$ 时，分别写出

   $$
   P_A(\mathrm{actgt}),\quad P_A(\mathrm{actac}),\quad
   P_B(\mathrm{actgt}),\quad P_B(\mathrm{actac}).
   $$
...
For Machine A, take the letters to be independent, and take separate output sequences to be independent samples. Then
...
Provided the observation has positive probability:
...
The only possible input sequences are `actgt` and `actac`.

#### Case 1: Input Sequence is `actgt`

If the input sequence is `actgt`, its initial `ac` must not be converted. Hence, the probability that the sequence remains unchanged is $1 - \epsilon$.
...
- Probability of converting `actac` to `actgt`: $(1-\epsilon)\epsilon$, since the first `ac` must remain unchanged.
...
$$
p_A=P_A(\text{actgt}),\qquad p_B=P_B(\text{actgt}),
$$

and, for $M\in\{A,B\}$, let

$$
q_M=(1-\epsilon)\bigl(P_M(\text{actgt})+\epsilon P_M(\text{actac})\bigr)
$$

be the probability that C outputs `actgt` from an input generated by $M$.

Choosing two sequences and then choosing which one enters C is equivalent to choosing an ordered pair of distinct sequences. The source pairs $AA,BB,AB,BA$ therefore have probabilities $3/10,1/10,3/10,3/10$, respectively. If $E$ is the observed event, their likelihoods are

$$
q_Ap_A,\qquad q_Bp_B,\qquad q_Ap_B,\qquad q_Bp_A.
$$

Hence, provided the denominator is positive,

$$
P(\text{same machine}\mid E)
=\frac{3q_Ap_A+q_Bp_B}
{3q_Ap_A+q_Bp_B+3q_Ap_B+3q_Bp_A}.
$$
