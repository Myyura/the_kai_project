...
tags:
  - Tokyo-University
  - Probability-Statistics.Probability-Distributions-and-Asymptotics.Probability-Generating-Function
  - Probability-Statistics.Probability-Distributions-and-Asymptotics.Probability-Generating-Function-of-Random-Sum
...
对任意取非负整数值的随机变量 $X$，定义其概率生成函数

$$
\varphi_X(s)=E\{s^X\}=\sum_{k=0}^{\infty}s^k\Pr\{X=k\},
$$

其中 $s$ 为实数。回答下列问题：

1. 证明 $\varphi_X(1)=1$ 以及

   $$
   \left.\frac{d\varphi_X}{ds}\right|_{s=1}=E\{X\}.
   $$

2. 以下设 $N,U_1,U_2,\ldots$ 为相互独立、同分布且取非负整数值的随机变量。对正整数 $n$ 定义

   $$
   Y_n=\sum_{i=1}^{n}U_i,
   $$

   证明 $\varphi_{Y_n}(s)=\varphi_N(s)^n$。
3. 定义

   $$
   W=\sum_{n=1}^{\infty}Y_n I(N=n),\qquad
   I(N=n)=
   \begin{cases}
   1,&N=n,\\
   0,&N\ne n,
   ...
   证明 $\varphi_W(s)=\varphi_N(\varphi_N(s))$。可使用提示：对正整数 $k$，

   $$
   \Pr\{W=k\}=\sum_{n=1}^{\infty}\Pr\{Y_n=k\}\Pr\{N=n\}.
   $$

4. 当 $\Pr\{N=k\}=q^k(1-q)$、$0<q<1$ 时，计算 $E\{N\}$ 与 $E\{W\}$。

## **Kai**
We work on $0\le s\le1$, where the generating functions are finite. The derivative at $s=1$ is understood from the left; it may be $+\infty$ if $E[X]=+\infty$. For $0<s<1$, termwise differentiation is valid, and monotone convergence gives the limit as $s\uparrow1$.
...
$$
\begin{aligned}
\frac{d \varphi_X}{ds}(1) &= \sum_{k=1}^{\infty} k \Pr\{X = k\} \\
&= E\{X\}
\end{aligned}
$$
...
We need to show $\varphi_W(s) = \varphi_N(\varphi_N(s))$ where $W = \sum_{n=1}^{\infty} Y_n I(N = n)$.
...
&=\Pr\{N=0\}+\sum_{k=0}^{\infty}s^k
  \sum_{n=1}^{\infty}\Pr\{Y_n=k\}\Pr\{N=n\} \\
&=\Pr\{N=0\}+\sum_{n=1}^{\infty}\Pr\{N=n\}
  \sum_{k=0}^{\infty}s^k\Pr\{Y_n=k\} \\
&=\Pr\{N=0\}+\sum_{n=1}^{\infty}\Pr\{N=n\}\varphi_{Y_n}(s) \\
&= \sum_{n=0}^{\infty} \Pr\{N = n\} \varphi_N(s)^n \quad \text{(from result of part 2)} \\
&= \varphi_N(\varphi_N(s))
\end{aligned}
$$
