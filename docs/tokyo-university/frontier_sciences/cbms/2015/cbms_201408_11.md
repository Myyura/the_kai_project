...
- **概率生成函数**：由生成函数的定义验证归一化性质，并通过在 $s=1$ 处求导提取随机变量的期望。
- **独立随机和的生成函数**：利用独立同分布性把固定项数之和 $Y_n$ 的生成函数化为单项生成函数的 $n$ 次幂。
- **随机项数之和**：对随机指标 $N$ 条件化，推导复合生成函数 $\varphi_N(\varphi_N(s))$，再据此计算期望。
- **几何分布**：对给定的从 $0$ 开始计数的几何型概率质量函数求生成函数及均值。
