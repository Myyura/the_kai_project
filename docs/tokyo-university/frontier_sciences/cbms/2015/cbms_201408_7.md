...
1. If $f(n) \geq g(n)$, then $\max(f(n),g(n))=f(n)$ and

$$
f(n)\leq f(n)+g(n)\leq 2f(n).
$$

2. If $g(n) \geq f(n)$, then $\max(f(n),g(n))=g(n)$ and
...
$$
\frac{T(n)}{2\max(f(n),g(n))}
\leq \frac{T(n)}{f(n)+g(n)}
\leq \frac{T(n)}{\max(f(n),g(n))}.
$$

If the rightmost ratio is bounded, so is the middle ratio. Conversely, if the middle ratio is bounded, the left inequality gives

$$
\frac{T(n)}{\max(f(n),g(n))}
\leq 2\frac{T(n)}{f(n)+g(n)},
$$

so the rightmost ratio is also bounded. Therefore,
