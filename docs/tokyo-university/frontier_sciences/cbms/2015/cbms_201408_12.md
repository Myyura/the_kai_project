...
4. 为更精确地评价长间隙，改用上文含 $X_1,Y_1,X_2,Y_2$ 的两套间隙状态迭代。当 $d_2>d_1$ 且 $e_2>e_1$ 时，按 $k$ 的取值范围分类，写出长度为 $k$ 的间隙得分的一般形式。

#### 考点

- **全局序列比对的动态规划**：辨认匹配状态与两个方向的间隙状态，并设置能覆盖序列首端间隙的正确边界值。
- **仿射间隙罚分**：从开启罚分 $d$ 和延伸罚分 $e$ 推导长度为 $k$ 的间隙得分，并解释开启间隙通常比继续延伸代价更高的生物学含义。
- **分段间隙模型**：比较两套线性间隙罚分，在交点处按间隙长度分类写出实际采用的最优得分。
