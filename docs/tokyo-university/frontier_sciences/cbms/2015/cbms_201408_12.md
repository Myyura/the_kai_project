...
(4) In order to treat the score of long gaps more accurately, let us consider alignments using the following iterations. Show the general form of the gap score of length $k$, when $d_2 > d_1 > e_1 > e_2$ holds. Note that classification on the value of $k$ is necessary.
...
(4) 为了更准确地处理长间隙的得分，让我们考虑使用以下迭代进行比对。展示当 $d_2 > d_1 > e_1 > e_2$ 时长度为 $k$ 的间隙得分的一般形式。注意对 $k$ 值的分类是必要的。
...
对两个序列 $x=x_1,\ldots,x_m$ 与 $y=y_1,\ldots,y_n$ 做全局比对。上文迭代式 (A) 用 $M(i,j)$ 表示以 $x_i$、$y_j$ 对齐结尾的最优得分，用 $X(i,j)$、$Y(i,j)$ 表示分别以两个方向的间隙结尾的最优得分；字符配对得分为 $s(x_i,y_j)$，间隙开启、延伸参数满足 $d>0$、$e>0$，任意长度为 $k$ 的间隙采用同一得分函数 $g(k)$。完成以下任务：

1. 由迭代式 (A) 写出 $g(k)$ 的一般形式。
2. 为使对 $i=1,\ldots,m$、$j=1,\ldots,n$ 的迭代最终给出

   $$
   \max\{M(m,n),X(m,n),Y(m,n)\},
   $$

   求边界值 $X(i,0)$（$i=1,\ldots,m$）与 $Y(0,j)$（$j=1,\ldots,n$）。已知其余初值为

   $$
   \begin{aligned}
   M(0,0)&=0,&X(0,0)&=Y(0,0)=-\infty,\\
   M(i,0)&=Y(i,0)=-\infty&&(i=1,\ldots,m),\\
   M(0,j)&=X(0,j)=-\infty&&(j=1,\ldots,n).
   \end{aligned}
   $$

3. 简要说明生物序列比对中通常取 $d>e$ 的原因。
4. 为更精确地评价长间隙，改用上文含 $X_1,Y_1,X_2,Y_2$ 的两套间隙状态迭代。当 $d_2>d_1>e_1>e_2$ 时，按 $k$ 的取值范围分类，写出长度为 $k$ 的间隙得分的一般形式。
...
3. **Computational cost**: This parameter choice does not change the asymptotic complexity of the dynamic program.

### (4)

For the more complex model with two sets of gap penalties $(d_1, e_1)$ and $(d_2, e_2)$, where $d_2 > d_1 > e_1 > e_2$, the general form of the gap score $g(k)$ is
...
$$
k^* = \frac{d_2 - d_1}{e_1 - e_2} + 1
$$

Equivalently,

$$
g(k)=\max\{-d_1-(k-1)e_1,\,-d_2-(k-1)e_2\}.
$$

Thus no rounding convention is needed when $k^*$ is not an integer; compare the integer $k$ directly with $k^*$.
