...
对含 $n$ 个实数的数组 $H[1,\ldots,n]$，若

$$
H[\lfloor i/2\rfloor]\ge H[i]\qquad(1<i\le n),
$$

则称其为堆；这里 $\lfloor i/2\rfloor$ 是 $i$ 除以 $2$ 的整数商。在对应二叉树中，$H[1]$ 为根，$H[\lfloor i/2\rfloor]$ 为 $H[i]$ 的父节点。回答下列问题：

1. 判断数组 $[6,3,4,1,0,3,4,2]$ 是否为堆并说明理由。
2. 证明若 $H[1,\ldots,n]$ 为堆，则任意前缀 $H[1,\ldots,j]$（$1\le j<n$）也为堆。
3. 若 $H[1,\ldots,n]$ 是堆而加入 $H[n+1]$ 后不再是堆，描述仅通过交换元素、在最坏 $O(\log n)$ 时间内恢复 $H[1,\ldots,n+1]$ 堆性质的过程。
4. 证明堆顶 $H[1]$ 是 $H[1,\ldots,n]$ 的最大元素；再设以 $H[n]$ 替换 $H[1]$ 后 $H[1,\ldots,n-1]$ 不再为堆，描述最坏 $O(\log n)$ 时间的交换恢复过程。
5. 描述通过交换元素在最坏 $O(n)$ 时间内把任意 $H[1,\ldots,n]$ 建成堆的算法。
6. 利用上述过程，描述最坏时间为 $O(n\log n)$ 的数组排序算法。
...
**Time Complexity:** A node of height $h$ costs $O(h)$ to heapify. The number of nodes whose height is at least $h$ is at most $n/2^h$, so the total cost is

$$
O\!\left(\sum_{h=1}^{\lfloor\log_2 n\rfloor}\frac{n}{2^h}\right)=O(n),
$$
