...
设 $n\ge2$，随机变量 $X_1,\ldots,X_n$ 相互独立、均取非负实数，且共同密度为
...
2. 对给定 $k\ne i$，在 $X_i=x_i$ 条件下求
   $$
   P(X_i\le X_k\mid X_i=x_i).
   $$
3. 将第 2 问结果乘以 $f(x_i)$ 并对 $x_i$ 积分，求 $P(X_i\le X_k)$。
4. 令
   $$
   X_{\min}=\min_{k=1,\ldots,n}X_k.
   $$
   对给定正实数 $x$，求尾概率 $P(x\le X_{\min})$。
5. 给定正实数 $\lambda_1,\ldots,\lambda_n$，定义
   $$
   Z_i=\frac{X_i}{\lambda_i}.
   $$
   在 $X_i=x_i$ 条件下，对 $k\ne i$ 求 $P(Z_i\le Z_k\mid X_i=x_i)$。
6. 在同一条件下，求
   $$
   P\!\left(\bigcap_{\substack{k=1,\ldots,n\\k\ne i}}
   \{Z_i\le Z_k\}\,\middle|\,X_i=x_i\right).
   $$
7. 令
   $$
   I=\underset{k=1,\ldots,n}{\arg\min}\,Z_k,
   $$
   求索引 $I$ 的分布 $P(I=i)$（$i=1,\ldots,n$）。

#### 考点

- **指数分布**：由密度积分求均值、方差和生存函数，并利用独立性得到多个变量最小值的分布。
- **条件概率与全概率公式**：先固定 $X_i=x_i$ 计算比较事件，再对 $x_i$ 的密度积分消去条件。
- **指数竞赛与次序统计量**：识别 $Z_i=X_i/\lambda_i$ 具有速率 $\lambda_i$，计算每个缩放变量成为总体最小值的概率。
