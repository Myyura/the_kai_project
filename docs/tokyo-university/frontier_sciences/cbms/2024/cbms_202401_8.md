...
Thus the eigenvalues of $\mathbf A^2$ are $\lambda_i^2$ (with algebraic multiplicity), and each $\boldsymbol\alpha_i$ is a corresponding eigenvector. For any nonzero eigenvalue $\mu$ of $\mathbf A^2$, choose $r\in\mathbb C$ with $r^2=\mu$. Its full eigenspace is

$$
E_\mu(\mathbf A^2)=E_r(\mathbf A)\oplus E_{-r}(\mathbf A).
$$

Indeed, if $\mathbf A^2v=r^2v$, then

$$
v_+=\frac{v+\mathbf Av/r}{2},\qquad
v_-=\frac{v-\mathbf Av/r}{2}
$$

satisfy $\mathbf Av_+=rv_+$, $\mathbf Av_-=-rv_-$ and $v=v_++v_-$. The intersection is zero because $r\ne0$.

For $\mu=0$, the full eigenspace is $\ker(\mathbf A^2)$: it contains the zero-eigenvalue eigenvectors and any generalized eigenvectors belonging to chains of length two. Every nonzero vector in one of these spaces is an eigenvector of $\mathbf A^2$.
...
1. "Matrix Analysis" by Roger A. Horn and Charles R. Johnson, Chapter 3: Canonical Forms.
