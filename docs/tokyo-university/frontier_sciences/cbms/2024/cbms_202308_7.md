...
$$
f^{(0)}(n)=n,\qquad
f^{(i)}(n)=f(f^{(i-1)}(n))\quad(i>0).
$$

若存在 $i$ 使 $f^{(i)}(n)\le1$，定义

$$
f^*(n)=\min\{i>0\mid f^{(i)}(n)\le1\};
$$
...
1. 当 $f(n)=n/2$ 时，证明
   ...
   其中 $\lceil x\rceil$ 为不小于 $x$ 的最小整数。
2. 当 $f(n)=\log_2n$ 时，求满足 $f^*(n)=5$ 的最小 $n$。
3. 仍取 $f(n)=\log_2n$，比较 $f(f^*(n))$ 与 $f^*(f(n))$ 的大小关系。原定义只显式规定自变量 $n>1$；题面未另行说明当复合表达式的中间自变量不大于 1 时如何扩展 $f^*$ 的定义域。
...
Exactly five iterations means $f^{(4)}(n)>1$ and $f^{(5)}(n)\le1$. Applying the increasing inverse function $2^x$ repeatedly gives

$$
2^{16}<n\le 2^{65536}.
$$

Since $n$ is real, there is no minimum; the infimum is $2^{16}=65536$. If $n$ were restricted to integers, the minimum would be $65537$.
...
- $f^*(n)=1$ and $f(f^*(n))=0$.
- However, $f(n)\le1$, so $f^*(f(n))$ is outside the stated domain of $f^*$ and is undefined.
...
In summary, where both sides are defined, equality holds for $2<n\le4$, and
$f(f^*(n))<f^*(f(n))$ for $n>4$.
