...
\end{pmatrix},
\qquad
0<p<1,\quad0<q<1,\quad p+q\ne1,
$$

回答：

1. 证明 $\binom p{-q}$ 是 $\mathbf P$ 的特征向量，并确定相应特征值。
2. 求 $n$ 步转移矩阵 $\mathbf P^n$。
3. 求极限 $\lim_{n\to\infty}\mathbf P^n$。

另对任意 $n\times n$ 实矩阵 $\mathbf A=(a_{ij})$，定义
...
4. 若 $\mathbf A$ 的特征值为 $\lambda_1,\ldots,\lambda_n$，证明
...
5. 证明：若存在自然数 $m$ 使
   ...
   其中 $\mathbf O$ 为零矩阵，则 $\operatorname{tr}\mathbf A=0$。
...
Writing $r=(1-p-q)^n$ and multiplying gives

$$
\mathbf P^n=\frac1{p+q}
\begin{pmatrix}
q+pr & p(1-r)\\
q(1-r) & p+qr
\end{pmatrix}.
...
Over $\mathbb C$, write $\mathbf A=\mathbf P\mathbf D\mathbf P^{-1}$ in Jordan form. The matrix $\mathbf D$ is upper triangular and its diagonal entries are the eigenvalues, counted with algebraic multiplicity. The identity $\operatorname{tr}(UV)=\operatorname{tr}(VU)$ follows by summing $\sum_{i,j}U_{ij}V_{ji}$. Thus
