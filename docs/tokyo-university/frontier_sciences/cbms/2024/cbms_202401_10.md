...
有 $N$ 条长度均为 $L$ 的 DNA 序列，存于数组 $\mathbf A$，其中 $\mathbf A[i]$ 为第 $i$ 条序列（$1\le i\le N$）；把一整条序列从一个内存位置复制到另一个位置视为常数时间。

1. 序列可含 `a`、`c`、`g`、`t`。编写伪代码，按最左端碱基的字母顺序排序，并把结果写入另一数组 $\mathbf B$；运行时间须与 $N$ 成线性关系。
2. 以下仅考虑由 `a`、`c` 组成的序列。对给定 $1\le j\le L$，编写按第 $j$ 个碱基排序的伪代码；结果必须原地保存在 $\mathbf A$，不得使用任何其他数组，运行时间须为 $O(N)$。
3. 以第 2 问算法为子程序，编写按完整序列字典序排序的伪代码，并使最坏运行时间为 $O(LN)$。
...
3. **第三问**：最后一个问题要求我们按整个序列的字母顺序进行排序。此时，我们可以使用**基数排序（Radix Sort）**，一种非比较型排序算法。从最高位开始，使用第二问的原地划分把序列分成 a、c 两组，再分别递归处理下一位。通过这种方式，我们可以保证整体排序的时间复杂度是 O(L \times N)，其中 L 是序列长度，N 是序列数量。
...
function countingSortFirstBase(A, N):
    ...
    // Step 3: Calculate ending index for each base in sorted order
    for j = 2 to 4:
        count[j] += count[j - 1]
...
1. **Initialize Count Array**: We first initialize a count array of size 4 to count the occurrences of the bases 'a', 'c', 'g', and 't'. The array indices represent the bases: `count[1]` for 'a', `count[2]` for 'c', `count[3]` for 'g', and `count[4]` for 't'.
...
3. **Calculate Ending Indices**: We compute the ending index for each base in the sorted array by accumulating the counts. This step helps determine where to place the sequences in array $\mathbf{B}$.
...
2. **Partitioning**: The goal is to move all sequences whose $j$-th base is 'a' to the left and those whose $j$-th base is 'c' to the right.
3. **Sorting Process**:
...
This in-place sorting ensures that all sequences in the given subarray are sorted based on the $j$-th base without using additional memory.
...
1. **Radix Sort**: This is a most-significant-digit radix sort. After partitioning by position $j$, it recursively sorts the `a` and `c` blocks by position $j+1$.
2. **Correctness**: Every sequence in the first block is lexicographically smaller than every sequence in the second block; recursion orders equal-prefix sequences. Stability of the two-pointer partition is therefore unnecessary.
3. **Efficiency**: At each of the $L$ positions, the disjoint subarrays contain $N$ sequences in total. Thus the worst-case running time is $O(LN)$.

## **Knowledge**
### 难点思路

- **基数排序的方向**：此处按最高位优先递归划分，不要求划分稳定；最低位优先的逐位排序则需要稳定性。
