...
1. 完全三分图 $K_{r,s,t}$ 的顶点分为大小分别为 $r,s,t$ 的三组；不同组的任意两点相连，同组内任意两点不相连。画出 $K_{2,2,2}$。
2. 求 $K_{r,s,t}$ 的边数。
3. 设简单图有 $n$ 个顶点、$m$ 条边；简单图不含自环和重边，$\binom ab$ 表示从 $a$ 个元素中选 $b$ 个的组合数。证明若
   ...
   则该图必连通。
4. 欧拉图定义为具有欧拉回路的图。证明欧拉图的每个顶点度数均为偶数。
5. 证明逆向充分性：若连通图的每个顶点度数均为偶数，则该图是欧拉图。
...
Partition the connected components into two nonempty groups, and let $G_1,G_2$ be their unions. No edge joins the two groups.

Let $n_1$ and $n_2$ denote their numbers of vertices, and $m_1,m_2$ their numbers of edges. Then $1\le n_1\le n-1$.
...
### (5)
We prove the claim by induction on the number of edges. The case $|E|=0$ is trivial (a connected graph then has one vertex).

For $|E|>0$, let $G=(V,E)$ denote the graph. Let $v \in V$ be an arbitrary vertex.
Pick an arbitrary unused edge adjacent to $v$.
Continue to pick unused edges and walk around the graph until we return to $v$ (we will never get stuck since every vertex has even degree, if there is an incoming edge, there is an outgoing edge).
...
Each nonempty connected component of $G_v$ has fewer edges than $G$, so by induction it has an Euler circuit.
...
- Starting from any vertex, follow unused edges until no continuation is possible. Even degrees imply that this maximal trail returns to its starting vertex.
- If unused edges remain, connectivity gives a vertex of the current circuit incident to an unused edge. Construct another closed trail there and splice it into the current circuit.
- Repeating this finite process uses every edge exactly once, producing an Eulerian circuit.
