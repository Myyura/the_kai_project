...
1. 解释上文算法报告的内容。该算法设置 $H_{i,m+1}=0$、$H_{0,j}=0$，对 $i=1,\ldots,m$ 及 $j=m,\ldots,i$，在
   ...
   时令 $H_{i,j}=H_{i-1,j+1}+1$，否则置零；若 $H_{i-1,j+1}\ge k$，则报告区间对

   $$
   [i-H_{i-1,j+1}+1,i-1],\qquad
   [j+1,j+H_{i-1,j+1}].
   $$
...
Let $L=H_{i-1,j+1}\ge k$. The length-$L$ reverse-complementary ranges represented by this entry are $[i-L,i-1]$ and $[j+1,j+L]$. The printed first range starts at $i-L+1$, so as written the algorithm reports ranges of lengths $L-1$ and $L$; its first endpoint appears to be off by one.
...
def max_reverse_complementary_alignment(x):
    ...
    # Restrict to i <= j, as in (2-1), and scan j downwards.
