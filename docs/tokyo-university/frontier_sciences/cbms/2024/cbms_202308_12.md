...
&\quad \quad \text{else:} \\
&\quad \quad \quad H_{i,j} = 0 \\
&\quad \quad \quad \text{if } H_{i-1,j+1} \ge k \text{ then } \\
&\quad \quad \quad \quad \text{report a pair of ranges } [i - H_{i-1,j+1}, i - 1] \text{ and } [j + 1, j + H_{i-1,j+1}].
\end{align*}
$$

(2-2) Let us define the 'reverse complementary alignment score' of two subsequences $x_{i} \cdots x_{i+p}$ and $x_{j-q} \cdots x_{j}$ of length $p + 1$ and $q + 1$ as the maximum score of global alignment of $x_i\cdots x_{i+p}$ and $x_j\cdots x_{j-q}$, with the second sequence in reverse order.
...
&\quad \quad \text{否则：} \\
&\quad \quad \quad H_{i,j} = 0 \\
&\quad \quad \quad \text{如果 } H_{i-1,j+1} \ge k \text{ 那么 } \\
&\quad \quad \quad \quad \text{报告一对范围 } [i - H_{i-1,j+1}, i - 1] \text{ 和 } [j + 1, j + H_{i-1,j+1}].
\end{align*}
$$

(2-2) 定义两个子序列 $x_{i} \cdots x_{i+p}$ 和 $x_{j-q} \cdots x_{j}$ 的“反向互补对齐得分”为 $p + 1$ 和 $q + 1$ 的全局对齐的最大得分。比对时第二个子序列按 $x_j\cdots x_{j-q}$ 的次序使用。
...
**第 1 部分：序列比对。** 给定序列 $x=x_1\cdots x_m$、$y=y_1\cdots y_n$，字符配对得分为 $s(a,b)$，间隙开启与延伸参数满足 $0<e<d$。上文三状态动态规划以 $M_{i,j}$ 表示字符对齐结尾状态，以 $X_{i,j},Y_{i,j}$ 表示两个方向的间隙结尾状态：新开间隙扣 $d$，同向延伸扣 $e$。

1. 写出长度为 $k$ 的间隙罚分公式。
2. 已知

   $$
   M_{0,0}=0,\qquad X_{0,0}=Y_{0,0}=-\infty,
   $$

   $$
   M_{0,j}=X_{0,j}=-\infty\quad(j=1,\ldots,n),
   $$

   $$
   M_{i,0}=Y_{i,0}=-\infty\quad(i=1,\ldots,m),
   $$

   求其余边界值 $X_{i,0}$ 与 $Y_{0,j}$。
3. 在相同仿射间隙罚分下，写出计算局部比对最大得分的三状态迭代式。
4. 说明如何利用第 3 问的填表结果恢复一组得分最大的局部比对。

**第 2 部分：反向互补片段。** 序列 $x=x_1\cdots x_m$ 仅含 $a,c,g,t$，互补映射为

$$
\operatorname{comp}(a)=t,\quad
\operatorname{comp}(c)=g,\quad
\operatorname{comp}(g)=c,\quad
\operatorname{comp}(t)=a.
$$

1. 解释上文算法报告的内容。该算法设置 $H_{i,m+1}=0$、$H_{0,j}=0$，对 $i=1,\ldots,m$ 及 $j=m,\ldots,i$，在

   $$
   x_i=\operatorname{comp}(x_j)
   $$

   时令 $H_{i,j}=H_{i-1,j+1}+1$，否则置零，并在此时检查：若 $H_{i-1,j+1}\ge k$，则报告区间对

   $$
   [i-H_{i-1,j+1},i-1],\qquad
   [j+1,j+H_{i-1,j+1}].
   $$

2. 对正向片段 $x_i\cdots x_{i+p}$ 与按反向次序使用的片段 $x_{j-q}\cdots x_j$，把“反向互补比对得分”定义为二者全局比对最大得分，其中
   ...
   且长度为 $k$ 的间隙罚分为 $k$。设计一个算法，报告 $x$ 中一对允许为空、使反向互补比对得分最大的子序列。
...
For $i=1$, $X_{1,0}=M_{0,0}-d=-d$; for $i>1$, $X_{i,0}=X_{i-1,0}-e$. Hence $X_{i,0}=-d-(i-1)e$.
...
3. Find a cell and state (M, X or Y) with the maximum score across all three matrices.
4. Follow the predecessor state chosen by each recurrence, recording a letter pair or gap as appropriate, until the score is 0 or a boundary is reached.
...
The algorithm scans a sequence $x$ for reverse complementary matches. It uses a matrix $H$ to record the length of matching substrings that are reverse complements. When an inward extension fails and the preceding match length is at least $k$, the algorithm reports that pair of substrings.

Specifically:

- $H_{i,j}=L$ means that $x_{i-L+1}\cdots x_i$ is reverse-complementary to $x_j\cdots x_{j+L-1}$.
...
- When $x_i\ne\operatorname{comp}(x_j)$ and $H_{i-1,j+1}\ge k$, it reports the preceding matching ranges.

Let $L=H_{i-1,j+1}\ge k$. The length-$L$ reverse-complementary ranges represented by this entry are $[i-L,i-1]$ and $[j+1,j+L]$. The two ranges are disjoint, have equal length $L$, and cannot be extended inward by this pair of characters.
...
The worst-case time complexity of this algorithm is $O(m^2)$, where $m$ is the length of the sequence $x$. The space complexity is also $O(m^2)$ due to the dynamic programming matrix.
...
def max_reverse_complementary_alignment(x):
    m = len(x)
    # dp[i][j]: best local score ending at x[i] and reverse position x[j]
    dp = [[0] * (m + 2) for _ in range(m + 1)]
    parent = [[None] * (m + 2) for _ in range(m + 1)]
    max_score = 0
    max_pos = (0, m + 1)
    ...
    # Compare all interval pairs, including overlapping intervals.
    for i in range(1, m+1):
        for j in range(m, 0, -1):
            choices = [
                (0, None),
                (dp[i-1][j+1] + s(x[i-1], x[j-1]), 'D'),
                (dp[i-1][j] - 1, 'U'),
                (dp[i][j+1] - 1, 'R'),
            ]
            dp[i][j], parent[i][j] = max(choices, key=lambda z: z[0])
            if dp[i][j] > max_score:
                max_score = dp[i][j]
                max_pos = (i, j)
    ...
    return (
        max_score,
        x[i:end_i], x[start_j-1:j-1],
        ''.join(reversed(seq1)), ''.join(reversed(seq2)),
    )
```
