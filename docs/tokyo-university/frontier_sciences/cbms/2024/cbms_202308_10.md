...
**Caveat.** The procedure is not a correct quicksort as printed. For example, on $[1,2]$ the loop is skipped because $l=h$, after which the unconditional swap produces $[2,1]$. On $[0,1,2]$, scan (C) can move $h$ past $p$; Python negative indexing then leads to an infinite recursion, while ordinary pseudocode accesses outside the subarray.
...
For the printed procedure there is no largest finite $d_{\max}$. With $\mathbf V=[0,1,2]$, Python semantics alternate the full interval between $[0,1,2]$ and $[2,1,0]$, so $d$ increases without bound. Thus question (3) has no finite answer unless the partition code is corrected. For an ordinary corrected quicksort, a monotone array with distinct keys gives the familiar one-sided recursion.
