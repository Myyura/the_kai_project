...
设 $T(n)$ 为输入规模 $n$ 对应的运行时间，规定 $T(0)=1$，且 $\lfloor x\rfloor$ 表示不超过 $x$ 的最大整数。可按需使用大 $O$ 等 Landau 记号。

1. 分别求下列递推的 $T(n)$ 复杂度：

   $$
   \begin{aligned}
   \text{(1)}\quad&T(n)=T(n-1)+2n,\\
   \text{(2)}\quad&T(n)=T(\lfloor n/3\rfloor)+1,\\
   \text{(3)}\quad&T(n)=2T(\lfloor n/2\rfloor)+n+1.
   \end{aligned}
   $$
...
2. 先判断下列命题真伪，再证明：
   ...
   $$
   T(n)\in O(2^n)\quad\Longleftrightarrow\quad T(n)\in O(e^n).
   $$

3. 对正常数 $a$，求递推

   $$
   T(n)=aT(\lfloor n/2\rfloor)+\lfloor n\log_2n\rfloor
   $$
...
For arbitrary $n$, the recurrence implies that $T(n)$ is nondecreasing. Bounding $n$ between consecutive powers of two extends the same asymptotic bound to all integers.

### (4)
The statement is False.

Counter example: $T(n)=2.5^n$. Since $2<2.5<e$, we have $(2.5/e)^n\to0$ but $(2.5/2)^n\to\infty$. Thus this function belongs to $O(e^n)$ and does not belong to $O(2^n)$.
...
$$
\begin{aligned}
    &T(n) = \Theta(n \log_2 n) &\text{if } a < 2 \\
    &T(n) = \Theta(n \log_2^2 n) &\text{if } a = 2 \\
    &T(n) = \Theta(n^{\log_2 a}) &\text{if } a > 2
\end{aligned}
$$
...
Here $n^{\log_3 1}=1$ and $f(n)=\Theta(1)$, so case 2 of the Master Theorem gives $T(n)=\Theta(\log n)$.
...
At level $\ell$, the total nonrecursive cost is
$n\log(n/2^\ell)$. Hence, for $n$ a power of two,

$$
T(n)=nT(1)+n\sum_{\ell=0}^{\log_2n-1}(\log_2n-\ell)
=\Theta(n\log^2 n).
$$
