...
纳米孔测序仪以电流变化读取 DNA。某待测序列在时刻 $t=1,\ldots,n$ 的电流为

$$
\{A_t\},\qquad0\le A_t\le1,
$$

已知其中包含一段参考序列；单独读取参考序列时，在 $t=1,\ldots,m$ 得到参考波形

$$
\{B_t\},\qquad0\le B_t\le1.
$$

由于读取速度波动，需要允许时间轴错位。假设存在对应位置

$$
1\le p_1\le p_2\le\cdots\le p_m\le n,
$$
...
其中 $W$ 为正整数。把参考波形与测序输出部分的差异度定义为

$$
\sum_{t=1}^{m}(B_t-A_{p_t})^2,
$$

并以使其最小的 $p_1,\ldots,p_m$ 作为正确对应。结合上图回答：

1. 当 $m=1$ 时，给出求一组最优 $p_1$ 的算法。
2. 当 $m=2$ 时，给出求一组最优 $(p_1,p_2)$ 的算法。
3. 用一行说明参数 $W$ 对应何种物理限制。
4. 定义 $D_{i,j}$：在 $B_i$ 与 $A_j$ 对应的条件下，参考前缀 $\{B_1,\ldots,B_i\}$ 与输出前缀 $\{A_1,\ldots,A_j\}$ 的最小差异度。用满足 $x<i,y\le j$ 的先前状态 $D_{x,y}$ 表示 $D_{i,j}$，并体现单调对应与相邻位置差不超过 $W$ 的约束。
5. 设计计算完整参考波形 $\{B_t\}$ 对 $\{A_t\}$ 最小差异度的算法，并用 $n,m,W$ 表示时间复杂度。
...
The parameter $W$ bounds local time stretching: at most $W$ output sampling intervals may elapse between two adjacent reference samples.
...
D_{i,j}
&=(B_i-A_j)^2+
\min_{\max(1,j-W)\le k\le j}D_{i-1,k}
\qquad(i\ge2).
\end{aligned}
$$
...
3. For each $i = 2, 3, \dots, m$, do:
   - For each $j = 1, 2, \dots, n$, do:
     - Set $D_{i,j} = (B_i-A_j)^2+\min_{x = \max(1, j - W)}^{j}D_{i-1,x}$.
4. Return $\min_{1\le j\le n}D_{m,j}$.
