...
- **单比特密度矩阵**：由 Hermitian 二阶矩阵的特征方程及 Bloch 向量长度约束证明半正定性。
- **量子门共轭演化与测量**：计算 $\rho\mapsto U\rho U^\dagger$ 后的左上角元素，分别得到 H 门、Y 门后的 Z 基测量概率。
- **复矩阵与 Bloch 球**：利用酉变换保持密度矩阵的特征值或纯度，判断 Bloch 向量平方长度在一般量子门下的变化。
...
$$
\begin{aligned}
a' &= (|u_{00}|^2-|u_{01}|^2)(1+a) + (u_{00}'u_{01}'+u_{00}''u_{01}'')2b \\
&+ (u_{00}'u_{01}''-u_{00}''u_{01}')2c + (|u_{10}|^2-|u_{11}|^2)(1-a)
\end{aligned}
$$

$$
\begin{aligned}
b' &= (u_{00}'u_{10}'+u_{00}''u_{10}'')(1+a) + (u_{01}'u_{11}'+u_{01}''u_{11}'')(1-a) \\
&+ (u_{00}'u_{11}'+u_{00}''u_{11}''+u_{01}'u_{10}'+u_{01}''u_{10}'')b - (u_{00}'u_{11}''-u_{00}''u_{11}'-u_{01}'u_{10}''+u_{01}''u_{10}')c
\end{aligned}
$$

$$
\begin{aligned}
c' &= (u_{00}'u_{10}''-u_{00}''u_{10}')(1+a) + (u_{01}'u_{11}''-u_{01}''u_{11}')(1-a) \\
&+ (u_{00}'u_{11}''-u_{00}''u_{11}'-u_{01}'u_{10}''+u_{01}''u_{10}')b + (u_{00}'u_{11}'+u_{00}''u_{11}''-u_{01}'u_{10}'-u_{01}''u_{10}'')c
\end{aligned}
$$
...
$$
\det P = \frac{1 - a^2 - b^2 - c^2}{4}
$$

$$
\det P' = \det(UPU^*) = \det(PU^*U) = \det P = \frac{1 - a^2 - b^2 - c^2}{4}
$$
