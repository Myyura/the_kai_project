...
tags:
  - Tokyo-University
  - Computer-Science.Quantum-Computing.Quantum-Computing
...
## **Author**
[zephyr](https://inshi-notes.zephyr-zdz.space/), 祭音Myyura (with the help of an anonymous contributor)
...
**Additional conditions (independent summary):** A gate $U$ is a complex $2\times2$ unitary matrix, $U^\dagger U=I$. Here $U^\dagger$ is its conjugate transpose, and applying the gate changes the state to $\rho'=U\rho U^\dagger$. The real and imaginary parts of every entry of $U$ are real numbers. [Official paper, question 11](https://www.cbms.k.u-tokyo.ac.jp/media/files/past/2024B_CBMS_exam_J.pdf).
...
表示，其中 $i^2=-1$，$a,b,c\in\mathbb R$ 且

$$
a^2+b^2+c^2\le1.
$$

在计算基（Z 基）测量时，得到状态 0、1 的概率分别是对角元

$$
p_0=\frac{1+a}{2},\qquad p_1=\frac{1-a}{2}.
$$

要求给出数学推导并回答：

1. 证明 $\rho$ 的全部特征值均为非负实数。
2. 对状态施加 Hadamard 门
...
4. 一般量子门写为
   ...
   U=
   \begin{pmatrix}
   u'_{00}+iu''_{00}&u'_{01}+iu''_{01}\\
   u'_{10}+iu''_{10}&u'_{11}+iu''_{11}
   \end{pmatrix}.
   $$
...
$$
\begin{aligned}
H\rho H^\dagger &= \frac{1}{4}\begin{pmatrix} 1 & 1 \\ 1 & -1 \end{pmatrix} \begin{pmatrix} 1+a & b-ic \\ b+ic & 1-a \end{pmatrix} \begin{pmatrix} 1 & 1 \\ 1 & -1 \end{pmatrix} \\
...
$$
\begin{aligned}
Y\rho Y^\dagger &= \frac12\begin{pmatrix} 0 & -i \\ i & 0 \end{pmatrix} \begin{pmatrix} 1+a & b-ic \\ b+ic & 1-a \end{pmatrix} \begin{pmatrix} 0 & -i \\ i & 0 \end{pmatrix} \\
&= \frac12\begin{pmatrix} 1-a & -b-ic \\ -b+ic & 1+a \end{pmatrix}
\end{aligned}
$$
...
### 4. Compute $a'^2 + b'^2 + c'^2$ after applying U gate (by zephyr)
...
As a quantum gate, $U$ is unitary. The transformed state is

$$
\rho' = U\rho U^\dagger = \begin{pmatrix} \frac{1+a'}{2} & \frac{b'-ic'}{2} \\ \frac{b'+ic'}{2} & \frac{1-a'}{2} \end{pmatrix}
$$

Rather than expanding its entries, use $UU^\dagger=I$: unitary conjugation preserves the purity of the density matrix.
...
### 4. Compute $a'^2 + b'^2 + c'^2$ after applying U gate (by an anonymous contributor)

Note that

$$
\det \rho = \frac{1 - a^2 - b^2 - c^2}{4}
$$

$$
\det \rho' = \det(U\rho U^\dagger) = \det(\rho U^\dagger U) = \det \rho = \frac{1 - a^2 - b^2 - c^2}{4}
$$
