...
设 $n$ 为正整数，$f(n),g(n)$ 表示处理规模 $n$ 问题的运行时间。定义

$$
O(g(n))=\{f(n)\mid \exists c_0,n_0>0,\ \forall n\ge n_0,\ 0\le f(n)\le c_0g(n)\},
$$

$$
\Omega(g(n))=\{f(n)\mid \exists c_0,n_0>0,\ \forall n\ge n_0,\ 0\le c_0g(n)\le f(n)\}.
$$

对下列各情形，分别按定义判断所给上界或下界是否成立，并给出证明：

1. $f(n)=2^{2n}$ 时，判断 $f(n)\in O(2^n)$ 与 $f(n)\in\Omega(2^n)$。
2. $f(n)=\sum_{i=1}^{n}\frac1i$ 时，判断 $f(n)\in O(\log n)$ 与 $f(n)\in\Omega(\log n)$。
3. 令 $\lfloor x\rfloor$ 表示不超过 $x$ 的最大整数。若

   $$
   f(1)=1,\qquad f(n)=f(\lfloor n/2\rfloor)+n\quad(n\ge2),
   $$

   判断 $f(n)\in O(n)$ 与 $f(n)\in\Omega(n^2)$。
...
For large $n$, $2^n$ grows exponentially, so any fixed positive $c_0$ will be less than $2^n$ for sufficiently large $n$. For example, choose $c_0=1$ and $n_0=1$:
...
In this case, $\log_b a = \log_2 1 = 0$, and $g(n) = n$. Since $g(n)=\Theta(n)$ has exponent $1>0$ and $g(n/2)=g(n)/2$, case 3 of the Master Theorem applies, giving us:
