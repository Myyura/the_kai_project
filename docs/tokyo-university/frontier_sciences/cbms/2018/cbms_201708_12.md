...
给定长度分别为 $n,m$（$n,m\ge0$）的字符串

$$
\mathbf x=x[1]\cdots x[n],\qquad
\mathbf y=y[1]\cdots y[m].
$$

公共子序列用两列严格递增的下标

$$
\begin{pmatrix}i_1,\ldots,i_r\\j_1,\ldots,j_r\end{pmatrix}
$$

表示，其中 $r$ 为正整数，

$$
1\le i_1<\cdots<i_r\le n,\qquad
1\le j_1<\cdots<j_r\le m,
$$

且对每个 $k=1,\ldots,r$ 都有 $x[i_k]=y[j_k]$。最长公共子序列长度记为 $l(x,y)$；若不存在公共子序列，则定义 $l(x,y)=0$。

1. 令 $z_k^p,z_k^s$ 分别为字符串 $z$ 的长度 $k$ 前缀和后缀，并定义

   $$
   \alpha[i,j]=l(x_i^p,y_j^p),\qquad
   \beta[i,j]=l(x_{n-i+1}^s,y_{m-j+1}^s).
   $$

   对 $1<i,1<j$，给出由 $\alpha[i-1,j-1]$、$\alpha[i-1,j]$、$\alpha[i,j-1]$ 计算 $\alpha[i,j]$ 的递推；对 $i<n,j<m$，给出由 $\beta[i+1,j]$、$\beta[i,j+1]$、$\beta[i+1,j+1]$ 计算 $\beta[i,j]$ 的递推。
2. 对 $\mathbf x=\mathrm{ACTGG}$、$\mathbf y=\mathrm{ACACG}$，计算完整矩阵 $\alpha,\beta$。
3. 已知 $\alpha$，编写取得一个最长公共子序列的伪代码。
4. 已知 $\alpha,\beta$，编写伪代码，对指定 $1\le i\le n$ 求所有包含 $\mathbf x$ 第 $i$ 个位置的公共子序列中的最大长度。
...
Use the boundary values $\alpha[0,j]=\alpha[i,0]=0$.

#### Recurrence for $\beta[i, j]$

The value of $\beta[i, j] = l(x_{n-i+1}^s, y_{m-j+1}^s)$ can be computed as follows:

- If $x[i] = y[j]$, then $\beta[i, j] = \beta[i+1, j+1] + 1$.
- If $x[i] \neq y[j]$, then $\beta[i, j] = \max(\beta[i+1, j], \beta[i, j+1])$.

This can be summarized as:

$$
\beta[i, j] = \begin{cases}
\beta[i+1, j+1] + 1 & \text{if } x[i] = y[j] \\
\max(\beta[i+1, j], \beta[i, j+1]) & \text{if } x[i] \neq y[j]
\end{cases}
$$

Use the boundary values $\beta[n+1,j]=\beta[i,m+1]=0$.
...
Let's fill the matrix $\beta$ for $\mathbf{x} = \text{ACTGG}$ and $\mathbf{y} = \text{ACACG}$. The row and column indices are displayed in descending order:
...
- The resulting list of matched index pairs represents one of the longest common subsequences; their characters can be read from $x$.
...
Given matrices $\alpha$ and $\beta$, match $x[i]$ with some equal character $y[j]$. Combine a common subsequence before these positions, this matched pair, and a common subsequence after them.

#### Explanation

- For each position $j$ in $\mathbf{y}$:
  - If $x[i]=y[j]$, the maximal length using this pair is
    $\alpha[i-1,j-1]+1+\beta[i+1,j+1]$.
...
If no character of $y$ matches $x[i]$, there is no common subsequence containing this position; the function returns $0$ in that case.
...
- [東京大学 CBMS 公式過去問](https://www.cbms.k.u-tokyo.ac.jp/media/files/past/H30_senmon.pdf)
