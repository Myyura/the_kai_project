...
设实值随机变量 $X_1,\ldots,X_n$ 相互独立且同分布，其共同分布函数为

$$
F(x)=P(X_i\le x),\qquad i=1,\ldots,n.
$$
...
1. 求最小次序统计量 $X_{(1)}$ 的分布函数。
2. 求最大次序统计量 $X_{(n)}$ 的分布函数。
3. 对任意 $k$，求 $X_{(k)}$ 的分布函数。
4. 当共同分布为区间 $[0,1]$ 上的均匀分布时，求 $E[X_{(1)}]$。
...
Since $\mathbf{X_{(n)}}$ is the largest of the $\mathbf{X_1}, \ldots, \mathbf{X_n}$, $\mathbf{X_{(n)}} \leq x$ means that all $\mathbf{X_i} \leq x$. Thus:
...
$$
\int_{0}^{1} x (1 - x)^{n-1} \, \mathrm{d}x = \frac{1}{n(n+1)}.
$$

Thus:

$$
\mathbb{E}[\mathbf{X_{(1)}}] = \frac{1}{n+1}.
$$
