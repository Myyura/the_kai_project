...
1. 证明无向图所有顶点的度数之和等于边数的两倍。
2. 对顶点集为 $\mathbf V$、边集为 $\mathbf E$ 的图 $\mathbf G(\mathbf V,\mathbf E)$，证明若 $\mathbf G$ 是树，则

   $$
   |\mathbf E|=|\mathbf V|-1.
   $$

3. 给定无向图 $\mathbf G(\mathbf V,\mathbf E)$ 及边集 $\mathbf T\subseteq\mathbf E$；若 $\mathbf S(\mathbf V,\mathbf T)$ 为树，则称其为生成树，加权图中边权总和最小的生成树称为最小生成树。求出上图的一棵最小生成树。
4. 设 $\mathbf S(\mathbf V,\mathbf T)$ 与 $\mathbf S'(\mathbf V,\mathbf T')$ 是 $\mathbf G$ 的两棵不同生成树。证明：对任意

   $$
   \mathbf e'\in\mathbf T'\setminus\mathbf T,
   $$

   都存在 $\mathbf e\in\mathbf T\setminus\mathbf T'$，使

   $$
   (\mathbf T\setminus\{\mathbf e\})\cup\{\mathbf e'\}
   $$
...
**Inductive Step**: Assume the statement is true for every tree with $n$ vertices. Any tree with $n+1$ vertices has a leaf $v$. Removing $v$ and its unique incident edge leaves a tree with $n$ vertices, hence $n-1$ edges by the induction hypothesis. Restoring $v$ and its edge gives $n$ edges, so $\left| \mathbf{E} \right| = \left| \mathbf{V} \right| - 1$.

Hence, by induction, the proposition is proved.

### (3)
Label the vertices A (leftmost), B (upper left), C (lower left), D (upper right), and E (lower right).
...
Thus, the minimum spanning tree includes edges A-C, B-D, D-E, and A-B, with total weight $2+4+4+5=15$.
...
In this cycle, there must be at least one edge $\mathbf{e}$ that is in $\mathbf{T}$ but not in $\mathbf{T'}$: otherwise every other edge of the cycle, together with $\mathbf e'$, would belong to $\mathbf T'$, contradicting that $\mathbf T'$ is acyclic. Removing this edge $\mathbf{e}$ breaks the cycle while preserving connectivity.
