...
二叉堆树是每个节点带整数标签、且任一父节点标签均不小于其子节点标签的二叉树；树 $\mathbf T$ 的高度记为 $h(\mathbf T)$。

1. 从上图所列候选树中选出所有二叉堆树。
2. 对高度为 $2$ 的二叉树 $\mathbf T$，给出至多交换一次某节点与其子节点标签、使其成为二叉堆树的算法。
3. 已有两棵二叉堆树 $\mathbf T_1,\mathbf T_2$，新建根节点 $\mathbf r$，并使两棵树的根成为 $\mathbf r$ 的两个子节点，得到树 $\mathbf T$。给出把 $\mathbf T$ 调整为二叉堆树 $\mathbf T'$ 的算法，使最坏时间为

   $$
   O(\max(h(\mathbf T_1),h(\mathbf T_2))).
   $$

4. 对任意二叉树 $\mathbf T$，给出反复交换父子标签以建成二叉堆树的算法，并证明最坏时间为

   $$
   O(2^{h(\mathbf T)}).
   $$

5. 利用第 4 问的操作设计整数数组降序排序算法，并用数组元素数表示最坏时间复杂度。
...
We count tree height by the number of levels, so a tree of height 2 has a root and its children.
...
2. Let $\mathbf{c_1}$ and $\mathbf{c_2}$ be the existing children of $\mathbf{r}$.
3. Compare the children, then compare the larger child with $\mathbf{r}$.
4. If $\mathbf{r}$ is less than the larger child, exchange the labels of $\mathbf{r}$ and the larger child.

This algorithm ensures that the tree satisfies the binary heap property with at most one exchange.
...
**Algorithm:**
1. Visit non-leaf nodes in postorder, processing both child subtrees before their parent.
2. For each node, perform the full `heapify` operation from Question 3:
   - Compare the node with its existing children.
   - If it is smaller than the larger child, swap their labels and continue at that child.
   - Continue downwards until the heap property holds or a leaf is reached.
3. Repeat the process until the root node is processed.

If $C(h)$ is the worst-case work for height $h$, bottom-up heapify gives
$C(h)\leq 2C(h-1)+O(h)=O(2^h)$. Hence the worst-case time is
$O(2^{h(\mathbf T)})$.
...
4. Reverse the resulting ascending array to obtain descending order.
