...
设 $\mathbf A$ 是秩为 $r>0$ 的 $n\times m$ 实矩阵，其薄奇异值分解为

$$
\mathbf A=\mathbf U\mathbf\Sigma\mathbf V^T,
$$

其中 $\mathbf U\in\mathbb R^{n\times r}$、$\mathbf V\in\mathbb R^{m\times r}$，满足

$$
\mathbf U^T\mathbf U=\mathbf I_r,\qquad
\mathbf V^T\mathbf V=\mathbf I_r,
$$

$\mathbf\Sigma$ 是 $r\times r$ 对角矩阵，且

$$
\Sigma_{kk}=\sigma_k,\qquad
\sigma_1\ge\cdots\ge\sigma_r>0.
$$

回答下列问题：

1. 写出 $\mathbf A^T\mathbf A$ 的全部正特征值及对应的单位特征向量。
2. 对线性映射

   $$
   T_{\mathbf A}:\mathbb R^m\to\mathbb R^n,\qquad T_{\mathbf A}(\mathbf x)=\mathbf A\mathbf x,
   $$
...
3. 定义 Moore–Penrose 伪逆
   ...
   $$
   \mathbf B=\mathbf I_m-\mathbf A^+\mathbf A,\qquad
   T_{\mathbf B}(\mathbf x)=\mathbf B\mathbf x.
   $$

   证明 $\operatorname{Im}(T_{\mathbf B})$ 与 $\ker(T_{\mathbf A})$ 线性同构。
4. 对

   $$
   \mathbf x_1=\mathbf B\mathbf x,\qquad
   \mathbf x_2=\mathbf x-\mathbf x_1,
   $$

   证明 $\mathbf x=\mathbf x_1+\mathbf x_2$ 是正交分解。
5. 给定 $\mathbf b\in\mathbb R^n$，令 $\mathbf x_0=\mathbf A^+\mathbf b$，证明 $\mathbf x=\mathbf x_0$ 最小化
   ...
   =\mathbf A(\mathbf x-\mathbf x_0)+(\mathbf A\mathbf x_0-\mathbf b).
   $$
...
The matrix $\mathbf{\Sigma}^2$ is diagonal with the diagonal elements $\sigma_k^2$ ($k = 1, \ldots, r$). Thus, the positive eigenvalues of $\mathbf{A}^T \mathbf{A}$ are exactly the $\sigma_k^2$, and the corresponding columns $\mathbf v_k$ of $\mathbf V$ are normalized eigenvectors. If a singular value is repeated, every unit vector in the span of the corresponding $\mathbf v_k$ is an associated normalized eigenvector.
...
$$
\mathbf{A}\mathbf{B}
=\mathbf{A}(\mathbf{I}_m-\mathbf{A}^+\mathbf{A})
...
Let $\mathbf P=\mathbf A^+\mathbf A=\mathbf V\mathbf V^T$. Then $\mathbf P$ is symmetric and idempotent, and $\mathbf B=\mathbf I_m-\mathbf P$. Hence:

$$
\mathbf{x}_1^T\mathbf{x}_2
=\mathbf{x}^T(\mathbf I_m-\mathbf P)\mathbf P\mathbf{x}
...
Since $\mathbf{x}_0 = \mathbf{A}^+ \mathbf{b}$, the residual
$\mathbf r=\mathbf A\mathbf x_0-\mathbf b=(\mathbf A\mathbf A^+-\mathbf I_n)\mathbf b$
is orthogonal to $\operatorname{Im}(\mathbf A)$. Thus $\mathbf r$ is orthogonal to $\mathbf A(\mathbf x-\mathbf x_0)$, and:

$$
\|\mathbf A\mathbf x-\mathbf b\|^2
=\|\mathbf A(\mathbf x-\mathbf x_0)\|^2+\|\mathbf A\mathbf x_0-\mathbf b\|^2
\geq \|\mathbf A\mathbf x_0-\mathbf b\|^2.
$$

Therefore, $\mathbf{x}_0$ is a minimizer.
