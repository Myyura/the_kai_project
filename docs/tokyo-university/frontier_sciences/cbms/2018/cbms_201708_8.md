...
- **奇异值分解与谱关系**：由薄 SVD 识别 $\mathbf A^T\mathbf A$ 的正特征值 $\sigma_k^2$ 及右奇异向量。
- **线性映射的核与像**：以秩 $r$ 和定义域、值域维数判定单射与满射，并刻画核空间。
- **Moore–Penrose 伪逆**：证明 $\mathbf I-\mathbf A^+\mathbf A$ 投影到 $\ker\mathbf A$，并给出核空间与行空间的正交分解。
- **最小二乘法**：利用残差在 $\operatorname{Im}\mathbf A$ 上的正交性证明伪逆解使残差平方范数最小。
