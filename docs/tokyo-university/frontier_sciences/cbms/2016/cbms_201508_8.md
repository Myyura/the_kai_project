...
### (3)
证明，如果一个可逆矩阵 **A** 的特征值是 $\lambda_1, \dots, \lambda_n$，那么其逆矩阵 **A**$^{-1}$ 的特征值是 $1/\lambda_1, \dots, 1/\lambda_n$。
...
1. 求矩阵
   ...
   的逆矩阵。
2. 对二维数据点 $(x_i,y_i)$（$i=1,\ldots,n$），定义

   $$
   \sigma_x=\frac1n\sum_{i=1}^{n}(x_i-\bar x)^2,\qquad
   \sigma_y=\frac1n\sum_{i=1}^{n}(y_i-\bar y)^2,
   $$

   $$
   \sigma_{xy}=\frac1n\sum_{i=1}^{n}(x_i-\bar x)(y_i-\bar y),
   $$

   其中 $\bar x,\bar y$ 为两坐标的均值。
   1. 对数据点
   ...
   2. 求该方差—协方差矩阵的全部特征值与特征向量。
3. 证明：若可逆矩阵 $\mathbf A$ 的特征值为 $\lambda_1,\ldots,\lambda_n$，则 $\mathbf A^{-1}$ 的相应特征值为 $1/\lambda_1,\ldots,1/\lambda_n$。
...
Thus, all eigenvectors corresponding to $\lambda_1=4$ are
$c\begin{pmatrix}1\\1\end{pmatrix}$ with $c\ne0$, and all eigenvectors corresponding to $\lambda_2=1$ are
$c\begin{pmatrix}1\\-1\end{pmatrix}$ with $c\ne0$.
...
Since $\mathbf A$ is invertible, every $\lambda_i$ is nonzero. Conversely, an eigenvalue $\mu$ of $\mathbf A^{-1}$ gives the eigenvalue $1/\mu$ of $\mathbf A$ by the same argument. Thus, the eigenvalues of $\mathbf{A}^{-1}$ are exactly $\frac{1}{\lambda_i}$ for $i = 1, 2, \ldots, n$.
