...
一个罐中有 $m$ 个黑球与 $l-m$ 个白球，其中 $0<m<l$。独立地进行 $n>0$ 次有放回抽取，并对答案说明理由：

1. 对 $1<k<n$，求第 $k$ 次抽取时首次抽到黑球的概率。
2. 已知首次黑球出现在第 $k$ 次，求余下 $n-k$ 次中至少再出现一个黑球的概率。
3. 定义
...
5. 求
   ...
   $$
   \operatorname{Var}(R)=E[R^2]-E[R]^2.
   $$
...
$$
2\sum_{1\le j<k\le n}jk
=\left(\sum_{j=1}^{n}j\right)^2-\sum_{j=1}^{n}j^2,
$$

the terms containing $p^2\left(\sum j\right)^2$ cancel. Hence

$$
\begin{aligned}
\mathrm{Var}[R]
&=(p-p^2)\sum_{j=1}^{n}j^2\\
&=p(1-p)\frac{n(n+1)(2n+1)}{6}\\
&=\boxed{\frac{m(l-m)}{l^2}\frac{n(n+1)(2n+1)}{6}}.
\end{aligned}
$$
