...
设 $T(n)$ 为处理规模 $n\ge1$ 输入的算法最坏运行时间，$\lfloor x\rfloor$ 表示不超过实数 $x$ 的最大整数，常数 $c\ge1$，且 $T(1)=c$。分别考虑 $n>1$ 时的五个递推：

$$
\begin{aligned}
\text{(1)}\quad&T(n)=T(\lfloor3n/4\rfloor)+cn,\\
\text{(2)}\quad&T(n)=2T(n-1)+cn,\\
\text{(3)}\quad&T(n)=T(n-1)+c(n^2+n),\\
\text{(4)}\quad&T(n)=T(\lfloor n/2\rfloor)+c,\\
\text{(5)}\quad&T(n)=2T(\lfloor n/2\rfloor)+cn.
\end{aligned}
$$
...
Since the exponent $1>\log_ba=0$ and $f(3n/4)=3f(n)/4$, the regularity condition holds and $T(n)=\Theta(n)$.
...
The exact expression is $T(n)=c(2^{n+1}-n-2)$, and hence:
...
& = T(1) + c \sum_{k=2}^{n} (k^2 + k) \\
& = c + c \sum_{k=2}^{n} (k^2 + k)
\end{aligned}
$$
...
$$
\sum_{k=2}^{n} k^2 = \frac{n(n+1)(2n+1)}{6}-1
$$
...
$$
\begin{aligned}
T(n) & = c + c \left( \frac{n(n+1)(2n+1)}{6}
 + \frac{n(n+1)}{2}-2 \right) \\
& = c\left(\frac{n(n+1)(n+2)}{3}-1\right) \\
& = \Theta(n^3)
\end{aligned}
$$
...
- **Recurrence 2**: $T(n) = \Theta(2^n)$, so the smallest listed class is $O(3^n)$.
