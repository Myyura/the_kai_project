...
**Note**: The connectedness claim requires the convention that $G$ has no isolated vertices. Otherwise, an Eulerian cycle together with an isolated vertex is a counterexample. Under the convention implicit in the question, every vertex is incident to an arc.
