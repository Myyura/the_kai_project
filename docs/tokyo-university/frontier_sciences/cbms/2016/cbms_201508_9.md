...
对整数数组 $x$ 排序，并假定比较、加减、内存读取和写入等整数操作各耗费单位时间。围绕上文给出的 C 代码完成以下任务：

1. `merge_two_arrays` 的输入子数组 $x[s,\ldots,m-1]$ 与 $x[m,\ldots,e-1]$ 均已升序排列；补全代码块 (A)、(B)，使其合并得到升序数组 $y[s,\ldots,e-1]$，必要时可填写多行。
2. 临时数组 $y$ 与 $x$ 等长；补全 `merge_sort` 中的代码块 (C)，使函数把输入区间 $x[s,\ldots,e-1]$ 原地更新为升序结果。
3. 求 `merge_sort()` 对长度为 $n$ 的整数数组排序时的最坏时间复杂度。
4. 现有硬件函数 `cmp(x1,x2,x3)`：若三个参数中的最小值依次为 `x1`、`x2`、`x3`，则分别返回 1、2、3；一次 `cmp` 调用及按返回值分支都各耗费单位时间。说明如何利用该三路比较函数加速归并排序。

#### 考点

- **二路归并**：补全两个游标比较、写入输出并推进索引的逻辑，正确处理任一子数组耗尽后的剩余元素。
- **归并排序递归**：递归排序左右半区后合并并回写，建立并求解最坏时间递推式。
- **多路归并优化**：利用一次硬件操作比较三个候选首元素，设计多路划分与合并以减少归并层数或比较开销。
