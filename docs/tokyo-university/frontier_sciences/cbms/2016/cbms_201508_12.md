...
其中 $s(x_i,y_j)$ 是字符配对得分，$d>0$，长度为 $k$ 的间隙得分记为 $g(k)$。完成下列已录入的小问：
...
原 Description 称题目包含第 (1) 至 (5) 问，但现有正文只录入了上述第 (1) 至 (4) 问，未提供第 (5) 问的题干或作答要求。

#### 考点

- **最小编辑距离式动态规划**：把字符配对、在任一序列中插入间隙三种转移统一到二维前缀状态，并正确设置边界。
- **比对复杂度分析**：根据需计算的 $m\times n$ 个状态及每个状态的常数次比较，给出运行时间量级。
- **最优路径回溯**：用前驱指针 $\pi(i,j)$ 保存最优转移，从终点反向恢复一组达到最大得分的全局比对。
