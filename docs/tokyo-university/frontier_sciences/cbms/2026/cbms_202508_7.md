...
1. 求 $A_0$ 的全部特征值与相应特征向量。
2. 求一个非零向量 $\binom xy$，使
   $$
   \lim_{n\to\infty}A_0^n\binom xy=\binom00.
   $$

再设 $A$ 为 $m\times n$ 实矩阵、$B$ 为 $l\times n$ 实矩阵，并假设对每个 $\boldsymbol x$ 都有

$$
A\boldsymbol x=\boldsymbol0
\quad\Longrightarrow\quad
B\boldsymbol x=\boldsymbol0,
$$

即 $\ker A\subseteq\ker B$。证明：

3. $\operatorname{rank}A\ge\operatorname{rank}B$。
4. 存在 $l\times m$ 实矩阵 $C$ 使
   $$
   B=CA.
   $$
...
- **特征分解与矩阵极限**：把初始向量分解到两个特征方向，选择绝对值小于 1 的稳定特征空间以使幂迭代趋零。
- **核空间与秩**：由 $\ker A\subseteq\ker B$ 配合秩—零化度定理比较两个线性映射的秩。
- **映射因子分解**：证明 $B\boldsymbol x$ 只依赖于 $A\boldsymbol x$，先在 $\operatorname{Im}A$ 上定义线性映射，再延拓并写成矩阵 $C$。
...
That is, if we denote the null space (kernel) of $A$ and $B$ as $\ker(A)$ and $\ker(B)$ respectively, then:
