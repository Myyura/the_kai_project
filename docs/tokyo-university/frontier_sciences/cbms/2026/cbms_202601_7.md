...
1. 求 $A$ 的全部特征值。
2. 求每个特征空间的一组基。
3. 求一个正交矩阵 $P$，使 $P^TAP$ 为对角矩阵。
4. 对 $\boldsymbol x=(x_1,x_2,x_3)^T\in\mathbb R^3$，令
   $$
   f(\boldsymbol x)=\boldsymbol x^TA\boldsymbol x.
   $$
   已知使 $f$ 达到最小值的最优解集合在三维空间中是一条直线，写出表示该直线的方程。
5. 对任意实对称方阵，证明若其全部特征值互不相同，则对应的全部特征向量两两正交。

#### 考点

- **实对称矩阵的特征分解**：利用矩阵的全一方向及其正交补求特征值、特征空间，并选取标准正交基。
- **正交对角化**：把单位特征向量按列组成正交矩阵 $P$，验证 $P^TAP$ 的对角形式。
- **二次型最小化**：在正交特征坐标中把二次型写成特征值加权平方和，确定零特征空间形成的最优直线。
- **特征向量正交性**：由对称性比较 $\boldsymbol u^TA\boldsymbol v$ 的两种写法，在特征值不同时推出内积为零。
