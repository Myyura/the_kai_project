...
- 矩阵对角化：由互异特征值及对应特征向量构造相似变换，把矩阵化为对角形。
- 矩阵幂：在特征基下逐项计算对角元的 $n$ 次幂，再变换回原基。
- 可交换算子的同时对角化：利用 $AB=BA$ 说明 $B$ 保持 $A$ 的各一维特征子空间，从而共享特征向量。
- 正定矩阵平方根：在共同的特征基中对 $A$ 的正特征值取正平方根，以构造满足 $B^2=A$ 的正谱平方根。
- 固定迹下的行列式最大化：把 $AX$ 的正特征值之和固定为 $d$，用等分条件或多元极值求其乘积最大值。
