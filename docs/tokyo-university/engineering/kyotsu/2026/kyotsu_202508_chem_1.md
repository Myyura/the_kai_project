...
I. 对区间 $0\le x\le l$ 的一维无限深势阱，电子满足
$$
-\frac{h^2}{8\pi^2m}\psi''=E\psi,\qquad
\psi=A\cos\alpha x+B\sin\alpha x,\quad
\alpha^2=\frac{8\pi^2mE}{h^2},
$$
并有 $\psi(0)=\psi(l)=0$。证明能级
$E_n=n^2h^2/(8ml^2)$；由归一化确定 $A,B$；再把含 11 个共轭双键的 $\beta$-胡萝卜素的 22 个 $\pi$ 电子视为在长度 $L$ 势阱中的粒子，求 HOMO–LUMO 跃迁吸收波长关于 $L,m,h,c$ 的表达式。
...
\ce{CO2* <=> CO2 + *}.
$$
吸附种间无相互作用。用 Gibbs 自由能说明等温等压 CO 吸附为何放热；仅有 CO 吸附时推导朗缪尔覆盖率；当 CO、O、CO2 竞争吸附且表面反应为速率控制步骤、$r=k\theta_{\rm CO}\theta_{\rm O}$ 时，用各平衡常数、气体分压和 $k$ 写出速率式。

III. 对 $\ce{E+S <=> ES -> E+P}$，在结合步骤平衡近似下，用 $K_{\rm S},V_{\max},[\mathrm S]$ 写出速度 $v$。随后分别讨论抑制剂只结合游离酶 $\ce{E+I<=>EI}$ 和只结合酶—底物复合物 $\ce{ES+I<=>ESI}$ 两种情形，推导速度式并画出对应 Lineweaver–Burk 双倒数图。

#### 考点

- 一维势阱与 HOMO–LUMO 跃迁：由边界条件和归一化量子化波函数，再按电子占据确定最高占据与最低空能级间距。
- 朗缪尔—欣谢尔伍德 CO 氧化：用竞争吸附平衡和位点守恒求各覆盖率，并代入表面双分子反应速率。
- 酶抑制与 Lineweaver–Burk 图：区分竞争抑制和反竞争抑制，比较双倒数直线的斜率与截距变化。
