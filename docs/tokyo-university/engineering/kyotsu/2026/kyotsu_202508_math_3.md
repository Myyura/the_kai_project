...
- Möbius 变换：求逆变换并把半平面不等式化成圆盘方程，确定边界对应关系。
- Laurent 级数：在本性奇点处展开指数函数，逐项识别负幂和 $z^{-1}$ 系数。
- 留数定理：由单位圆内唯一奇点的留数直接计算闭合积分。
- 调和函数与调和共轭：识别实部中的 $\operatorname{Re}z^n$ 结构，或用 Cauchy–Riemann 方程恢复虚部。
