...
- Fourier 变换：直接积分矩形函数，并用频移性质处理指数衰减函数乘余弦。
- 卷积：识别高斯函数与 $e^{-|x|}$ 的卷积，用卷积定理把变换化为乘积。
- Fourier 级数：利用奇偶性和正交积分求阶跃函数及半频余弦在 $2\pi$ 周期延拓下的系数。
- 由 Fourier 级数求无穷级数：在合适的连续点代入级数，得到交错奇数倒数和。
