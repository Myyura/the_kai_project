...
$$
先假设存在二次多项式特解
$y=A_0+A_1x+A_2x^2$，求三个系数；再利用该特解把 Riccati 方程化为一阶线性方程，求一般解。解答须在原方程和所得表达式有定义的各区间内理解，并保留代换过程中被排除的特解。

#### 考点

- 二阶线性常微分方程：由特征方程求齐次通解，再叠加与指数型右端相配的特解。
- 柯西—欧拉方程：以 $y=x^m$ 求幂函数基本解，并用同型幂函数处理非齐次项。
- Riccati 方程：先找一个多项式特解，再令 $y=y_p+1/u$ 化为带积分因子的一阶线性方程。
