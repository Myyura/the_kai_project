...
- 信道容量：由输入输出熵和条件熵构造互信息，并对输入分布优化可传输信息率。
- 有记忆有限状态信道：根据内部状态随历史输入或输出的演化，分析多次使用信道时的可辨识码字和容量。
- $z$ 变换：把离散时间系统的差分关系转换为传递函数，并由此求冲激响应和频率响应。
- 无限冲激响应滤波器的传递函数与实现：比较不同反馈结构的传递函数参数，并进行减少延时单元的等效实现。
