...
- 全加器与临界路径：由真值表选择逻辑门实现半加器、全加器，并累加门延迟确定最慢输入—输出路径。
- 行波进位加法器：串接各位全加器，让进位逐级传播并分析随位数增长的延迟。
- 组合二进制乘法器：用与门生成部分积，再以半加器和全加器归约并确定输出位宽。
- 二叉搜索树的插入与删除：按键值递归定位插入位置，并区分零、一、两个孩子的删除情形重接子树。
- 二叉树中序遍历：按左子树、根、右子树递归访问，即可升序枚举搜索树键值。
- 树排序复杂度：结合树高分析查找和插入；平衡时平均为 $O(\log n)$，退化时需注意最坏 $O(n)$。
