...
## **Author**
[donguri0912](https://sekakota.hatenablog.com/entry/eeis-2022-4), [meitmt](https://github.com/meitmt), 祭音Myyura
...
I. 可用 AND、OR、NOT、XOR 门的传播延迟分别为 $50$ ps、$50$ ps、$30$ ps、$80$ ps，忽略连线延迟；临界路径须以输入—输出对表示，若有多条须全部列出。

1. 对图 2 的全加器：
   - 为由 $A,B$ 产生和 $D$、进位 $E$ 的半加器 $X$ 写真值表；先仅用 AND、OR、NOT 门实现并给出临界路径及延迟。
   - 允许使用全部门时，设计临界路径更短的半加器并给出路径和延迟。
   - 以 $CI,D,E$ 为输入、$S,CO$ 为输出，写出后级 $Y,Z$ 的真值表并实现最小临界路径电路；再结合前一小问的 $X$，求整个全加器的临界路径和延迟。
2. 用上述全加器构成带进位输入的无符号三位加法器，输入为 $a_2a_1a_0,b_2b_1b_0,ci$，输出为 $s_2s_1s_0,co$；画图并求临界路径及延迟。
3. 设计三位数 $a_2a_1a_0$ 与二位数 $b_1b_0$ 的无符号组合乘法器：先确定积 $p$ 所需位宽 $N$；再至少各用一个半加器和全加器实现，尽量缩短临界路径，并列出路径及延迟。

II. 使用 C 结构体结点构成二叉搜索树管理学号 `sid`，每个结点的值大于左子树所有值、小于右子树所有值。

1. 补全递归函数 `add()` 的空格 A、B、C，使其插入新结点。
2. 从空树开始，分别按两组给定顺序插入 `1040,1042,2001,2004,2010,2012` 和 `2001,1042,2010,1040,2004,2012`，画出结果。
3. 补全 `enumerate()` 的 D、E、F、G，使其按 `sid` 从小到大输出结点。
4. 分别说明删除叶结点、只有一个孩子的结点和有两个孩子的结点的步骤。
5. 说明插入已有 `sid` 时 `add()` 的行为及其问题。
...
(1-iii) の全加算器で A から CO の伝搬遅延時間は $180\text{ps}$ なので、
...
追加する `new` の `left` と `right` はあらかじめ `NULL` に初期化する。
...
空の木にも対応させる場合は、元の `enumerate()` の冒頭に `if (root == NULL) return;` を追加する。ここでの空欄 D〜G は、呼び出し時の `root` が非 NULL であることを前提としている。

#### (4)
$0$ 個の場合、親からそのノードへのリンク（根なら根ポインタ）を `NULL` に変更し、そのノードを解放する。
...
問題点は一つの sid を複数個保持できず、`new` を別途解放しなければ未使用ノードのメモリが残ることである。
...
ただし $O(\log n)$ は木が十分平衡な場合であり、偏った木では探索・追加とも最悪 $O(n)$ となる。
