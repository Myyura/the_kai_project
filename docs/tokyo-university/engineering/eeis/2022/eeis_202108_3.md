...
- 熵：根据二元信源概率计算单个符号的平均信息量。
- 信源扩展：把两个独立输出合并为四元符号，并据联合概率计算平均码长。
- 霍夫曼编码：按符号概率构造高效前缀码，使平均码长尽量接近熵。
- 互信息：由 $H(Y)-H(Y\mid X)$ 量化输入经非对称信道后保留的信息。
- 信道容量：把互信息写成输入概率 $p$ 的函数，并通过优化判断容量对应的输入分布。
- 因果响应与希尔伯特变换关系：利用因果性从频率响应的实部恢复虚部，体现幅相分量间的约束。
- 单边带调制：通过正交载波与希尔伯特变换构造相消项，只保留载频一侧的频谱。
