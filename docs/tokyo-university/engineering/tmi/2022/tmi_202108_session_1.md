...
- Bernoulli 方程：用负幂代换把非线性一阶方程化为线性方程，再反代得到一般解。
- 矩阵对角化：由互异特征值和特征向量构造特征基。
- 矩阵幂：在对角基中对特征值取 $n$ 次幂，再变换回原坐标。
- 高斯积分：把二维积分化为极坐标径向积分，并通过缩放得到一般参数的一维积分。
- 诊断检验的阳性预测值：应用 Bayes 公式综合患病率、灵敏度和假阳性率。
- 约束优化：把假阳性率代入阳性预测值，使其成为区间上的单变量函数并求最大点。
