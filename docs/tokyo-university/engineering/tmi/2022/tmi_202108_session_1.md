...
1. 对 Bernoulli 型方程
   ...
   令 $u=y^{-3}$，把它化为关于 $u$ 的一阶线性方程，并据此求原方程的一般解。
2. 对

   $$
   A=\begin{pmatrix}1&-1&-2\\2&4&2\\1&1&4\end{pmatrix},
   $$

   求全部特征值和对应特征向量，再利用对角化结果求自然数次幂 $A^n$。
3. 先用极坐标计算二维高斯积分

   $$
   I_1=\int_{-\infty}^{\infty}\int_{-\infty}^{\infty}
   e^{-(x^2+y^2)/2}\,\mathrm dx\,\mathrm dy,
   $$

   再利用结果求正参数 $a$ 下

   $$
   I_2=\int_{-\infty}^{\infty}e^{-ax^2}\,\mathrm dx.
   $$

4. 某病毒人群感染率为 $x=0.001$，感染者检测阳性的概率为 $y$，未感染者假阳性的概率为 $z$。当 $y=0.8,z=0.001$ 时，求阳性者实际感染的概率；再在
   $z=0.001y^2+0.0005$、$0\le y\le1$ 的约束下，求使这一阳性预测值最大的 $y$。
...
ただし、この置換では $y=0$ を除いているので、$3x+C\ne0$ の各区間上の上記解に加えて $y\equiv0$ も解である。
...
$$
I_1=\int_0^{2\pi}\int_0^\infty e^{-r^2/2}r\,dr\,d\theta=2\pi.
$$

#### 2.

$u=\sqrt{2a}\,x$ とおけば、問 1 より $\int_{-\infty}^{\infty}e^{-u^2/2}du=\sqrt{2\pi}$ だから、

$$
I_2=\frac{1}{\sqrt{2a}}\sqrt{2\pi}=\sqrt{\frac{\pi}{a}}.
$$
...
P(\text{感染}\mid\text{陽性})
=\frac{xy}{xy+(1-x)z}
=\frac{0.001\cdot0.8}{0.001\cdot0.8+0.999\cdot0.001}
=\frac{800}{1799}\simeq0.445.
$$

#### 2.

$x$ は一定なので、上の確率を最大化するには

$$
\frac{y}{z}=\frac{y}{0.001y^2+0.0005}
$$

を最大化すればよい。その導関数の符号は $0.0005-0.001y^2$ の符号に一致するから、
