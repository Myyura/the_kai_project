...
- 二阶线性常微分方程：由特征根构造齐次通解，并与各非齐次特解叠加。
- 待定系数法：分别为三角函数和“多项式乘指数”右端选择试探形式并解系数。
- 含参数矩阵的可对角化性：按参数导致的特征值重合分类，比较代数重数和特征空间维数。
- Gamma 分布：由密度归一化及 Gamma 积分计算形状参数为 2 时的矩。
- 生日碰撞问题：先计算所有登录秒互异的概率，再取补集并用对数比较阈值。
