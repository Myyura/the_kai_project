...
3. 举出 $K/\mathbb{Q}$ 的两个中间域 $M_1,M_2$，使它们各自在 $\mathbb{Q}$ 上都是次数为 $8$ 的非交换伽罗瓦扩张，并且群 $\mathrm{Gal}(M_1/\mathbb{Q})$ 与 $\mathrm{Gal}(M_2/\mathbb{Q})$ 不同构。
...
まず $S$ は $\mathbb{Q}$ 上 $4$ 次巡回拡大で、その唯一の非自明な真の中間体は $\mathbb{Q}(\sqrt{2})$ である。一方、$\beta=\sqrt{3+\sqrt3}$ とおくと、$3+\sqrt3$ の $\mathbb Q(\sqrt3)/\mathbb Q$ に関するノルムは $6$ で平方数でないから、$[T:\mathbb Q]=4$ である。
