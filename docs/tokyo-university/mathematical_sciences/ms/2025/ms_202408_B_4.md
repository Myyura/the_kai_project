...
tags:
  - Tokyo-University
  - Mathematics.Abstract-Algebra.Galois-Group-and-Intermediate-Fields-of-Nested-Radicals
---
# 東京大学 数理科学研究科 数理科学専攻 2024年8月実施 専門科目B 第4問

## **Author**
[藍色日和](https://mathlog.info/articles/8NhyTodyCA2cQQEYMUWg), 祭音Myyura
...
(3) $K/\mathbb{Q}$ の中間体 $M_1,M_2$ で、いずれも $\mathbb{Q}$ 上の非可換ガロア拡大であり、拡大次数が等しく、かつ $\mathrm{Gal}(M_1/\mathbb{Q})$ と $\mathrm{Gal}(M_2/\mathbb{Q})$ が群同型でないようなものの例を挙げなさい。
...
1. 证明扩张 $K/\mathbb{Q}$ 是伽罗瓦扩张，并计算扩张次数 $[K:\mathbb{Q}]$。
2. 列出 $K/\mathbb{Q}$ 的全部中间域中在 $\mathbb{Q}$ 上次数为 $4$ 的那些中间域；列举时，每个中间域都必须用 $K$ 中某个元素 $\alpha$ 写成 $\mathbb{Q}(\alpha)$ 的形式。
3. 举出 $K/\mathbb{Q}$ 的两个中间域 $M_1,M_2$，使它们各自在 $\mathbb{Q}$ 上都是非交换伽罗瓦扩张，扩张次数相等，并且群 $\mathrm{Gal}(M_1/\mathbb{Q})$ 与 $\mathrm{Gal}(M_2/\mathbb{Q})$ 不同构。
...
$\alpha=\sqrt{2+\sqrt2}$、$\alpha'=\sqrt{2-\sqrt2}$ とおく。$\alpha$ の多項式 $X^4-4X^2+2$ は素数 $2$ に関する Eisenstein の判定法で既約である。$\alpha'=\sqrt2/\alpha\in S$ であり、$\alpha\mapsto\alpha'\mapsto-\alpha\mapsto-\alpha'\mapsto\alpha$ は位数 $4$ の自己同型を与える。従って $S/\mathbb Q$ は $4$ 次巡回拡大で、その唯一の非自明な真の中間体は $\mathbb Q(\sqrt2)$ である。一方、$\beta=\sqrt{3+\sqrt3}$ とおくと、$3+\sqrt3$ の $\mathbb Q(\sqrt3)/\mathbb Q$ に関するノルムは $6$ で平方数でないから、$[T:\mathbb Q]=4$ である。

$\sqrt2\in T$ と仮定すると、$E=\mathbb Q(\sqrt2,\sqrt3)\subset T$ かつ $[E:\mathbb Q]=[T:\mathbb Q]=4$ より $T=E$ となる。しかし $\beta=u+\sqrt2v$ $(u,v\in\mathbb Q(\sqrt3))$ と書いて平方すると $uv=0$ である。$N=N_{\mathbb Q(\sqrt3)/\mathbb Q}$ とおけば、$v=0$ なら $N(u)^2=6$、$u=0$ なら $N(v)^2=3/2$ となり、いずれも $N(u),N(v)\in\mathbb Q$ に矛盾する。従って $\sqrt2\notin T$ である。

また $T$ がガロアなら $\sqrt{3-\sqrt3}\in T$ なので、$\sqrt6\in T$、従って $\sqrt2\in T$ となり矛盾する。ゆえに $T$ はガロアでない。以上と $S$ の中間体の記述から $S\cap T=\mathbb Q$ が従う。よって
...
で定義される $\sigma_{a,b,c,d}$ $(a,b,c,d\in\{0,1\})$ 全体の集合で書ける。$G=\mathrm{Gal}(K/\mathbb{Q})$ の部分群
...
である。$N$ の生成元を $x=\sigma_{0,0,0,1}$、$H$ の生成元を $y=\sigma_{0,1,0,0}$ と取ると、$G$ は表示
...
とおく。$K_7$ の生成元は $\sqrt6\alpha\in K$ である。各生成元を固定する自己同型は次の通りである。添字の条件は $\mathbb Z/2\mathbb Z$ で考える。

| 中間体 | 固定する $\sigma_{a,b,c,d}$ の条件 |
| --- | --- |
| $K_1$ | $b=d=0$ |
| $K_2$ | $a=b=0$ |
| $K_3$ | $c=d=0$ |
...
これらは互いに異なる位数 $4$ の部分群なので、ガロア対応により $K_1,\ldots,K_7$ は互いに異なる $4$ 次中間体である。位数 $4$ の部分群は全部で $7$ 個だから、これで全てである。
...
は位数の同じ群の間の全射準同型を定めているから同型であり、よって $G_2$ は四元数群 $Q_8$ に等しい。
...
実際、$\sqrt2,\beta$ を共に固定する自己同型の条件は $b=c=d=0$ なので、その固定群は $\{1,y^2\}$ である。また $w=\alpha\beta$ とおくと

$$
\sqrt3=\frac{w^2}{2+\sqrt2}-3.
$$

従って $\sqrt2,w$ を共に固定する条件は $b=d=0,\ a=c$ であり、その固定群は $\{1,x^2y^2\}$ である。よって上記二つの体はともに $\mathbb Q$ 上 $8$ 次で、所望の固定体に等しい。
