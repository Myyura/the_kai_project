...
- 迭代正弦函数的一致收敛：需要利用正弦迭代的单调性、对称性以及对全体 $x\in\mathbb{R}$ 都成立的统一上界，判断 $f_n$ 的一致极限。
- LogSumExp 的一致逼近：需要把 $\frac1n\log(e^{nx}+e^{ny}+e^{nz})$ 与 $\max\{x,y,z\}$ 比较，并给出与 $(x,y,z)$ 无关、随 $n$ 趋于零的误差界。
