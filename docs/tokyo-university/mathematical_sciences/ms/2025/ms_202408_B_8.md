...
即 $S(r)$ 是 $\mathbb{R}^n$ 中半径为 $r$ 的球面；并用 $\Omega^k(\mathbb{R}^n)$ 表示 $\mathbb{R}^n$ 上所有 $C^\infty$ 级 $k$-形式组成的集合。回答下列问题：
...
1. 证明对任意 $\omega\in\Omega^{n-1}(\mathbb{R}^n)$，当 $r\to0^+$ 时，
   ...
   收敛到某个有限值。
2. 设 $k$ 为正常数。求所有满足下述条件的实数 $a$ 的范围：对任意 $\omega\in\Omega^{n-1}(\mathbb{R}^n)$，当 $r\to0^+$ 时，
...
### (2)
以下、$x_1^k$ が $\mathbb R^n$ 上の滑らかな実関数となる自然な解釈として $k\in\mathbb Z_{>0}$ とする。
