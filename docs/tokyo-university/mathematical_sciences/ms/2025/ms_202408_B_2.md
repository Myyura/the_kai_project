...
## **Author**
[藍色日和](https://mathlog.info/articles/y2uBoNYg240OJG9vgrLF), 祭音Myyura
...
(1) 将 $B$ 表示为若干个域的直积 $\prod_i K_i$。

(2) 令 $p_i:B\twoheadrightarrow K_i$ 为自然投影。令 $C_i$ 为 $K_i$ 中由 $p_i\!\left(\frac{x}{2}\right)$ 在 $\mathbb{Z}$ 上生成的子代数，并令 $C=\prod_i C_i$。证明在嵌入

$$
A\subseteq B\simeq\prod_i K_i
$$

下，$A$ 的像包含于 $C$；并把作为 $\mathbb{Z}$-模的商 $C/A$ 表示为循环群的直积。

(3) 将单位群 $A^\times$ 表示为循环群的直积。
...
を満たす一方、$a-b\in4\mathbb{Z}$ かつ $c\in2\mathbb{Z}$ を満たす整数 $a,b,c$ を任意に取ったとき、
