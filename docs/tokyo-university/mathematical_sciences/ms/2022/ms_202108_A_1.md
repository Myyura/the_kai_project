...
tags:
  - Tokyo-University
  - Mathematics.Linear-Algebra.Simultaneous-Diagonalization-of-Parameterized-Operators
---
...
设 $V$ 为以 $x$ 为变量、次数不超过 $2$ 的全体实系数多项式组成的实线性空间。设 $a,b$ 为实数，并定义线性映射 $S,T:V\to V$：

$$
S(f(x))=\frac{d}{dx}\bigl((ax+2)f(x)\bigr),\qquad
T(f(x))=(bx+3)\frac{d}{dx}f(x)
\qquad (f(x)\in V).
$$

(1) 求关于 $a$ 的充要条件，使得存在一组能够将 $S$ 对角化的 $V$ 的基底。

(2) 求关于 $a,b$ 的充要条件，使得存在一组能够同时将 $S$ 与 $T$ 对角化的 $V$ 的基底。
...
とおくと、 $v=w=0$ を得るので、固有空間は1次元であることがわかる。
...
十分性は $3a=2b$ のとき $T=\frac32S-\frac{3a}{2}I$ となることからも従う。$a\ne0$ なら $S$ の固有基底はそのまま $T$ の固有基底でもある。
