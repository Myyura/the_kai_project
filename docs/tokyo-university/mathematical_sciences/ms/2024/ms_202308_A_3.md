...
tags:
  - Tokyo-University
  - Mathematics.Geometry.Maximum-Size-of-Pairwise-Obtuse-Vector-Set
---
# 東京大学 数理科学研究科 数理科学専攻 2023年8月実施 専門科目A 第3問

## **Author**
[藍色日和](https://mathlog.info/articles/EsejOfCbSTU4s97NcVNq), 祭音Myyura
...
考虑带有标准内积的线性空间 $\mathbb{R}^n$。求满足下述条件的自然数 $m$ 的最大值：

- 存在向量 $v_1,\ldots,v_m\in\mathbb{R}^n$，使得对任意互不相同的 $i,j$，都有 $v_i\cdot v_j<0$。
...
を満たすような $(a_1,\cdots,a_{n+2})$ 全体の集合 $W$ を考える。この集合は次元 $\geq2$ の線型空間である。線型写像 $W\to\mathbb R$、$a\mapsto\sum_i a_i$ の核は少なくとも1次元なので、その核から非零の元を取れば正と負の成分をともにもつ。したがって、$W$ の元 $a=(a_1,\cdots,a_{n+2})$ で、$a_i>0$ を満たすような $i$ と $a_j<0$ を満たすような $j$ が一つ以上存在するようなものを取ることができ、これを一つ固定する。いま
...
$$
\varepsilon<\sqrt{\min_{i,j}\left\{|v_i\mathbin{\cdot}v_j|\middle|i\neq j\right\}}
$$
