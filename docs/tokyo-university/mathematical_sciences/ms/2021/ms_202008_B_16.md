...
tags:
  - Tokyo-University
  - Physics.Mechanics.Small-Oscillations-of-a-Chain-Pendulum
  - Mathematics.Linear-Algebra.Generalized-Eigenvalue-Characteristic-Polynomial-Recurrence
---
...
准备 $N$ 个质量均为 $m$ 的重物，如图所示，用 $N$ 根长度均为 $l$ 的细绳依次连接，并悬挂在天花板上。细绳始终绷直且不能伸缩；绳的质量可以忽略；各重物只在一个包含竖直轴的固定平面内运动。以 $t$ 表示时间，以 $g$ 表示重力加速度。

在运动所在的平面内建立图示的 $xy$ 坐标系：以天花板与最上方细绳的固定点为原点，水平方向为 $x$ 轴，竖直向下为 $y$ 轴。由上向下数第 $i$ 个重物的水平位置坐标记为 $x_i$。

(1) 假设所有 $|x_i|$ 都充分小，求该系统的拉格朗日量 $L$，以及由 $L$ 得到的 Euler–Lagrange 方程。这里要求所得的 Euler–Lagrange 方程为线性微分方程。

(2) 考虑 (1) 中线性微分方程的如下形式的解：

$$
x_i=A_i\sin\omega t \qquad (i=1,2,\ldots,N),
$$

其中 $\omega$ 为常数。证明：存在一个首项系数为 $1$ 的整系数 $N$ 次多项式 $P_N(X)$，使上述形式的解存在的充要条件可以写成

$$
P_N\left(\omega^2\frac{l}{g}\right)=0.
$$

(3) 求一个能够刻画多项式序列

$$
\{P_N(X)\mid N=1,2,3,\ldots\}
$$
...
## **Kai**
### (1)
以下の成分表示は $N\ge2$ に対するものである。$N=1$ では $L=\frac{m}{2}\dot x_1^2-\frac{mg}{2l}x_1^2+\text{定数}$、$\ddot x_1=-(g/l)x_1$ となる。
...
初期条件は $P_0(X)=1$、$P_1(X)=X-1$ である。同じ展開により漸化式は $N=0$ から成立し、特に $P_2(X)=X^2-4X+2$ となる。これらの初期条件と漸化式が多項式列を一意に定める。
