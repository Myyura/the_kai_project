...
若一个下三角矩阵的所有对角元都等于 $1$，则称其为单位下三角矩阵。回答下列问题。

（1）设 $L,L'$ 是两个 $n\times n$ 下三角矩阵，证明乘积 $LL'$ 仍是下三角矩阵。

（2）设 $L,L'$ 是两个 $n\times n$ 单位下三角矩阵，证明 $LL'$ 仍是单位下三角矩阵。

（3）分别求下列矩阵的逆矩阵：

$$
L_1=\begin{pmatrix}1&0\\2&1\end{pmatrix},\qquad
L_2=\begin{pmatrix}1&0&0\\2&1&0\\3&2&1\end{pmatrix}.
$$

（4）设可逆矩阵 $A\in\mathbb{R}^{n\times n}$ 有两种分解

$$
A=LU=L'U',
$$

其中 $U,U'$ 为上三角矩阵，$L,L'$ 为单位下三角矩阵。证明 $L=L'$ 且 $U=U'$。
...
For $k<i$, $L'_{k,i}=0$, and for $k>i$, $L_{i,k}=0$. Hence

$$
(LL')_{i,i}
=\sum_{k=1}^{n}L_{i,k}L'_{k,i}
=L_{i,i}L'_{i,i}=1.
$$
...
Since $A$ is invertible and $L,L_1$ are unit lower triangular, $U=L^{-1}A$ and $U_1=L_1^{-1}A$ are invertible.
