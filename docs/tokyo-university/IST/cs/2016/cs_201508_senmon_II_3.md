...
（1）用字符串对象（字符数组）的链表表示 $S$，给出内存量和平均查询复杂度，并简要说明理由。

（2）用开放寻址哈希表表示 $S$，给出内存量、平均查询复杂度，并简要说明理由；可以自行决定哈希函数定义等细节。

（3）用按字典序比较字符串、每个结点保存一个字符串的二叉搜索树表示 $S$，回答同样问题并简要说明理由；按给定顺序插入 $S_0$ 并画出树。

（4）用 trie 表示 $S$。根到叶的路径对应一个字符串，每个内部结点保存含 $m$ 个子指针的数组。回答同样问题并简要说明理由，画出 $S_0$ 的 trie。
...
$\boxed{\Theta(n\ell+n)}$。一次查询最多比较 $n$ 个字符串；与第 $i$ 个字符串比较至多需要 $O(1+\min(|w|,|w_i|))$ 时间。因此总查询时间的上界为

$$
O\left(\sum_{i=1}^{n}(1+|w_i|)\right)=\boxed{O(n(\ell+1))}.
$$
...
取表长 $T=\Theta(n)$ 并保持负载因子 $\alpha=n/T\le\alpha_0<1$，其中 $\alpha_0$ 为固定常数。采用开放寻址，并在均匀散列模型下分析：每个键的探测序列为表槽的独立均匀随机排列，计算字符串的散列值需 $O(|w|+1)$，生成后续探测位置需 $O(1)$。

在此模型下，失败查询的期望探测次数至多为 $1/(1-\alpha)$；成功查询也只需常数次期望探测。每次字符串比较需 $O(|w|+1)$，所以平均查询时间为 $\boxed{O(\ell+1)}$。内存为字符串的 $\Theta(n\ell)$ 加表槽的 $\Theta(n)$，即 $\boxed{\Theta(n\ell+n)}$。

### (3)

内存仍为 $\boxed{\Theta(n\ell+n)}$。树高为 $h$ 时，查询一个长度为 $|w|$ 的字符串需 $O((|w|+1)(h+1))$，因而平均为 $O((\ell+1)(h+1))$。平衡树的平均查询时间为 $\boxed{O((\ell+1)\log(n+1))}$；随机插入次序也给出这一期望上界，退化时则为 $O(n(\ell+1))$。
