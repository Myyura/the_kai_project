...
### (2)
For $N\geq1$, the minimum height is $1$: one root with $2^N-1$ leaves. (For $N=0$, it is $0$.)
...
### (4)
Define the new **MERGE**$(A,B)$ by attaching the root of the smaller-height tree below the root of the other tree. This is union by rank, where the rank is an upper bound on the height. The rank increases only when the two ranks are equal.

To maximize the height, merge trees of the same height: merging two height-$h$ trees produces a height-$(h+1)$ tree. Thus a maximum-height tree with $2^N$ nodes is obtained recursively from two maximum-height trees with $2^{N-1}$ nodes, and

$$
H(2^N)=1+H(2^{N-1})=N.
$$

Equivalently, a root of rank $h$ contains at least $2^h$ nodes, so no tree with $2^N$ nodes can have height greater than $N$. Equal-rank merges attain this bound.
