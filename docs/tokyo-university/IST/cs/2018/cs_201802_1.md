...
记 $\mathbb{R}$ 为实数集，$\mathbb{R}^N$ 为 $N$ 维实列向量集合；
$\boldsymbol{v}^{\mathsf T}$ 表示 $\boldsymbol{v}$ 的转置，$I$ 为
$N\times N$ 单位矩阵。考虑实对称矩阵 $A\in\mathbb{R}^{N\times N}$ 的特征方程
$$
A\boldsymbol{x}=\lambda\boldsymbol{x},
$$
并以 $\lambda_{\max}(M)$ 表示矩阵 $M$ 的最大特征值。

可以不加证明地使用：实对称矩阵有 $N$ 个线性无关的特征向量并构成正交基，且其所有特征值均为实数。回答下列问题。

（1）证明：若 $\boldsymbol{x}$ 是 $A$ 的特征向量，则对任意
$\mu\in\mathbb{R}$，它也是 $A+\mu I$ 的特征向量。

（2）证明 Rayleigh 商刻画
$$
\lambda_{\max}(A)
=\max\{\boldsymbol{v}^{\mathsf T}A\boldsymbol{v}
\mid \boldsymbol{v}\in\mathbb{R}^N,
\boldsymbol{v}^{\mathsf T}\boldsymbol{v}=1\}.
$$

（3）证明对任意 $\boldsymbol{v}\in\mathbb{R}^N$，
$$
\boldsymbol{v}^{\mathsf T}
\bigl(\lambda_{\max}(A)I-A\bigr)\boldsymbol{v}\ge0.
$$

（4）设 $B$ 也是 $N\times N$ 实对称矩阵，证明
$$
\lambda_{\max}(A+B)
\le\lambda_{\max}(A)+\lambda_{\max}(B).
$$

#### 考点

- **实对称矩阵的特征分解**：在正交特征向量基下展开向量，分析平移矩阵的特征值。
- **Rayleigh 商**：证明单位球面上二次型的最大值等于最大特征值。
- **半正定矩阵与谱不等式**：识别 $\lambda_{\max}(A)I-A$ 的半正定性，并据此证明最大特征值的次可加性。
