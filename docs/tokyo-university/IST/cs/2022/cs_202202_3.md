...
$$
L=\{w^Rbaw\mid w\in\Sigma^*\}
$$

不是正则语言；可以使用正则语言泵引理。其中 $w^R$ 表示 $w$ 的逆序，例如
$(abb)^R=bba$。

（4）判断第（3）问的 $L$ 是否为上下文无关语言。若是，构造生成 $L$ 的上下文无关文法；若不是，证明其非上下文无关性。
...
Indeed, $Q_0,Q_1,Q_2$ are pairwise distinguishable: $Q_2$ is accepting,
while the suffix $a$ is accepted from $Q_1$ but not from $Q_0$.
...
The pumped word contains a unique $b$, so in any representation
$u^Rbau$ that $b$ must be the delimiter.  The two surrounding $a$-blocks would then have to have equal length, but their lengths are $p+j$ and $p$.  Hence the pumped word is not in $L$, a contradiction.
...
This CFG generates strings of the form $w^{\mathbf{R}}baw$ by recursively placing matching symbols around the center substring $ba$.
