...
设 $G=(V,E)$ 为简单无向图，
$V=\{v_i\mid i=1,\ldots,n\}$。对
$\boldsymbol{x}=(x_1,\ldots,x_n)\in\{-1,+1\}^n$，定义
$$
f_G(\boldsymbol{x})
=\sum_{(v_i,v_j)\in E}\frac{1-x_ix_j}{2}.
$$
回答下列问题。

（1）当 $G$ 为 $n$ 个顶点的完全图 $K_n$ 时，求
$$
a_n=\max_{\boldsymbol{x}\in\{-1,+1\}^n}f_G(\boldsymbol{x}).
$$

（2）令 $b_n$ 为 $K_n$ 的边数，计算
$$
\lim_{n\to\infty}\frac{a_n}{b_n}.
$$

（3）对任意简单无向图 $G$，若各 $x_i$ 独立地以 $1/2$ 概率取
$-1$ 或 $+1$，求 $f_G(\boldsymbol{x})$ 的期望。可以使用期望的线性性。

（4）证明对任意简单无向图 $G$，存在
$\boldsymbol{x}\in\{-1,+1\}^n$ 使
$$
f_G(\boldsymbol{x})\ge\frac{|E|}{2}.
$$
...
The odd case differs from $n^2/4$ by only $1/4$, so after division by $b_n$ the difference tends to zero. Thus both parities have the same limit:
