...
- **有限状态转导**：输出每个位置是否匹配固定模式，并处理重叠匹配。
- **像语言的自动机构造**：用有限状态保存模式匹配窗口，同时模拟原 DFA，构造输出字母表上的 NFA。
- **正则表达式**：刻画固定模式 `aba` 的所有可能起点标记序列。
- **上下文无关语言闭包性质**：判断有限状态转导下的像是否保持上下文无关性，并以 PDA/CFG 乘积构造论证。
