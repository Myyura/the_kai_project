...
- **Instruction 5** uses `x4` from **Instruction 1** in the first iteration: `5 → 1`
- **Instruction 5** in iteration $n$ uses `x4` from **Instruction 5** in iteration $n-1$: `5 → 5` (loop dependency)
- **Instruction 4** in iteration $n$ uses `x5` from **Instruction 4** in iteration $n-1$: `4 → 4` (loop dependency)
...
**Execution Cycles**:
- Dynamic instructions: $3+16\times4+1=68$.
- With no stalls, a five-stage pipeline takes $68+4=72$ cycles.
- The 16 load-use hazards add $16$ cycles.
- The 16 branches add $16\times2=32$ cycles.

Total clock cycles: $72+16+32=120$ cycles.

**Average IPC Calculation**:
- Total instructions: $3 + 16 \times 4 + 1 = 68$
- IPC: $\frac{68}{120} \approx 0.57$

Equivalently, the issue stream is Instructions 0, 1, 2, followed by 16 repetitions of `(Instruction 3, one load-use stall, Instructions 4, 5, 6, two branch stalls)`, and finally Instruction 7; four cycles are then needed to drain the pipeline.
...
A two-bit saturating counter in the BHT quickly learns that Instruction 6 is taken for the first 15 executions; a branch-target buffer supplies the target `Loop`, so Instruction 3 can be fetched speculatively instead of waiting two cycles. On the final not-taken execution, the prediction may be wrong and the speculative instructions are flushed. The exact cycle count requires the predictor's initial state, target-prediction mechanism, and misprediction penalty, none of which are specified.
