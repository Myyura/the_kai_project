...
tags:
  - Tokyo-University
  - Mathematics.Linear-Algebra.Lower-Upper-Decomposition
...
对矩阵 $M$，以 $M_{i:j,k:l}$ 表示由第 $i$ 至 $j$ 行、第 $k$ 至
$l$ 列组成的子矩阵；$M_{i,k:l}$ 和 $M_{i:j,k}$ 分别表示相应行向量、列向量。

设 $A$ 是可作 LU 分解的 $n\times n$ 非奇异带状矩阵：存在正整数 $w$，使
$|i-j|>w$ 时 $A_{i,j}=0$。记
$A=LU$，其中 $L$ 为单位下三角矩阵，$U$ 为上三角矩阵。题中原地算法 P 对
$k=1,\ldots,n-1$ 依次执行
...
终止后 $A$ 的严格下三角部分存放 $L$ 的相应元素，上三角部分存放 $U$。
...
（2）证明当 $|i-j|>w$ 时，$L_{i,j}=0$ 且 $U_{i,j}=0$。

（3）设 $n\gg1$、$w\ll n$、$m\gg n$，需要求解
$Ax_i=b_i\ (i=1,\ldots,m)$。从算术运算量比较：

- 先求 $A^{-1}$，再计算每个 $x_i=A^{-1}b_i$；
- 先对 $A$ 作 LU 分解，再分别前代求 $Ly_i=b_i$、回代求 $Ux_i=y_i$。
...
$$
A_{i,k}=0\quad(i>k+w),\qquad
A_{k,j}=0\quad(j>k+w).
$$

The multipliers stored in column $k$ therefore vanish below row $k+w$.  Moreover, the rank-one update
$A_{i,j}\leftarrow A_{i,j}-A_{i,k}A_{k,j}$ can be nonzero only when
$k<i,j\le k+w$; such indices satisfy $|i-j|<w$.  Thus the update creates no entry outside the band.  Induction over $k$ proves
$L_{i,j}=U_{i,j}=0$ whenever $|i-j|>w$.

### (3)

By (2), one elimination step updates at most $w^2$ entries, so the LU factorization costs $O(nw^2)$.  A forward/back substitution with the banded factors costs $O(nw)$.

In method (I), forming the generally dense inverse by solving for its $n$ columns costs $O(n^2w)$, and each multiplication $A^{-1}b_i$ costs $O(n^2)$.  Its total cost is

$$
O(nw^2+n^2w+mn^2)=O(mn^2).
$$
...
Since $w\ll n$ and $m\gg n$, method (II) uses asymptotically fewer arithmetic operations, by a factor of order $n/w$ in the dominant term.
