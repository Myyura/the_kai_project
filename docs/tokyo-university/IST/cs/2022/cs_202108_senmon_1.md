...
（2）采用非抢占式最短作业优先调度，求平均周转时间和平均响应时间。

（3）采用时间片为 $10\ {\rm ms}$ 的轮转调度，求平均周转时间和平均响应时间。若当前进程提前完成，下一时间片立即开始；新进程到达时加入队尾；若多个进程同时到达队尾，剩余计算时间较短者优先。

（4）实际系统不能忽略上下文切换开销。结合 CPU 调度和内存管理，说明轮转调度在考虑该开销时的优缺点。

（5）说明用于确定进程优先级的老化（Aging）机制的基本思想，以及它相对于静态优先级方案的优势。

#### 考点

- **CPU 调度指标**：逐段绘制抢占式/非抢占式 SJF 和轮转调度时间线，计算周转与首次响应。
- **轮转调度权衡**：分析时间片、公平性、交互响应、上下文切换及工作集/缓存影响。
- **优先级老化**：随等待时间提高优先级，以避免静态优先级调度中的饥饿。
