...
（1）题中 C 程序用联合体把整数 `0x12345678` 与四字节字符数组共享存储，并按地址递增顺序输出各字节。在处理器 A 上输出
`0x12, 0x34, 0x56, 0x78`，在处理器 B 上则逆序输出。结合处理器体系结构解释差异原因。

（2）用具体例子解释无转发机制的流水线处理器中的数据冒险和控制冒险。

（3）有一个四路组相联缓存，数据容量为
$32\times2^{10}$ 字节，地址宽度为 $32$ 位，缓存行为 $64$ 字节。分别计算缓存索引位宽和标签位宽，并计算存放全部标签所需 RAM 总位数。

（4）处理器具有指令缓存和数据缓存；二者均命中时基础 CPI 为 $C$，任一缓存每次未命中额外罚时 $P$ 个周期。程序中加载/存储指令占比为
$R_{ls}$，指令缓存未命中率为 $R_i$，数据缓存未命中率为 $R_d$，处理器 IPC 为
$I$。用 $C,R_i,R_d,R_{ls},P$ 表示 $I$。

#### 考点

- **大小端序**：理解多字节整数在低地址到高地址上的字节排列差异。
- **流水线冒险**：识别指令间数据依赖与分支造成的控制不确定性，以及停顿需求。
- **组相联缓存地址划分**：由容量、路数和块大小计算组数、索引、块内偏移与标签位数。
- **缓存对性能的影响**：把指令取用和数据访问的未命中罚时计入平均 CPI，再取倒数得到 IPC。
