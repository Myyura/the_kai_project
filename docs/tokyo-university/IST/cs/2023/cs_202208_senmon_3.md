...
（3）假定可使用硬件支持的计数器，且每个页表项含一个相应字段。从以下三方面说明如何实现精确 LRU：

- 计数器何时递增；
- 计数器值何时复制到页表字段；
- 如何选择被换出的页面。

（4）从实际实现角度说明 LRU 的一个缺点。

（5）假定每个页表项具有引用位，说明如何实现一种 LRU 近似算法。

#### 考点

- **最优与 LRU 页面置换**：逐次跟踪页框内容、未来访问或最近访问顺序并统计缺页。
- **页表时间戳实现**：记录页面最近引用时刻，并在置换时选择最旧时间戳。
- **LRU 实现开销**：分析每次访问更新元数据以及置换时搜索的硬件、时间成本。
- **引用位近似算法**：用时钟/二次机会等机制周期性检查并清除引用位来近似最近使用信息。
