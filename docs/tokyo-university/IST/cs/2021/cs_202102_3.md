...
tags:
  - Tokyo-University
  - Computer-Science.Formal-Languages.Left-Quotient
...
令 $\Sigma=\{a,b\}$。对语言 $L_1,L_2\subseteq\Sigma^*$，定义左商

$$
L_1\triangleleft L_2
=\{w\in\Sigma^*\mid
\text{存在 }v\in L_1\text{ 使 }vw\in L_2\}.
$$

例如，若 $L_1=\{ab,bb\}$、$L_2=\{aa,abb,bbab\}$，则
$L_1\triangleleft L_2=\{b,ab\}$。记 $L(A)$ 为有限自动机 $A$ 接受的语言。
回答下列问题。

（1）令
$L_3=\{aa,b,bb\}$、
$L_4=\{a,b,ab,bb,aaa,bbab\}$，求
$L_3\triangleleft L_4$。

（2）$L_5,L_6$ 分别由正则表达式 $(a^*b)^*$ 和 $(abba)^*$ 表示。
用正则表达式表示 $L_5\triangleleft L_6$。

（3）设 DFA
$A_i=(Q_i,\Sigma,\delta_i,q_{i,0},F_i)\ (i=1,2)$，且转移函数均为全函数。
构造识别 $L(A_1)\triangleleft L(A_2)$ 的 NFA 并简要说明；允许使用
$\varepsilon$ 转移。

（4）判断命题“对每个上下文无关语言 $L$ 和正则语言 $R$，
$L\triangleleft R$ 都是正则语言”是否正确。若正确，给出证明概要；若错误，给出反例。
...
1. For $v=aa$, the only word of $L_4$ having this prefix is $aaa$, so $w=a$.
2. For $v=b$, the words $b,bb,bbab$ give $w=\epsilon,b,bab$.
3. For $v=bb$, the words $bb,bbab$ give $w=\epsilon,ab$.
...
1. Words in $L_5$ are $\epsilon$ or words ending in $b$.
2. Words in $L_6$ are repetitions of `abba`.
3. A prefix of a word in $L_6$ that belongs to $L_5$ is either $\epsilon$, $(abba)^t ab$, or $(abba)^t abb$ for some $t\ge0$.

The corresponding suffixes are:

- for $v=\epsilon$, any word in $(abba)^*$;
- after a prefix $(abba)^t ab$, a word in $ba(abba)^*$;
- after a prefix $(abba)^t abb$, a word in $a(abba)^*$.
...
Run the product automaton $A_1\times A_2$ from
$(q_{1,0},q_{2,0})$, and put

$$
S=\{q\in Q_2\mid
\exists v\in\Sigma^*:\delta_1^*(q_{1,0},v)\in F_1,
\ \delta_2^*(q_{2,0},v)=q\}.
$$

Take a copy of $A_2$, add a new initial state with
$\epsilon$-transitions to every state in $S$, and retain $F_2$ as the accepting set.  It accepts $w$ exactly when some $v\in L(A_1)$ satisfies $vw\in L(A_2)$.

### (4)

The statement is true.  Let a DFA $A=(Q,\Sigma,\delta,q_0,F)$ accept
$R$, and set

$$
S=\{\delta^*(q_0,v)\mid v\in L\}\subseteq Q.
$$

An NFA consisting of $A$ with all states in $S$ as initial states accepts exactly
$L\triangleleft R$.  Since $Q$ is finite, this language is regular.  (The argument in fact holds for arbitrary $L$.)
...
对于问题 3，先用积自动机找出 $A_2$ 在读完某个 $L(A_1)$ 中的前缀后可能处于的全部状态，再把这些状态作为读取剩余输入的初态。
