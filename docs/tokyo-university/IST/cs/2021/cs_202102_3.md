...
- **语言左商**：按定义枚举能从第二个语言字符串前端消去的第一个语言字符串。
- **正则表达式化简**：分析两个周期性正则语言的可消前缀与剩余后缀。
- **$\varepsilon$-NFA 构造**：先让两个自动机同步读取被消去的前缀，再从可由第一个自动机接受的位置开始只运行第二个自动机。
- **正则语言闭包性质**：判断上下文无关语言与正则语言参与左商时结果的正则性，并用有限自动机状态集合论证。
