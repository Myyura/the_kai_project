...
tags:
  - Tokyo-University
  - Computer-Science.Information-Theory.Optimal-Alphabetic-Binary-Tree
  - Computer-Science.Information-Theory.Kraft-Inequality-for-Binary-Trees
  - Computer-Science.Information-Theory.Entropy-Lower-Bound-on-Weighted-External-Path-Length
---
...
有序二叉树的每个结点至多有两个有次序的子结点。记 $T_n$ 为具有
$n$ 个叶结点的所有有序二叉树的集合，$d_T(v)$ 为结点 $v$ 在树 $T$ 中的深度，即根到
$v$ 的路径所含边数。对 $n$ 个正数组成的序列
$P=(c_1,\ldots,c_n)$，定义

$$
S_P=\sum_{i=1}^n c_i,\qquad
H_P=-\sum_{i=1}^n c_i\log_2(c_i/S_P).
$$

对 $T\in T_n$，若 $v_i$ 是从左到右第 $i$ 个叶结点，定义

$$
W_P(T)=\sum_{i=1}^n c_i\,d_T(v_i).
$$

回答下列问题。

（1）当 $P=(4,2,1,1)$ 时，给出使 $W_P(T)$ 最小的
$T\in T_4$。

（2）证明对任意具有叶结点 $v_1,\ldots,v_n$ 的
$T\in T_n$，

$$
\sum_{i=1}^n2^{-d_T(v_i)}\le1.
$$

（3）令正数 $x_1,\ldots,x_n$ 满足 $\sum_i x_i=1$。证明对任意正数序列
$P=(c_1,\ldots,c_n)$，当 $x_i=c_i/S_P$ 时，

$$
\sum_{i=1}^n c_i\log_2x_i
$$

达到最大值。

（4）证明对任意正数序列 $P$ 和任意 $T\in T_n$，
$W_P(T)\ge H_P$。
...
Here $\mathbf H_P=14$ as well, so the lower bound in (4) proves optimality. The displayed tree also preserves the required left-to-right leaf order.
...
First contract every node having exactly one child. This only decreases the depths of its descendant leaves, so it can only increase the sum. It therefore suffices to prove the result for full ordered binary trees, in which every internal node has two children.
...
**Inductive Step**: Assume that for any full ordered binary tree with $k$ leaves, the inequality holds:

$$
\sum_{i=1}^{k} 2^{-\mathrm{d_T}(v_i)} \leq 1
$$

Now, we need to prove that the inequality holds for a full ordered binary tree with $k+1$ leaves.

1. Consider a full ordered binary tree with $k+1$ leaves.
2. Let's denote the depth of the leaves in this tree by $\mathrm{d_T}(v_1), \mathrm{d_T}(v_2), \ldots, \mathrm{d_T}(v_{k+1})$.

Every full tree with $k+1$ leaves contains two sibling leaves. Contracting such a pair produces a full tree with $k$ leaves; conversely, the original tree is obtained by splitting that contracted leaf into two children.
...
$$
\sum_{i=1}^{k} 2^{-\mathrm{d_T}(v_i)}-2^{-d} + 2^{-(d+1)} + 2^{-(d+1)} \leq 1
$$
...
Moreover, $\partial^2\mathcal L/\partial x_i^2=-c_i/(x_i^2\ln 2)<0$. Hence the objective is strictly concave on the constraint set, so this stationary point is the unique global maximum.
...
$$
K=\sum_{i=1}^{n}2^{-\mathrm{d_T}(v_i)}\leq1,
\qquad x_i=\frac{2^{-\mathrm{d_T}(v_i)}}{K}.
$$

Then $x_i>0$ and $\sum_i x_i=1$. By question (3),

$$
\sum_{i=1}^{n}c_i\log_2x_i
\leq \sum_{i=1}^{n}c_i\log_2\frac{c_i}{\mathbf{S_P}}
=-\mathbf{H_P}.
$$
...
$$
\sum_{i=1}^{n}c_i\log_2x_i
=-\mathbf{W_P}(T)-\mathbf{S_P}\log_2K
\geq-\mathbf{W_P}(T),
$$

because $K\leq1$. Therefore $-\mathbf{W_P}(T)\leq-\mathbf{H_P}$, or

$$
\mathbf{W_P}(T)\geq\mathbf{H_P}.
$$
