...
$\boldsymbol{y}\in\mathbb R^n$，并假定 $XX^{\mathsf T}$ 可逆。用拉格朗日乘子法求解

$$
\min_{\boldsymbol{w}}\frac12\|\boldsymbol{w}\|^2
\quad\text{满足}\quad
...
设 $X$ 的奇异值分解为
$X=U\Lambda V^{\mathsf T}$，其中 $U,V$ 的列分别构成
$\mathbb R^n,\mathbb R^d$ 的标准正交基，
$\Lambda\in\mathbb R^{n\times d}$ 的前 $n$ 个对角元为正数
$\lambda_1,\ldots,\lambda_n$，其余为零。将这些非零奇异值取倒数形成
$\Lambda^-$，并定义

$$
X^-=V(\Lambda^-)^{\mathsf T}U^{\mathsf T}.
$$
...
（2）仅用 $U$ 和 $\lambda_i\ (i=1,\ldots,n)$ 表示
$XX^{\mathsf T}$。

（3）将 $L$ 的驻点写成
$\boldsymbol{w}=A\boldsymbol{y}$、
$\boldsymbol{\mu}=B\boldsymbol{y}$，仅用 $X$ 表示
$A\in\mathbb R^{d\times n}$ 与
...
$$
\mathbf{X^-} = \mathbf{V} (\mathbf{\Lambda}^-)^\top \mathbf{U}^\top,
$$
...
$$
\mathbf{XX^-X} = (\mathbf{U} \mathbf{\Lambda} \mathbf{V}^\top)(\mathbf{V} (\mathbf{\Lambda}^-)^\top \mathbf{U}^\top)(\mathbf{U} \mathbf{\Lambda} \mathbf{V}^\top).
$$
...
\begin{aligned}
\mathbf{XX^-X}
&= \mathbf{U} \mathbf{\Lambda} (\mathbf{V}^\top \mathbf{V}) (\mathbf{\Lambda}^-)^\top \mathbf{U}^\top \mathbf{U} \mathbf{\Lambda} \mathbf{V}^\top \\
&= \mathbf{U} \mathbf{\Lambda} (\mathbf{\Lambda}^-)^\top \mathbf{\Lambda} \mathbf{V}^\top \\
&= \mathbf{U} \mathbf{\Lambda} \mathbf{V}^\top = \mathbf{X}.
\end{aligned}
$$
...
$$
\mathbf{X} \mathbf{X}^- = \mathbf{U} \mathbf{\Lambda} \mathbf{V}^\top \mathbf{V} (\mathbf{\Lambda}^-)^\top \mathbf{U}^\top = \mathbf{U} \mathbf{\Lambda} (\mathbf{\Lambda}^-)^\top\mathbf{U}^\top = \mathbf{U} \mathbf{I}_n \mathbf{U}^\top = \mathbf{I}_n.
$$

This identity alone does not determine a right inverse uniquely. It remains to show that

$$
\mathbf{X}^\top (\mathbf{X} \mathbf{X}^\top)^{-1} = \mathbf{X^-}.
$$

To see this, use the given SVD of $\mathbf{X}$ and $\mathbf{X}\mathbf{X}^\top$ from Question 2:
...
$$
\mathbf A
= \mathbf{X}^\top (\mathbf{X} \mathbf{X}^\top)^{-1}
$$
...
0 & 0 & \cdots & 0
\end{pmatrix} \mathbf{U}^\top
= \mathbf{V}(\mathbf{\Lambda}^-)^\top\mathbf{U}^\top
= \mathbf{X^-}.
$$
