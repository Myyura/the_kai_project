...
tags:
  - Tokyo-University
  - Computer-Science.Formal-Languages.Language-Square-Root
...
$\{q_1\}$。构造识别 $\mathcal H(L_3)$ 的状态数最少的 DFA。

（3）判断命题 1 是否成立：对 $\Sigma$ 上每个正则语言
$L$，$\mathcal H(L)$ 也为正则语言。若成立则证明；否则给出反例并简要说明。
...
- **语言平方根运算**：分析同一字符串连续出现两次这一约束，并化简具体正则语言。
- **DFA 构造与最小化**：跟踪一个前半串诱导的状态变换，再验证相同字符串的第二次作用，并合并等价状态。
- **正则语言闭包性质**：利用有限转换幺半群或有限状态变换证明平方根运算下的正则性。
- **上下文无关语言闭包反例**：考察复制语言约束与 CFL 无法一般比较两个任意相同子串的限制。
