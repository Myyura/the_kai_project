...
（2）设计并图示电路 ALT，使输出如上表所示，每个时钟周期在 $0,1$ 间翻转。可用第（1）问所选触发器及 AND、OR、NOT 门。

（3）设计并图示用冒泡排序将 $8$ 个 4 位无符号整数排序的电路。输入为 $I_0,\ldots,I_7$ 和 1 位装载信号 $L$；输出为 $O_0,\ldots,O_7$ 和 1 位完成信号 $V$，各 $I_i,O_i$ 均为 4 位无符号整数。$L=1$ 时用 32 个触发器保存输入，记为 $v_0,\ldots,v_7$。$L$ 变为 $0$ 后，首个时钟周期比较交换
...
每对中前者大于后者时交换，否则保持原值；每周期将更新后的 $v_0,\ldots,v_7$ 输出至 $O_0,\ldots,O_7$。$L=0$ 期间交替重复这两层比较。若执行这两层的连续两个周期均无交换则令 $V=1$，否则令 $V=0$。可使用所选触发器、ALT、4 位比较器 CMP、4 位 2 选 1 MUX 及 AND、OR、NOT 门。
