...
处理器 $P$ 配有直接映射数据缓存：数据容量 $256$ 字节，缓存行
$16$ 字节；`lw`、`sw` 的地址和数据均为 $32$ 位。对地址
$a_{31}\cdots a_0$，缓存索引为 $a_7a_6a_5a_4$，块内偏移为
$a_3a_2a_1a_0$。处理器有寄存器 `x0` 至 `x31`，其中 `x0` 恒为零。

程序 $S$ 对首地址 `0x1000`、含 $402$ 个 32 位有符号整数的数组
$A$ 施加长度为 $3$ 的一维均值滤波，并把 $400$ 个结果写入首地址
`0x2000` 的数组 $B$。寄存器 `x5,x6,x7` 初值分别为
`0x1640,0x1000,0x2000`。程序先令 `x2=3`，每轮从
`x6+0`、`x6+4`、`x6+8` 取三个相邻元素，按题中指令累加并除以
$3$，将结果存至 `x7`；随后 `x6,x7` 各加 $4$，在
`x6<x5` 时继续循环。

（1）假定执行开始时所有缓存行均无效且没有预取器，计算程序的数据缓存命中率，保留三位小数。

（2）计算程序 IPC，保留三位小数。假定指令存储无延迟，取指、译码、数据转发和寄存器写回均无延迟；处理器每周期至多启动一条指令，且前一条完成前不启动后一条。
`add`、`addi`、`blt` 各需 $1$ 周期，`div` 需 $4$ 周期；
`lw`、`sw` 命中时需 $1$ 周期，未命中时需 $4$ 周期。

（3）在不改变缓存总容量的前提下，说明一种能提高程序 $S$ 命中率的数据缓存硬件修改，并解释原因。

（4）在不修改处理器或缓存的情况下，结合具体例子说明一种提高命中率的软件级优化。

#### 考点

- **直接映射缓存冲突**：按地址索引跟踪数组 $A,B$ 的块映射及读写相互驱逐，精确统计命中。
- **可变访存延迟下的 IPC**：汇总各类指令数量与命中/未命中周期，计算总指令数除以总周期数。
- **缓存组织优化**：在容量不变时通过提高相联度等方式减少冲突未命中。
- **缓存感知程序优化**：复用滑动窗口已加载元素、调整地址或利用寄存器，减少访存和冲突。
