...
- **倒数位置语言的 NFA**：非确定性地猜测目标字符并用固定长度后缀验证其位置。
- **正则表达式**：把任意前缀、指定字符和长度为 $k-1$ 的任意后缀连接起来。
- **泵引理与稀疏位置集合**：分析平方数距离条件，证明无限并所得语言是否能由有限状态记忆。
- **DFA 状态复杂度**：用长度 $k$ 的不同后缀或 Myhill–Nerode 可区分性给出 $2^k$ 下界。
