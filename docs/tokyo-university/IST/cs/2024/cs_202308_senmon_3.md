...
设 $G=(V,E)$ 为无自环、无重边的无向图，
$|V|=n$、$|E|=m$。若连通图 $G$ 中的顶点 $v$ 被删除后所得图不连通，则称
$v$ 为 $G$ 的割点。回答下列问题。

（1）描述判断 $G$ 是否连通的算法，并估计时间复杂度。

（2）给定连通图 $G$，描述一个 $O(m)$ 时间算法求其生成树 $T$。

（3）设 $T$ 是连通图 $G$ 的生成树，$v$ 是 $T$ 的非叶结点（在
$T$ 中度至少为二），且 $v$ 不是 $G$ 的割点。令 $e$ 为 $T$ 中一条与
$v$ 相接的边。证明存在 $G$ 中另一条边 $f\ne e$，用 $f$ 替换
$e$ 后仍得到 $G$ 的一棵生成树。

（4）给出一个耗时为 $o(mn)$ 的算法，输入连通图 $G$，找出其所有割点。

#### 考点

- **连通性与图搜索**：用 DFS/BFS 标记可达顶点并同时生成搜索树。
- **生成树交换性质**：删除树边得到两个分量，再利用非割点条件寻找跨分量替代边。
- **割点算法**：在 DFS 树上计算发现时间与 low 值，用子树是否存在回边判定割点。
- **复杂度分析**：以邻接表确保遍历顶点和边的总成本为线性，从而满足严格优于 $mn$ 的要求。
