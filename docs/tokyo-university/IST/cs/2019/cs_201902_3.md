...
- **子序列语言的 NFA**：用自环跳过不选择的字符，用状态推进记录已匹配的子序列前缀。
- **自动机上的存在量词**：通过非确定性选择输入字符，模拟原 DFA 在某个子序列上的运行。
- **子集状态与全称条件**：确定性地跟踪所有子序列可能到达的原自动机状态，并据接受状态集合定义判定条件。
- **构造正确性证明**：归纳证明读入任一前缀后，状态集合恰好对应其全部子序列的可达状态。
