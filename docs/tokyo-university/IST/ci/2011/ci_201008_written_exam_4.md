...
- **高阶函数**：说明把函数作为参数或返回值以抽象通用计算模式的编程方式。
- **公钥基础设施**：说明证书、认证机构和信任链如何把公钥绑定到实体身份。
- **自治系统编号**：说明 AS 号在互联网域间路由中标识独立路由管理域的作用。
- **同步定位与地图构建**：说明移动机器人从传感观测同时估计自身轨迹与环境地图的 SLAM 问题。
- **多关节机器人连杆表示**：说明用坐标系和连杆参数描述相邻关节变换并串联得到机械臂姿态。
- **广义逆与伪逆**：说明非方阵或奇异线性系统的最小二乘、最小范数解。
- **\(H^\infty\) 控制**：说明以系统最坏情况下的增益界为指标设计具有鲁棒性的控制器。
- **主成分分析**：说明通过协方差矩阵特征分解寻找最大方差正交方向以降维。
