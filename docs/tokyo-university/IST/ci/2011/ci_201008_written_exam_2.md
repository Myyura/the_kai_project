...
- **汇编指令格式设计**：在固定 16 位宽度内权衡操作码、寄存器号、寻址字段和立即数范围，并保证所需控制流及访存能力。
- **双操作数与三操作数、寄存器—存储器与加载—存储架构**：比较代码密度、寄存器破坏、指令数量、译码复杂度和流水线实现。
- **数据通路与指令执行**：依据自定义指令集组织 PC、指令寄存器、寄存器堆、ALU、存储器和控制器，并跟踪算术指令的数据流。
