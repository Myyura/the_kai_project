...
如图 1，质量为 \(M\) 的小车位于倾角 \(\theta\) 的斜面，可沿斜面 \(x\) 轴方向施力 \(f\)。力足以把小车拉上坡，忽略小车与斜面的摩擦及空气阻力。时刻 \(t\) 的力、位置、速度分别为 \(f(t),x(t),v(t)\)，重力加速度大小为 \(g\)。初始 \(x(0)=0,v(0)=0\)，目标是移到 \(x=L\)。

1. 以恒力 \(f(t)=F\)（\(F>0\)）加速至 \(t_1\)，求小车的位置和速度。
2. 从 \(t_1\) 到 \(t_2\ge t_1\) 改施恒力 \(f(t)=-F\)，要求 \(x(t_2)=L,v(t_2)=0\)。求实现该运动的 \(t_1,t_2\)。

随后采用比例位置反馈 \(f(t)=k_1\{L-x(t)\}\)，其中 \(k_1>0\)。
...
再加入速度反馈：
\[
f(t)=k_1\{L-x(t)\}-k_2v(t),\qquad k_1,k_2>0.
\]

5. 说明加入 \(-k_2v(t)\) 的效果及产生原因。
6. 求使 \(x(t)\) 不振荡的 \(k_1,k_2\) 条件。可使用：齐次方程
   \[
   x''+ax'+bx=0
   \]
   的特征方程为 \(r^2+ar+b=0\)。若有两个不同实根 \(p,q\)，通解为 \(C_1e^{pt}+C_2e^{qt}\)；若有共轭复根 \(h\pm ki\)，通解为 \(e^{ht}(C_1\cos kt+C_2\sin kt)\)；若有重根 \(p\)，通解为 \(e^{pt}(C_1+C_2t)\)。
7. 在第 6 问条件下画 \(x(t)\) 曲线。

最后加入位置误差积分项：
\[
f(t)=k_1\{L-x(t)\}-k_2v(t)
+k_3\int_0^t\{L-x(\tau)\}\,d\tau,\qquad k_3>0.
\]
...
- **斜面上的受力与分段匀加速运动**：把重力沿斜面分量计入运动方程，联立末端位置、速度条件求切换时刻。
- **比例与比例—微分反馈**：分析比例控制的稳态偏差、速度反馈的阻尼作用及特征根决定的振荡条件。
- **积分控制**：说明误差积累如何提供抵消恒定重力扰动的控制力，从而消除稳态误差。
- **阻尼振子**：由二阶特征方程判别欠阻尼、临界阻尼和过阻尼响应并绘制位置曲线。
