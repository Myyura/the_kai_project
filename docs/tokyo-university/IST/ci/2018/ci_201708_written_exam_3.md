...
The optimization problem is NP-hard, and its decision version is NP-complete. Suppose there are $N$ cities; enumerating the $(N-1)!$ tours from a fixed starting city and evaluating each in $O(N)$ time gives an $O(N!)$ brute-force algorithm and an $O(N^22^N)$ state-compression DP algorithm exist.
