...
- **分治法**：说明把问题分成同类子问题、递归求解并合并结果的算法结构。
- **B 树**：说明多路平衡搜索树的结点容量、关键字次序、叶层平衡以及其降低磁盘 I/O 的用途。
- **采样与奈奎斯特频率**：解释采样率的一半所表示的最高可辨频率及超过该范围产生的混叠。
- **冲激响应与阶跃响应**：说明线性时不变系统中阶跃响应是冲激响应的积分、冲激响应是阶跃响应的导数。
- **向量量化**：说明以码本中的代表向量近似输入向量并按失真准则选择码字的压缩过程。
- **乱序执行**：说明处理器在保持程序可见语义的同时，按操作数和功能单元就绪情况重排指令执行。
- **正则文法与正则语言**：用实例说明线性产生式如何生成可由有限自动机识别的语言。
- **CGI**：说明 Web 服务器如何调用外部程序、传递请求数据并接收其动态响应。
