...
- **文件格式与覆盖语义**：解析逗号分隔的区间、赋值对，并按最右声明覆盖及去重规则处理。
- **静态区间分析**：沿赋值序列传播变量来源区间，分别累计结束时与整个执行过程的可能值域。
- **区间一致性检查**：比较实际运行值域与声明区间，并检查赋值右值区间是否包含于左值声明。
- **区间约束求解**：为未声明变量选择区间，使赋值图上的包含关系同时成立，识别无解情况。
