...
- **最大似然估计**：从独立样本似然与对数似然推导硬币参数、正态均值和方差估计。
- **高斯混合模型**：写出类别先验、条件高斯密度及含隐变量的联合或边缘似然。
- **EM 算法**：把 Jensen 下界作为辅助函数，在 E 步求后验责任度，在 M 步更新混合权重、均值、方差。
- **单调性与 Jensen 不等式**：证明交替最大化辅助函数不会降低原目标，并理解下界取等条件。
