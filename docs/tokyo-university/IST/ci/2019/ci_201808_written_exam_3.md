...
- **逆运动学**：说明由机器人末端目标位姿反求关节变量及多解、无解问题。
- **隐马尔可夫模型**：说明隐藏状态转移、观测发射和序列概率推断。
- **Minimax**：说明零和博弈树中双方分别最大化、最小化效用的递归决策。
- **NP 完全性**：说明问题属于 NP 且所有 NP 问题可多项式时间归约到它。
- **光线追踪**：说明从相机发射光线求交并递归计算阴影、反射和折射。
- **SIMD**：说明一条指令并行作用于多个数据元素的数据级并行体系。
- **值调用与引用调用**：比较传入对象副本与传入可修改引用时对实参和副作用的影响。
- **公钥密码**：说明公钥、私钥的配对及其在加密和签名中的用法。
