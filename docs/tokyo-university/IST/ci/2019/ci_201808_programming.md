...
文本文件由 \(0\)～\(255\) 的整数构成，相邻整数以一个空格分隔，整数总数是 3 的倍数。每三个数依次组成一个像素的红、绿、蓝强度 \((r,g,b)\)。像素按文件中的三元组顺序从 0 起编号；例如

```text
19 7 0 17 13 1 29 3 27 5 11 23
```

表示编号 0～3 的四个像素 \((19,7,0),(17,13,1),(29,3,27),(5,11,23)\)。把像素从左到右排放，达到图像宽度后换到下一行，形成矩形图像。必要时编程回答：

1. 在答题纸写出 `image1.txt` 中的像素数。
2. 用该文件构图。每行最右像素均为白色 \((255,255,255)\)，且图中不存在另一条从顶到底的全白竖线。求图像宽度并写在答题纸上。
3. 按像素强度 \(r^2+g^2+b^2\) 升序排序 `image1.txt` 中的 \(n\) 个像素；同强度时，编号较大的像素视为更小。\(n\) 为偶数，编程输出排序后第 \(n/2\) 个像素的三元组及原编号，并把结果写在答题纸上；排序下标从 0 起。
4. 对 `image2.txt` 的 \(n\) 个像素，在上述排序后选 \(k\) 个像素 \(e_i\)（\(0\le i<k\)），其中 \(e_i\) 是第 \(ni/k\) 个像素，且 \(n\) 是 \(k\) 的倍数。输出各三元组和编号；并把 \(k=4\) 时全部结果写在答题纸上。
5. 对给定文件，用下列迭代选取代表全部像素的 \(k\) 种颜色，\(k\) 为输入。
   1. 以第 4 问的 \(e_i\) 初始化代表像素 \(p_i^{(0)}=e_i\)。
   2. 对每个像素 \(q_j\)，找最近的 \(p_i^{(t)}\)，归入簇 \(C_i^{(t)}\)；代表像素自身必须属于该簇。
   3. 求每簇 RGB 三分量的平均值，除法结果向下取整，得到质心；选簇中离质心最近的实际像素为新代表 \(p_i^{(t+1)}\)。
   4. 用新代表重新分簇。
   5. 重复十次，得到 \(p_i^{(10)}\)，其三元组即代表色。

   两三元组距离为
   \[
   |r_i-r_j|+|g_i-g_j|+|b_i-b_j|.
   \]
   第 2、3 步若有等距像素，选原编号最大的。对 `image2.txt` 取 \(k=128\)，写出 \(i=40,80,120\) 的 \(p_i^{(10)}\)；对 `image3.txt` 取 \(k=8\)，写出 \(i=2,4,6\) 的结果。
6. 编写程序按第 5 问把图像每个 \(C_i^{(10)}\) 中像素改为代表色 \(p_i^{(10)}\)，并输出 `image.tif`。假设图像为正方形。对 `image2.txt` 取 \(k=32\)，把生成文件保存到 U 盘。

   `image.tif` 由 104 字节属性信息和像素数据组成，前 104 字节依次为下列十进制值：
...
For `image3.txt`:
```py
with open('image3.txt') as i2:
    i2r = i2.read()
    n_triplets = len(i2r.split()) // 3
i2rs = [int(d) for d in i2r.split()]
image2 = []
width = -1
...
new_image2_bytes = []
big_endian_w = [(w//(256**3))%256,(w//(256**2))%256,(w//256)%256,w%256]
big_endian_h = [(h//(256**3))%256,(h//(256**2))%256,(h//256)%256,h%256]
s = w*h*3
big_endian_s = [(s//(256**3))%256,(s//(256**2))%256,(s//256)%256,s%256]
for i in range(n_triplets):
    tmp = p[c[i]].tolist()
    # tmp = [i2rs[3*i], i2rs[3*i+1], i2rs[3*i+2]]
    new_image2_bytes += tmp
tif = [
    77,77,0,42,0,0,0,8,0,7,1,0,0,4,0,0,0,1,
]+big_endian_w+[
    1,1,0,4,0,0,0,1,
]+big_endian_h+[
    1,2,0,3,0,0,0,3,0,0,0,98,1,6,0,3,0,0,0,1,0,2,0,0,1,17,0,4,0,0,0,1,0,0,0,104,1,21,0,3,0,0,0,1,0,3,0,0,1,23,0,4,0,0,0,1,
]+big_endian_s+[
    0,0,0,0,0,8,0,8,0,8
]
assert len(tif) == 104
tif += new_image2_bytes
with open('image.tif', 'wb') as imw:
    imw.write(bytes(tif))
