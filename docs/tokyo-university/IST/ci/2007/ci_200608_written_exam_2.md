...
- **传感器采集流水线调度**：在多路选择、采样保持、A/D 转换和计算阶段之间安排重叠执行，画出满足依赖关系的最短时序。
- **流水线与并行资源配置**：比较转换时间和计算时间所形成的瓶颈，推导达到最小周期所需的 A/D 转换器数或计算机数。
- **性能分析**：从各阶段延迟、吞吐率、周期时间和资源利用率量化一次性处理及稳态重复处理的耗时。
