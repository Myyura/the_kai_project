...
必要时可使用文末的 ASCII 码表。本题只处理 ASCII 码 32～126 以及换行符，共 96 种字符；所用文件均仅含 ASCII 字符，每行不超过 76 个字符。

**第 1 题：凯撒密码。** 这种密码把英文字母按密钥指定的位数循环后移，密钥为 1～25 的整数。例如密钥为 4 时，`Japan` 加密为 `Neter`；密钥为 25 时，`IBM` 加密为 `HAL`。大写字母仍映射为大写，小写仍映射为小写；空格、换行、数字、标点等非字母字符不加密。读入 `q1.txt`，推断加密密钥并恢复原英文文本。

**第 2 题：单表替换密码。** 该密码用一张英文字母间的一一映射表作为密钥。

1. 文件 `q21.txt` 含数万字符的英文文章，来源为 C. A. R. Hoare 的 1980 年 ACM 图灵奖演讲论文 *The Emperor’s Old Clothes*（*Communications of the ACM*, 24(2), 1981；原文所列链接为 `www.braithwaite-lee.com/opinions/p75-hoare.pdf`）。编写程序统计其中各英文字母的出现次数，统计时将大写并入对应小写，并按频数从高到低输出字母—频数对，输出格式不限。
2. `q22.txt` 含一段用某张替换表加密的文本；非字母不变，字母大小写保持不变。编写按替换表转换字符的程序，并结合英语知识反复尝试解密，将得到的明文输出到 `a22.txt`。文本来源为原文所列的 Dijkstra EWD340 链接。

**第 3 题：哈夫曼编码。** 单表替换密码会保留明文字符频率，因而容易受到频率分析；若先以高频字符分配短二进制串、低频字符分配长二进制串的方式压缩文本，再加密，则二进制模式的频率更趋均匀。

1. 统计 `q21.txt` 中上述 96 种字符各自的频数，并实现题目规定的哈夫曼编码。构造一棵所有字符均为叶子的有序二叉树；从根走向叶子时，走左支记 0、走右支记 1，沿途比特串即为该字符的编码。自底向上建树时，先将每个字符作为权值等于其频数的结点（题设保证频数均非零）；反复从尚无父结点的结点中选权值最小和次小者，分别作为新结点的右、左孩子，新结点权值为二者之和，直至形成整棵树。权值相同时，字符结点中 ASCII 码较小者置于右侧；涉及已合并结点时，较新建立者置于右侧。图 1 给出了 \(A,B,C,D,E\) 的频数分别为 \(50,20,33,15,40\) 时的构造示例。输出须能看出每个字符与其二进制串的对应关系，空格显示为 `SP`，换行显示为 `NL`，其余格式不限。
2. 题设指出，不考虑频率时对含换行在内的 96 种字符进行二进制编码，平均每字符需要 6.5 位。编写程序计算第 3-1 问所得哈夫曼编码的加权平均码长，权重为各字符出现频率，结果至少保留小数点后两位。

题目所需的哈夫曼树示意图与 ASCII 码表沿用原文图片。

#### 考点

- **凯撒密码与循环移位**：保持字母大小写并跳过非字母字符，实现移位解密并从自然语言文本推断密钥。
- **单表替换密码与频率分析**：统计忽略大小写的字母频率，借助英文频率规律和试探性的映射更新恢复密文。
- **字符频数统计与文件处理**：按题设限定的 96 种 ASCII 字符读取、计数、排序并将结果或明文写入指定文件。
- **哈夫曼编码**：严格按给定的左右孩子和同权结点规则构造有序二叉树，生成前缀码并计算按字符频率加权的平均码长。
