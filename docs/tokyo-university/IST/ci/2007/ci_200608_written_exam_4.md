...
- **采样定理与混叠**：说明连续信号的最高频率与无失真采样所需采样率之间的关系，以及采样不足造成的频谱混叠。
- **RISC 与 CISC 架构**：比较两类处理器在指令复杂度、指令长度、执行周期和硬件实现等方面的取舍。
- **TCP 与 UDP**：比较两种传输层协议的连接方式、可靠性、顺序保证、开销及典型应用。
- **堆排序**：用完全二叉堆说明建堆和反复取出堆顶的排序过程及复杂度。
- **函数式编程**：说明一等函数、不可变数据、表达式求值及递归等语言特征。
- **分支定界法**：结合候选空间的分支、上下界估计和剪枝解释组合优化求解过程。
- **语素与形态分析**：用具体词例说明最小意义单位及其在自然语言分词、构词分析中的作用。
- **齐次坐标**：说明通过增加一个坐标分量统一表示仿射变换和投影变换的方法。
