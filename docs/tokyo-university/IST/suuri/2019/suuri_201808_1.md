...
- **列随机矩阵**：利用列和为 1 证明谱半径为 1，并识别概率单纯形的不变性。
- **PageRank 传送机制**：理解均匀传送项使矩阵严格正并保证概率不动点。
- **不动点定理**：在概率单纯形上建立连续自映射以证明存在性。
- **1-范数压缩**：对零和向量证明 $B$ 的收缩因子至多为 $\alpha$。
- **幂迭代收敛**：由压缩估计得到向稳态向量的几何收敛率。
