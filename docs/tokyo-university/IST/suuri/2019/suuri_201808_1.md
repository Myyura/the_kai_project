...
tags:
  - Tokyo-University
  - Mathematics.Linear-Algebra.PageRank-Teleportation-Fixed-Point-and-Contraction
---
...
设实矩阵 $A=(a_{ij}),B=(b_{ij})\in\mathbb R^{n\times n}$ 满足

$$
a_{ij}\ge0,\qquad
\sum_{i=1}^na_{ij}=1\quad(j=1,\ldots,n),
$$

$$
B=\alpha A+\frac{1-\alpha}{n}\mathbf1\mathbf1^\top,
\qquad0<\alpha<1,
$$

其中 $\mathbf1=(1,\ldots,1)^\top$。向量的 1-范数定义为
...
1. 求 $A$ 的特征值模的最大值。
2. 证明存在分量非负的概率向量 $x$ 满足

   $$
   Bx=x,\qquad\mathbf1^\top x=1.
   $$

   可使用“非空紧凸集到自身的连续映射存在不动点”这一事实。
3. 对任意满足 $\mathbf1^\top q=0$ 的向量
   $q=(q_1,\ldots,q_n)^\top$，证明对每个 $i$，

   $$
   \left|\sum_{j=1}^nb_{ij}q_j\right|
   \le\sum_{j=1}^nb_{ij}|q_j|
   -\frac{1-\alpha}{n}\|q\|_1.
   $$

4. 对第 2 问的任一不动点 $x$ 和任意正整数 $N$，证明

   $$
   \left\|B^N\frac{\mathbf1}{n}-x\right\|_1
   \le\alpha^N\left\|\frac{\mathbf1}{n}-x\right\|_1.
   $$
...
$$
\begin{aligned}
    \left|\sum_{j=1}^n a_{ji}x_j\right|
    \leq \sum_{j=1}^n a_{ji}|x_j|
    \leq |x_i|\sum_{j=1}^n a_{ji}
    =|x_i|<|\lambda|\,|x_i|=|\lambda x_i|
\end{aligned}
$$

つまり、$|\lambda|>1$ なら、この不等式に反し矛盾。

よって、$A$ の固有値の絶対値で最大となるものは $1$ である。

### (2)
関数 $f:S \to S$ を $f(\boldsymbol{x})=B\boldsymbol{x}$ で定義する。

ただし、$S=\{\boldsymbol{x}\in \mathbb{R}_{\geq 0}^n \mid \boldsymbol{1}^T\boldsymbol{x}=1 \}$ である。

まず $B\boldsymbol{x}\in S$ を示す。
...
という表式と、$a_{ij},\alpha$ の値域より、$B\boldsymbol{x}\in \mathbb{R}_{\geq 0}^n$ は明らか。
...
\begin{aligned}
    \boldsymbol{1}^TB\boldsymbol{x} & =\boldsymbol{1}^T\left(\alpha A\boldsymbol{x}+\frac{1-\alpha}{n}\boldsymbol{1}\boldsymbol{1}^T\boldsymbol{x}\right) \\
                    ...
                    & =\alpha \boldsymbol{1}^T\boldsymbol{x}+(1-\alpha)\boldsymbol{1}^T\boldsymbol{x}                     \\
                    & =\alpha+(1-\alpha)                                                  \\
                    & =1
\end{aligned}
$$

よって、$f(S)\subset S$ である。

線形写像 $f$ は連続である。$S$ は非空な凸集合であり、また有界閉集合なのでコンパクトである。
...
\begin{aligned}
    \boldsymbol{1}^T(B\boldsymbol{q}) & =\boldsymbol{1}^T \left(\alpha A+\frac{1-\alpha}{n}\boldsymbol{1}\boldsymbol{1}^T\right)\boldsymbol{q} \\
                      ...
                      & =\alpha \boldsymbol{1}^T\boldsymbol{q}+(1-\alpha)\boldsymbol{1}^T\boldsymbol{q}                \\
                      & =\boldsymbol{1}^T\boldsymbol{q}                  \\
...
\begin{aligned}
\|B\boldsymbol{q}\|_1
&\leq \alpha\sum_{i=1}^n\sum_{j=1}^n a_{ij}|q_j| \\
&=\alpha\sum_{j=1}^n\left(\sum_{i=1}^na_{ij}\right)|q_j| \\
