...
tags:
  - Tokyo-University
  - Mathematics.Numerical-Analysis.Lie-Trotter-Product-Formula-and-Operator-Splitting
...
对复方阵 $X\in\mathbb C^{n\times n}$，定义

$$
e^X=\sum_{k=0}^\infty\frac{X^k}{k!},\qquad X^0=I.
$$
...
2. 令 $A,B\in\mathbb C^{n\times n}$、$m\ge1$，并设

   $$
   P=e^{(A+B)/m},\qquad Q=e^{A/m}e^{B/m}.
   $$

   利用

   $$
   P^m-Q^m=\sum_{i=0}^{m-1}P^i(P-Q)Q^{m-1-i}
   $$
   ...
   $$
   \|P^m-Q^m\|
   \le m\|P-Q\|e^{\frac{m-1}{m}(\|A\|+\|B\|)}.
   $$

3. 定义 $f(X)=e^X-I$、$g(X)=e^X-I-X$，证明

   $$
   P-Q=g\!\left(\frac{A+B}{m}\right)
   -g\!\left(\frac A m\right)-g\!\left(\frac B m\right)
   -f\!\left(\frac A m\right)f\!\left(\frac B m\right).
   $$
...
4. 进一步证明误差估计
   ...
   $$
   \|P^m-Q^m\|
   \le\frac2m(\|A\|+\|B\|)^2e^{\|A\|+\|B\|}.
   $$

5. 对初值问题

   $$
   \dot x(t)=(A+B)x(t),\qquad x(0)=v,
   $$

   定义算子分裂序列 $\tilde x^0=v$，并交替执行

   $$
   \tilde x^{k+1}=
   \begin{cases}
   e^{B/m}\tilde x^k,&k\text{ 为偶数},\\
   e^{A/m}\tilde x^k,&k\text{ 为奇数}
   \end{cases}
   \quad(k=0,\ldots,2m-1).
   $$
...
\|P^iQ^{m-1-i}\|
&\leq\|P\|^i\|Q\|^{m-1-i}
\leq e^{\frac{m-1}{m}(\|A\|+\|B\|)}.
\end{aligned}
$$

ただし、途中で $e$ の単調性つまり、$x \leq y\Rightarrow e^x \leq e^y$ を用いた。

一般には $P=Q$ とは限らないため、行列の指数関数に可換性を仮定した指数法則は適用できない。
...
\begin{aligned}
    \|P^m-Q^m\| & =\left\lVert \sum_{i=0}^{m-1}P^i(P-Q)Q^{m-1-i}\right\rVert       \\
                & \leq \sum_{i=0}^{m-1}\|P^i\|\,\|P-Q\|\,\|Q^{m-1-i}\| \\
...
$$
\begin{aligned}
    \tilde{x}^{2m}=\left(e^{\frac{A}{m}}e^{\frac{B}{m}}\right)^m v=Q^m v
\end{aligned}
$$
