...
- **矩阵指数与算子范数**：逐项估计指数级数及一、二阶余项。
- **Lie–Trotter 乘积公式**：比较 $e^{A+B}$ 与 $(e^{A/m}e^{B/m})^m$。
- **望远镜型矩阵恒等式**：在不要求矩阵交换的条件下控制幂差。
- **算子分裂法**：把联合动力系统拆成 $A$、$B$ 两个易求解子流交替推进。
- **全局误差阶**：由单步 $O(m^{-2})$ 误差累积得到终点 $O(m^{-1})$ 估计。
