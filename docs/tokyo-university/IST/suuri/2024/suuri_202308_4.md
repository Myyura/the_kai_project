...
p_Y(y)=
\begin{cases}
2\lambda\rm{e}^{-2\lambda y} & (y\geq0),\\
0 & (y<0).
\end{cases}
$$
...
p_{X_1+\dots+X_n}(x)=
\begin{cases}
\dfrac{\lambda^n x^{n-1}}{(n-1)!}\rm{e}^{-\lambda x} & (x\geq0),\\
0 & (x<0).
\end{cases}
$$
...
$n\geq1$ ではこれが最尤推定値である。$n=0$ では
$L(\lambda)=e^{-\lambda a}$ は $\lambda>0$ 上で狭義単調減少するため、
問題文の母数空間のままでは最尤推定値は存在せず、上限は
$\lambda\downarrow0$ で達せられる。母数空間の閉包
$\lambda\geq0$ を用いる通常の拡張最尤推定量は
