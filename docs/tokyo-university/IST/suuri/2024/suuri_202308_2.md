...
- **凸性与连续 Lyapunov 分析**：对 $E(t)$ 求导并使用凸函数的一阶不等式，证明加速梯度流的能量单调不增。
- **连续时间收敛率**：从 $E(t)\leq E(0)$ 直接提取函数值误差的 $O(t^{-2})$ 上界。
- **前向差分恒等式**：使用离散乘积法则推导平方范数的差分公式，保留关键的负二次修正项。
- **离散能量与收敛率**：把离散方程代入 $\delta^+E^{(k)}$ 并利用凸性证明能量衰减，再得到 $O(k^{-2})$ 的函数值误差界。
