...
- **逼近误差的比较**：用三角不等式把相邻近似函数之差控制在给定的 $O(n^{-3})$ 误差内。
- **Bernstein 不等式**：对 $t_{n+1}-t_n$ 应用导数范数估计，建立相邻导函数之差的可求和上界。
- **完备性与一致收敛**：证明 $\{t_n'\}$ 在 $\mathrm C[0,2\pi]$ 中为 Cauchy 列，并借助完备性得到连续周期极限。
- **一致极限下的可微性**：结合 $t_n\to f$ 与 $t_n'\to s$ 的一致收敛，通过积分关系证明 $f'=s$。
