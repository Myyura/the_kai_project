...
tags:
  - Tokyo-University
  - Mathematics.Real-Analysis.Differentiability-from-Polynomial-Approximation-Rate
  - Mathematics.Real-Analysis.Bernstein-Inequality-for-Trigonometric-Polynomials
  - Mathematics.Functional-Analysis.Uniform-Convergence
---
# 東京大学 情報理工学研究科 数理情報学 2023年8月実施 第3問

## **Author**
[Kurosu9991](https://github.com/Kurosu9991), 祭音Myyura
...
以 $\mathbb{R}$、$\mathbb{C}$ 分别表示实数集和复数集，以
$\mathrm{i}$ 表示虚数单位，以 $\mathrm{e}$ 表示自然对数的底。
对以 $2\pi$ 为周期的函数 $g:\mathbb{R}\to\mathbb{C}$，定义
...
\mathcal{T}_n=
\left\{
t:\mathbb{R}\to\mathbb{C}\ \middle|\
\begin{array}{l}
\text{存在一个复系数的 $n$ 次多项式 $P$，}\\
t(\theta)=P(e^{\mathrm{i}\theta})
\end{array}
\right\}.
$$

设 $f:\mathbb{R}\to\mathbb{C}$ 是以 $2\pi$ 为周期的连续函数，并且
对每个正整数 $n$，均存在 $t_n\in\mathcal{T}_n$ 满足

$$
\|f-t_n\|\leq\frac{c}{n^3},
$$

其中 $c>0$ 是与 $n$ 无关的常数。回答下列问题。
...
(2) 对 $t\in\mathcal{T}_n$，以 $t'$ 表示其导函数。证明函数序列
$\{t_n'\}$ 在 $[0,2\pi]$ 上一致收敛于某个以 $2\pi$ 为周期的
连续函数 $s:\mathbb{R}\to\mathbb{C}$。

可以不加证明地使用以下两个事实：

- 以 $\mathrm{C}[0,2\pi]$ 表示 $[0,2\pi]$ 上所有复值连续函数
  的集合，则赋范空间
  $(\mathrm{C}[0,2\pi],\|\cdot\|)$ 是完备的。

- 对每个正整数 $n$ 和任意 $t\in\mathcal{T}_n$，都有

  $$
  \|t'\|\leq n\|t\|.
  $$

(3) 证明 $f$ 在 $\mathbb{R}$ 上可微。
...
$$
\exists s\in\rm{C}[0,2\pi], \quad t_n'\rightrightarrows s
$$
...
$$
f(\theta)-f(0)
=\lim_{n\to\infty}\{t_n(\theta)-t_n(0)\}
=\lim_{n\to\infty}\int_0^\theta t_n'(u)\,du
=\int_0^\theta s(u)\,du
$$

となる。したがって $f'(\theta)=s(\theta)$ である。
また、各 $t_n'$ は $2\pi$ 周期であるから、その一様極限 $s$ も
$2\pi$ 周期である。よって $f$ は $\mathbb{R}$ 上微分可能である。
