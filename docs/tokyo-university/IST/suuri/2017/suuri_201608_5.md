...
- **邻接矩阵与游走计数**：用矩阵幂统计固定长度的顶点间游走。
- **谱与图直径**：利用最小多项式或 Krylov 向量空间证明不同特征值个数控制距离。
- **图 Laplacian**：识别零特征向量 $\mathbf1$、非零特征空间与其正交关系。
- **半正定性**：把二次型写成边差平方和，证明 Laplacian 特征值非负。
- **一致性动力系统**：将梯度流写成 $\dot x=-Lx$，由代数连通度决定向平均共识的指数收敛速率。
