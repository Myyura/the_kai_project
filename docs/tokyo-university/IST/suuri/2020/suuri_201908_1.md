...
tags:
  - Tokyo-University
  - Mathematics.Linear-Algebra.Sylvester-Equation-with-Positive-Definite-Coefficient
---
...
设 $n$ 为正整数，$A$ 为 $n$ 阶实方阵，$B$ 为 $n$ 阶实对称正定矩阵。
...
\begin{aligned}
                    & AB=BA                                                             \\
    \Leftrightarrow & CB^2=B^2C.
\end{aligned}
$$

$C'=Q^TCQ$, $D=\operatorname{diag}(d_1,\ldots,d_n)$ とおくと、最後の条件は

$$
(d_j^2-d_i^2)c'_{ij}=0\quad(1\leq i,j\leq n)
$$

である。$d_i,d_j>0$ より $d_i^2=d_j^2$ と $d_i=d_j$ は同値なので、これは $(d_j-d_i)c'_{ij}=0$、すなわち $C'D=DC'$ を意味する。よって $CB=BC$ である。
