...
tags:
  - Tokyo-University
  - Discrete-Mathematics.Graph-Theory.Heat-Equation-on-Cycle-Graph-by-Discrete-Fourier-Transform
---
...
考虑定义于整数 $n$ 和 $t\ge0$ 的函数 $x(n,t)$，满足离散热方程
$$
\frac{\partial x(n,t)}{\partial t}
=x(n-1,t)+x(n+1,t)-2x(n,t)
$$
以及周期条件
$$x(n+N,t)=x(n,t),$$
其中整数 $N\ge3$。定义 Fourier 模式
$$
e(m,n)=\exp\!\left(i\frac{2\pi mn}{N}\right).
$$

1. 对整数 $m$，设
   $$x(n,t)=e(m,n)f_m(t),\qquad f_m(0)=c_m,$$
   其中 $c_m\in\mathbb C$。在方程与周期条件下求
   $f_m(t)$。
2. 给定初始向量 $(g_0,\ldots,g_{N-1})\in\mathbb C^N$，在
   $$x(n,0)=g_n\quad(n=0,\ldots,N-1)$$
   下，用离散 Fourier 展开求周期解 $x(n,t)$。
3. 求该解的长期极限
   $$\lim_{t\to\infty}x(n,t).$$

#### 考点

- **循环图上的离散热方程**：把右端识别为周期离散 Laplacian。
- **离散 Fourier 变换**：用复指数模式同时对角化循环差分算子。
- **常微分方程模态分解**：每个 Fourier 系数独立按相应特征值指数演化。
- **扩散与平均共识**：非零频率模式衰减，零频模式保留初值平均。
