...
より、$M$ は $g_i$ の倍数を除いて $c\,x_i^{a_i-m_i}y_i^{b_i-m_i}\prod_{j\ne i}x_j^{a_j}y_j^{b_j}$ に置き換えられる。これを全ての $i$ と全ての単項式に行えば、
...
可換単位的環 $R$ の部分集合 $S$ が生成するイデアルは以下の通りである（$S$ が有限なら有限生成である）。
