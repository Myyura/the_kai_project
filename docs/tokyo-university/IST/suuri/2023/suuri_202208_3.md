...
- **单位圆参数化**：通过 $z=e^{i\theta}$ 将周期积分 $I(f)$ 改写为单位圆上的围道积分。
- **极点与留数**：识别 $z^N=1$ 的全部单位根为简单极点，计算留数并把离散求和 $I_N(f)$ 表示为围道积分。
- **解析函数的误差估计**：在半径 $1<r'<r$ 的圆周上比较两个积分核，结合一致范数后令 $r'\nearrow r$，得到指数级误差界。
- **最优常数**：构造随 $N$ 变化的全纯测试函数，使上界在渐近意义下达到 $2\pi$，从而证明常数不可改进。
