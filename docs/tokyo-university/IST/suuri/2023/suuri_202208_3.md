...
tags:
  - Tokyo-University
  - Mathematics.Numerical-Analysis.Exponential-Convergence-of-Trapezoidal-Rule-for-Analytic-Periodic-Functions
...
$$
\begin{align}
|I(f) - I_N(f)| \le \frac{2 \pi \lVert f \rVert}{r^N-1} \tag{*}
\end{align}
$$
...
以 $\mathbb{C}$ 表示复数集，以 $i$ 表示虚数单位。对实数
$r>1$，令
...
对 $D_r$ 上的全纯函数 $f:D_r\to\mathbb{C}$，定义一致范数

$$
\lVert f\rVert=\sup_{z\in D_r}|f(z)|,
$$
...
\qquad
I_N(f)=\frac{2\pi}{N}\sum_{k=1}^N f(e^{2\pi ik/N}).
$$

回答下列问题。

(1) 对实数 $R>0$，令 $\Gamma(R)$ 为复平面上以 $0$ 为圆心、半径为
$R$、按正向（逆时针方向）定向的圆周。证明

$$
I(f)=\oint_{\Gamma(1)}\frac{-i}{z}f(z)\,dz.
$$

(2) 定义

$$
g_N[f](z)=\frac{-iz^{N-1}}{z^N-1}f(z).
$$

求 $g_N[f]$ 在 $D_r$ 内的全部极点，并求它在每个极点处的留数。

(3) 假设 $\lVert f\rVert<\infty$。证明

$$
|I(f)-I_N(f)|
\leq\frac{2\pi\lVert f\rVert}{r^N-1}.
\tag{*}
$$

(4) 证明式 $(*)$ 右端的常数 $2\pi$ 是最优的，即

$$
\limsup_{N\to\infty}
\left(
r^N\sup_f\frac{|I(f)-I_N(f)|}{\lVert f\rVert}
\right)=2\pi,
$$

其中上确界取遍所有满足 $\lVert f\rVert<\infty$ 的
$D_r$ 上全纯函数 $f:D_r\to\mathbb{C}$。
...
### (2)
$D_r$ 内の特異点候補は、$z^N-1=0$ より
$z=\zeta_N^k\ (k=1,\ldots,N)$ である。ただし、
$\zeta_N=e^{\frac{2\pi i}{N}}$ とする。
$f(\zeta_N^k)\ne0$ ならば1位の極であり、
$f(\zeta_N^k)=0$ ならば可除特異点となる。
...
\begin{aligned}
  |{I(f)-I_N(f)}| & =\left |\oint_{\Gamma(1)} \frac{-i}{z}f(z) \text{d}{z} - \frac{2\pi}{N} \sum_{k=1}^{N} f(\zeta_N^k) \right| \\
                    ...
                    & \leq \lVert f \rVert \oint_{\Gamma(r')} \left |\frac{-i}{z} - \frac{-iz^{N-1}}{z^N-1}\right| |\mathrm{d}z|          \\
                    & = \lVert f \rVert \oint_{\Gamma(r')} \left | \frac{1}{z(z^N-1)} \right | |\mathrm{d}z|                                 \\
...
### (4)
比の上限では $f\not\equiv0$ とする。(3) より、各 $N$ に対して

$$
r^N\sup_f\frac{|I(f)-I_N(f)|}{\|f\|}
\le\frac{2\pi r^N}{r^N-1}\longrightarrow2\pi.
$$
...
となり、$\lVert f\rVert=r^N$ である。

よって、

$$
\begin{aligned}
  \limsup_{N \to \infty} \left(r^N \sup_f \frac{|I(f)-I_N(f)|}{\lVert f \rVert} \right)
  \geq r^N\frac{|0-2\pi|}{r^N}=2\pi
\end{aligned}
$$
