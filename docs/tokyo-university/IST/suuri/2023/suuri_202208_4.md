...
- **随机变量变换**：处理 $Y=1/X^2$ 的两个原像及 Jacobian，求出其重尾概率密度。
- **拉普拉斯变换微分方程**：在积分号下求导并作适当变量代换，建立并求解 $L(u)$ 所满足的一阶方程。
- **特征函数与复平方根**：选择复平方根的正确分支，通过拉普拉斯变换的解析延拓和边界极限求 $\phi(u)$。
- **稳定分布与分布收敛**：利用独立性将归一化和的特征函数写成乘积，识别 $n^2$ 缩放下的极限并还原其密度。
