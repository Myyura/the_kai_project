...
tags:
  - Tokyo-University
  - Probability-Statistics.Probability-Distributions-and-Asymptotics.Levy-Distribution-from-Inverse-Chi-Square
...
设随机变量 $X$ 服从均值为 $0$、方差为 $1$ 的正态分布，并以
$f(y)$ 表示随机变量
...
所服从分布的概率密度函数。以 $i$ 表示虚数单位，以
$\mathbb{R}$ 表示实数集，并以 $\mathbb{E}[Z]$ 表示随机变量
$Z$ 的期望。回答下列问题。
...
$$
\frac{dL(u)}{du}=-\frac{1}{\sqrt{2u}}L(u).
$$
...
(4) 设随机变量 $Y_1,\ldots,Y_n$ 相互独立且同分布，其共同分布的
概率密度函数为 $f(y)$。证明当 $n\to\infty$ 时，

$$
\frac{1}{n^2}(Y_1+\cdots+Y_n)
$$

依分布收敛，并求其极限分布的概率密度函数。
...
\begin{aligned}
  \frac{\text{d}L(u)}{\text{d}u} & =\frac{\text{d}}{\text{d}u} \int_0^{\infty} {e^{-uy}f(y)}\text{d}{y}                                                                        \\
               ...
               e^{-uy-\frac{1}{2y}}y^{-\frac{1}{2}} \text{d}{y}.
\end{aligned}
$$
...
$$
\int_0^\infty e^{-uy-\frac{1}{2y}}y^{-\frac12}\,dy
=\frac{1}{\sqrt{2u}}
\int_0^\infty e^{-uz-\frac{1}{2z}}z^{-\frac32}\,dz.
$$
...
微分と積分の交換は、固定した $u>0$ の近傍で
$y e^{-uy}$ が有界であり、定数倍の $f(y)$ で優収束定理を適用できることから従う。
...
実軸正の部分から、$u \searrow 0$ とすれば、$M(u)=L(u^2) \to L(0)=1$ となる。
...
\begin{aligned}
  \left \lvert \frac{e^{-(u+h)y}-e^{-uy}}{h} \right \rvert & = \left \lvert \frac{1}{h}\int_u^{u+h} -y e^{-ky} \text{d}{k}\right \rvert                  \\
                                      ...
                                      & =y e^{-\frac{a}{2}y}
                                      \leq \frac{2}{ae}=:C \quad (h \text{によらず有界})
\end{aligned}
$$
...
よって、特性関数の一意性より各 $n$ について
$n^{-2}\sum_{i=1}^nY_i$ は $Y$ と同分布であり、特に $Y$ に分布収束する。
極限分布の確率密度関数は

$$
\begin{cases}
\dfrac{1}{\sqrt{2\pi}}e^{-\frac{1}{2y}}y^{-\frac32} & (y>0),\\
0 & (y\leq0)
\end{cases}
$$
