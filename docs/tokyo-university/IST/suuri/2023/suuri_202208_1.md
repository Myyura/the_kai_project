...
tags:
  - Tokyo-University
  - Mathematics.Real-Analysis.Exponential-Kernel-Sequence-Divergence-and-Continuum-Limit
---
...
对正整数 $m,n$ 以及有限实数序列
$A=\{a_i\}_{i=1}^m$、$B=\{b_j\}_{j=1}^n$，定义
...
(1) 证明 $f(A,B)\geq 0$，并求使 $f(A,B)=0$ 成立的 $A,B$ 的充要条件。

(2) 证明：对任意三个非空有限实数序列 $A,B,C$，均有
...
$$
A_m(s)=\left\{s+\frac{i}{m}\right\}_{i=1}^m,
\qquad
B_n=\left\{\frac{j}{n}\right\}_{j=1}^n,
$$
...
#### <center> 図1: $h(z)$ のグラフ</center>

よって、
$q(z)=1/h(z)$ と $J(s)=\int_s^{s+1}q(z)\,dz$ をおく。
$h(z)$ は $z=1/2$ からの距離に関して狭義単調減少するので、
$q(z)$ はその距離に関して狭義単調増加する。また、

$$
J'(s)=q(s+1)-q(s)
$$

である。$s<0$ なら $|s-1/2|>|s+1-1/2|$ より $J'(s)<0$、
$s>0$ なら $J'(s)>0$ である。したがって $J(s)$、従って
$g(s)=\log J(s)$ は $s=0$ で一意に最小となる。
