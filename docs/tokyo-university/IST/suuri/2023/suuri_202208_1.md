...
- **非负性与等号条件**：利用 $e^{-|a_i-b_j|}\leq1$ 估计定义式，并从逐项取等刻画 $f(A,B)=0$。
- **指数核与三角不等式**：由 $|a-c|\leq|a-b|+|b-c|$ 得到指数核的乘法型下界，进而证明序列泛函的三角不等式。
- **黎曼和与连续极限**：把等距有限序列上的两重平均依次化为积分，并对含绝对值的核积分 $h(z)$ 作分段计算。
- **对称性与最优化**：分析 $h(z)$ 的对称性和单调性，比较长度为 $1$ 的滑动区间上的积分以确定 $g(s)$ 的最小点。
