...
- **平衡点与变量平移**：由 $F(x^*)=0$ 消去 $v$，再令 $z=x-x^*$ 将广义 Lotka–Volterra 方程写成矩阵形式。
- **熵型 Lyapunov 函数**：沿轨道求 $L$ 的导数，把其符号归结为由 $CA+A^TC$ 决定的二次型，并证明题设的充要关系。
- **加权梯度表示**：计算 $\nabla H_w(z)=Wz$，用 $A,W,X^*,Z$ 明确写出满足运动方程的矩阵函数 $G(z)$。
- **对角稳定性与局部衰减**：依据 $CA+A^TC$ 的负定性选择权重 $w$，并控制邻域内的扰动项 $Z$，使 $H_w$ 沿非平衡轨道严格下降。
