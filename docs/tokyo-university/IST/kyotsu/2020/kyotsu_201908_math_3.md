...
アルバイトの採用において次のような戦略をとる。ただし、$1 < r \leq n$ とする。
...
采用阈值策略（$1<r\le n$）：无条件拒绝前 $r-1$ 人；此后遇到比前
...
绝对第一名出现在第 $k$ 位的概率为 $1/n$，故当 $2\le r\le k\le n$ 时

$$
P = \frac{1}{n}\frac{r-1}{k-1}.
$$

### (4)

$$
P_n(r) = \frac{r-1}{n} \left( \frac{1}{r-1} + \frac{1}{r} + \cdots + \frac{1}{n-1} \right) \qquad (2\le r\le n)
$$
...
As $n\to\infty$ with $r/n\to q\in(0,1]$, the harmonic sum is a Riemann sum for $\int_q^1dt/t=-\ln q$. Thus $P_n(r)\to-q\ln q$. Since $f''(q)=-1/q<0$, the stationary point $q=1/e$ is the unique maximizer, with maximum $1/e$.
