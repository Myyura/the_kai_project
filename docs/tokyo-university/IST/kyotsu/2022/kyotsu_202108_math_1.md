...
（1）仅用关于 $x,y$ 的不等式表示 $\Omega$。

（2）在 $xy$ 平面上画出 $\Omega$；若边界与坐标轴相交，标明交点坐标。

（3）$\Omega$ 边界中的曲线段可由单位圆的若干圆弧经某个线性变换
$X$ 得到。求一个这样的 $X$，并要求单位圆上的 $(1,0)$ 被映到该曲线段曲率最大的点。

（4）求第（3）问线性变换矩阵的行列式。

（5）求 $\Omega$ 的面积。可以使用：线性变换使面积乘以其行列式的绝对值。

#### 考点

- **消元与平面区域刻画**：把关于 $z$ 的两个开区间存在公共点转化为
  $x,y$ 不等式。
- **二次曲线与线性像**：识别边界椭圆弧，并由单位圆构造满足指定方向的线性变换。
- **矩阵行列式与面积缩放**：利用 $|\det X|$ 把圆扇形或圆弧围成区域的面积转换到目标图形。
