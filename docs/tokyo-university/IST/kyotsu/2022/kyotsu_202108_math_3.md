...
在单位正方形
$R=\{(x,y)\mid0<x<1,\ 0<y<1\}$ 上均匀随机选点
$A=(X,Y)$。从 $A$ 分别向 $y$ 轴、$x$ 轴作垂线，和原点
$O$ 围成轴对齐矩形；令其面积随机变量为
$S=XY$。回答下列问题。

（1）求 $\mathbb E[S]$。

（2）对 $0<r<1$，求 $P(S\le r)$。

（3）求 $S$ 的概率密度函数。

再在 $R$ 中独立、均匀地随机选择 $n$ 个点
$A_1,\ldots,A_n$，令 $S_i$ 为第 $i$ 个点对应矩形的面积，并令
$$
Z=\min(S_1,\ldots,S_n).
$$
...
- **独立均匀变量之积**：将矩形面积写为 $XY$，用二维区域积分求其分布函数。
- **分布函数求密度**：对含对数项的 $P(S\le r)$ 求导。
- **最小次序统计量**：利用独立性写出
  $P(Z>z)=P(S_1>z)^n$，再求导得到密度。
