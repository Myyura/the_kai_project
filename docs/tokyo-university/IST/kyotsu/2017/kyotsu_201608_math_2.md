...
实值函数 $u(x,t)$ 定义在 $0\le x\le1,\ t\ge0$ 上。求热方程
$$
...
（1）对正整数 $n,m$，计算
$$
\int_0^1\sin(n\pi x)\sin(m\pi x)\,\mathrm dx.
$$

（2）设可分离变量
$u(x,t)=\xi(x)\tau(t)$。用任意分离常数 $C$ 写出
$\xi,\tau$ 分别满足的常微分方程。可以使用：若仅依赖 $x$ 的函数与仅依赖
$t$ 的函数对任意 $x,t$ 恒等，则二者均为常数。

（3）求解第（2）问的常微分方程，并证明满足边界条件的一组解可写为
$$
u_n(x,t)=e^{\alpha t}\sin(\beta x),
$$
用正整数 $n$ 表示 $\alpha,\beta$。

（4）满足边界、初始条件的解写成
$$
u(x,t)=\sum_{n=1}^{\infty}c_nu_n(x,t).
$$
求各系数 $c_n$；可以使用第（1）问的正交积分结果。

#### 考点

- **正弦函数正交性**：计算 Fourier 正弦基在 $[0,1]$ 上的内积。
- **偏微分方程分离变量法**：把热方程化为时间指数方程和空间特征值问题。
- **Dirichlet 热方程**：由零边界条件得到正弦特征函数及衰减率。
- **Fourier 正弦级数**：把初始函数 $x-x^2$ 展开并确定各模态系数。
