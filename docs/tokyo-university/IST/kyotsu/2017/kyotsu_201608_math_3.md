...
tags:
  ...
  - Probability-Statistics.Stochastic-Processes.Exponential-Race-and-Order-of-Arrival
---
...
\qquad \lambda>0.
$$
即 $T$ 服从参数为 $\lambda$ 的指数分布。求 $T$ 的均值及分布函数
$F(t)=P(T\le t)$。
...
$\lambda_0$ 的指数分布。求最先完成者所需时间的分布函数和均值。

（4）学生 A、B 的完成时间相互独立，分别服从参数
$\lambda_A,\lambda_B$ 的指数分布。求 A 先于 B 完成的概率。
...
上式对 $t\ge0$ 成立；$t<0$ 时分布函数为 $0$。对 $n$ 个相互独立且服从参数为 $\lambda_0$ 的指数分布的随机变量，其中的最小值服从参数为 $n\lambda_0$ 的指数分布。
