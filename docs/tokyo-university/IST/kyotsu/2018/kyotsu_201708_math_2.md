...
（2）对 $n\ge2$ 定义
$g_n(x)=x^{a_n}-x^p$。利用 $a_n\ge1$，证明 $g_n$ 在某点
$x_n$ 取得最大值，并求 $x_n$。
...
（4）令 $d_n=(c_n)^{q^n}$。证明
$d_{n+1}/d_n$ 在 $n\to\infty$ 时收敛到有限正值。可以使用
$\lim_{t\to\infty}(1-1/t)^t=e^{-1}$。
...
- **非线性函数递推**：归纳识别幂函数形式并分析指数、系数数列。
- **一致误差的极值分析**：对 $x^{a_n}-x^p$ 求导，定位区间上的最大偏差。
- **渐近数列**：求 $a_n\to p$，并通过指数缩放后的 $d_n$ 分析 $c_n$ 极限。
- **函数列收敛**：结合指数与系数极限证明 $f_n$ 的逐点收敛，并以最大误差理解更强收敛。
