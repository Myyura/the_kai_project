...
(2) $z_n = 1$ である確率を $P_n$, $z_n = i$ である確率を $Q_n$ とする。$P_n, Q_n$ についての漸化式を立てよ。
...
袋中有两张红牌和一张白牌。设复随机变量
$z_n,w_n\ (n=0,1,\ldots)$ 初值均为 $1$。每轮先有放回地抽一张牌，并按
...
更新 $w$。每张牌等概率被抽到，$i$ 为虚数单位。回答下列问题。

（1）证明 $n$ 为奇数时 $\operatorname{Re}(z_n)=0$，为偶数时
$\operatorname{Im}(z_n)=0$。

（2）记
$P_n=P(z_n=1)$、
$Q_n=P(z_n=i)$，建立关于 $P_n,Q_n$ 的递推关系。

（3）分别求 $z_n=1,i,-1,-i$ 的概率。

（4）证明 $\mathbb E[z_n]=(i/3)^n$。

（5）求 $P(z_n=w_n)$。

（6）求 $\mathbb E[z_n+w_n]$。
...
则当 $n=2k+2$ 时，$z_{2k+2}, w_{2k+2}$ 可能取值为 $\pm iz_{2k+1}, \pm iw_{2k+1}$ 为实数；$n=2k+3$ 时，$z_{2k+3}, w_{2k+3}$ 可能取值为 $\pm iz_{2k+2}, \pm iw_{2k+2}$ 为纯虚数。
...
$$
E[z_nw_n] = E[z_n]E[w_n] = (\frac{1}{9})^n
$$
