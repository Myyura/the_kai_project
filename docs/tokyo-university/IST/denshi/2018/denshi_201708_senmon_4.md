...
パケット数でウィンドウを $N$ と表し、ヘッダの占める割合を無視するモデルでは $W=NM$ だから、$N\ge\lceil B_{\min}R/M\rceil$ となる。さらに ACK も $M$ Bytes、各ルータは蓄積交換、処理・待ち行列遅延を無視すると、

$$
R=2\sum_{i=1}^3\left(D_i+\frac{M}{B_i}\right)
$$

を用いればよい。実際の TCP のウィンドウはデータのバイト数を数えるので、ヘッダを含む IP パケット長とは区別する。
