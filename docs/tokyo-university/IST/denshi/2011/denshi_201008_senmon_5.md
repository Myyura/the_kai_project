...
ここで $x(l)=0\ (l<0)$ を用いた。片側 $z$ 変換で $m=-r<0$ とする場合は、

$$
\mathcal Z\{x(n+r)\}=z^r\left(X(z)-\sum_{k=0}^{r-1}x(k)z^{-k}\right)
$$

となる。両側 $z$ 変換なら任意の整数 $m$ に対してシフト則が成り立つ。
...
### (5)
因果的なシステムなので、$H(z)$ の収束領域は $|z|>|ab|$ である。
