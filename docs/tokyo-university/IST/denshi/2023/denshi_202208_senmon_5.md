...
tags:
  - Tokyo-University
  - Electrical-Electronic.Signal-Processing.Z-Transform-of-Geometric-Sequence
  - Electrical-Electronic.Signal-Processing.Discrete-Time-System-Transfer-and-Frequency-Response
---
...
(1) 对定义于 $n\ge0$ 的离散信号序列 $x_n$（$n=0,1,2,\ldots$），写出其 $\mathcal Z$ 变换 $X(z)$ 的定义，其中 $z$ 为复数。

(2) 利用定义求实数 $p$ 所确定的几何序列 $x_n=p^n$ 的 $\mathcal Z$ 变换。

(3) 考虑上图所示离散时间系统的零状态响应，其中 $a,b,c$ 为实数。设系统内部状态 $q_n$ 的 $\mathcal Z$ 变换为 $Q(z)$，用 $Q(z)$ 表示系统输入 $x_n$、输出 $y_n$ 的 $\mathcal Z$ 变换 $X(z),Y(z)$。

(4) 求该系统的传递函数 $H(z)$。

(5) 利用 $H(z)$ 求系统的频率响应。连续复指数

$$
x(t)=e^{j\omega t}\qquad(t\ge0)
$$

以间隔 $T$ 采样后得到输入序列 $x_n=e^{jn\omega T}$。推导 $Y(z)$，进而求输出序列 $y_n$。

#### 考点

- 几何序列的 $\mathcal Z$ 变换：要求由定义求和得到基本有理式，并用于处理复指数输入。
- 离散系统传递与频率响应：要求从图示内部状态关系建立 $X(z),Y(z),H(z)$，再逆变换得到系统对采样复指数的响应。
