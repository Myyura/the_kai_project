...
- **三角形支撑集上的联合密度**：标签表明题目涉及定义在三角形区域上的二维联合密度及其归一化或概率计算，但具体函数与区域缺失。
- **条件密度与边缘密度**：标签表明需要由联合分布求条件密度、边缘密度，并据此判断随机变量是否独立；原文未提供所需公式。
- **仿射变换下的协方差**：标签表明题目涉及随机变量线性变换后的协方差关系，具体变量与系数未给出。
