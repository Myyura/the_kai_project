...
- **拉普拉斯分布的矩、分布函数与分位数**：标签表明题目涉及推导或计算拉普拉斯分布的这些基本量，但具体参数与问题缺失。
- **拉普拉斯位置参数的单侧检验**：标签表明题目涉及针对位置参数构造单侧假设检验并计算检验功效，但原文未给出显著性水平、样本或备择假设等条件。
- **统计功效**：标签表明需要理解或计算拒绝错误原假设的概率，具体检验设置未提供。
