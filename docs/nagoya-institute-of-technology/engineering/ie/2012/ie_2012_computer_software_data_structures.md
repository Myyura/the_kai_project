---
sidebar_label: "2012年度 計算機ソフトウェア I"
tags:
...
$A_n\ge B_n$ とする。$A_n-B_n$ の減算に必要な時間計算量を $D(n)$ とするとき、$D(n)=O(n)$ を示せ。
...
$T[i]\in\{0,1\}$ を $i$ 桁目へ入る借りとし、$T[0]=0$ とする。各 $i=0,\ldots,n-1$ について
...
と評価できる。$F(n)=M(n+2)$ とおけば $F(n)\le3F(n/2)+O(n)$ となるので、マスター定理より
