---
sidebar_label: "2016年度 計算機ソフトウェア（データ構造とアルゴリズム）"
tags:
...
# 名古屋工業大学 工学研究科 情報工学専攻 2016年度 計算機ソフトウェア（データ構造とアルゴリズム）
...
上の 3 頂点グラフについて書き換え後の擬似コードを実装すると、`distance[s,a,b]=[2,3,2]` となった。一方、全単純路を列挙すると正しい最長距離は `[0,1,2]` であり、特に頂点 $a$ で反例になることを確認した。
