...
- **二次型与正交对角化**：由实对称矩阵的标准正交特征基旋转坐标，将二次曲线化为标准形。
- **实对称矩阵的谱性质**：用 Hermitian 共轭和内积证明特征值为实数。
- **径向场的散度**：逐分量计算 $\boldsymbol r/r^3$ 在原点外的散度。
- **Gauss 散度定理与奇点通量**：区分闭曲面是否包围原点，并以小球挖孔处理场在原点的奇性。
