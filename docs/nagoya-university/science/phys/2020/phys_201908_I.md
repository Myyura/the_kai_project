...
- **圆的渐开线运动学**：由接触点位置与切向展开线段构造质点坐标，再求速度。
- **Lagrangian 力学**：用曲线约束后的广义坐标写动能，推导非线性运动方程并验证初值解。
- **循环坐标与守恒广义动量**：在圆柱可转动时识别 $\psi$ 不显含于 Lagrangian，由此得到系统总角动量型守恒量。
- **能量与角动量守恒辨析**：区分固定约束下质点动能、关于 O 的角动量，以及可动约束中质点与圆柱之间的能量交换。
