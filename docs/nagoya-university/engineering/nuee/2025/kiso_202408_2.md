...
- **特征值与特征向量**：求具有重特征值的矩阵谱及各特征空间的参数表示。
- **广义特征向量**：解 $(A-\lambda_2I)\boldsymbol v_3=\boldsymbol v_2'$，并施加单位范数条件。
- **Jordan 标准形与矩阵幂**：由特征向量链构造 $P^{-1}AP=J$，利用 Jordan 块幂中的 $n\lambda^{n-1}$ 项求指定矩阵元素。
