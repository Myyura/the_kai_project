...
---
# 名古屋大学 工学研究科 電気系専攻 2023年8月実施 基礎2
...
1. 对
   ...
   求全部特征值及相应特征向量；用正交矩阵 $U$ 将 $A$ 对角化，并求自然数 $n$ 对应的 $A^n$。
2. 设二阶实方阵 $A,P,Q$ 满足

   $$
   A=\lambda_1P+\lambda_2Q,\qquad P+Q=E,
   $$

   其中 $\lambda_1,\lambda_2$ 为 $A$ 的特征值且 $|\lambda_1|\le|\lambda_2|$，$E$、$O$ 分别为二阶单位矩阵与零矩阵。对

   $$
   A=\begin{pmatrix}a&0\\2&a+2\end{pmatrix},\qquad a>0,
   $$

   求 $\lambda_1,\lambda_2$ 和 $P,Q$；证明 $P^2=P$、$PQ=O$；并用 $a,n$ 写出 $A^n$ 的全部元素。
...
とおけば $U$ は直交行列であり、$U^TAU=\operatorname{diag}(-3,-1)$ である。
...
#### 4)
$PQ=O$ であり、$Q=E-P$ なので $QP=O$ でもある。したがって、
