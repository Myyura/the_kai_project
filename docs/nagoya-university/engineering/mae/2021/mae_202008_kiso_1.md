...
1. 不定積分 $\int\sin^4x\cos^3x\,dx$ を求める。
2. $a\ne0$ とする。
   - 三角関数による置換を用いて $\int dx/(x^2+a^2)$ を計算する。
   - 正の整数 $n$ について、次の漸化式を示す。
   ...
   - この式から $\int dx/(x^2+4)^3$ を求める。
3. カージオイド $r=a(1+\cos\theta)$ が囲む面積と曲線全体の長さを計算する。
...
,
\\
\frac{d}{dx} \left( \frac{x}{2na^2(x^2+a^2)^n} + \frac{2n-1}{2na^2} \int \frac{1}{(x^2+a^2)^n} dx \right)
&= \frac{1}{2na^2} \frac{(x^2+a^2)^n - 2nx^2 (x^2+a^2)^{n-1}}{(x^2+a^2)^{2n}} + \frac{2n-1}{2na^2} \frac{1}{(x^2+a^2)^n}
\\
&= \frac{1}{(x^2+a^2)^{n+1}}
\end{aligned}
$$
...
$$
  ...
  &= \frac{x}{16(x^2+4)^2} + \frac{3}{16} \int \frac{1}{(x^2+4)^2} dx + C_0
  \ \ \ \ \ \ \ \ ( C_0 \text{ は任意定数 } )
  \\
  &= \frac{x}{16(x^2+4)^2} + \frac{3}{16} \frac{x}{8(x^2+4)} + \frac{3}{16} \frac{1}{8} \int \frac{1}{x^2+4} dx + C_1
  \ \ \ \ \ \ \ \ ( C_1 \text{ は任意定数 } )
  \\
