...
- **NumPy 数组运算**：理解广播、点积、切片、按轴归约和 reshape 的精确输出。
- **Python 可变对象、不可变对象与作用域**：区分就地修改对象和局部重新绑定，识别字符串不可变及 `global/nonlocal` 相关错误。
- **浮点数比较**：解释二进制舍入使精确相等条件不可靠。
- **素数枚举**：补全试除逻辑并注意函数按降序遍历。
- **滑动窗口模式匹配**：在二维数组上比较局部块的逐元素误差，并把匹配区域替换为指定值。
