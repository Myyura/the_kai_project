...
**[1]** 有两枚硬币：A 的正面概率为 $1/3$，B 的正面概率为 $2/3$。答案写成最简分数。

1. 抛 A 五次，求恰有两次正面的概率。
2. 抛 A 四次，求出现至少两个连续正面的概率。
3. 随机等概率选择一枚硬币，抛四次后恰有一次正面；求所选为 A 的后验概率。
...
求归一化常数 $k$、累积分布函数 $F(x)$；分别求满足 $F(x)=1/2$ 与 $F(x)=5/32$ 的 $x$；再令 $Y=2X+1$，求 $Y$ 的密度 $g(y)$。

#### 考点

- **二项与组合概率**：计算固定正面次数，并通过补集或枚举处理连续正面事件。
- **Bayes 定理**：结合两枚硬币的先验概率及观测似然求后验概率。
- **密度与分布函数**：归一化分段多项式密度并积分得到完整 CDF。
- **随机变量线性变换**：由反函数及 Jacobian 求 $Y=2X+1$ 的密度和支持集。
