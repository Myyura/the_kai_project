...
- 硬币 A 出现正面、反面的概率分别为 $\dfrac13,\dfrac23$；
- 硬币 B 出现正面、反面的概率分别为 $\dfrac23,\dfrac13$。

下列各题均以最简分数作答。

1. 将硬币 A 投掷 $5$ 次，求正面恰好出现 $2$ 次的概率；
2. 将硬币 A 投掷 $4$ 次，求投掷序列中出现至少两个连续正面的概率；
3. 从 A、B 中等概率随机选取一枚，投掷 $4$ 次后只出现 $1$ 次正面。求所选硬币为 A 的条件概率。
...
1. 求 $k$；
2. 求累积分布函数 $F(x)$；
3. 分别求满足 $F(x)=\dfrac12$ 与 $F(x)=\dfrac5{32}$ 的 $x$；
4. 定义 $Y=2X+1$，求 $Y$ 的概率密度函数 $g(y)$。
