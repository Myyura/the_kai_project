...
$$
\frac{d^3x}{dt^3}-3\frac{d^2x}{dt^2}-6\frac{dx}{dt}+8x=0 \tag{1}
$$

回答下列问题。

1. 令 $\boldsymbol u^\top=\left(x,\dfrac{dx}{dt},\dfrac{d^2x}{dt^2}\right)$，将式 (1) 写成 $A\boldsymbol u=\dfrac{d\boldsymbol u}{dt}$，求矩阵 $A$；
2. 求 $A$ 的全部特征值；
3. 解微分方程 (1)。

**[2]** 回答下列复数问题。

1. 分别求 $a_1=\sqrt3+i$ 与 $a_2=2i$ 的模和辐角；
2. 若复数 $z_1,z_2,z_3$ 对应的三点构成正三角形，证明
...
x^{2/3}+y^{2/3}+z^{2/3}\le1,\ x\ge0,\ y\ge0,\ z\ge0\}.
$$
...
\begin{cases}
\displaystyle
\frac{(n-1)(n-3)\cdots3\cdot1}{n(n-2)\cdots4\cdot2}\cdot\frac{\pi}{2},
& n\ge2\text{ 且 }n\text{ 为偶数},\\[6pt]
\displaystyle
\frac{(n-1)(n-3)\cdots4\cdot2}{n(n-2)\cdots5\cdot3}\cdot\frac{\pi}{2},
& n\ge3\text{ 且 }n\text{ 为奇数}.
\end{cases}
$$

用平面 $z=k$ 截取 $V$，所得截面记为 $S$。

1. 对 $S$ 作变量代换 $x=r\cos^3t,\ y=r\sin^3t$，求 $r,t$ 各自的取值范围；
2. 求截面 $S$ 的面积；
3. 求立体 $V$ 的体积。
