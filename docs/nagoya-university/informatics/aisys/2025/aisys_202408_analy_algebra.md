...
回答：令 $\boldsymbol u^\top=(x,x',x'')$，求使 $\boldsymbol u'=A\boldsymbol u$ 的矩阵 $A$；求 $A$ 的特征值；解原微分方程。

**[2] 复平面几何**：

1. 求 $a_1=\sqrt3+i$、$a_2=2i$ 的模与辐角。
...
x^{2/3}+y^{2/3}+z^{2/3}\le1,\ x,y,z\ge0\}.
$$

可使用题面给出的 $\int_0^{\pi/2}\sin^nx\,dx$、$\int_0^{\pi/2}\cos^nx\,dx$ 公式。以平面 $z=k$ 截得截面 $S$：

1. 对变换 $x=r\cos^3t,\ y=r\sin^3t$，求 $r,t$ 的范围。
2. 求 $S$ 的面积。
3. 求 $V$ 的体积。

#### 考点

- **高阶线性微分方程与一阶系统**：构造伴随矩阵，通过特征值求三阶方程通解。
- **复数极形式与复平面几何**：计算模辐角，并把正三角形的旋转关系转化为复数恒等式。
- **变量代换与 Jacobian**：用三次三角参数化星形线截面，计算面积元后再对高度积分求体积。
