...
1. 选一个点切成两段，求至少一段长度大于 $0.7$ 的概率。
2. 选两个点切成三段，求至少一段长度大于 $0.5$ 的概率。

**[2] 随机变量变换**：$X$ 服从 $[1,3]$ 上的连续均匀分布。写出 $X$ 的密度；令 $Y=X^2+2$，求 $Y$ 的密度。

**[3] 二项分布**：公平六面骰各点数等概率。

1. 掷 $n$ 次，以 $X$ 表示出现点数 1 的次数，写出 $P(X=x)$。
2. 为使一次点数 1 都不出现的概率不小于 $0.4$，最多可掷多少次？
3. 掷 10 次时，出现点数 1 的次数不超过 1 的概率是否不小于 $0.5$？计算并说明。

#### 考点

- **几何概率**：把切点位置表示为一维或二维均匀区域，通过事件区域长度/面积求概率。
- **均匀分布的变量变换**：由单调变换 $Y=X^2+2$ 的反函数和导数求密度及支持区间。
- **二项分布**：识别单次成功概率 $1/6$，计算点概率与累积事件，并用对数不等式确定最大试验次数。
