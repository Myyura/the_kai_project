...
この構造体は１つの頂点を表現しその頂点が保持する整数を格納するメンバ value, 左の子へのポインタを格納するメンバ left, 右の子へのポインタを格納するメンバ right から構成される.
...
insert 関数, eliminate 関数はポインタ tree が指す２分木に対して引数で与えられた整数を保持する頂点をそれぞれ挿入, 削除する関数である.
...
\[3\] 三項演算子 ? : は「式1 ? 式2 : 式3」のように記述されて式を構成する.「式1」を評価した結果が 0 でない時は「式2」を, 0 である時は「式3」を評価した結果を式の値とする. 例えば, 式 `x <= 0 ? x+1 : x+2` を評価した値は x の値が -1 の時には 0 となり, 5 の時は 7 となる. この三項演算子を用いて, 53 行目から 56 行目のコード
...
\[6\] 21 行目の末尾まで実行した時点でポインタ tree が指す２分木は図 1(b) になっている. このあと, 22 行目を実行した際に出力される文字列を示せ.
...
考虑每个节点保存一个整数的二叉树。节点由结构体 `vertex` 表示，包含整数成员 `value` 以及分别指向左、右孩子的指针 `left`、`right`；不存在的孩子以 `NULL` 表示，空树也以 `NULL` 表示。原题程序以全局指针 `tree` 指向树根，并定义了节点创建 `create`、插入 `insert`、删除 `eliminate` 和先序输出 `display` 等函数。完整代码与示例树见上文。

回答下列问题。

1. 分别画出 `main` 函数执行到第 18 行末尾和第 19 行末尾时，`tree` 所指向的二叉树。
2. 在第 22 行空格 (ア) 中写出一组 `insert(x);` 或 `eliminate(x);` 指令，使执行完后得到图 1(b) 的树。每个作为函数参数的整数至多出现一次。
3. 使用三目运算符 `?:`，将第 53—56 行根据 `x` 与 `p->value` 的大小选择左、右孩子的代码改写为一条赋值语句，填写空格 (イ)。
4. `eliminate` 函数可能发生非法内存访问，例如在第 19 行之后执行 `eliminate(10);`。指出应在哪两行之间插入哪条语句来修复。
5. 现有 `eliminate` 不释放已删除节点的内存。说明反复插入和删除可能造成的问题，并在函数的两个适当位置分别加入使用 `free` 的语句；按“在某行与下一行之间插入某语句”的形式作答。
6. 假设执行第 22 行后树为图 1(b)，写出随后执行 `display(tree)` 时输出的字符串。
7. 修改 `display`，使节点值按从大到小的顺序输出；填写空格 (ウ) 至 (オ)，输出节点值时使用 `printf("%d,", p->value)`。
8. 补全函数 `member(int x)` 的空格 (カ) 至 (ケ)：若树中存在保存 `x` 的节点返回 `1`，否则返回 `0`。
