...
2. 在第 21 行空格 (ア) 中写出一组 `insert(x);` 或 `eliminate(x);` 指令，使执行完后得到图 1(b) 的树。每个作为函数参数的整数至多出现一次。
3. 使用三目运算符 `?:`，将第 53—58 行根据 `x` 与 `p->value` 的大小选择左、右孩子的代码改写为一条赋值语句，填写空格 (イ)。
...
6. 假设执行第 21 行后树为图 1(b)，写出随后执行 `display(tree)` 时输出的字符串。
7. 修改 `display`，使节点值按从大到小的顺序输出；填写空格 (ウ) 至 (オ)，输出节点值时使用 `printf("%d,", p->value)`。
8. 补全函数 `member(int x)` 的空格 (カ) 至 (ケ)：若树中存在保存 `x` 的节点返回 `1`，否则返回 `0`。

#### 考点

- **二叉搜索树**：理解节点插入、查找和删除时依据键值选择左右子树的规则。
- **树遍历**：掌握先序遍历以及通过“右子树—根—左子树”的次序实现降序输出。
- **指针与动态内存**：正确更新父子指针、处理空指针，并用 `free` 释放删除节点。
- **C 语言三目运算符**：将条件分支等价改写为条件表达式。
- **程序状态追踪**：根据连续插入和删除操作画出各阶段的树形并推导输出。
