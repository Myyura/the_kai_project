...
函数 `index(string, p)` 返回第 $p$ 个字符首字节的地址；`length_b` 返回字符串的字节数；`concat` 连接两个字符串；`concat_p` 连接第一个字符串在地址 `l` 之前的前缀与第二个字符串；`replace(string1, string2, p)` 用仅含一个字符的 `string2` 替换 `string1` 的第 $p$ 个字符。完整程序及四个测试字符串的十六进制字节序列见上文。

回答下列问题。凡要求写出指针所指字符串的内容时，须按题中示例列出每个地址对应的字节值，并包含结尾 `'\0'`。

1. 填写空格 A 至 E，完成 `concat`。
2. 写出执行 `r = index(str1, 1);` 后 `r` 所指的字符串。
3. 写出执行 `r = index(str2, 2);` 后 `r` 所指的字符串。
4. 写出执行 `r = index(str3, 3);` 后 `r` 所指的字符串。
5. 执行 `r = replace(str3, str4, 3);` 时，在 `tmp2 = concat_p(string1, string2, tmp1);` 刚执行完后，写出 `tmp2` 的内容。
6. `concat_p` 当前为 `result` 固定分配 100 字节，某些参数下空间不足。将该分配长度改为根据输入准确计算的表达式。
7. 填写空格 F 至 H，完成 `replace`，使其跳过被替换字符所占的一个或三个字节。

#### 考点

- **变长字符编码**：根据首字节最高位判断字符占一个还是三个字节。
- **指针运算**：按字符边界移动指针，并区分字符数与字节数。
- **字符串拼接与替换**：正确复制前缀、替换字符和剩余后缀，并保留字符串终止符。
- **动态内存分配**：根据实际字节长度为结果字符串分配足够空间。
- **十六进制字节追踪**：逐地址推导多字节字符串切片和拼接后的内存内容。
