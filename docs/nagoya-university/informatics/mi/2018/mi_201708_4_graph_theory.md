...
考虑无向图 $G=(V,E)$。若图可在平面上画成边互不交叉的形式，则该画法称为平面图；边分割出的每个区域（包括外部区域）称为面。闭路是首尾相同且相邻顶点之间均有边的顶点序列。树是无闭路的连通图。$n$ 个顶点的完全图记为 $K_n$。相关示例见图 1 至图 3。

回答下列问题。

1. 分别求图 1、图 2 的顶点数、边数和面数。
2. 平面图中的一个面是否总与一个闭路一一对应？若是则证明；若否，给出一个反例。
3. 已知连通平面图满足 Euler 公式

   $$
   |V|-|E|+f=2,
   $$

   其中 $f$ 为面数。利用此公式证明平面图满足
   ...
   可使用“每个面至少由三条边围成”的事实。
4. 对 $n=3,4,5,\ldots$，判断完全图 $K_n$ 是否为平面图并说明依据。
...
$K_n \ (n=6, 7, 8, \ldots)$ は $K_5$ を含むので、平面グラフではない。
...
- 頂点は消していないので $|V'|=|V|$
- 辺は 1 本除去したので $|E'|=|E|-1$
- $e$ の両側の 2 つの面が 1 つにつながったので $f'=f-1$
