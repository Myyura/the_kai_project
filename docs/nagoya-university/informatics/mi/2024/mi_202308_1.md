...
原文的题目描述缺失，以下内容依据现有解答整理。

1. 对二维向量 $\boldsymbol v,\boldsymbol v_1,\boldsymbol v_2$ 与向量
   $\boldsymbol w,\boldsymbol w_1,\boldsymbol w_2$，以及标量 $\alpha$，根据张量积的分量定义证明双线性：
   $$
   ...
   =\alpha(\boldsymbol v\otimes\boldsymbol w).
   $$
2. 若 $A\boldsymbol v=\alpha\boldsymbol v$ 且
...
3. 令
   ...
   B=\begin{pmatrix}3&1\\1&3\end{pmatrix},\qquad
   M=A\otimes B.
   $$
   求四阶矩阵 $M$ 的全部特征值及相应特征向量。

#### 考点

- **张量积**：掌握向量与矩阵 Kronecker 积的分量定义。
- **双线性**：证明张量积对两个因子分别满足加法与数乘分配律。
- **Kronecker 积的谱**：由两个因子矩阵的特征对构造积矩阵的特征对。
- **特征值与特征向量**：求二阶对称矩阵的谱并组合成四阶矩阵的完整谱。
