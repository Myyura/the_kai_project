...
- **冒泡排序**：掌握相邻元素交换、逐趟确定元素位置以及中间状态追踪。
- **二叉搜索树**：理解插入顺序对树形和平均查找代价的影响。
- **最优二叉搜索树**：根据不等查找概率，用动态规划等方法最小化期望查找代价。
- **Dijkstra 算法**：维护已确定节点及暂定距离，逐步求解单源最短路径。
- **负权边限制**：理解贪心确定最短距离后无法被后续负权路径修正的问题。
