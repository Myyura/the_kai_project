...
回答下列关于排序、二叉搜索树和最短路径的问题。

1. 使用冒泡排序将数组
   `18, 21, 13, 15, 22, 17, 26, 20, 10, 29`
   按升序排列，并记录每一趟扫描结束后的数组状态，直至排序完成。
2. 将集合 $\{1,2,\ldots,7\}$ 的元素按随机顺序依次插入二叉搜索树。
   - 说明如何遍历全部 $7!$ 种插入顺序并计算平均查找代价；
   - 给出一个能使平均查找代价最小的插入顺序，并画出所得二叉搜索树；
   - 当各元素的查找概率不相等时，说明如何构造最优二叉搜索树。
3. 对节点 $A$ 至 $F$ 的加权有向图，边为
   $A\to B(2)$、$A\to C(5)$、$B\to C(1)$、$B\to D(2)$、
   $C\to D(3)$、$C\to E(1)$、$D\to F(1)$、$E\to F(2)$。
   - 使用 Dijkstra 算法展示从 $A$ 出发求各节点最短路径的步骤，求出到所有节点的最短距离，并画出相应的加权有向图；
   - 解释 Dijkstra 算法不能直接用于含负权边图的原因。
...
各挿入順を等確率としたとき，この計算で得る平均ノード深さは $472/245\approx1.92653$ である。
...
ここでは探索時の比較回数（深さ $+1$）をコストとする。深さそのものをコストとする場合、以下の $W(i,j)$ を $W(i,j)-p_r$ に置き換えればよく、最適な根は同じである。
...
左右部分木の各ノードには 1 回分の比較が加わり、根でも 1 回比較するため、区間全体の確率和 $W(i,j)$ が追加される。
...
Root に保存した区間の根を選び，左右の区間について同じ操作を再帰的に行えば，最適な木を構成できる。確率和を前計算すると，この DP の時間計算量は $O(n^3)$，空間計算量は $O(n^2)$ である。
...
この確定操作の正しさは，すべての辺の重みが $0$ 以上なら保証される。負の辺を許す一般のグラフでは保証されない。
...
この反例では B が距離 $2$ で先に確定されるが，経路 $A\to C\to B$ の距離は $5-10=-5$ である。したがって一度確定した距離を変えない方式では誤った結果になる。
