...
tags:
  - Nagoya-University
  - Mathematics.Real-Analysis.Functions-of-Bounded-Variation
---
...
出典：[名古屋大学公表問題](https://www.math.nagoya-u.ac.jp/ja/admission/gs/download/exam-mc-2021-2_day2.pdf)。
...
$$
\sum_{j=0}^{n-1}|\varphi(t_{j+1})-\varphi(t_j)|\le M,
$$

则称 $\varphi$ 在 $I$ 上有界变差。

1. 设 $f$ 在包含 $I$ 的开区间上可微，且 $f'$ 在 $I$ 上有界，证明 $f$ 在 $I$ 上有界变差。
2. 分别判断下列函数在 $I$ 上是否有界变差，并说明理由：
...
とできる.
ここで, $f'$ の有界性より $|f'(t)|\le M\ (t\in I)$ となる $M>0$ をとると
...
である. よって、$0<x\le1$ に対して

$$
|g'(x)| \leq \left| 2x \sin \frac{1}{x} - \cos \frac{1}{x}  \right| \leq 2|x| + 1 \leq 3
$$

$x=0$ でも $|g'(0)|=0\le3$ である。従って $g$ は $I$ を含む開区間で微分可能で、その導関数 $g'$ は $I$ で有界である。
...
$$
2 \sum_{j=1}^n \left( \frac{2}{4j\pi} + \frac{2}{(4j-2)\pi}\right) = \frac{2}{\pi} \sum_{j=1}^{2n} \frac{1}{j} \rightarrow +\infty \quad (n \rightarrow +\infty)
$$
