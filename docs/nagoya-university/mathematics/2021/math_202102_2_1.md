...
设 $n$ 阶实方阵 $A$ 满足 $A^2=A^\top$。对 $x,y\in\mathbb C^n$，采用标准 Hermite 内积

$$
\langle x,y\rangle=\sum_{j=1}^nx_j\overline{y_j}.
$$

1. 若 $v$ 是 $A$ 对应特征值 $\alpha$ 的特征向量，通过考察 $\langle v,Av\rangle$ 证明 $\alpha^2=\overline\alpha$。
2. 证明 $A$ 的不同特征值所对应的特征向量相互正交。
3. 以下令 $n=3$，并设 $A^2=A^\top$、$A$ 可逆且 $A\ne I$。证明 $A$ 至少有一个实特征值，进而求出全部特征值，并证明 $A$ 可由酉矩阵对角化。
4. 给出一个满足上述条件的 $A$ 的例子。
...
である. よって, 固有ベクトルは非零であり $\langle v,v\rangle>0$ なので $\alpha^2 = \overline{\alpha}$ である.
...
さらに $AA^{\top}=A^3=A^{\top}A$ より $A$ は正規行列なのでユニタリ対角化可能である.
したがって, 固有値がすべて $1$ ならば $A=I$ となる. 条件より $A\ne I$ なので実数でない固有値 $\lambda$ ももつ.
