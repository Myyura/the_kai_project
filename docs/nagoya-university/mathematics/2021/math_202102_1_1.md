...
1. 求 $A$ 的所有具有形如 $(1,1,t)^\top$（$t$ 为实数）特征向量的特征值。
2. 求 $A$ 的全部特征值。
3. 当 $b=1$ 时，求特征值 $1$ 的特征空间维数。
4. 求 $A$ 不可对角化时参数 $a,b$ 满足的充要条件。
...
と計算できる. したがって $\lambda=at+1$ かつ

$$
a t(1-t)=0
$$

である. $a\neq0$ ならば $t=0,1$ であり、$a=0$ ならば任意の $t$ に対して $\lambda=1$ である。したがって、求める固有値は $1,a+1$ であり、
