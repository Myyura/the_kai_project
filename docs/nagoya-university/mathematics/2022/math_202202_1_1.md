...
出典：[名古屋大学公表問題](https://www.math.nagoya-u.ac.jp/ja/admission/gs/download/exam-mc-2022-2_day1.pdf)。
...
设 $c\in\mathbb R$。令 $V\subseteq\mathbb R^4$ 由

$$
\begin{pmatrix}c+2\\c-2\\c\\c-2\end{pmatrix},
\quad
\begin{pmatrix}c+1\\c-2\\c-1\\c-1\end{pmatrix},
\quad
\begin{pmatrix}c\\c\\c-2\\c\end{pmatrix}
$$

张成，令 $W\subseteq\mathbb R^4$ 由
...
张成。求 $\dim V$、$\dim(V+W)$ 和 $\dim(V\cap W)$；答案须考虑参数 $c$ 可能导致的不同情形。
