...
- **行列式与齐次线性方程组**：以系数矩阵行列式为零判断非平凡解存在条件。
- **分部积分与三角代换**：计算含 $x\log x$ 和根式的定积分。
- **可分离变量与二阶线性方程**：分别通过分离积分和齐次解加特解求通解。
- **Fourier 变换的尺度与频移性质**：由定义推导频域缩放、平移对应的时域变化，并计算截断正弦信号频谱。
- **指数分布**：验证密度、计算均值和指定区间概率。
