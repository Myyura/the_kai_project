...
1. 填写图 1 的空白 `(a)`、`(b)`，使 `search1`、`search2` 均能被正确调用。
2. 以 `./a hiroshima io` 运行程序（`a` 为可执行文件名），写出输出。
3. 以 `./a hiroshima z` 运行时，程序不会按上述规格工作。分别回答：
   1. 实际处理与规格有何不同；
   2. 应如何修改第 6 行的 `search1` 与第 12 行的 `search2`；
   3. 修改后程序的输出是什么。
4. 将 `search1`、`search2` 改成输出目标字符最后一次出现位置的 `search1_last`、`search2_last`；填写图 2、图 3 中的 `(c)`、`(d)`、`(e)`、`(f)`，完成代码。

图 1 是原程序，图 2、图 3 分别给出两个“查找最后位置”函数的待补全代码。

#### 考点

- 字符串处理：依据 C 字符串的 `'\0'` 终止标记和 `strlen` 限定合法搜索范围，正确判断字符是否出现。
- 递归：分析递归搜索的终止条件与参数推进方式，并改写为查找最后一次出现位置。
- 指针与数组：正确传递命令行字符数组、索引字符，并避免搜索过程中越过数组边界。
