...
1. 表 1 给出鸟取（Tr）、岛根（S）、冈山（O）、广岛（H）、山口（Y）、德岛（Tk）、爱媛（E）七座城市两两之间的距离。以城市为顶点、城市间距离为边权构造图 $G$，求其最小生成树。所有边权以表 1 的数值为准。
2. 对任意规模、所有边权互不相同的带权连通图 $G$，说明一种求最小生成树的算法。
3. 证明第 2 问算法所得子图确为最小生成树。
4. 给出第 2 问算法的时间复杂度并说明理由。
5. 证明：若带权连通图 $G$ 的所有边权互不相同，则 $G$ 的最小生成树唯一。

#### 考点

- 最小生成树：在距离完全图上执行生成树算法，并用割性质或交换论证证明算法正确性、唯一性及其实现复杂度。
