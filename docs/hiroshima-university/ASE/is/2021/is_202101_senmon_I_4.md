...
现要在地点 $v$ 至 $z$ 之间建设电力网。图 1 的图 $G$ 中，每个顶点对应地点 $v,w,x,y,z$ 之一；每条边表示可在其两个端点对应地点之间铺设电力线，边旁数字表示建设费用。

1. 写出图 $G$ 的 $0$-$1$ 邻接矩阵 $A$ 及相应的拉普拉斯矩阵 $L$。
2. 图 $G$ 的生成树对应一个连通且无环、覆盖所有地点的电力网。求不同生成树的总数，并说明依据。
3. 在第 2 问的所有生成树中找出总费用最低的一棵，并给出其总费用。
