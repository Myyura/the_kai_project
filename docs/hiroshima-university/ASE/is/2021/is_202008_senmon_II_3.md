...
图 1 是 C 源程序 `counting.c`，它应完成两类计数：

- `c_count1`、`c_count2`、`c_count3` 返回字符串 `source` 中字符 `target` 出现的次数；
- `n_count1`、`n_count2`、`n_count3` 返回整数 `source` 的各位中，与整数 `target` 的个位数字相同的数字个数。

图 2 给出编译 `counting.c` 后运行可执行文件 `a.out` 的输出示例。为使程序按上述说明工作，完成以下各问：

1. 填写 `c_count1` 中的 `<1-1>` 至 `<1-3>`。
2. 填写 `c_count2`、`c_count3` 中相同的空白 `<2-1>`。
3. 填写 `n_count1` 中的 `<3-1>` 至 `<3-2>`。
4. 填写 `n_count2` 中的 `<4-1>` 至 `<4-3>`。
5. 填写 `n_count3` 中的 `<5-1>` 至 `<5-5>`。

所有函数、递归调用和空白位置均以图 1 的完整代码为准，运行输出参照图 2。

#### 考点

- 字符串处理：利用空字符判断字符串结束，并逐字符统计目标字符的出现次数。
- 指针与数组：在数组下标遍历和字符指针递增两种写法之间正确传递 `source` 与 `target`。
- 递归：为字符计数和十进制数位计数设置基例，并在每次调用中缩小输入规模。
