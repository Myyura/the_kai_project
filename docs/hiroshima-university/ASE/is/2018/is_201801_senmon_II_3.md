...
图 1、图 2 给出两个 C 函数 `power1` 和 `power2`：对整数参数 $x,y$（其中 $y$ 为正整数），分别用循环和递归计算并返回 $x^y$。

1. 以 $x=2,y=4$ 运行图 1 的 `power1`，写出其标准输出。
2. 图 2 的 `power2` 递归计算 $x^y$；填写空白 `[a]`、`[b]`、`[c]`，使函数完整。
3. 在某一环境中以 $x=10,y=10$ 运行 `power1` 时，标准输出包含图 3 所示的
   `9: 1000000000` 和 `10: 1410065408`。指出其中的异常，并从数据类型角度说明出现该结果的原因。
4. 参考 `power2`，定义函数 `factorial`，以递归方式计算并返回正整数 $x$ 的阶乘。

代码分别见图 1（循环版 `power1`）、图 2（待补全的递归版 `power2`）以及图 3（异常输出）。

#### 考点

- 递归：补全幂函数的递归终止条件与递推式，并仿照其结构实现递归阶乘。
- 整数溢出：结合 C 语言 `int` 的表示范围解释 $10^{10}$ 输出异常的原因。
