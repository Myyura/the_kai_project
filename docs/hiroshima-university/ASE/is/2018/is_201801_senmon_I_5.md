...
1. 展开 $(a+b)^5$，求其中 $a^3b^2$ 项的系数。
2. 在恒等式

   $$
   (a+b)^n=X_0a^n+X_1a^{n-1}b+X_2a^{n-2}b^2+X_3a^{n-3}b^3+\cdots+X_{n-1}ab^{n-1}+X_nb^n
   $$

   中，分别求 $X_1,X_2,X_3$。
3. 利用 $11^5=(10+1)^5$ 计算 $11^5$。
4. 展开 $(a+b+c)^7$，求 $a^2b^3c^2$ 项的系数。
5. 展开 $(2a+b+c)^7$，求 $a^2b^3c^2$ 项的系数。

#### 考点

- 二项式系数：运用二项式定理和多项式系数的组合计数，求指定项系数并完成数值幂的展开计算。
