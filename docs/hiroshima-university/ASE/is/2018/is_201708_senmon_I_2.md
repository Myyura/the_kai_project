...
1. 设 $\alpha>0$，求函数 $f(x)=x^\alpha e^{-x}$ 在区间 $[0,\infty)$ 上的最大值。
2. 证明当 $x>0$ 时，广义积分
...
- 局部极值：通过导数符号判断 $x^\alpha e^{-x}$ 的单调区间及全局最大值。
- 广义积分：分别控制积分在 $0$ 附近和无穷远处的行为，以证明 $\Gamma(x)$ 在 $x>0$ 时收敛。
- 伽马函数：利用分部积分建立 $\Gamma(x+1)=x\Gamma(x)$ 的递推关系。
- 极限：借助伽马函数递推关系与 $\Gamma(1)$ 求 $x\to0^+$ 时的极限。
