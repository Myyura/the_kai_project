...
算法 1 将整数序列 $a_0,a_1,\ldots,a_{n-1}$ 按升序排列。它以序列 $Q$ 为参数，$|Q|$ 表示其长度，$\operatorname{head}(Q)$ 表示首元素，$Q[i]$ 表示第 $i$ 个元素；当 $Q$ 为空时规定 $\operatorname{head}(Q)=\infty$。`Delete-head(Q)` 删除 $Q$ 的首元素，`Allocate(L,k)` 分配长度为 $k$ 的序列 $L$。算法 1 的完整伪代码见上文代码块。

1. 对输入序列 $21,1,26,45,29,28,2,9$ 执行算法 1，写出各阶段序列的变化，并给出比较次数。若已知参与比较的一方为 $\infty$，该次比较不计入次数。
2. 给出一个含 $8$ 个元素、能使算法 1 的比较次数达到最大的输入序列。
3. 写出算法 1 的最坏时间复杂度，并说明理由。
