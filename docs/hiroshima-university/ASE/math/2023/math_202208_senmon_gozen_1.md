...
したがって，、 $A$ の階数が $2$ となるのは $a=1$ のときである。
...
したがって，方程式が解をもつための必要十分条件は $2p-q+r=0$ である。
...
が奇数であるとする。このとき，$A$ が $1$ を固有値にもつことを示す。
...
もし $\dim X$ が奇数であると仮定する。このとき，実線形空間 $X$ 上の線形写像 $A_X$ の特性多項式は奇数次の実係数多項式であるから，少なくとも1つの実根をもつ。

したがって，$A_X$ は実固有値 $\lambda$ をもつ。
...
\lambda^2+\lambda+1
=
\left(\lambda+\frac12\right)^2+\frac34>0
$$

であるから，これは実数 $\lambda$ に対して成り立たない。
...
したがって，$y$ は $A$ の固有値 $1$ に対応する固有ベクトルである。

ゆえに，

$$
\boxed{A\text{ は }1\text{ を固有値にもつ}}
$$
