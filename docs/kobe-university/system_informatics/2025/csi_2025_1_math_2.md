...
1. 求全部满足 $g(z)=0$ 的复数。
2. 利用第 1 问所得零点都是 $f(z)/g(z)$ 的一阶极点，计算
...
3. 证明
   ...
   $$
   \lim_{R\to\infty}
   \int_{C_2}\frac{f(z)}{g(z)}\,dz=0,
   $$

   $$
   \lim_{R\to\infty}
   \int_{C_4}\frac{f(z)}{g(z)}\,dz=0.
   $$
...
- **复指数方程与极点**：求 $e^z=-1$ 的周期解，并判断简单零点对应一阶极点。
- **留数定理**：选取高度 $2\pi$ 的矩形路径，计算路径内部极点的留数。
- **竖边积分估计**：分别在右、左半平面控制指数函数大小，证明边界贡献消失。
- **周期平移与实积分**：比较 $C_1,C_3$ 上的积分，利用 $f(z+2\pi i)$ 的相位因子求目标积分。
