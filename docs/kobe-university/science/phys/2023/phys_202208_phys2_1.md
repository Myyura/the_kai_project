...
三个量子态 $|1\rangle,|2\rangle,|3\rangle$ 满足标准正交关系
...
对常数 $\varepsilon>0$，系统 Hamiltonian 为
...
2. 若
   ...
   $$
   a|1\rangle+b|2\rangle+a|3\rangle
   $$

   是 $\hat H$ 的本征态，求 $b/a$ 和相应本征能量。
3. 把状态 $|1\rangle$ 表示为 $\hat H$ 本征态的线性叠加。
4. 以 $|\Psi(t)\rangle$ 表示 $t\geq0$ 时的状态。若
...
- **量子态时间演化**：把初态投影到能量本征态并附加相位 $e^{-iEt/\hbar}$。
