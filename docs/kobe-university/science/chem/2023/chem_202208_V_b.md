...
tags:
  - Kobe-University
  - Physics.Quantum-Mechanics.Particle-in-Circular-Well
...
质量为 $m_0$ 的粒子被限制在半径为 $L$ 的圆内，圆内势能为零。其在平面极坐标 $(r,\theta)$ 中的 Schrödinger 方程为
...
其中 $\hbar=h/(2\pi)$，$h$ 为 Planck 常数。

1. 将波函数 $\Psi(r,\theta)$ 对 $r,\theta$ 分离变量，分别求出径向与角向的独立微分方程。
2. 写出波函数 $\Psi(r,\theta)$ 应满足的边界条件。
3. 若粒子径向被限制在常数 $r=r_0$（$0<r_0<L$），一个可能的波函数为

   $$
   \Psi_m(r_0,\theta)=Ae^{im\theta},
...
円周を無限に高い壁とすると、外周で波動関数は零であり、角度について一価かつ滑らかである。
...
$$
  ...
  , \\
  \Psi(r, 2 \pi) &= \Psi(r, 0),\\
  \partial_\theta\Psi(r,2\pi)&=\partial_\theta\Psi(r,0)
  \end{aligned}
$$

また、原点で波動関数は有限かつ一価でなければならない。分離解では発散する動径解を除き、角方向の周期性から分離定数は $\alpha=-m^2$（$m\in\mathbb Z$）となる。
...
を得る。
半径 $r_0$ の円周に拘束された粒子は、円弧長 $s=r_0\theta$ に沿う一次元自由粒子として扱う。その Hamiltonian は

$$
\hat H=-\frac{\hbar^2}{2m_0}\frac{d^2}{ds^2}
=-\frac{\hbar^2}{2m_0r_0^2}\frac{d^2}{d\theta^2}.
$$

これを $Ae^{im\theta}$ に作用させると、
