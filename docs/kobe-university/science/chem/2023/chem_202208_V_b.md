...
- **极坐标 Schrödinger 方程的分离变量**：令 $\Psi=R(r)\Theta(\theta)$，得到 Bessel 型径向方程和周期角向方程。
- **圆形无限深势阱边界条件**：使用波函数有限、单值、周期以及在 $r=L$ 处为零的条件。
- **圆环上的角动量量子化**：将 $e^{im\theta}$ 代入角向动能算符，得到整数 $m$ 对应的能级。
