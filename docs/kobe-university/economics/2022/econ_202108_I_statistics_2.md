...
随机变量 $X$ 的密度为 $f(x;\theta)$，参数 $\theta$ 只能取 0 或 1。两种密度分别为
...
1. 求常数 $a$。（15 分）
2. 当 $\theta=0$ 时，求 $X$ 的期望和方差。（20 分）
3. 当 $\theta=0$ 时，求 $X$ 的矩母函数。（15 分）
4. 设 $X_1,\ldots,X_n$ 是来自 $f(x;\theta)$ 的随机样本，求 $\theta$ 的最大似然估计量。（20 分）
...
$X_1X_2\cdots X_n=1/2^n$ のときは $l(0)=l(1)$ なので、$0,1$ のいずれも最尤推定値である。
