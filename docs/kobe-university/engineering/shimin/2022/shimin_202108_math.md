...
3. 令
   ...
   $$
   f(x,y)=\frac{x+5y}{x^2+2y^2+1}.
   $$

   在 $xy$ 平面上，沿从原点 $(0,0)$ 到 $(1,1)$ 的线段
...
- **概率密度归一化与矩**：通过多项式积分求常数、尾概率、均值和方差。
- **Jordan 标准形与广义特征向量**：由相似不变量确定参数，并构造特征向量链组成 $\boldsymbol P$。
- **标量场的弧长线积分**：参数化直线并使用 $ds=\|\boldsymbol r'(t)\|dt$。
- **一阶线性微分方程**：整理为标准形式并用积分因子求通解。
