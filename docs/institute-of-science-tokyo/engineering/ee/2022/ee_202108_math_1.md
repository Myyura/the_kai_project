...
## **Description**
$\int_{-\infty}^{\infty}|f(t)|\,dt$ が有限で絶対積分可能な実関数 $f(t)$ に対して，フーリエ変換 $F(\omega)$ を
...
[公式問題 PDF・2ページ](https://admissions.isct.ac.jp/plugins/cms/component_download_file.php?contentsDataId=&contentsId=&fileName=exam_ee_20218&key=cd5c2ecd3d4dc49c5c29e8de8374f19b.pdf&pageId=3186&prevId=&type=1)
...
以下、必要な積分と微分の交換ができ、部分積分の境界項が $0$ になるとする。
...
\int_{-\infty}^{\infty}t^2f(t)e^{-j\omega t}dt &= -\frac{d^2F(\omega)}{d\omega^2} = G(\omega)
\end{aligned}
$$
...
\begin{aligned}
\frac{d^2f(t)}{dt^2} - t^2f(t) &= af(t) \\
-\omega^2F(\omega) - \left(-\frac{d^2F(\omega)}{d\omega^2}\right) &= aF(\omega) \\
...
$$
\begin{aligned}
\frac{d^2}{dt^2}\left(H_n(t)e^{-\frac{t^2}{2}}\right) &= \frac{d}{dt}\bigg(\frac{dH_n(t)}{dt}e^{-\frac{t^2}{2}} - H_n(t)te^{-\frac{t^2}{2}}\bigg) \\
&= \frac{d^2H_n(t)}{dt^2}e^{-\frac{t^2}{2}} - 2t\frac{dH_n(t)}{dt}e^{-\frac{t^2}{2}} - H_n(t)e^{-\frac{t^2}{2}} + H_n(t)t^2e^{-\frac{t^2}{2}}\\
\end{aligned}
$$
...
$$
\begin{aligned}
\frac{dH_n(t)}{dt} &= 2tH_n(t)+(-1)^ne^{t^2}\frac{d^{n+1}}{dt^{n+1}}(e^{-t^2}) \\
&= 2tH_n(t) - H_{n+1}(t)
\end{aligned}
$$
