...
<figure style="text-align:center;">
  <img src="https://raw.githubusercontent.com/Myyura/the_kai_project_assets/main/kakomonn/institute-of-science-tokyo/engineering/ee_201908_electrical_circuit_1_p1.png" width="500" alt=""/>
</figure>
...
1. 端子 $ab$ 间施加电压 $v(t)$，电容电压为 $v_c(t)$。用 $R,C,v(t),v_c(t)$ 写出电路方程。
2. 将图 1.2 所示电压 $v(t)$ 加在端子 $ab$ 上，其中 $V>0$。解电路方程，求 $t\geq0$ 时的 $v_c(t)$。
3. 求经过充分长时间后电容器的静电能。
4. 把图 1.3 所示、从 $2V$ 变为 $V$ 的电压 $v(t)$ 加在端子 $ab$ 上。仍规定 $t=0$ 时电容器电荷为 $0$，且 $V>0,a>0$。求 $v_c(t)$。
5. 在第 4 问中，若 $t\geq a$ 后电容器发生放电，求 $a,R,C$ 必须满足的关系。
6. 在第 4 问中调节 $R,C$，使 $t\geq a$ 时电路电流为 $0$。用 $C,V$ 表示从 $t=0$ 到 $t=a$ 期间电阻 $R$ 消耗的电能。
7. 在第 6 问条件下，用 $C,V$ 表示从 $t=0$ 到 $t=a$ 期间端子 $ab$ 提供的电能。
