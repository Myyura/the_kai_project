...
$$
\begin{align}
z = \bigg(\frac{\sqrt{3}}{8} + \frac{j}{8}\bigg)^{20} = a + jb \tag{2.1}
\end{align}
$$
...
[公式問題 PDF・3ページ](https://admissions.isct.ac.jp/plugins/cms/component_download_file.php?contentsDataId=&contentsId=&fileName=exam_ee_20238&key=3461b477e949262187dc4964e80871b3.pdf&pageId=3186&prevId=&type=1)
...
1. 若
   ...
   $$
   z=
   \left(\frac{\sqrt3}{8}+\frac j8\right)^{20}
   =a+jb,
   \tag{2.1}
   $$
...
3. 令积分路 $C$ 为 $|z|=3$ 的逆时针圆周，计算

   $$
   \int_C\frac{5z-1}{2z^2+3z-2}\,\mathrm dz.
   \tag{2.3}
   $$
...
4. 考虑定积分
   ...
   $$
   \int_0^{2\pi}\frac{\mathrm d\theta}
   {3+2\cos\theta}.
   \tag{2.4}
   $$

   1. 令 $z=e^{j\theta}$，其中 $0\leq\theta\leq2\pi$，把 (2.4) 写成关于复数 $z$ 的围道积分。
   2. 计算 (2.4) 的值。
...
<figure style="text-align:center;">
  <img src="https://raw.githubusercontent.com/Myyura/the_kai_project_assets/main/kakomonn/institute-of-science-tokyo/engineering/ee_202308_math_2_p1.png" width="400" alt=""/>
</figure>
...
$$
\begin{aligned}
\int_C \frac{5z - 1}{2z^2 + 3z - 2}dz
&=2\pi j\left\{\operatorname{Res}(f,-2)+\operatorname{Res}(f,\tfrac12)\right\}\\
&=2\pi j\left(\frac{11}{5}+\frac{3}{10}\right)\\
&=5\pi j
\end{aligned}
$$
