...
$$
\begin{aligned}
\sum_{n = 0}^{N - 1} f(-n)e^{-j\frac{2\pi}{N}kn} &= \sum_{n = 0}^{N - 1}f(N - n)e^{-j\frac{2\pi}{N}kn} \quad \big(\text{Substitute } N - n \text{ with } m) \\
&= \sum_{m = 1}^N f(m)e^{j\frac{2\pi}{N}km}
\end{aligned}
$$
...
$m$ を $N$ で割った余りに置き換えても両辺は変わらないので、$0\leq m\leq N-1$ としてよい。
...
\begin{aligned}
&\sum_{n = 0}^{N - 1}f(n - m)e^{-j\frac{2\pi}{N}kn} \\
=& \bigg\{\sum_{n = N - m}^{N-1} f(n) e^{-j\frac{2\pi}{N}kn} + \sum_{n = 0}^{N - 1 - m}f(n)e^{-j\frac{2\pi}{N}kn}\bigg\} e^{-j\frac{2\pi}{N}km} \\
...
- (iii) $k = 2$ のとき ($1 \ [\text{Hz}]$)、$F(2) = 0$ より、成分なし。

よって、$g(t)$ は、DC および、$\frac{1}{2} \ [\text{Hz}]$ の $\sin,\cos$ 成分からなる。

$\frac{1}{2} \ [\text{Hz}]$ の成分について、$\omega t = 2\pi ft = \pi t$、$g(t) = A + B\cos\pi t + C\sin \pi t$ より、$t = [0,\frac{1}{2},1,\frac{3}{2}]$ と $f(n) = [3,0,-1,2]$ が対応し、
