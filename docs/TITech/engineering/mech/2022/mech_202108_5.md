...
- 对称矩阵的谱分解：求 $2\times2$ 对称矩阵的正交归一特征基，并由对角矩阵的幂计算 $A^n$。
- 幂级数与高阶极点留数：使用几何级数展开，并对二阶极点应用导数公式。
- Fourier 级数与 Parseval 恒等式：计算偶矩形脉冲的复系数、构造有限谐波近似，并由正交性求平方积分。
- 缺失题面边界：环面几何和一阶方程的 Kai 记录不足且自相矛盾，不能据此编造原条件或考点细节。
