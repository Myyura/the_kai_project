...
- 梯度、散度与旋度：利用 $\operatorname{div}(\operatorname{rot}\boldsymbol V)=0$，把无散条件化为标量势的 Laplace 算子条件。
- 极点与留数：比较 $e^z-1$、$\sin^2z$ 在原点的最低次 Taylor 项，确定简单极点及留数。
- 二阶常系数方程：由共轭复特征根写出实通解，并用待定系数法求多项式非齐次项的特解。
- 缺失题面边界：第 1 问及向量场 $\boldsymbol V$ 的原始分量没有材料依据，不补造对应考查内容。
