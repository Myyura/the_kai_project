...
- 方差与原点矩：展开中心化平方，利用期望线性性推导 $V(S)=E[S^2]-E[S]^2$。
- Poisson 分布矩母函数：在 $t=0$ 处求一、二阶导数，从而得到均值与方差。
- 几何分布矩母函数：对支持集从 $1$ 开始的概率质量函数求几何级数，并注明收敛条件。
- 独立随机变量之和：把 $e^{t(X+Y)}$ 分解并利用独立性证明矩母函数相乘，再用于具体均值计算。
