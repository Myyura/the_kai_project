...
- C 语言短路求值：按 `&&`、`||` 对左右操作数的条件求值规则，结合 `printf` 返回值准确追踪副作用与输出。
- 运算符优先级和结合性：区分成员访问、一元取地址/解引用的绑定顺序，并用括号写出唯一解析。
- 有序单链表插入：正确处理空表、头插和中间插入的边界条件，并从候选式补全遍历条件。
- 二级指针：让遍历指针始终指向“当前链接字段”，统一修改头指针或结点的 `next`。
- 链表反转：分别追踪回溯式递归、尾递归状态传递和三指针循环中的链接变化及中间状态。
