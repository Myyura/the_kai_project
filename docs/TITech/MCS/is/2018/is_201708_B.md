...
设数列 $\{a_n\}_{n=0}^{\infty}$ 满足

$$
a_{n+1}\leq\frac12a_n,\qquad a_n\geq0.
$$

1. 证明当 $n\to\infty$ 时，数列 $\{a_n\}_{n=0}^{\infty}$ 收敛。
2. 判断级数
   ...
   是否收敛；若收敛则给出证明，若不收敛则给出反例。

#### 考点

- 数列收敛性：由非负性和递推不等式得到几何衰减上界，并确定数列极限。
- 正项级数比较判别：将 $a_n$ 与以 $a_0$ 为首项、公比为 $1/2$ 的几何数列比较，判断无穷级数。
