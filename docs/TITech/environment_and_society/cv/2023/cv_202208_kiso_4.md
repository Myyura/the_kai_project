...
- 随机变量变换：由 $X=e^Y$ 的单调变换及 Jacobian $1/x$ 推导对数正态密度。
- 对数正态分布的矩：对 Gaussian 指数矩配方积分，求 $E[e^Y]$、$E[e^{2Y}]$ 和方差。
- 对数正态参数的最大似然估计：对样本似然取对数并分别对 $\lambda,\zeta$ 求导，得到对数样本均值及以 $n$ 为分母的离差尺度。
- 缺失题面边界：第 3(c) 无任何可用信息，不增加推测性考点。
