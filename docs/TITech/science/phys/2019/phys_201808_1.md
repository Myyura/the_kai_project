...
> **题面缺失边界：** 当前 Description 与全部本地 Git 历史版本均为空，原机械装置图也没有留存在允许材料中。Kai 可确认装置含一根均匀细杆和一圆盘、广义坐标 $\theta,\phi$ 及下列几何和能量关系；连接点名称、角度零方向和第 6 问候选示意图本身无法恢复。

设均匀细杆质量为 $m$、长度为 $l$，绕一端转动；均匀圆盘质量为 $M$、半径为 $a$。杆角为 $\theta$，圆盘转角为 $\phi$，重力加速度为 $g$。Kai 中圆盘中心 O 的坐标关系为
...
2. 以 $\theta=\phi=0$ 为势能零点，求系统重力势能 $V(\theta,\phi)$。Kai 唯一保留的高度关系给出
...
5. 在极限
   ...
   求两个正常模角频率 $\omega_\pm$。
6. 再取 $l\gg a$，求两个正常模的振幅比 $B/A$ 的主导近似，并从原题候选图中选择相应模式。Kai 记录
...
- 耦合刚体正常模：代入同频简谐试探解，令系数行列式为零求两支本征频率，并在 $m/M\to0$、$l\gg a$ 下分析同相与反相振幅。
