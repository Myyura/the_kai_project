...
$$
D=\{(x,y,z)\in\mathbb R^3\mid x^2+y^2+1\leq z\leq5\}.
$$

回答下列问题。

1. 求向量场 $A$ 的散度 $\nabla\cdot A$ 和旋度 $\nabla\times A$。
2. 设 $\partial D$ 是取外向定向的区域边界，$n$ 为外向单位法向量。计算通量积分
...
3. 定义曲面和平面
   ...
   $$
   S=\{(x,y,z)\in\mathbb R^3\mid x^2+y^2+1=z\},
   $$

   $$
   T=\{(x,y,z)\in\mathbb R^3\mid x+y=0\}.
   $$

   令曲线 $C\subseteq S\cap T$ 从 $P(0,0,1)$ 指向 $Q(1,-1,3)$。取 $C$ 的适当参数表示 $r=r(t)$，计算线积分
