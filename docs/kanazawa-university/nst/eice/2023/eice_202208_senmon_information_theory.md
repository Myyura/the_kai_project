...
无记忆信源 $S=\{s_1,s_2\}$ 中，$s_1$ 的出现概率为

$$
P(s_1)=p,\qquad0\leq p\leq1.
$$

令 $a_1=s_1,a_2=s_2$，构成发送符号集 $A=\{a_1,a_2\}$。发送 $a_i$ 经过信道 $C$ 后接收 $B=\{b_1,b_2\}$ 中的符号 $b_j$；信道矩阵为
...
其中第 $i$ 行第 $j$ 列是发送 $a_i$ 时收到 $b_j$ 的概率。
...
并在 $x=\frac12$ 时取得最大值 1。

1. 求二阶扩展信源 $S^2$ 的熵 $H(S^2)$，并用 $H(p)$ 表示。
2. 编码 $C$ 对 $S^2$ 的四个信源符号编码如下：
   ...
   对码字 $c_k,c_\ell\in C$，以 Hamming 距离定义

   $$
   d_{\min}(C)
   =
   \min_{\substack{c_k\ne c_\ell\\c_k,c_\ell\in C}}
   h(c_k,c_\ell).
   $$

   已知 $d_{\min}(C)=3$，且 $c_3,c_4$ 的 Hamming 重量均为 3。求可能的码字对 $\{c_3,c_4\}$ 的数量。若同一对候选码字 $c_a,c_b$ 可用，则交换 $c_3=c_a,c_4=c_b$ 与 $c_3=c_b,c_4=c_a$ 只计作一种组合。
3. 令
   ...
   求接收符号的熵 $H(B)$，并用 $H(\alpha)$ 表示。
4. 求后验概率 $P(a_1\mid b_1)$ 与 $P(a_1\mid b_2)$。
5. 用熵函数表示条件熵 $H(A\mid B)$，并求信道 $C$ 的信道容量。
...
### 問4
以下では条件事象の確率が正、すなわち $\alpha>0$ および $1-\alpha>0$ とする。確率が $0$ の受信記号に対する事後確率は定義されない。
...
となる。端点では $0\log_2 0=0$ とし、上式を連続的に解釈する。
...
である。$q=\frac12$ のときは任意の $p$ で $\alpha=\frac12$ となり、同じ式から $C_{\mathrm{cap}}=0$ を得る。
