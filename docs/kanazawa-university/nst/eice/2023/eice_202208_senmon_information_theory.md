...
- **无记忆信源扩展与熵可加性**：列出二阶符号概率并利用独立性求 $H(S^2)$。
- **最小 Hamming 距离与码字枚举**：在固定长度、重量和距离约束下组合计数可用码字对。
- **二元对称信道**：由输入先验和交叉概率求输出分布、后验概率及条件熵。
- **互信息与信道容量**：使用 $I(A;B)=H(A)-H(A\mid B)$ 或 $H(B)-H(B\mid A)$，并对输入分布最大化。
