...
$$
f(x)=\pi-2x,\qquad0\leq x\leq\pi,
$$

$$
g(x)=x(\pi-x),\qquad0\leq x\leq\pi.
$$

1. 分别求 $f(x)$ 的 Fourier 正弦级数和 $g(x)$ 的 Fourier 余弦级数。
2. 将微分方程

   $$
   y''-4y=g(x),\qquad0<x<\pi
   $$
...
これは $y'(0)=y'(\pi)=0$ を満たす解である。問題文には境界条件がないので解の一意性は主張できず、微分方程式の一般解は上式に $C_1e^{2x}+C_2e^{-2x}$ を加えたものとなる。
