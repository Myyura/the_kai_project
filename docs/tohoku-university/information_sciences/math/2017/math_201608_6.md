...
tags:
  ...
  - Probability-Statistics.Computational-Statistics.Antithetic-Variates
...
(1) 期待値 $E(Y),E(Z)$ と分散 $V(Y),V(Z)$ を $m, g$ を用いて表せ．
...
$$
A_n = \frac{1}{2n} \sum_{i=1}^{2n} g(X_i), \ \ \ B_n = \frac{1}{2n} \sum_{i=1}^{n} (g(X_i) + g(1 - X_i))
$$
...
$$
Y=g(X),\qquad Z=\frac{g(X)+g(1-X)}2.
$$

1. 用 $m$ 和 $g$ 表示 $E(Y)$、$E(Z)$ 以及 $V(Y)$、$V(Z)$。
2. 证明对任意 $x,y\in\mathbb R$，

   $$
   (g(x)-g(y))(g(1-x)-g(1-y))\le0.
   $$

3. 证明

   $$
   \int_0^1g(x)g(1-x)\,dx\le m^2.
   $$

4. 令 $\{X_1,X_2,\ldots\}$ 为 $[0,1]$ 上独立同分布的均匀随机变量，并定义

   $$
   A_n=\frac1{2n}\sum_{i=1}^{2n}g(X_i),\qquad
   B_n=\frac1{2n}\sum_{i=1}^{n}\bigl(g(X_i)+g(1-X_i)\bigr).
   $$

   比较二者作为 $m$ 的估计量的效率，判断哪一个更有效并说明理由。
...
両者とも期待値は $m$ であるから、同じ $2n$ 回の関数評価では $B_n$ の分散（したがって平均二乗誤差）は $A_n$ 以下である。特に $g$ が定数の場合には両者の分散は $0$ であり、効率は同じである。

## **Reference**

- [東北大学 情報科学研究科 数学教室 2016年8月試験（PDF）](https://www.math.is.tohoku.ac.jp/admission/docs/1608.pdf)
