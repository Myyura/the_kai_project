...
1. 函数 $f$ 在闭区间 $[3,10]$ 上连续且严格单调递减，并满足
   $$f(3)=10,\qquad f(10)=5,\qquad \int_3^{10}f(x)\,dx=42.5.$$
   计算
   $$\int_5^{10}f^{-1}(x)\,dx.$$
2. 令
...
- **反函数积分**：利用函数图像关于 $y=x$ 对称或反函数积分公式。
- **单调连续函数**：由端点值确定反函数定义域和积分边界。
- **矩阵乘法与幂**：先计算稀疏矩阵 $J'+2J^2$ 的低次幂，再识别周期或标量倍关系。
