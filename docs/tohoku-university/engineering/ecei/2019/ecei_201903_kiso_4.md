...
- 手続き $Q(i，j)$ は、配列 $A$ の $i$ 番目と $j$ 番目の要素を入れ替える.ただし、もし $i=j$ なら配列 $A$ は変化しない.
...
(3) Fig. 4(b) に示す整列アルゴリズム Alg2 について、手続き $Q$の総呼び出し回数が $N- 1$ 回以下となるように，(A), (B)，(C) に入る適切な疑似コードを示せ.ただし、疑似コードは Fig.4(a) に示す疑似コードの表記に従うものとする.

(4) Fig. 4($c$) に示す整列アルゴリズム Alg3 の疑似コードを考える.ただし、Fig. 4($c$) 中に示す手続き $R$ が利用できる.
...
(2) Consider the pseudocode for the sorting algorithm **Alg1** shown in Fig. 4(a). Give the number of calls to the procedure $P$ as an expression of $N$. Moreover, explain your derivation of the result.
...
## **Kai**
### (1)
只需考虑 $N$ 个元素互异的输入。比较决策树至少有 $N!$ 个叶结点，而每次调用 $P$ 只有两种结果。设树高为 $h$，则最坏情形下
...
固定 $i$ 时，内层循环调用 $P$ 恰好 $N-i$ 次，与输入数值无关。因此总调用次数为
...
#### (a)
先从后向前调用 $R$，将数组建成最大堆；随后把堆顶最大值与堆末元素交换，缩小堆范围，并调用 $R$ 恢复堆性质，直至堆只剩一个元素。
...
最后两次输出相同，因为最后的 $Q(1,2)$ 交换了两个相等元素，但仍须输出。
