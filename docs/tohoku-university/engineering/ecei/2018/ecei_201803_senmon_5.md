...
因 $b-(b-a)=a$，先用后果规则将第一式的前置条件改为 $I(b-a,a)$，再应用后果规则与顺序规则，得到 $\vdash\{I\land a>0\}[a=b-a;b=b-a]\{I\}$。由循环规则，退出时满足 $I\land a\le0$；而 $F_i>0$ 对 $i\ge1$ 成立，故只能 $i=0$，于是 $(a,b)=(0,1)$。最后用后果规则即得所证三元组。
