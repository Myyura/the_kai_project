...
1. 图 2(a) 为并联 RLC 电路，电源电压与角频率为 $V,\omega$，忽略内阻；$R=50\,\Omega$、$L=2\,\mathrm{mH}$、$C=10\,\mu\mathrm F$。
   ...
   2. 当 $\omega$ 从 $10^4\,\mathrm{rad/s}$ 减小到 $10^0\,\mathrm{rad/s}$ 时，用超前、滞后、同相描述 $I$ 相位的变化；求谐振频率 $f_0$，并用必要的 $V,R,L,C$ 表示谐振时流过 $R$ 的电流。
...
$$
\arg\left(\frac{I}{V}\right)=\arg Y_{ab}=\arctan\!\left[50\left(10^{-5}\omega-\frac{1}{2\cdot10^{-3}\omega}\right)\right]
$$

按题目从 $10^4$ 降到 $10^0\,\mathrm{rad/s}$ 的方向，电流最初超前电压；降至共振频率时同相；继续降低频率后，电流滞后电压。
...
完全耦合 $M^2=L_1L_2$ 时，由两个线圈的电压方程可得电压比等于匝数比，也等于自感比的平方根。有限自感仍意味着存在励磁电流，不能把整个器件直接等同于没有励磁支路的理想变压器：
