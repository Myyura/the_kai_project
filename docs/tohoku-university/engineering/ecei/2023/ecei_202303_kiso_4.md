...
1. 考虑由顶点集 $V$ 与带权边集 $E$ 构成的连通无向图 $G=(V,E)$，图 4 的 $G_1$ 是一个具体例子。
   1. 写出 $G_1$ 的邻接矩阵与邻接表；可忽略边权；
   2. 对含 $n$ 个顶点、$m$ 条边的图，分别用大 $O$ 记号给出存储邻接矩阵和邻接表所需空间；
   3. 生成树是包含 $G$ 全部顶点的连通无环子图，最小生成树是总边权最小的生成树。求图 4 中 $G_1$ 的最小生成树。
2. 整数矩阵 $M$ 有 $n$ 行 $m$ 列，每行从左到右非降序、每列从上到下非降序。设计并概述一个时间复杂度为 $O(n+m)$ 的算法：给定整数 $x$，若 $M$ 含有 $x$ 输出 `Yes`，否则输出 `No`。

#### 考点

- **图的表示**：在邻接矩阵和邻接表之间转换并比较空间复杂度。
- **最小生成树**：用 Prim 或 Kruskal 算法选择不成环的最轻边。
- **渐近空间分析**：邻接矩阵为 $O(n^2)$，邻接表为 $O(n+m)$。
- **有序矩阵搜索**：从右上角或左下角出发，每次排除一行或一列。
