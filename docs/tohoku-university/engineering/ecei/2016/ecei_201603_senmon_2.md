...
上述した振幅変調波 $g_{\mathrm{AM}}(t)$ の受信機モデルを Fig. 2 に示す。受信機内では両側電力スペクトル密度が $N_0/2$ である白色雑音 $n(t)$ が付加されるものとする。バンドパスフィルタ（BPF）は中心周波数 $f_c$，通過帯域 $2f_r$ の次式で表される理想的な通過特性を持つものとする。
$$
H(f)=\begin{cases}1&\bigl||f|-f_c\bigr|\le f_r\\0&\bigl||f|-f_c\bigr|>f_r\end{cases}
$$
なお $f_m,f_c$ および $f_r$ は $f_m<f_r\ll f_c$ の関係を満足する。このとき，以下の問に答えよ。
...
4. 接收端叠加双边功率谱密度为 $N_0/2$ 的白噪声，经过理想带通滤波器、包络检波和直流隔离。带通在 $\big||f|-f_c\big|\le f_r$ 时增益为 $1$，其他频率为 $0$，且 $f_m<f_r\ll f_c$。假设载波足够强，求输出信噪比与包络检波器输入信噪比 $(S/N)_{in}$ 的关系及其最大值。
...
带通输出噪声功率为 $P_{n,in}=2N_0f_r$。把窄带噪声写为

$$
n_I(t)\cos(2\pi f_ct)-n_Q(t)\sin(2\pi f_ct),
$$

其同相分量功率为 $E[n_I^2]=2N_0f_r$。强载波近似下，去除直流后的包络输出为
...
$$
\left(\frac SN\right)_{out}=\frac{m^2A_c^2}{4N_0f_r},\qquad
\left(\frac SN\right)_{in}=\frac{A_c^2(2+m^2)}{8N_0f_r},
$$
