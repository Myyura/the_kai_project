...
- (a) このフィードバック制御系の閉ループ伝達関数 $G_d(s)$ を求めよ。
- (b) このフィードバック制御系の固有周波数 $\omega_n$ を求めよ。また，減衰率が $\zeta=0.5$ となる $K_T$ を求めよ。
- (c) 目標値が単位ランプ関数 $r(t)=t$ であるときの定常速度偏差 $e_{ss}$ を求めよ。
...
- (c) このフィードバック制御系のナイキスト線図の概形を描け。また，位相交差周波数 $\omega_{pc}$ を求めよ。
...
1. 当 $G_c(s)=1$ 时：(a) 求闭环传递函数；(b) 求固有角频率 $\omega_n$，以及使阻尼比 $\zeta=0.5$ 的 $K_T$；(c) 输入为单位斜坡 $r(t)=t$ 时，求稳态误差。
...
$$
\boxed{G_d(s)=\frac{10}{s^2+(1+10K_T)s+10}.}
$$

比较 $s^2+2\zeta\omega_ns+\omega_n^2$，有

$$
\boxed{\omega_n=\sqrt{10},\qquad K_T=\frac{\sqrt{10}-1}{10}\quad(\zeta=0.5).}
$$

速度误差常数为 $K_v=\lim_{s\to0}sP_{\rm in}(s)=10/(1+10K_T)$，所以

$$
\boxed{e_{\rm ss}=\frac{1+10K_T}{10}.}
$$

若使用 (b) 的 $K_T$，则 $e_{\rm ss}=1/\sqrt{10}$。
