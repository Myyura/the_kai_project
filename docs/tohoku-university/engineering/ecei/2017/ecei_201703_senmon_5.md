...
(a) 自然数同士の乗算を行う関数 $mul$ を含むプログラムを与えよ。
...
2. (a) 用此语言编写自然数乘法 `mul`；(b) 编写 `le(m,n)`，使其在 $m\le n$ 时输出 $1$，否则输出 $0$。
...
```text
mul(m,n) = ifz m then 0 else f(n,mul(pred(m),n))
le(m,n) = ifz m then 1 else
              (ifz n then 0 else le(pred(m),pred(n)))
```

乘法按第一个参数归纳即得 $\operatorname{mul}(m,n)=mn$；比较函数同时减一，直到一方为零，因此恰在 $m\le n$ 时返回 $1$。
