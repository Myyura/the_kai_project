...
Fig. 5 に示す再帰関数 $f$ を考える。ここで，入力 $x$ と $y$ は任意の正の整数であるとする。関数 $\operatorname{div}(x,y)$ は，$x$ を $y$ で割った商を整数で返し，$\operatorname{mod}(x,y)$ は，$x$ を $y$ で割った余りを整数で返す。演算子 ‘−’ と ‘*’ は，整数の減算と乗算を各々表す。また，式 “if $e_1=e_2$ then $e_3$ else $e_4$” の値は，$e_1$ の値が $e_2$ の値に等しければ $e_3$ の値に，そうでなければ $e_4$ の値に等しい。$f$ に関する以下の問に答えよ。
