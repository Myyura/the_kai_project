...
#### <center>图 1：结构体 `node` 的定义

#### <center>表 1：操作结构体 `node` 的函数
...
- **二叉树遍历**：区分前序、中序与后序遍历的访问顺序，并能用递归函数实现。
- **二叉搜索树**：利用键值大小关系递归选择左、右子树并完成插入。
- **递归调用计数**：根据插入路径和树高统计函数调用次数。
- **树形与插入顺序**：分析插入顺序如何影响二叉搜索树的高度和操作代价。
