...
[大学公表の原題](https://www.eps.sci.kyoto-u.ac.jp/education-md/exam-m/docs/kiso05_fix.pdf)
### \[1\]
次の積分を計算せよ． 
...
2. 方阵对角元素之和称为迹。设 $\boldsymbol A,\boldsymbol B$ 为 $n\times n$ 方阵，证明 $\boldsymbol A\boldsymbol B$ 与 $\boldsymbol B\boldsymbol A$ 的迹相等。
...
5. 考虑矩阵
   ...
   对 $\boldsymbol x^{t}=(x_1,x_2)$，计算

   $$
   \int_{-\infty}^{\infty}
   \int_{-\infty}^{\infty}
   \exp(\boldsymbol x^{t}\boldsymbol A\boldsymbol x)
   \,dx_1\,dx_2.
   $$
