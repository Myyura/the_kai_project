...
tags:
  ...
  - Physics.Thermodynamics.Canonical-Partition-Function
  - Physics.Quantum-Mechanics.Rectangular-Barrier-Tunneling
...
[大学公表の原題](https://www.scphys.kyoto-u.ac.jp/education/inshi/kakomon/R2.pdf)
### I-2A（統計力学）
...
为研究固体晶格三维振动所导致的比热温度依赖，假设晶体中各原子的振动可视为相互独立的谐振子。每个原子质量为 $m$，原子数为 $N$，谐振子角频率为 $\omega$。

1. 先作经典处理。第 $i$ 个振子相对平衡位置的位移和动量分别为 $\vec r_i,\vec p_i$，系统 Hamiltonian 为
...
2. 若一自由度谐振子的量子能级为
   ...
   $$
   E_n=\left(n+\frac12\right)\hslash\omega,
   \qquad n=0,1,2,\ldots,
   $$

   使用正则分布计算比热，其中 $\hslash$ 是 Planck 常数除以 $2\pi$。

3. 对第 2 问的量子情形，说明充分高温和充分低温下比热随温度变化的特征。

### I-2B（量子力学）

质量为 $m$ 的粒子在一维空间中以能量 $E>0$ 从负 $x$ 方向入射高度为 $V_0>0$、宽度为 $a$ 的势垒。势能为
...
粒子从 $x<0$ 区域沿正方向入射，波函数

$$
\psi(x,t)=e^{-iEt/\hslash}\phi(x)
$$
...
这里 $\hslash$ 是 Planck 常数除以 $2\pi$。

1. 势能 $V(x)$ 在 $x=0$ 与 $x=a$ 处不连续。证明 $\phi(x)$ 的空间一阶导数在这两点连续。
2. 考虑入射能量 $E<V_0$ 的情形，粒子仍以一定概率穿过势垒到达 $x>a$。定义

   $$
   k=\sqrt{\frac{2mE}{\hslash^2}},
   \qquad
   \beta=\sqrt{\frac{2m(V_0-E)}{\hslash^2}}.
   $$

   用 $a,k,\beta$ 表示透射率。
3. 势垒厚度 $a$ 满足怎样的条件时，透射率会无限趋近于 0？
...
\frac{3}{2} N \hbar \omega
\cdot\left(-\frac{1}
{ \sinh^2 \frac{\hbar \omega}{2k_BT} }\right)
\cdot \left( - \frac{\hbar \omega}{2k_BT^2} \right)
\\
&=
