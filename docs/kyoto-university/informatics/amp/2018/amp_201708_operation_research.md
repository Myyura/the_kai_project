...
$$
\Omega=\left\{\boldsymbol{x}\in\mathbb{R}^n\ \middle|\ 0\leqq x_i\leqq1,\ i=1,\ldots,n\right\}.
$$

设 $f:\mathbb{R}^n\to\mathbb{R}$ 连续可微，并且对任意
$\boldsymbol{x},\boldsymbol{y}\in\mathbb{R}^n$ 和 $\alpha\in[0,1]$ 都有

$$
\alpha f(\boldsymbol{x})+(1-\alpha)f(\boldsymbol{y})
\geqq
f\!\left(\alpha\boldsymbol{x}+(1-\alpha)\boldsymbol{y}\right)
...
$$
\begin{aligned}
P:\quad &\text{最小化}\quad -f(\boldsymbol{x})\\
&\text{约束于}\quad \boldsymbol{x}\in\Omega,
\end{aligned}
$$

以及以 $\boldsymbol{z}\in\Omega$ 为参数、以
$\boldsymbol{x}\in\mathbb{R}^n$ 为决策变量的凸二次规划
...
+\frac12(\boldsymbol{x}-\boldsymbol{z})^\top
(\boldsymbol{x}-\boldsymbol{z})\\
&\text{约束于}\quad \boldsymbol{x}\in\Omega.
\end{aligned}
$$

已知对每个 $\boldsymbol{z}\in\Omega$，$Q(\boldsymbol{z})$ 都有唯一最优解，记为
$\bar{\boldsymbol{x}}(\boldsymbol{z})$。回答下列问题：
...
2. 写出问题 $Q(\boldsymbol{z})$ 的 Karush–Kuhn–Tucker（KKT）条件。
3. 证明对任意 $\boldsymbol{z}\in\Omega$，

$$
f(\boldsymbol{z})-f\!\left(\bar{\boldsymbol{x}}(\boldsymbol{z})\right)
\leqq
-\left(\bar{\boldsymbol{x}}(\boldsymbol{z})-\boldsymbol{z}\right)^\top
\left(\bar{\boldsymbol{x}}(\boldsymbol{z})-\boldsymbol{z}\right).
$$

4. 判断命题（A）是否成立：若 $\boldsymbol{z}\in\Omega$ 且
   $\bar{\boldsymbol{x}}(\boldsymbol{z})=\boldsymbol{z}$，则 $\boldsymbol{z}$ 是问题 $P$ 的局部最优解。若成立，请证明；若不成立，请给出反例。
