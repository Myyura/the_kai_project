...
对有限集合 $A$，以 $|A|$ 表示元素个数。给定简单连通无向图 $G=(V,E)$，且 $|V|\ge3$，选定起点 $s\in V$。对任意 $v\in V$，令 $\operatorname{dist}(v)$ 为从 $s$ 到 $v$ 的最短路边数，$\sigma(v)$ 为这类最短路的总条数，并定义

$$
d_{\max}=\max_{v\in V}\operatorname{dist}(v),\qquad
V_i=\{v\in V\mid\operatorname{dist}(v)=i\}
\quad(i=0,\ldots,d_{\max}).
$$

回答：

1. 给出在 $O(|E|)$ 时间内计算 $d_{\max}$ 及全部 $V_i$ 的方法。
2. 给出在 $O(|E|)$ 时间内计算所有 $\sigma(v)$ 的方法。
3. 对给定 $t\in V\setminus\{s\}$ 与
   $U\subseteq V\setminus\{s,t\}$，在 $O(|E|)$ 时间内计算从 $s$ 到 $t$ 且至少经过一个 $U$ 中顶点的最短路条数。
4. 对给定的上述 $t,U$ 及整数 $1\le k\le|U|$，在 $O(|E|)$ 时间内判定是否存在从 $s$ 到 $t$ 且至少经过 $k$ 个 $U$ 中顶点的最短路。

#### 考点

- **广度优先搜索分层**：在线性时间内求无权图单源距离、最大层数和各距离层。
- **最短路 DAG 上的动态规划**：沿层次累加最短路总数，并扩展状态统计经过指定集合的路径。
- **路径属性的线性时间判定**：在最短路子图上求一条路径可经过的 $U$ 中顶点最大数，与阈值 $k$ 比较。
