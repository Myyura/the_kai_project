...
$n=0,1$ は $f_0=1,f_1=2x$ より直接確認できる。$n\ge2$ では (i) と (ii) から
...
$t,s$ を原点近傍の有界区間に制限すると、各階の被積分関数の導関数は可積分な「多項式 $\times e^{-x^2+C|x|}$」で抑えられるので、積分と係数抽出を交換できる。他方、左辺で生成関数を展開したときの $t^ns^n$ の係数は
