...
设 $a(t)$ 是半无限区间 $[1,\infty)$ 上的连续函数，考虑微分方程

$$
t\frac{d^2x}{dt^2}+(t^2-1)a(t)\frac{dx}{dt}-ta(t)x=0,
\qquad t\geqq1. \tag{1}
$$

已知 $x=\dfrac{t^3}{t^2+1}$ 是方程 (1) 的一个解。回答：

1. 求函数 $a(t)$。
2. 在 (1) 中作代换 $x=\dfrac{t^3}{t^2+1}y$，求 $y$ 满足的微分方程。
3. 解第 2 问的方程，从而求出 (1) 的一般解。
4. 求初值
   $$
   (x_0,v_0)=\left(x(1),\frac{dx}{dt}(1)\right)
   $$
   应满足的充要条件，使解 $x(t)$ 在 $[1,\infty)$ 上有界。

#### 考点

- **二阶线性常微分方程的降阶**：利用已知非零解作乘积代换，导出较低阶方程并求一般解。
- **解的渐近有界性**：分析一般解各独立分量在 $t\to\infty$ 时的行为，把有界条件转化为初值之间的充要关系。
