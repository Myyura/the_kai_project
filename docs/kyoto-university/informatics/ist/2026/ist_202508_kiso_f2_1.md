...
1. 下图为一张无向图，边旁数字表示边长，其中顶点 $a,b$ 之间的边长 $x$ 为正实数。两个顶点 $s,t$ 之间总边长最小的路径称为最短路径，其总边长称为 $s,t$ 之间的距离。路径用不含重复顶点的顶点序列表示。
...
2. 给定全集 $U=\{1,2,\ldots,n\}$ 和集合族 $S=\{S_1,S_2,\ldots,S_m\}$，其中每个 $S_i\subseteq U$。集合覆盖问题要求找出

   $$
   \mathcal C=\{S_{c_1},S_{c_2},\ldots,S_{c_k}\}\subseteq S,
   $$

   使 $S_{c_1}\cup S_{c_2}\cup\cdots\cup S_{c_k}=U$ 且 $k$ 最小。这里 $n,m$ 为正整数，并假设 $S_1\cup\cdots\cup S_m=U$。以下贪心算法计算该问题的一个近似解：

   $$
   \begin{array}{ll}
   1 & \mathcal{C}\leftarrow\{\};\ \mathcal{P}\leftarrow S;\ V\leftarrow U;\\
   2 & \mathbf{while}\ V\neq\{\}\ \mathbf{do\ begin}\\
   3 & \quad h\leftarrow\max(\{\#(S_j\cap V)\mid S_j\in\mathcal P\});\\
   4 & \quad i\leftarrow\min(\{j\mid\#(S_j\cap V)=h,\ S_j\in\mathcal P\});\\
   5 & \quad \mathcal P\leftarrow\mathcal P\setminus\{S_i\};\
   ...
   其中 $\#(X)$、$\max(X)$、$\min(X)$ 分别表示集合 $X$ 的元素个数、最大元素和最小元素；$X\setminus Y$ 表示从 $X$ 中删除同时属于 $Y$ 的元素所得的集合差，例如

   $$
   \{1,3,4,5\}\setminus\{2,3,5\}=\{1,4\}.
   $$
   ...
   $$
   U=\{1,2,3,4,5\},\qquad
   S=\bigl\{\{1,3,5\},\{2,4\},\{2,3,5\}\bigr\},
   $$

   当 $S_1=\{1,3,5\}$、$S_2=\{2,4\}$、$S_3=\{2,3,5\}$ 时，算法输出

   $$
   \mathcal C=\bigl\{\{1,3,5\},\{2,4\}\bigr\};
   $$

   而当 $S_1=\{2,3,5\}$、$S_2=\{2,4\}$、$S_3=\{1,3,5\}$ 时，算法输出

   $$
   \mathcal C=\bigl\{\{2,3,5\},\{2,4\},\{1,3,5\}\bigr\}.
   $$

   对固定的 $U,S$，考察 $S$ 中元素的所有排列。将算法所有可能输出中 $\mathcal C$ 的最小基数记为 $A_{\min}(U,S)$，最大基数记为 $A_{\max}(U,S)$。上述例子中 $A_{\min}(U,S)=2$、$A_{\max}(U,S)=3$。回答下列问题，并说明每个答案的理由。

   （1）令

   $$
   U=\{1,2,\ldots,10\},\qquad
   S=\{\{i,i+1\}\mid i=1,2,\ldots,9\},
   $$

   求 $A_{\min}(U,S)$ 与 $A_{\max}(U,S)$。

   （2）令

   $$
   U=\{1,2,\ldots,100\},\qquad
   S=\{\{i,j\}\mid 1\leq i<j\leq100,\ i,j\text{ 为整数}\},
   $$
   ...
   $$
   U=\{1,2,\ldots,63\},\qquad
   S=\{\{i,2i\},\{i,2i+1\}\mid i=1,2,\ldots,31\},
   $$
...
- **最短路径与必经顶点**：枚举或比较候选路径长度，并通过参数化距离判断某顶点是否属于所有最短路径。
- **集合覆盖的贪心算法**：逐步选择覆盖最多尚未覆盖元素的集合，并理解编号在平局处理中的作用。
- **贪心算法的顺序敏感性**：构造最佳与最差排列，推导不同集合系统下输出规模的极值。
- **图结构与组合论证**：将路径图、完全图和满二叉树边集上的覆盖过程转化为匹配或边覆盖分析。
