...
\begin{array}{l}
\hline
\textbf{Algorithm 2: } \text{MST-KRUSKAL}(G, w) \\
\hline
...
\mathbf{11}\quad \\
\mathbf{12} \quad \text{FindSet}(v) \\
\mathbf{13} \quad \quad \textbf{if } v \neq v.parent \\
\mathbf{14} \quad \quad \quad \textbf{return } \text{FindSet}(v.parent) \\
\mathbf{15} \quad \quad \textbf{return } v.parent \\
\mathbf{16}\quad \\
\mathbf{17} \quad \text{Union}(u, v) \\
\mathbf{18} \quad \quad ru = \text{FindSet}(u) \\
\mathbf{19} \quad \quad rv = \text{FindSet}(v) \\
\mathbf{20} \quad \quad \textbf{if } ru == rv \\
\mathbf{21} \quad \quad \quad \textbf{return} \\
\mathbf{22} \quad \quad ru.parent = rv
\end{array}
$$
...
3. Kruskal 算法按边权升序依次选取不会产生环的边，从而求得最小生成树。算法 2 使用并查集判断所选边是否产生环；并查集通过树结构表示集合，以高效判断两个元素是否属于同一集合。
   ...
   \mathbf{10}\quad \textbf{return }MST\\
   \mathbf{11}\quad \\
   \mathbf{12}\quad \operatorname{FindSet}(v)\\
   \mathbf{13}\quad |\quad \textbf{if }v\ne v.parent\\
   \mathbf{14}\quad |\quad |\quad \textbf{return }\operatorname{FindSet}(v.parent)\\
   \mathbf{15}\quad |\quad \textbf{return }v.parent\\
   \mathbf{16}\quad \\
   \mathbf{17}\quad \operatorname{Union}(u,v)\\
   \mathbf{18}\quad |\quad ru=\operatorname{FindSet}(u)\\
   \mathbf{19}\quad |\quad rv=\operatorname{FindSet}(v)\\
   \mathbf{20}\quad |\quad \textbf{if }ru==rv\\
   \mathbf{21}\quad |\quad |\quad \textbf{return}\\
   \mathbf{22}\quad |\quad ru.parent=rv\\
   \hline
