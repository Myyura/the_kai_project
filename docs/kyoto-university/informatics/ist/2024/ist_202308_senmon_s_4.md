...
### Correction from Issue #121

**Author:** sure

Q2的答案在用拉格朗日乘数法时，对p和q的偏导求错了，具体是，在求以2为底p的对数的导数的时候求错了，把以2为底当成了以自然对数为底，所以得到的等式错了，应该是λ=log2p+ln2分之一=log2q+ln2分之一。后面熵的最大值也求错了，在p=q=2m分之一的情况下，熵应该是以2为底2m的对数。另外，Q6的题干描述错了，输入大小应该是2而不是n。
