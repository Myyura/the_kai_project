...
tags:
  - Kyoto-University
  - Probability-Statistics.Estimation-and-Hypothesis-Testing.Fisher-Exact-Test
...
1. $X_1,\ldots,X_n$ 是均值为 $\mu$ 的总体的随机样本。给出加权和
   $\sum_iw_iX_i$ 成为 $\mu$ 的无偏估计量的充要条件。
2. $X_1,\ldots,X_n$ 来自 $N(\mu,\sigma^2)$。
   1. 推导 $\mu,\sigma^2$ 的对数似然 $L(\mu,\sigma^2)$；
   2. 由其求 $\mu,\sigma^2$ 的最大似然估计量。
3. $X,Y$ 的均值均为 1、方差均为 2。令
   $S=X+2Y$、$T=X-2Y$，求 $\operatorname{Cov}(S,T)$。
4. 在药 A、B 效果无差异的零假设下，对表作 Fisher 精确检验：
   ...
   1. 固定边缘总数，从 9 人随机选 5 人为“有效”，求其中 A 组 3 人、B 组 2 人的概率。
   2. 双侧 $p$ 值定义为所有边缘相同且概率不高于观察表的表之概率和，求该 $p$ 值。

#### 考点

- **无偏估计**：对加权和取期望，得到权重和为 1 的充要条件。
- **正态模型最大似然**：推导均值与方差（分母为 $n$）的 MLE。
- **协方差的线性变换**：展开协方差并观察交叉项抵消。
- **Fisher 精确检验**：使用固定边缘下的超几何分布计算观察表概率及双侧尾概率。
