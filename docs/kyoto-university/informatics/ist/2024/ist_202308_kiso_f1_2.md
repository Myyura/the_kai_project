...
1. 计算：
   1. $\displaystyle\int_0^\infty\frac{dx}{(x^2+1)^2}$；
   2. 对椭圆域
      $D=\{(x,y)\mid x^2+y^2/4\le1\}$，计算
      $\displaystyle\iint_Dx^2y^2\,dx\,dy$。
2. 回答：
   1. 将 $\ln(1.02)$ 四舍五入到小数点后 6 位。
   2. 对 $x>0$ 证明
      $$
      x-\frac{x^2}{2}<\ln(1+x)<
      1-\frac{x^2}{2}+\frac{x^3}{3}.
      $$
3. 在约束 $3x^2+2y^2+z^2=1$ 下，求 $xyz$ 的最大值与最小值。

#### 考点

- **广义积分与椭圆变量代换**：用三角代换计算有理广义积分，并把椭圆缩放为圆后用极坐标积分。
- **Taylor 展开与余项估计**：计算对数数值近似，并用展开、导数符号或积分比较证明上下界。
- **多元约束极值**：用 Lagrange 乘子或加权 AM–GM 求椭球面上乘积的全局极值。
