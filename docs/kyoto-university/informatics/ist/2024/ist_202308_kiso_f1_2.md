...
The polynomial $x^3-x-1$ has exactly one positive root $\alpha$. Newton--Raphson iteration gives

$$
\alpha=1.324717957\ldots .
$$

Since $f'$ changes from negative to positive at $\alpha$, this is the minimum point, and

$$
\min_{x>0}f(x)=f(\alpha)=0.053868\ldots>0.
$$
...
For $1<x\leq x_0$, $f(x)\geq f(1)=e^{5/6}>x_0+1\geq x+1$. For $x>x_0$, the tangent bound gives
