...
1. 题图给出含 7 个顶点 $v_1,\ldots,v_7$、所有边权均为 1 的加权 DAG $G_1$。
   ...
   <figure style="text-align:center;">
     <img src="https://raw.githubusercontent.com/Myyura/the_kai_project_assets/main/kakomonn/kyoto_university/informatics/ist_202402_kiso_f2_1_p1.png" width="600" alt="七顶点 DAG"/>
   </figure>
...
2. 令 $\mathbb N$ 为非负整数集。在 $\mathbb N^3$ 上定义全序：
   ...
   <figure style="text-align:center;">
     <img src="https://raw.githubusercontent.com/Myyura/the_kai_project_assets/main/kakomonn/kyoto_university/informatics/ist_202402_kiso_f2_1_p2.png" width="450" alt="两个最小堆枚举算法"/>
   </figure>

#### 考点

- **DAG 上的路径计数与最短路 DP**：按拓扑序对入邻接点的路径数求和、最短距离取最小。
- **二叉最小堆操作**：模拟批量插入、抽取最小元及上浮下沉。
- **多路有序归并**：把每个固定坐标产生的有序序列视为一路，只保留每路当前候选，将堆空间限制为 $N+1$ 并证明无遗漏有序输出。
...
There are two length-$2$ paths from $v_1$ to $v_4$:
