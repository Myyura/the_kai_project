---
sidebar_label: "2021年7月実施 専門科目 S-2"
tags:
  - Kyoto-University
  ...
  - Probability-Statistics.Estimation-and-Hypothesis-Testing.One-Sample-Proportion-Z-Test
...
---
# 京都大学 情報学研究科 知能情報学専攻 2021年7月実施 専門科目 S-2
...
(1) $\mu - \mu_0 = 1.2, n = 16$ のときの検出力を考える。検出力は、確率変数 $Z$ が標準正規分布に従う時、確率 $\Pr[Z \geq u]$ として表すことができる。そのときの $u$ の値を求めよ。
...
(2) $X$ と $Y$ にそれぞれ次の一次変換を施して新しい確率変数 $U$ と $V$ を作った。
...
1. 独立样本 $x_i\sim N(\mu,4^2)$。检验
   $H_0:\mu=\mu_0$ 对 $H_1:\mu>\mu_0$，单侧显著性水平
   $\alpha=0.05$；可用 $\Pr(Z\le1.645)=0.95$。
   1. 当 $\mu-\mu_0=1.2,n=16$ 时，把检验功效写成
      $\Pr(Z\ge z)$，求其中 $z$。
   2. 当 $\mu-\mu_0=1.2$ 时，求使功效至少 $95\%$ 的最小 $n$。
2. 制造法 A 的不良率为 $p$。
   1. $p=0.5$，生产 8 件，求不良品不超过 1 件的概率。
   2. $p=1/4000$，生产 10000 件，求无不良品的精确概率及用
...
So we reject $H_0$ and find evidence of a difference in the defect rates under this normal approximation.
