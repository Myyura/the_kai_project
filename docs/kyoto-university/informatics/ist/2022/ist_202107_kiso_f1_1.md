---
sidebar_label: 2021年7月実施 情報学基礎 F1-1
tags:
  - Kyoto-University
...
---
# 京都大学 情報学研究科 知能情報学専攻 2021年7月実施 情報学基礎 F1-1
...
1. 给定实数 $r$、非零实向量 $\boldsymbol{x}$ 以及实矩阵
...
其中 $\|\boldsymbol{x}\|$ 表示向量长度。完成：

   1. 求 $A^3$。
   2. 求 $A^{-1}$。
   3. 求 $A^{15}$。
   4. 当自然数 $n\to\infty$ 时，求
      $\|(AB)^n\boldsymbol{x}\|$ 的极限；答案应按参数 $r$ 的取值讨论，包括极限不存在或发散的情形。

2. 设 $A$ 是 $m\times n$ 实矩阵，且 $m>n$。定义
...
其中 $\top$ 表示转置，最后一个公式定义 $l$ 维向量
$\boldsymbol{a},\boldsymbol{b}$ 的内积。必要时可以使用以下事实：实对称矩阵的特征值全为实数；对它的任一特征值，都可以选取实特征向量。

   1. 证明 $B$ 和 $C$ 都是实对称矩阵。
   2. 证明 $B$ 的所有特征值均非负。
   3. 实对称矩阵可以用正交矩阵对角化。设该正交矩阵中对应于 $B$ 的正特征值
      $\lambda_i,\lambda_j$ 的列特征向量分别为
      $\boldsymbol{p}_i,\boldsymbol{p}_j$，并定义
...
\qquad
\boldsymbol{q}_j
=\frac{1}{\sqrt{\lambda_j}}A\boldsymbol{p}_j.
$$

证明 $\boldsymbol{q}_i,\boldsymbol{q}_j$ 是 $C$ 的特征向量，对应特征值分别为
$\lambda_i,\lambda_j$；证明
...
$$
\boldsymbol{p}_i
=\frac{1}{\sqrt{\lambda_i}}A^\top\boldsymbol{q}_i.
$$
...
$$
\lambda = \frac{x^TA^TAx}{x^Tx} = \frac{\|Ax\|^2}{\|x\|^2} \geq 0
$$
...
Similarly, we have $Cq_j = \lambda_j q_j$ . Therefore, $q_i$ and $q_j$ are eigenvectors of C corresponding to eigenvalues $\lambda_i$ and $\lambda_j$ .
...
If $i \neq j$ , then $p_i^Tp_j = 0$ because $p_i,p_j$ are distinct columns of the orthogonal diagonalizing matrix. If $i = j$ , $p_i^Tp_j = 1$
Therefore, we have
