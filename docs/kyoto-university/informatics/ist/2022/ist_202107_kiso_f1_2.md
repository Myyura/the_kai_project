---
sidebar_label: "2021年7月実施 情報学基礎 F1-2"
tags:
  - Kyoto-University
...
---
# 京都大学 情報学研究科 知能情報学専攻 2021年7月実施 情報学基礎 F1-2
...
$$
x^2+y^2-1=0,\qquad 0\leq x,y\leq1
$$

的条件下，求 $f(x,y)=x^3+2y^3$ 的极值。

2. 地面上竖直立有一个半径为 $1\,\mathrm{m}$ 的圆柱。山羊颈部系着一根长度为
   $\pi\,\mathrm{m}$、不可伸长或缩短的绳，绳的另一端系在圆柱表面上与山羊颈部等高的一点。以圆柱中心轴为原点，并令圆柱上的系绳点为
   $(-1,0)$，在地面建立二维坐标系。

   1. 用从 $x$ 轴量起的角度 $\theta$ 表示绳保持绷紧时山羊在
      $x\geq-1,\ y\geq0$ 区域内的二维坐标。
...
#### (2)
まず、条件 $x^2+y^2-1=0$ より、 $x = \cos \theta, y = \sin \theta$ と表せる。
...
よって、局所的な極大値は $1,2$、極小値は $2/\sqrt{5}$ である。特に、最大値は $2$、最小値は $2/\sqrt{5}$ である。
...
本問では $r=1$ であり、求める範囲は $0\leq\theta\leq\pi$ である。
...
#### (3)
接点の角度を $\theta$、接点から伸びた直線部分の長さを $u$ とすると、

$$
(x,y)=(\cos\theta+u\sin\theta,\ \sin\theta-u\cos\theta),
\qquad 0\le u\le\theta\le\pi.
$$

この変数変換のヤコビアンの絶対値は $u$ なので、上側の面積は
$\int_0^\pi\int_0^\theta u\,du\,d\theta$ となる。
