...
对 $0<p\le1$ 定义
$N(p)=\lceil-\log_2p\rceil$。平稳无记忆信源
$S$ 的字母表为 $\Sigma=\{a_1,\ldots,a_n\}$，符号概率为
$p_1\ge\cdots\ge p_n>0$。定义累积概率

$$
P_1=0,\qquad P_i=\sum_{k=1}^{i-1}p_k\quad(i=2,\ldots,n),
$$

并将 $a_i$ 编为 $P_i$ 的二进制小数展开前 $N(p_i)$ 位，记为 $C(a_i)$。例如 $P_i=3/5,p_i=1/5$ 时，
$C(a_i)=100$。令信源熵为 $H(S)$，平均码长为 $\bar N$。
