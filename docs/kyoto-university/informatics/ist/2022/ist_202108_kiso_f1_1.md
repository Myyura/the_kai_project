...
1. 给定
   $$
   A=\begin{pmatrix}1&-\sqrt3\\\sqrt3&1\end{pmatrix},\quad
   B=\begin{pmatrix}r&0\\0&r\end{pmatrix},\quad
   x=\begin{pmatrix}x_1\\x_2\end{pmatrix},\quad\|x\|>0.
   $$
   1. 求 $A^3$；
   2. 求 $A^{-1}$；
   3. 求 $A^{15}$；
   4. 对正整数 $n$，求
      $\displaystyle\lim_{n\to\infty}\|(AB)^n x\|$。
2. 设 $A$ 为 $m\times n$ 实矩阵（$m>n$），定义
   $B=A^\top A$、$C=AA^\top$，内积
   $(a,b)=\sum_{k=1}^l a_kb_k$。
   1. 证明 $B,C$ 都是实对称矩阵。
   2. 证明 $B$ 的全部特征值非负。
   3. 设 $p_i,p_j$ 是 $B$ 对应正特征值
      $\lambda_i,\lambda_j$ 的正交单位特征向量，并定义
      $$
      q_i=\frac1{\sqrt{\lambda_i}}Ap_i,\qquad
      q_j=\frac1{\sqrt{\lambda_j}}Ap_j.
      $$
      证明 $q_i,q_j$ 分别是 $C$ 对应 $\lambda_i,\lambda_j$ 的特征向量，且
      $$
      (q_i,q_j)=\begin{cases}0,&i\ne j,\\1,&i=j,\end{cases}
      $$
      并证明
      $p_i=\frac1{\sqrt{\lambda_i}}A^\top q_i$。

#### 考点

- **旋转缩放矩阵幂与极限**：把 $A$ 识别为模长 2 的旋转，计算高次幂并按 $|2r|$ 分类讨论范数极限。
- **半正定矩阵**：由 $x^\top A^\top Ax=\|Ax\|^2$ 证明 $A^\top A$ 的谱非负。
- **奇异值分解**：建立 $A^\top A$ 与 $AA^\top$ 的正特征值、左右奇异向量及正交归一关系。
