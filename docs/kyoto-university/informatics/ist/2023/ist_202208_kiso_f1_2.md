...
仅用 $x$、$y$、$\dfrac{\partial^2z}{\partial x^2}$ 和
$\dfrac{\partial^2z}{\partial y^2}$ 表示
...
For $n\geq2$, Leibniz's rule gives
...
### 設問2
Write $D_u=x\partial_x+y\partial_y$ and $D_v=-y\partial_x+x\partial_y$. For a twice continuously differentiable $f$,

$$
z_{uu}=x^2z_{xx}+2xyz_{xy}+y^2z_{yy}+xz_x+yz_y,
$$

$$
z_{vv}=y^2z_{xx}-2xyz_{xy}+x^2z_{yy}-xz_x-yz_y.
$$
...
### 設問3
#### (1)
Let $I=\int_{-\infty}^{\infty}e^{-x^2}\,dx>0$. Using nonnegativity to exchange integrals and then polar coordinates,
