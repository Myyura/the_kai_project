...
図1の辺と距離（要約）：$(1,2):1$、$(2,1):2$、$(1,3):2$、$(3,4):2$、$(4,1):4$、$(4,2):1$。
...
Algorithm 1 の処理（要約）：配列を $\infty$ で初期化し、対角を $0$、各辺の成分をその距離にする。$k,i,j$ の順でそれぞれ $1,\ldots,N$ を走査し、条件 (a) が成立するときに処理 (b) を行い、最後に配列を返す。

### 题目描述

设 $G=(V,E)$ 为有向图，$V=\{1,\ldots,N\}$。边 $(u,v)$ 的长度
$l(u,v)>0$。路径 $p=(v_1,\ldots,v_m)$ 的中间顶点为端点之外的顶点，长度
$l(p)=\sum_{i=1}^{m-1}l(v_i,v_{i+1})$。题图给出一个具体图：

<figure style="text-align:center;">
  <img src="https://raw.githubusercontent.com/Myyura/the_kai_project_assets/main/kakomonn/kyoto_university/informatics/ist_202208_kiso_f2_2_p1.png" width="125" alt="带权有向图"/>
</figure>

1. 求图中从顶点 4 到顶点 3 的最短路径及长度。
2. 若路径中存在 $v_i=v_j$（$i<j$），称其含回路。证明任意最短路径不含回路。

对全源最短路，定义 $\delta(i,j,k)$ 为从 $i$ 到 $j$、所有中间顶点均在
$\{1,\ldots,k\}$ 中的最短距离；无路径时为 $\infty$，且
$\delta(i,i,k)=0$，$k=0$ 表示无中间顶点。

3. 证明对 $1\le k\le N$，

   $$
   \delta(i,j,k)=\min\{\delta(i,j,k-1),
   \delta(i,k,k-1)+\delta(k,j,k-1)\}.
   $$

4. 令 $d^{(k)}[i][j]=\delta(i,j,k)$。对题图依次求
   $d^{(0)},\ldots,d^{(4)}$，得到全部顶点对最短距离。
5. 证明算法无需保存 $N+1$ 个二维数组，一个二维数组即可原地更新。
6. 利用第 5 问填写下图算法空栏 (a)、(b)。

<figure style="text-align:center;">
  <img src="https://raw.githubusercontent.com/Myyura/the_kai_project_assets/main/kakomonn/kyoto_university/informatics/ist_202208_kiso_f2_2_p2.png" width="500" alt="Floyd-Warshall 伪代码"/>
</figure>
...
1. vertex $k$ is not an intermediate vertex on the path, then we have $l(p_{ijk}) = \delta (i, j, k-1)$.
2. vertex $k$ is an intermediate vertex on the path, then the path consists of a subpath from $i$ to $k$ and a subpath from $k$ to $j$. Each subpath can only contain intermediate vertices in $\{1, \ldots , k-1\}$, and also shortest paths from $i$ to $k$ and $k$ to $j$ respectively (otherwise a shorter path from $i$ to $j$ can be constructed), namely they have lengths $\delta(i, k, k-1)$ and $\delta(k, j, k-1)$, respectively. Hence $l(p_{ijk}) = \delta(i, k, k-1) + \delta(k, j, k-1)$.
...
\begin{align}
    ...
    &=\delta(i, k , k-1),\\
    \delta(k, j, k) &= \min (\delta(k,j,k-1), \delta(k, k, k-1)+\delta(k, j, k-1))\\
    &=\delta(k, j , k-1).
\end{align}
$$
...
The initialization with $d[i][j]\leftarrow l(i,j)$ assumes no self-loops. To allow positive self-loops while retaining the zero-length path from each vertex to itself, use $d[i][j]\leftarrow\min\{d[i][j],l(i,j)\}$ when reading edges, or reset the diagonal to zero afterward.
