...
tags:
  ...
  - Computer-Science.Data-Structures.Expression-Tree
  - Computer-Science.Data-Structures.Reverse-Polish-Notation
  ...
  - Computer-Science.Dynamic-Programming.Counting-Subset-Sum-Dynamic-Programming
---
...
1. 对下列 $f(n)$，给出最简洁的渐近上界 $g(n)$ 使
   $f(n)=O(g(n))$：
   1. $5\log n+2(\log n)^3+3n^3$；
   2. $n\log n+10n^2+100n$；
   3. $4n!+2n^n+8n\log n$。
2. 用栈机器计算
   $((5-3)*2)+((7-4)/(2+1))$。
   1. 画表达式树；
   2. 写逆波兰式；
   3. 写出递归遍历表达式树并输出逆波兰式的伪代码；
   4. 图示直到求值完成的栈变化。
3. 设 $A=\{a_1,\ldots,a_n\}$ 为互异正整数集，$s\ge0$。令
   $d(i,j)$ 为 $A_i=\{a_1,\ldots,a_i\}$ 中元素和为 $j$ 的子集个数。
   1. 对 $A=\{10,3,6,13,11,4\}$，求
      $d(4,16)$、$d(6,20)$，并列出相应全部子集。
   2. 用若干 $d(i-1,k)$ 表示 $d(i,j)$；约定
      $d(0,0)=1$、$d(0,j)=0$（$j>0$）。
...
![expression-tree](https://raw.githubusercontent.com/Myyura/the_kai_project_assets/main/kakomonn/kyoto_university/informatics/ist/2023/kyoto-ist-2022-expression-tree.svg)
