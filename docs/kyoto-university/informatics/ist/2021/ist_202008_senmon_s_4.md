...
$$
p(a)=\frac38,\quad p(b)=\frac14,\quad
p(c)=\frac14,\quad p(d)=\frac18.
$$

1. 求信源熵 $H(A)$。
2. 用下列二元码 $C_1$ 编码，判断是否唯一可译并说明理由。
...
3. 将信源产生的足够长序列用 $C_1$ 编码，从所得比特流中均匀抽取 1 bit，求抽到 `1` 的概率。
4. 考虑题图给出的非对称二元信道。当输入 $X$ 的概率由第 3 问确定时，求互信息 $I(X;Y)$。

   <figure style="text-align:center;">
     <img src="https://raw.githubusercontent.com/Myyura/the_kai_project_assets/main/kakomonn/kyoto_university/informatics/ist_202008_senmon_s_4_p1.png" width="200" alt="非对称二元信道图"/>
   </figure>

5. 改用固定长码 $C_2$ 编码信源符号，经第 4 问信道传输后再按 $C_2$ 译码，令译码符号为随机变量 $B$，求 $I(A;B)$。
...
&= \frac{3}{8}\log_2 \frac{8}{3} + \frac{1}{4} \log_2 4 + \frac{1}{4} \log_2 4 + \frac{1}{8} \log_2 8\\
...
任意の $C_1$ 符号列全体を反転すると、記号順を逆にした $C'_1$ 符号列になる。$C'_1$ が一意に復号可能なので、元の $C_1$ 符号列も一意に復号可能である。

### 設問3

$$
\frac{\frac{3}{8}n\cdot 0 + \frac{1}{4}n \cdot 1 + \frac{1}{4}n \cdot 2 + \frac{1}{8}n \cdot 3}{\frac{3}{8}n\cdot 1 + \frac{1}{4}n\cdot 2 + \frac{1}{4}n\cdot 3 + \frac{1}{8}n\cdot 3} = \frac{9}{16}
$$
...
### 設問6
設問4の通信路では入力 $1$ に誤りがないため、生起確率の最大の $a$ に $11$、最小の $d$ に $00$ を割り当てるのが有利である。24 通りの割当てについて $I(A;B)$ を比較すると、最適な割当ては

$$
a\mapsto 11,\qquad d\mapsto 00,\qquad
(b,c)\mapsto(01,10)\ \text{または}\ (10,01)
$$

であり、$I(A;B)\simeq1.456898$ bit となる。一方、$C_2$ では $I(A;B)\simeq1.351273$ bit なので、$C_2$ は最適ではない。
