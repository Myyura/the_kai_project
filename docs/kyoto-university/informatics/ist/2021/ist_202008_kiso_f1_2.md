...
1. 求下列约束条件下的最小值：
   ...
   1. 在 $x+y=3$ 的条件下，求 $x^2+y^2$ 的最小值。
   2. 在 $x^2+y^2=1$ 的条件下，求 $xy+x$ 的最小值。

2. 计算下列三个广义积分：

$$
\int_0^\infty e^{-x}x\,dx,\qquad
\int_0^\infty e^{-x^2}x^3\,dx,\qquad
\int_0^\infty e^{-x^2}x^{15}\,dx.
$$
...
$x^2+y^2=\frac{(x+y)^2+(x-y)^2}{2}=\frac92+\frac{(x-y)^2}{2}$ なので、
...
### 設問2
以下では $a>0$、$n$ は非負整数とする。漸化式は $I_n$ では $n\ge1$、$T_n$ では $n\ge2$ に適用する。
...
&= -\frac{1}{a}\left[ x^n e^{-ax}\right]_{0}^{\infty} + \frac{n}{a} \int_{0}^{\infty} x^{n-1} e^{-ax}dx \\
&= \frac{n}{a} I_{n-1}
\end{aligned}
$$
...
&=\left[-\frac{1}{2 a} x^{(n-1)} e^{-a x^{2}}\right]_{0}^{\infty}-\left(-\frac{1}{2 a}\right) \int_{0}^{\infty}(n-1) x^{(n-2)} e^{-a x^{2}} d x \\
...
$$
\int_0^{\infty} e^{-x^2}x^3dx =T_3 = \frac{1!}{2\cdot 1^{1+1}} = \frac{1}{2}
$$

#### (3)

$$
\int_0^{\infty} e^{-x^2}x^{15}dx = T_{15} = \frac{7!}{2\cdot 1^{7+1}} = \frac{7!}{2}
$$
