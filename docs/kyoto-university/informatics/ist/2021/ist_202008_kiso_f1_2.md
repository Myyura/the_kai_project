...
1. 求约束极值：
   1. 在 $x+y=3$ 下，求 $x^2+y^2$ 的最小值；
   2. 在 $x^2+y^2=1$ 下，求 $xy+x$ 的最小值。
2. 计算广义积分：
   1. $\displaystyle\int_0^\infty e^{-x}x\,dx$；
   2. $\displaystyle\int_0^\infty e^{-x^2}x^3\,dx$；
   3. $\displaystyle\int_0^\infty e^{-x^2}x^{15}\,dx$。

#### 考点

- **约束优化**：用代入、Cauchy 不等式或 Lagrange 乘子处理线性约束与单位圆约束。
- **Gamma 型广义积分**：通过分部积分或 $t=x^2$ 代换化为 Gamma 函数值并递推计算。
