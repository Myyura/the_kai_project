...
1. 在体积固定的直圆锥中，当高与底面半径之比为何值时，侧面积最小？
2. 计算：
   1. $\displaystyle\int_0^1\log(1+\sqrt x)\,dx$；
   2. $\displaystyle\iint_D(x^2+y^2)^{-2}\,dx\,dy$，其中
      $D=\{(x,y)\mid x^2+y^2\ge1\}$。

#### 考点

- **约束极值**：用定体积约束消去变量，将圆锥侧面积化为单变量函数并求全局最小。
- **定积分技巧**：通过根式代换和分部积分计算含对数积分。
- **极坐标广义二重积分**：将单位圆外区域化为 $r\ge1$，连同 Jacobian 判断收敛并求值。
