...
- **二项分布与精确双侧检验**：计算点概率，并按零假设下同等或更极端结果求小样本硬币检验 $P$ 值。
- **族错误率**：在独立检验下用补事件求至少一个第一类错误的概率。
- **已知方差正态均值置信区间**：使用标准误 $\sigma/\sqrt n$ 与正态临界值构造区间。
- **HARKing 与多重比较**：解释事后选择假设使名义 $P$ 值失真、增加第一类错误并夸大证据。
