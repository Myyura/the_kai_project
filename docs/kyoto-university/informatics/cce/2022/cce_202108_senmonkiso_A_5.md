...
- **Huffman 编码与即时码**：按概率合并构造最优二元前缀码，计算平均码长并阐明无需后续符号即可译码的性质。
- **信息熵**：计算离散信源熵并与 Huffman 平均码长比较。
- **循环码多项式运算**：用生成多项式整除性判断码字，并通过模二多项式除法得到系统编码。
- **最小距离与纠错能力**：由生成多项式确定码的最小距离，比较两个循环码的码率与纠错能力，并说明综合译码步骤。
