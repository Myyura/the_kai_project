...
- **信息熵与信源扩展**：计算二元无记忆信源熵，比较块 Huffman 编码的平均码长与熵界及扩展次数的影响。
- **Huffman 编码**：对二次扩展的四个联合符号按概率合并构造前缀码并计算归一化平均码长。
- **Markov 信息源**：由“输出 1 时换状态”写出转移矩阵，求平稳分布与熵率。
- **循环码与系统编码**：用生成多项式的整除性判断码字，并通过多项式除法生成系统码字。
- **最小距离纠错与失败概率**：由循环码最小距离确定纠错能力，并在二元对称信道上累加超出能力的错误事件概率。
- **信道编码定理**：说明低于信道容量的速率可实现任意小误码率，而高于容量则不可能可靠通信。
