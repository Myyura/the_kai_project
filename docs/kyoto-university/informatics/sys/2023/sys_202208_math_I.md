...
そのために、 $m$ 次正方行列 $A$ を $P - Q$ に分解し、方程式を $P x = Q x + b$ のように書き換え、適当な初期値 $x_0$ を与えて、 $P x_{n+1} = Q x_n + b$ 、つまり、 $x_{n+1} = P^{-1} (Q x_n + b)$ を繰り返し計算する。
...
を取るとする。
設問 (i) から、 $A$ は固有値 $\lambda_1 = 1$ と対応する固有ベクトル $v = u / \sqrt{2}$ を持つ。
...
给定初值 $\boldsymbol{x}_0$ 后，反复计算
$P\boldsymbol{x}_{k+1}=Q\boldsymbol{x}_k+\boldsymbol{b}$，即
