...
以下 $\mathbb R$ 表示实数集，$\pi$ 表示圆周率，矩阵和向量的转置记作 $\top$。
...
1. 回答下列问题。
   ...
   $$
   \sum_{i=1}^Nx_i=1,\qquad x_i>0\quad(i=1,\ldots,N)
   $$
   ...
   y=b\sin^3\theta,
   \qquad0<\theta<\frac\pi2,
   $$

   其中 $a,b>0$。曲线在点 $(x_0,y_0)$ 处的切线与 $x$ 轴、$y$ 轴分别交于 $P(p,0)$、$Q(0,q)$。

   1. 用 $a,b,x_0,y_0$ 表示 $p,q$。
   2. 当切点 $(x_0,y_0)$ 沿曲线移动时，求以 $O,P,Q$ 为顶点的三角形面积的最大值，以及取得最大值的切点所对应的 $\theta$。
...
2. 回答下列问题。
   ...
   （ii）设 $f(x)$ 是定义在 $\mathbb R$ 上的可微实函数，导函数为 $f'(x)$。对二维实向量
   ...
   $$
   f(\mathbf x)=(f(x_1),f(x_2))^\top,\qquad
   f'(\mathbf x)=(f'(x_1),f'(x_2))^\top.
   $$

   给定整数 $L\geq2$ 及 $L-1$ 个实矩阵

   $$
   A^{(1)},\ldots,A^{(L-1)}\in\mathbb R^{2\times2},
   $$

   二维实向量 $\mathbf x^{(1)},\ldots,\mathbf x^{(L)}$ 对所有 $\ell\in\{1,\ldots,L-1\}$ 满足

   $$
   \mathbf x^{(\ell+1)}
   =f\!\left(A^{(\ell)}\mathbf x^{(\ell)}\right).
   $$

   以下 $i,j\in\{1,2\}$、$\ell\in\{1,\ldots,L-1\}$。把 $\mathbf x^{(\ell+1)}$ 视为 $\mathbf x^{(\ell)}$ 的函数，定义
   ...
   把最终向量 $\mathbf x^{(L)}$ 视为 $\mathbf x^{(\ell)}$ 的函数，定义
   ...
   令 $U^{(\ell)},V^{(\ell)}$ 分别为以 $u_{i,j}^{(\ell)},v_{i,j}^{(\ell)}$ 为 $(i,j)$ 元素的 $2\times2$ 矩阵，并规定 $V^{(L)}$ 为 $2\times2$ 单位矩阵。记 $A^{(\ell)}$ 的 $(i,j)$ 元素为 $a_{i,j}^{(\ell)}$；必要时可把其第 $i$ 行记作 $\mathbf a_i^{(\ell)}$。还可使用
   ...
   1. 用 $\{A^{(m)}\}_{m=1}^{L-1}$、$\{\mathbf x^{(m)}\}_{m=1}^{L}$ 及其分量和 $f'$ 中必要的量表示 $U^{(\ell)}$。
   2. 用 $V^{(\ell+1)}$、$\{A^{(m)}\}_{m=1}^{L-1}$、$\{U^{(m)}\}_{m=1}^{L-1}$、$\{\mathbf x^{(m)}\}_{m=1}^{L}$ 和 $f'$ 中必要的量表示 $V^{(\ell)}$。
   3. 令 $k\in\{1,2\}$。把 $x_k^{(L)}$ 视为 $(A^{(1)},\ldots,A^{(L-1)})$ 的函数，用 $\{V^{(m)}\}_{m=1}^{L}$、$\{A^{(m)}\}_{m=1}^{L-1}$、$\{\mathbf x^{(m)}\}_{m=1}^{L}$ 及其分量和 $f'$ 中必要的量表示
