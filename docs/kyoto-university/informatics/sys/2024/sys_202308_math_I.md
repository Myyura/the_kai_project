...
1. 回答下列问题。
   ...
   $$
   x_1
   \begin{bmatrix}11\\11\\-11\\-11\end{bmatrix}
   +x_2
   \begin{bmatrix}12\\-12\\12\\-12\end{bmatrix}
   +x_3
   \begin{bmatrix}31\\31\\31\\31\end{bmatrix}
   +x_4
   \begin{bmatrix}41\\-41\\-41\\41\end{bmatrix}
   =
   \begin{bmatrix}23\\-11\\-51\\37\end{bmatrix}.
   $$

   若满足该式的实数组 $(x_1,x_2,x_3,x_4)$ 唯一存在，则给出 $x_3$；若解不唯一，回答“解不唯一”；若无解，回答“无解”。
...
3. 在内积为 $x^{\mathrm T}y$ 的三维实向量空间 $V$ 中，考虑二维子空间 $V_1$ 及其正交补 $V_2$。$V_1$ 的一组基为 $a,b$，$V_2$ 的一组基为 $c$，且

   $$
   a=\begin{bmatrix}1\\2\\k\end{bmatrix},
   \qquad
   c=\begin{bmatrix}1\\-1\\-1\end{bmatrix}.
   $$
   ...
   （ii）设 $p$ 为到 $V_1$ 的正交投影，求其投影矩阵 $P$ 及 $\operatorname{rank}(P)$。

   （iii）设 $q$ 为到 $V_2$ 的正交投影，求其投影矩阵 $Q$ 及 $\ker q$。
   ...
   求 $V_1$ 中与 $x$ 的 Euclidean 距离最小的向量 $y$，并把它写成 $a$ 与第（i）问所得 $b$ 的线性组合；同时求该最小距离 $d$。
