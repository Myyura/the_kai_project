...
在给定状态图中，初始状态为 S，目标状态为 G；边上的数字表示移动成本，节点括号内的数字表示启发式值 $h$。各节点的启发式值为

$$
h(S)=5,\ h(A)=5,\ h(B)=3,\ h(C)=4,\ h(D)=2,\ h(E)=2,\ h(F)=0,\ h(G)=0.
$$
...
$$
\begin{gathered}
S\!-\!A:2,\quad S\!-\!B:6,\quad A\!-\!B:2,\quad A\!-\!C:1,\\
B\!-\!F:4,\quad B\!-\!E:5,\quad C\!-\!E:2,\quad C\!-\!D:5,\\
D\!-\!E:1,\quad D\!-\!G:1,\quad E\!-\!G:5.
\end{gathered}
$$

1. 使用最佳优先搜索求出从 S 到 G 的路径，并展示开放列表与关闭列表的逐步变化。
2. 使用 A* 算法求出从 S 到 G 的路径，并同样展示两个列表的逐步变化。
3. 比较深度优先搜索、广度优先搜索、最佳优先搜索和 A* 管理开放列表的方法。说明中必须全部使用以下指定词语：“队列”“栈”“排序”“从起点到某状态的成本估计值”“从某状态到目标的成本估计值”。

#### 考点

- 最佳优先搜索：需要按启发式值 $h$ 管理开放列表，在给定带权状态图上记录节点展开顺序、两个列表的变化及所得路径。
- A* 算法：需要综合从起点到当前状态的成本估计值与从当前状态到目标的成本估计值，并与深度优先、广度优先及最佳优先搜索的开放列表管理方式进行比较。
