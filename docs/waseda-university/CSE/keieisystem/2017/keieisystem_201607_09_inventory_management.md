...
1. 推导经济订货量。
2. 当 $B=2,\ K=1000,\ d=40,\ p=2$ 时，求最优订货量和最优订货间隔。
3. 设单位采购成本为 $A$；当 $Q\geq V$ 时，$A$ 可享受九折优惠。在第 2 问的参数基础上，再取 $A=50,\ V=500$，利用成本函数图像求最优订货量。

#### 考点

- 经济订货量：需要从连续需求下的平均库存、保管成本和固定订货成本建立单位时间成本函数，推导最优 $Q$ 并计算相应订货间隔。
- 数量折扣：需要把 $Q=V$ 前后的采购价格写成分段成本函数，比较无折扣区间的最小成本与折扣门槛处的成本。
