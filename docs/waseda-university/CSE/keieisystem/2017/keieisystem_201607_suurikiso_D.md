---
sidebar_label: "2016年7月実施 数理基礎 D"
tags:
  - Waseda-University
...
1. 对向量
   ...
   b=\begin{pmatrix}y\\3\\6\end{pmatrix},
   $$
   分别求它们线性无关的条件和彼此正交的条件，并证明只要二者正交就一定线性无关。
2. 利用适用于可交换矩阵的二项式定理，求矩阵
   $$
   C=\begin{pmatrix}1&1\\0&1\end{pmatrix}
   $$
   的 $m$ 次幂。
...
- 线性无关：需要依据两个含参数向量何时成比例来确定线性无关条件，再由内积为零得到正交条件并证明正交蕴含线性无关。
- 矩阵幂：需要把 $C$ 分解为可交换的单位矩阵与幂零矩阵，使用矩阵二项式定理计算一般的 $C^m$。
