...
1. 假设简单线性回归模型
   ...
   估计 $\beta_0$ 和 $\beta_1$。
2. 零件 Q 的横向长度服从 $N(20,2^2)$。独立抽取 9 个零件并将它们横向连接成一个单元，求该单元横向总长度的分布。
3. 说明统计假设检验中的第一类错误与第二类错误。

#### 考点

- 简单线性回归：需要利用给定的三组 $(x_i,y_i)$ 数据估计回归截距 $\beta_0$ 与斜率 $\beta_1$。
- 正态分布：需要利用相互独立的正态随机变量之和仍服从正态分布，求 9 个零件总长度的均值与方差。
- 假设检验：需要分别说明在原假设为真和为假时可能发生的两类错误，并明确二者的判定含义。
