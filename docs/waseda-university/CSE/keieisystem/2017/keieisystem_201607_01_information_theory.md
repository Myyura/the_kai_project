---
sidebar_label: "2016年7月実施 情報数理応用 問題1"
tags:
  - Waseda-University
...
考虑平稳无记忆信源 $X$ 与 $Y$：$X$ 的符号 $a_j$ 以概率 $P(a_j)$ 独立出现，$Y$ 的符号 $b_k$ 以概率 $P(b_k)$ 独立出现，并以 $P(a_j\mid b_k)$ 表示条件概率。回答下列问题：

1. 写出信源 $X$ 的熵 $H(X)$ 的表达式。
2. 说明熵的含义及其在实际应用中的意义。
3. 给出条件熵 $H(X\mid Y)$ 的定义，并解释其含义。
4. 给出互信息量 $I(X;Y)$ 的定义，并解释其含义。
5. 说明典型序列与熵之间的关系，以及典型序列这一概念及其意义。
...
すなわち、長い系列のほとんどは、ほぼ等確率な約 $2^{nH(X)}$ 個の系列へ集中する。標準系列を短く表す一方、標準系列でない場合には識別用のフラグと通常の固定長表現を用いれば、全系列を無損失で符号化できる。非標準系列の確率が0に近づくため、1シンボル当たりの平均符号長を $H(X)$ bit に近づけられる。典型集合だけを符号化する固定長方式では、非標準系列が復号誤りとなるが、その誤り確率は0へ収束する。
