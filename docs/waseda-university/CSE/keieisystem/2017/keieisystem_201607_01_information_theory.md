...
- 信息熵：需要写出平稳无记忆信源 $X$ 的熵公式，并从不确定性、平均信息量及实际应用的角度解释它。
- 条件熵：需要根据 $P(a_j\mid b_k)$ 定义 $H(X\mid Y)$，说明已知 $Y$ 后 $X$ 仍保留的平均不确定性。
- 互信息量：需要定义 $I(X;Y)$，并说明观测 $Y$ 能够消除多少关于 $X$ 的不确定性。
- 典型序列：需要阐明长序列的概率与典型集合规模如何由熵刻画，并解释这一性质的意义。
