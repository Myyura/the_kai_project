...
- 最大似然估计：需要由正态样本的似然函数推导 $\mu$ 与方差参数 $\theta$ 的最大似然估计量。
- 矩估计法：需要把正态分布的总体矩与样本矩对应起来，求出 $\mu$、$\theta$ 的矩估计量。
- 条件密度：需要从给定的二维联合密度先求边缘密度，再计算 $f(x\mid y)$ 与 $E(X\mid Y=y)$。
- 大数定律：需要说明样本平均在样本量增大时趋近总体均值的概率极限定律及其含义。
