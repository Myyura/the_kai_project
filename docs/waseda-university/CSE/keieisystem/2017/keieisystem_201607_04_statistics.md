...
1. 独立随机变量 $x_1,\ldots,x_n$ 均服从正态分布 $N(\mu,\theta)$，其密度为
...
2. 对联合概率密度函数
   ...
   求 $E(X)$、条件密度 $f(x\mid y)$ 以及条件期望 $E(X\mid Y=y)$。
3. 说明大数定律。
...
ただし、標本分散が0ならば、$\mu=\bar x$ として $\theta\downarrow0$ のとき尤度が発散するため、$\theta>0$ の範囲に最尤推定量は存在しない。
...
標本分散が0ならば、$\hat\theta_{\mathrm{MM}}=0$ はパラメータ空間 $\theta>0$ の閉包上の値である。
...
であり、固定した $0<y<1$ に対して $x$ が $(0,1)$ の外なら0である。$Y$ の台の外での条件密度はこの比では定義できない。さらに
