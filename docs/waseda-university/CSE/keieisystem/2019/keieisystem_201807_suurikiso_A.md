...
1. 对 $0<x<\pi$，定义 $f(x)=(\sin x)^{\cos 2x}$，求 $f(x)$ 的导数。
2. 变量 $x,y$ 满足隐式方程
   ...
   求 $\dfrac{dy}{dx}$。
3. 设 $a,b,c\geq 1$。在约束 $x+y+z=1$ 下，使用拉格朗日乘数法求使 $x^ay^bz^c$ 取得最大值的 $x,y,z$。

#### 考点

- **函数求导**：需要对底数和指数都含变量的 $(\sin x)^{\cos 2x}$ 进行求导。
- **隐函数求导**：需要对含 $x,y$ 的五次隐式方程逐项求导并解出 $\dfrac{dy}{dx}$。
- **约束优化**：需要在一个等式约束下用拉格朗日乘数法确定三变量乘积的最大点。
