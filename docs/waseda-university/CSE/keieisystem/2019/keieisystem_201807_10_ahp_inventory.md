---
sidebar_label: "2018年7月実施 計画数理学 問題10"
tags:
  - Waseda-University
  - Operations-Research.Decision-Analysis.Analytic-Hierarchy-Process
  - Operations-Research.Decision-Analysis.Pairwise-Comparison-Consistency
  - Operations-Research.Inventory-Management.Newsvendor-Problem
---

# 早稲田大学 創造理工学研究科 経営システム工学専攻 2018年7月実施 計画数理学 問題10

## **Author**
祭音Myyura

## **Description**

1. AHP の一対比較行列を $A=(a_{ij})$、得られたウェイトを $w_1,w_2,w_3$、理想的一対比較行列を $D=(d_{ij})$、$d_{ij}=w_i/w_j$ とする。

   $$
   \beta=\frac19\sum_{i=1}^3\sum_{j=1}^3\frac{a_{ij}}{d_{ij}}
   $$

   と定義する。
   1. 一対比較が理想的なときの $\beta$ を求めよ。
   2. $a_{11}=a_{22}=a_{33}=1$、$a_{12}=3$、$a_{13}=9$ とする。$a_{23}\in\{1/3,1,3,5,7,9\}$ のどれで $\beta$ が最大になるか答えよ。ただし $\beta(a_{23}=1)>\beta(a_{23}=9)$ を用いてよい。
2. 前日在庫を $W$、発注量を $z$、当日需要を連続確率変数 $D$ とする。$D$ の分布関数を $F$、密度を $f$、単位品切れ費用を $s$、単位保管費用を $h$ とする。
   1. 総品切れ費用を max 関数で表せ。
   2. 総保管費用を max 関数で表せ。
   3. 当日損失の期待値を積分で表せ。
   4. $t=W+z$ とおき、最適解 $t^*$ が満たす $F(t^*)$ を求めよ。

### 题目描述

1. 在层次分析法（AHP）中，设成对比较矩阵为 $A=(a_{ij})$，所得权重为 $w_1,w_2,w_3$；理想成对比较矩阵为 $D=(d_{ij})$，其中 $d_{ij}=w_i/w_j$。定义

   $$
   \beta=\frac19\sum_{i=1}^3\sum_{j=1}^3\frac{a_{ij}}{d_{ij}}.
   $$

   回答：
   1. 成对比较为理想状态时，求 $\beta$。
   2. 已知 $a_{11}=a_{22}=a_{33}=1$、$a_{12}=3$、$a_{13}=9$，且

      $$
      a_{23}\in\left\{\frac13,1,3,5,7,9\right\}.
      $$

      判断哪一个 $a_{23}$ 使 $\beta$ 最大。可以使用条件 $\beta(a_{23}=1)>\beta(a_{23}=9)$。
2. 设前一日库存为 $W$，订货量为 $z$，当日需求为连续随机变量 $D$；$D$ 的分布函数和密度分别为 $F$ 与 $f$，单位缺货成本为 $s$，单位库存持有成本为 $h$。回答：
   1. 用 $\max$ 函数表示总缺货成本。
   2. 用 $\max$ 函数表示总库存持有成本。
   3. 用积分表示当日损失的期望。
   4. 令 $t=W+z$，求最优解 $t^*$ 所满足的 $F(t^*)$。

## **Kai**

### [小問 1-1]

一対比較が理想的なら

$$
a_{ij}=d_{ij}=\frac{w_i}{w_j}
$$

なので、9個の比はすべて1である。したがって

$$
\boxed{\beta=1}.
$$

### [小問 1-2]

現存の題意にはウェイトの算出法が明記されていない。以下は、**列和の逆数を正規化する方法**でウェイトを求める場合の解答である。手法を指定しないまま、$\beta$ が整合値からの距離だけで決まるとはいえない。

完全整合条件は $a_{13}=a_{12}a_{23}$ なので、整合する値は $a_{23}=3$ であり、このとき $\beta=1$ となる。

相反性 $a_{ji}=1/a_{ij}$ を用い、$x=a_{23}$ とおくと

$$
\beta(x)-1
=\frac{(x-3)^2(12x^2+355x+10)}{351x(x+10)(4x+1)}.
$$

候補値を代入して比較すると

$$
\beta(1/3)>\beta(1)>\beta(9)>\beta(7)>\beta(5)>\beta(3)=1
$$

である。よって、この算出法のもとでは

$$
\boxed{a_{23}=\frac13}
$$

で $\beta$ が最大となる。

### [小問 2-1]

利用可能量は $W+z$ なので、不足量は $\max\{D-W-z,0\}$ である。したがって総品切れ費用は

$$
\boxed{s\max\{D-W-z,0\}}.
$$

空欄 ① は $D-W-z$ である。

### [小問 2-2]

売れ残り量は $\max\{W+z-D,0\}$ なので、総保管費用は

$$
\boxed{h\max\{W+z-D,0\}}.
$$

### [小問 2-3]

$t=W+z$ とおく。$D\leq t$ では保管費用、$D>t$ では品切れ費用が生じるため

$$
\boxed{
L(t)=\int_0^t h(t-x)f(x)\,dx
+\int_t^\infty s(x-t)f(x)\,dx
}.
$$

したがって空欄は

$$
\boxed{\textcircled{2}=t=W+z,\qquad\textcircled{3}=h(t-x),\qquad\textcircled{4}=s(x-t)}.
$$

### [小問 2-4]

Leibniz の公式で微分すると、積分端点の被積分関数は0なので

$$
\begin{aligned}
L'(t)
&=h\int_0^tf(x)\,dx-s\int_t^\infty f(x)\,dx\\
&=hF(t)-s\{1-F(t)\}.
\end{aligned}
$$

最適点 $t^*$ で $L'(t^*)=0$ とおけば

$$
(h+s)F(t^*)=s.
$$

よって

$$
\boxed{F(t^*)=\frac{s}{h+s}}.
$$

これは $t^*>W$ となる内点解の場合である。$z\geq0$ の制約を含め、$q=F^{-1}(s/(h+s))$ とおけば、$t^*=\max\{W,q\}$ である。

空欄 ⑤ は $s/(h+s)$ である。
