...
2. 零件重量 $X\sim N(300,\sigma^2)$，且

   $$
   P(X\leq306.58)=0.95.
   $$

   标准正态分布的上侧 $5\%$ 分位点取 $1.645$，求 $\sigma$。
3. 设 $X_1,\ldots,X_n$ 相互独立且均服从 $N(\mu,\sigma^2)$，令

   $$
   S=\sum_i(X_i-\bar X)^2.
   $$

   已知 $S/\sigma^2$ 服从自由度 $n-1$ 的卡方分布。记该分布的下侧 $2.5\%$ 分位点为 $a$、上侧 $2.5\%$ 分位点为 $b$，求 $\sigma^2$ 的 $95\%$ 置信区间。
4. 在一个单因素配置实验中，因子 A 有 $4$ 个水平，每个水平重复 $5$ 次。求因子 A 和误差各自的自由度。
