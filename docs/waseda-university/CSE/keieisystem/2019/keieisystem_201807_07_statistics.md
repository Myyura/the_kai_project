...
1. 已知
   ...
   $$
   V(X)=V(Y)=V(Z)=4,\qquad
   C(X,Y)=0,\qquad C(X,Z)=1,\qquad C(Y,Z)=3.
   $$

   定义 $S=X+Y$、$T=X-Y$、$W=X+2Y-Z$，求 $V(S),V(T),V(W)$ 与 $C(S,T)$。
2. 独立随机变量 $X_i$ 的概率密度为
   ...
   求参数 $\theta$ 的最大似然估计量。
3. 疾病 K 的患病率为 $0.01$，检测灵敏度为 $0.95$，未患病者的假阳性率为 $0.20$。求检测结果为阳性的人实际患病的概率。
4. 因子 A、B、C、D 各有两个水平，采用 $L_8$ 正交表进行实验，只考虑交互作用 $A\times B$ 与 $A\times C$。A1 水平的数据总和为 14，A2 水平的数据总和为 10。回答：
   1. 求因子 A 的平方和。
   2. 求因子 A 的自由度。
   3. 求总自由度。
   4. 只将 $A\times C$ 并入误差项后，求误差自由度。
5. 说明多元回归分析中的残差与杠杆值。

#### 考点

- 方差与协方差：利用方差、协方差对线性组合的双线性展开，计算 $S,T,W$ 的方差以及 $S,T$ 的协方差。
- 最大似然估计：由给定密度构造独立样本的似然和对数似然，通过一阶、二阶导数确定 $\theta$ 的极大值点。
- 条件概率与贝叶斯公式：把患病率、灵敏度和假阳性率组合成阳性事件的全概率，再求阳性条件下的患病概率。
- 正交表与方差分析：根据 $L_8$ 两水平设计计算因子平方和与各项自由度，并理解把交互作用并入误差项时自由度如何转移。
- 多元回归与回归诊断：残差用于检查模型未解释的变动、线性和等方差等假设；杠杆值由帽子矩阵对角元刻画自变量位置对拟合的潜在影响。
