...
- 贝叶斯推断与 Beta–Bernoulli 模型：由均匀先验和含 $y$ 个 1 的伯努利样本构造后验分布，并从后验分布读取均值与众数。
- 最大似然估计：写出伯努利似然 $\theta^y(1-\theta)^{n-y}$，结合 $2\leq y\leq n-2$ 判断端点与内部极值，从而确定曲线形状和估计量。
- $k$-means 聚类：需要说明“分配样本—更新中心”的迭代机制，以及初值、簇数、尺度、离群点和簇形状对结果的影响。
- 朴素贝叶斯分类：利用给定类别下各维特征条件独立的假设分解联合概率，并结合高维分类说明其效率优势与独立性假设的局限。
- 形态素分析：说明自然语言文本的形态素切分及词性、原形等属性标注，并指出其作为后续语言处理前置步骤的作用。
- 信息熵：写出离散无记忆信源的熵公式，并从平均信息量、不确定性、编码长度与可压缩性等角度解释其意义。
