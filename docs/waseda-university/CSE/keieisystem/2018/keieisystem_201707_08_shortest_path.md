...
考虑顶点集合 $V=\{1,2,3,4\}$、有向边集合

$$
E=\{(1,2),(1,3),(2,3),(2,4),(3,4)\}
$$

所定义的有向图。各边旁的数字表示费用，具体为 $(1,2)$ 的费用为 1、$(1,3)$ 为 4、$(2,3)$ 为 2、$(2,4)$ 为 6、$(3,4)$ 为 3。
...
1. 将从顶点 1 到顶点 4 的最短路问题表述为线性规划问题。
2. 写出该线性规划的对偶问题。
3. 证明最优解满足互补条件。

#### 考点

- **最短路问题**：需要把从顶点 1 向顶点 4 发送单位流的路径选择表示为各有向边上的非负流量，并用节点流量守恒建立最小费用线性规划。
- **线性规划对偶**：需要由节点守恒等式引入自由的顶点势变量，写出每条边对应的对偶约束，并在最优原始流与对偶势之间核验互补条件。
