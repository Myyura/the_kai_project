...
1. 随机变量 $X$ 服从形状参数为 $\alpha$、尺度参数为 $\sigma$ 的伽马分布，其密度为
   ...
   1. 求 $E[X]$ 与 $\operatorname{Var}(X)$。
   2. 对估计量 $\hat\sigma(k)=kX$，求使其均方误差最小的 $k$。
2. 因素 A、B 各有 2 个水平，每个处理组合重复 2 次。根据下列观测值制作双因素试验的方差分析表。
...
- **伽马分布**：需要在形状参数为 4、尺度参数为 $\sigma$ 的参数化下计算均值与方差，并据此建立 $\hat\sigma(k)=kX$ 的均方误差来优化 $k$。
- **方差分析**：需要从 $2\times2$ 且每格重复 2 次的完整数据计算因素 A、因素 B、交互作用、误差与总变异的平方和、自由度、均方及相应 $F$ 值。
- **裂区设计**：需要说明主区与副区的两阶段因素配置和随机化方式，以及由此产生的不同误差层次。
