...
2. 随机变量 $X,Y$ 的联合密度为
   ...
   求条件密度 $f_{X\mid Y}(x\mid y)$ 和条件期望 $E[X\mid Y=y]$。
3. 对单因素试验证明平方和分解，并针对三组观测
   ...
   编制完整的方差分析表。
4. 说明 Mahalanobis 距离的定义及含义。

#### 考点

- **二重积分**：需要把一维高斯积分的平方化为平面二重积分，并通过极坐标完成证明。
- **条件密度**：需要先由联合密度求 $Y$ 的边缘密度，再计算条件密度和条件期望。
- **方差分析**：需要证明总平方和的组间、组内分解，并由给定三组数据计算自由度、平方和、均方与 $F$ 值。
- **马氏距离**：需要说明该距离如何借助协方差矩阵同时校正变量尺度和变量间相关性。
