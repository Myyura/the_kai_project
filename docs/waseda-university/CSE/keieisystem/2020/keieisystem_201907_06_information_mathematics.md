...
- **贝叶斯推断**：需要把伯努利样本的似然与先验结合，得到参数 $\theta$ 的后验量。
- **贝塔–伯努利模型**：需要利用贝塔先验与伯努利观测的共轭关系，写出后验密度并计算均值、众数。
- **典型序列**：需要用信息源熵刻画长序列中高概率集合的单条概率、规模及其统计意义。
- **随机森林**：需要说明随机抽样、随机选择特征和多棵决策树集成的机制及模型特性。
- **维数灾难**：需要具体解释高维空间中样本稀疏、距离区分力下降以及样本需求急剧增长等问题。
