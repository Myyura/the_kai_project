...
1. 一个伯努利信息源的参数为 $\theta$，其先验分布为 $\operatorname{Beta}(\alpha,\beta)$。在长度为 $n$ 的观测数据中出现了 $y$ 次 1，求 $\theta$ 的后验概率密度、后验均值和后验众数。
2. 对平稳无记忆信息源 $X$，考虑足够长的典型序列。用熵 $H(X)$ 表示每条典型序列的发生概率以及典型序列的条数，并说明典型序列的含义。
3. 设 $X_i\sim N(\mu,\sigma^2)$ 相互独立。对满足 $\sum_i\alpha_i=1$ 的估计量

   $$
   \bar X_\alpha=\sum_i\alpha_iX_i,
   $$

   求使 $\operatorname{Var}(\bar X_\alpha)$ 最小的各个系数 $\alpha_i$。
4. 说明随机森林方法的基本做法及其特点。
5. 说明由于“维数灾难”，特征空间维数增加时会出现哪些现象。
...
$A=\alpha+y, B=\beta+n-y$ とおく。$A\leq1, B\geq1$ で $(A,B)\neq(1,1)$ なら0、$B\leq1, A\geq1$ で $(A,B)\neq(1,1)$ なら1が最頻値である。$A<1, B<1$ なら両端、$A=B=1$ なら全点が最頻値である。
...
となる。すなわち、$H(X)<\log_2M$ なら全 $M^n$ 系列のうち比較的少数の標準系列が確率のほとんどを担い、1記号当たり約 $H(X)$ bit まで圧縮できる。
