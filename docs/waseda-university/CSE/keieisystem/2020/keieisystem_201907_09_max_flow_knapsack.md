...
- **单纯形法**：需要通过选取入基、出基变量和主元迭代求出二维线性规划的最优基解。
- **线性规划对偶性**：需要构造原线性规划及最大流模型的对偶，并利用主、对偶目标值关系判断最优性。
- **灵敏度分析**：需要确定第一条资源约束右端变化对最优解、最优值以及当前最优基有效范围的影响。
- **最大流**：需要根据三条有向弧的容量建立流量守恒模型，并联系其最小割对偶解释变量。
- **分支定界法**：需要以线性松弛值作为上界，对分数变量分支并通过界限剪枝。
- **0–1 背包问题**：需要在容量 6 和三个物品的给定重量、价值下比较松弛解与整数最优选择。
