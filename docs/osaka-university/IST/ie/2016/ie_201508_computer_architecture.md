...
1. CPU 与流水线。
   1. 从给定词表中填写五个空格：计算机的构成方式、当前主流的存储程序计算机类型、指出下一条指令地址的寄存器、机器指令中表示操作种类的字段，以及运算对象。
   2. 同步 CPU 将一条指令分为 $n$ 个阶段，每阶段一拍，时钟频率为 $f$。分别求执行 $m$ 条指令时：
      - 不采用流水线的执行时间；
      - 无停顿流水线的执行时间；
      - 当 $m\to\infty$ 时的吞吐率；
      - 解释增加流水级数为何可能提高吞吐率。
   3. 五级流水线依次为 IF、D、OF、EX、S。对原题给出的两组三条指令序列，画出逐拍流水时序。需同时考虑：
      - 程序与数据共用主存造成的存储器结构冲突；
      - 寄存器数据相关，且前一指令完成 S 后后一指令才能执行 OF；
      - 停顿阶段在完成拍标阶段名，其他等待拍标 `-`。

      第一组为从地址 A、B 分别加载到 `R1`、`R2`，再执行 `ADD R1,R2`；第二组为从 A 加载 `R1`、执行 `INC R1`，再把 `R1` 存至 B。
2. 缓存与主存层次。
   1. 缓存命中访问耗时 `2 ns`，未命中时缓存加主存总耗时 `50 ns`。
      - 命中率为 80% 时求平均访问时间；
      - 将命中耗时改为 `4 ns`、未命中仍为 `50 ns`，求保持上述平均时间所需命中率。
   2. 分别解释空间局部性与时间局部性。
   3. 说明由缓存和主存构成存储层次的优点及原因。

完整选项、指令语义和原题格式见上文。

#### 考点

- **存储程序体系结构**：理解 von Neumann 结构、程序计数器、操作码和操作数。
- **指令流水线**：计算延迟、吞吐率，并分析加深流水线与缩短时钟周期的关系。
- **流水线冒险**：识别统一存储器结构冲突和 RAW 数据相关所需停顿。
- **平均存储访问时间**：按命中、未命中概率加权计算并反求命中率。
- **引用局部性与缓存层次**：用程序访问模式解释小容量高速缓存的有效性。
