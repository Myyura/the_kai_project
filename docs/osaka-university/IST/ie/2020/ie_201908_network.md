...
- (1-1) プロトコル1ではデータと応答のシーケンス番号を0とする。正常なデータには `Ack(0)`、誤りのあるデータには `Nack(0)` が返り、応答には誤りがない。次の（あ）、（い）を埋めよ。`A` は何もしないアクションである。
...
- (1-3) 帯域1600 bit/s、片道伝搬遅延10 ms、端末処理時間0、ACK長8 bit、データパケット長24 bitとする。プロトコル3で1秒当たりに転送できる最大データパケット数を求めよ。
...
- (2-2-1) 次数 $r$ の生成多項式 $G(x)$、符号長 $n$、情報長 $m=n-r$ とし、情報多項式を $M(x)$ とする。符号語多項式 $F(x)$ の生成法を説明せよ。
- (2-2-2) $G(x)=x^4+x+1, n=15$ とし、情報ビット `00011010011` から $F(x)$ を生成せよ。
- (2-2-3) 受信語多項式から誤りを検出する方法を説明せよ。

### 题目描述

本题前半逐步构造停等可靠传输协议，考查ACK损坏、交替比特、吞吐量与超时重传；后半考查奇偶校验、最小距离、突发错误和CRC编码/检错。
