...
$V=\{v_1,\ldots,v_n\}$。其邻接矩阵 $A_G=(a_{ij})$ 在 $v_i,v_j$ 相邻时取 1，否则取 0。相邻顶点序列称为游走，允许重复顶点。

1. 写出题图 $G_1$ 的邻接矩阵。
2. 记 $f_G(k,i,j)$ 为从 $v_i$ 到 $v_j$、长度为 $k$ 的游走总数。
   1. 求 $f_{G_1}(3,3,2)$；
   2. 求 $\sum_{1\le k\le3}f_{G_1}(k,4,3)$；
   3. 证明 $A_G^k$ 的 $(i,j)$ 元等于 $f_G(k,i,j)$。
3. 记 $n$ 阶完全图为 $K_n$。
   1. 求 $K_n$ 的边数；
   2. 设 $A_G$ 的 $(i,j)$ 元为 $a_{ij}$，$A_G^2$ 的相应元素为 $b_{ij}$。说明如何利用 $a_{ij},b_{ij}$ 判断 $G$ 是否含三角形 $K_3$，并解释原因。
4. 对 $n\ge2$，用 $A_G$、单位矩阵 $I_n$ 和 $n$ 写出矩阵 $\alpha$，使“$\alpha$ 的所有元素均非零”等价于图 $G$ 连通。

#### 考点

- **邻接矩阵**：由图构造对称的 0–1 矩阵。
- **矩阵幂与游走计数**：通过归纳和矩阵乘法解释长度为 $k$ 的游走数。
- **三角形检测**：一条边两端若存在共同邻点，则形成 $K_3$。
- **图的连通性**：用 $I+A+\cdots+A^{n-1}$ 汇总有限长度可达关系。
