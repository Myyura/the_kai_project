...
```
#### <center> 図1. ハッシュ関数を用いてキーを配列に挿入するプログラム</center>
...
18
```
#### <center> 図2. input.txt の例 1</center>
...
空欄[ (A) ]
```
#### <center> 図3. input.txt の例 2</center>
...
```
#### <center> 図4. 関数 search1 と search2</center>
...
#### <center> 表1. 排他的論理和の計算例</center>

### 题目描述

考虑把互不重复的非负整数键插入数组，不进行删除。图 1 给出两种哈希方式：

- 方式 1：开放寻址。初始位置为 `key % size`，冲突后令探查值不断加 `delta` 再取模；
- 方式 2：使用两个长度相同的表。键在表 1 的候选位置为 `key % size`，在表 2 的候选位置为 `(key ^ delta) % size`；插入冲突时踢出旧键并在另一表继续放置。

输入文件依次给出 `delta`、键数和各键。完整程序、示例及异或表见上文。

1. 关于方式 1：
   1. 对示例 1，写出插入全部键后的 `table1`；
   2. 当插入键数等于 `TABLE1_SIZE=20` 时，列出 $0\le\delta\le10$ 中能保证任意键序列的探查都终止的全部值。
2. 关于方式 2：
   1. 对示例 1，写出 `table2_1`、`table2_2`；
   2. 对示例 2，求使踢出循环不终止的最小补充键 A。
3. 补全 `search2` 的 B 至 H，使存在时返回对应数组元素指针，不存在时返回 `NULL`；并比较 `search1` 与 `search2` 查找时最多或平均需要引用多少次数组，说明方式 2 的效率优势。
...
#### (3-2)
方式2 (`search2`) は二つの候補位置だけを調べるため、キー比較のための配列参照は高々2回であり、探索時間は $O(1)$ である。

方式1 (`search1`) は衝突した位置を順に調べる。キーが存在する場合、または探索列上の空き位置に到達する場合、調べる位置数は表サイズ $n$ に対して最悪 $O(n)$ である。ただし掲載コードには一周後に停止する条件がないため、**満杯の表に存在しないキーを探すと停止しない**。例えばサイズ20、`delta=1`、キー0～19を格納してから20を探すと、全位置を繰り返し調べる。`int` の加算が限界を超えると未定義動作にもなるため、原コードについて無条件に最悪 $O(n)$ とはいえない。

実装としては探索位置数を $n$ 回以内に制限すれば、不成功探索も $O(n)$ で終えられる。なお、原題のコードは[公式PDF（3ページ）](https://www.ist.osaka-u.ac.jp/files/examinees/admission/past-exam/5_Information_engineering_2023_JA.pdf)でも一周の判定を持たない。
