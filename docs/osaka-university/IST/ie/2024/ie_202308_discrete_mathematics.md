...
本题只考虑有限简单无向图。图 $G=(V,E)$ 由有限顶点集和不同顶点无序对组成的边集构成。若存在保持邻接关系的双射 $\varphi:V_1\to V_2$，则称 $G_1,G_2$ 同构，记为 $G_1\simeq G_2$。

记 $\mathcal G$ 为全体图的集合，$\mathcal G_n$ 为顶点集恰为 $\{1,\ldots,n\}$ 的全部标号图。

1. 集合 $\mathcal H$ 由题图所示六个图 $G_1,\ldots,G_6$ 构成。
   1. 列出全部满足 $i<j$ 且 $G_i\simeq G_j$ 的对 $(i,j)$；
   2. 求商集 $\mathcal H/\simeq$ 的元素数。
2. 令 $\mathcal I_n\subseteq\mathcal G_n$ 为一个最大子集，其中任意两个不同图都不同构；也就是每个同构类至多取一个代表。
   1. 求 $|\mathcal I_3|$ 并说明理由；
   2. 证明对任意 $G\in\mathcal G_n$，至多有 $n!$ 个不同的 $G'\in\mathcal G_n$ 与 $G$ 同构；
   3. 求 $|\mathcal G_n|$；
   4. 证明
      $$
      \log_2|\mathcal I_n|
      \ge\frac{n(n-1)}2-n\log_2n.
      $$
...
- **无标号图计数**：每个同构类选择一个代表。
- **标号简单图数量**：每个顶点对独立决定有边或无边，得到 $2^{\binom n2}$。
