...
图 1 的 ANSI C 程序在一个满足以下条件的加权无向图 $G$ 中求两点最短路径长度：顶点编号为 $0$ 至 $n-1$，所有边权为非负整数，图连通，且任意最短路长度小于 `INF`。邻接矩阵按一维数组存储为 `w[i*n+j]`；无边时权重为 `INF`，并令 `w[i*n+i]=0`。完整程序与示例图见上文。

1. 对示例图从顶点 0 运行 `compute`，分别写出 `while` 循环第 1 次和第 2 次迭代结束时 `Len[0]` 至 `Len[3]` 的值。
2. 用顶点数 $n$ 表示 `compute` 的渐近时间复杂度，并简述理由。
3. 修改程序，使其除最短距离外还输出一条最短路径。启用全局数组 `Prev`、相关初始化和调用，并插入递归函数
   `printpath(v)`。填写：
   - 在松弛 `Len[j]` 时如何更新空格 (ア) 的前驱；
   - 在 `main` 中空格 (イ) 如何调用路径输出函数。
4. 固定 $n=4$，仅允许增删 `compute`、`printf` 调用及修改其实参，输出一般无向图的所有顶点对最短距离。
   1. 对原版 `compute`（循环直至全部顶点访问完），说明调用方式和最少调用次数 $T$；
   2. 将循环条件改为 `while (visited[d] == 0)` 后，再说明调用方式和最少调用次数 $T$。

#### 考点

- **Dijkstra 算法**：维护已确定集合、暂定距离并选取最小未访问顶点。
- **复杂度分析**：数组实现每轮线性扫描，整体为二次复杂度。
- **最短路径恢复**：在松弛时记录前驱，并递归或逆序回溯路径。
- **全源最短路与无向图对称性**：利用一次单源运行产生多个终点结果，减少函数调用。
- **提前终止的影响**：区分计算到单一终点和计算完整单源距离表。
