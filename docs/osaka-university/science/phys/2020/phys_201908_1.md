---
sidebar_label: "2019年8月実施 問題1"
tags:
  - Osaka-University
  - Physics.Mechanics.Small-Angle-Pendulum
  - Physics.Mechanics.Lagrangian-Mechanics
  - Physics.Mechanics.Action-Integral-and-Adiabatic-Invariant
---
# 大阪大学 理学研究科 物理学専攻 2019年8月実施 問題1

## **Author**
[Miyake](https://miyake.github.io/exams/index.html)

## **Description**

### 題意の要約（暫定）

質量 $m$、長さ $l$ の単振り子を考える。重力加速度を $g$ とし、振れ角 $\theta$ は十分小さいとする。初期条件は

$$\theta(0)=\alpha,\qquad\dot\theta(0)=0.$$

である。

【I：微小振動】

1. 運動方程式から $\theta(t)$ と角振動数 $\omega$ を求めよ。
2. 振動の力学的エネルギー $E$ を求めよ。
3. $\theta^2$ の項まで残す近似の下で、糸の張力 $T(t)$ を求めよ。
4. 一周期にわたる平均張力 $\langle T\rangle$ を求めよ。

【II：振り子の長さの緩やかな変化】

振り子の長さを緩やかに $\delta l$ だけ変化させ、微小変化の一次の項のみを考える。

5. 角振動数の変化 $\delta\omega$ を求めよ。
6. 平均張力による仕事から、力学的エネルギーの変化 $\delta E$ を求めよ。
7. この緩やかな変化に対し、
   $$
   \frac{E+\delta E}{\omega+\delta\omega}
   =\frac E\omega+O(\delta l^2),
   $$
   すなわち $E/\omega$ が断熱不変量であることを示せ。

【III：位相空間】

8. 微小振動する単振り子の Lagrangian を書き、$\theta$ に共役な運動量 $p$ を求めよ。
9. エネルギー $E$ を固定したとき、位相平面 $(\theta,p)$ における楕円軌道の方程式を書け。
10. 作用積分
    $$\oint p\,d\theta,$$
    を求め、振り子の長さを緩やかに変えるときに不変であることを説明せよ。

### 题目描述

考虑质量为 $m$ 的单摆，摆长为 $l$，重力加速度为 $g$，摆角 $\theta$ 很小；初始条件为
$$\theta(0)=\alpha,\qquad\dot\theta(0)=0.$$

【I：小振动】

1. 由运动方程求 $\theta(t)$ 与角频率 $\omega$。
2. 求振动的机械能 $E$。
3. 在保留至 $\theta^2$ 的近似下，求摆线张力 $T(t)$。
4. 求一个周期内的平均张力 $\langle T\rangle$。

【II：缓慢改变摆长】

将摆长缓慢改变 $\delta l$，只保留一阶小量。

5. 求角频率变化 $\delta\omega$。
6. 由平均张力所做功求机械能变化 $\delta E$。
7. 证明在这一缓慢变化下
   $$
   \frac{E+\delta E}{\omega+\delta\omega}
   =\frac E\omega+O(\delta l^2),
   $$
   即 $E/\omega$ 为绝热不变量。

【III：相空间】

8. 写出小角度单摆的 Lagrangian，并求与 $\theta$ 共轭的动量 $p$。
9. 在固定能量 $E$ 下，写出相平面 $(\theta,p)$ 中轨道的椭圆方程。
10. 计算作用量
    $$\oint p\,d\theta,$$
    并说明它在摆长缓慢变化时保持不变。

## **Kai**
### I.
#### (1)
$\ddot{\theta} = d \dot{\theta} / dt$ と書くと、運動方程式は次のようになる：

$$
\begin{aligned}
ml \ddot{\theta} &= - mg \sin \theta
\\
\therefore \ \
\ddot{\theta} &= - \frac{g}{l} \sin \theta
\\
&\simeq - \frac{g}{l} \theta
\end{aligned}
$$

よって、 $\omega = \sqrt{g/l}$ とおいて、

$$
\begin{aligned}
\theta (t)
&= \alpha \cos \omega t
\\
&= \alpha \cos \left( \sqrt{\frac{g}{l}} t \right)
\end{aligned}
$$

が与えられた初期条件を満たす解である。

#### (2)
$\dot{\theta}(t) = - \alpha \omega \sin \omega t$
の最大値は $\alpha \omega$ であるから、

$$
\begin{aligned}
E
&= \frac{1}{2} m l^2 \alpha^2 \omega^2
\\
&= \frac{1}{2} mgl \alpha^2
\end{aligned}
$$

である。

#### (3)
糸の方向の運動方程式より、

$$
\begin{aligned}
ml \dot{\theta}^2 &= T - mg \cos \theta
\\
\therefore \ \
T
&= ml \dot{\theta}^2 + mg \cos \theta
\\
&\simeq ml \dot{\theta}^2 + mg - \frac{1}{2} mg \theta^2
\end{aligned}
$$

がわかる。

#### (4)
(1), (3) より、

$$
\begin{aligned}
T
&= ml \alpha^2 \omega^2 \sin^2 \omega t
+ mg - \frac{1}{2} mg \alpha^2 \cos^2 \omega t
\\
&= mg \alpha^2 \sin^2 \omega t
+ mg - \frac{1}{2} mg \alpha^2 \cos^2 \omega t
\\
&=mg \left( \frac{\alpha^2 + 4}{4} - \frac{3\alpha^2}{4} \cos 2 \omega t \right)
\\
\therefore \ \
\langle T \rangle
&= \frac{\alpha^2 + 4}{4} mg
\end{aligned}
$$

がわかる。

### II.
#### (5)

$$
\begin{aligned}
\delta \omega
&= \sqrt{\frac{g}{l + \delta l}} - \omega
\\
&= \omega \left( 1 + \frac{\delta l}{l} \right)^{-1/2} - \omega
\\
&\simeq \omega \left( 1 - \frac{\delta l}{2l} \right) - \omega
\\
&= - \frac{\delta l}{2l} \omega
\end{aligned}
$$

#### (6)

$$
\begin{aligned}
\delta W
&= - \langle T \rangle \delta l
\\
&= - \frac{\alpha^2 + 4}{4} mg \delta l
\end{aligned}
$$

また、 $\delta W = \delta E - mg \delta l$ より、

$$
\begin{aligned}
\delta E
&= \delta W + mg \delta l
\\
&= - \frac{\alpha^2}{4} mg \delta l
\end{aligned}
$$

である。

#### (7)
(2), (5), (6) の結果を使って、
$\delta l$ の1次までで次のように計算できる：

$$
\begin{aligned}
\frac{E + \delta E}{\omega + \delta \omega}
&= \frac{E}{\omega} \left(1 + \frac{\delta E}{E} \right)
\left(1 + \frac{\delta \omega}{\omega} \right)^{-1}
\\
&\simeq \frac{E}{\omega}
\left(1 + \frac{\delta E}{E} - \frac{\delta \omega}{\omega} \right)
\\
&= \frac{E}{\omega}
\left(1 - \frac{\delta l}{2l} + \frac{\delta l}{2l} \right)
\\
&= \frac{E}{\omega}
\end{aligned}
$$

### III.
#### (8)

$$
\begin{aligned}
L
&= \frac{1}{2} ml^2 \dot{\theta}^2 - mgl (1 - \cos \theta)
\\
\therefore \ \
p
&= \frac{\partial L}{\partial \dot{\theta}}
\\
&= ml^2 \dot{\theta}
\end{aligned}
$$

#### (9)

$$
\begin{aligned}
E
&= \frac{1}{2} ml^2 \dot{\theta}^2 + mgl (1 - \cos \theta)
\\
&\simeq \frac{1}{2} ml^2 \dot{\theta}^2 + \frac{1}{2} mgl \theta^2
\\
&= \frac{p^2}{2ml^2} + \frac{1}{2} mgl \theta^2
\\
\therefore \ \
&\frac{p^2}{2ml^2E} + \frac{\theta^2}{\frac{2E}{mgl}} = 1
\end{aligned}
$$

これは楕円の方程式である。

#### (10)
(9) より、

$$
\begin{aligned}
\oint p d \theta
&= \pi \sqrt{2ml^2E} \sqrt{\frac{2E}{mgl}}
\\
&= 2 \pi E \sqrt{\frac{l}{g}}
\\
&= 2 \pi \frac{E}{\omega}
\end{aligned}
$$

となるが、 (7) より、これは糸の長さが $\delta l$ 変化する前後で変わらない。
