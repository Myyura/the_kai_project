...
$$
\frac{\text{d}y}{\text{d}x} = y(y + 2) \qquad \qquad (1.2)
$$
...
1. 设 $y$ 为 $x$ 的函数，满足
   ...
   +4\frac{\mathrm dy}{\mathrm dx}+4y=0
   \tag{1.1}
   $$
   ...
   1. 解方程 (1.1)，求 $y$。
   2. 求所得函数 $y$ 的最大值 $y_{\max}$。

2. 设 $y$ 为 $x$ 的函数，$y\neq0,-2$，并满足

   $$
   \frac{\mathrm dy}{\mathrm dx}=y(y+2),
   \tag{1.2}
   $$
...
3. 设 $y$ 为 $x$ 的函数，$x\neq0$、$y\neq0$，并满足
   ...
   $$
   xy\frac{\mathrm dy}{\mathrm dx}=2y^2+x^2,
   \tag{1.3}
   $$
   ...
   1. 令 $u=y/x$，把 (1.3) 化为
      ...
      $$
      \frac{\mathrm du}{\mathrm dx}=\frac BA,
      $$

      其中 $A$ 只依赖 $u$，$B$ 只依赖 $x$。分别求 $A,B$。
   2. 解 (1.3) 后把结果写成 $y^2=C$，其中 $C$ 只依赖 $x$。求 $C$。
...
&\Rightarrow u(u + x\frac{\text{d}u}{\text{d}x}) = 2u^2 + 1 \\
