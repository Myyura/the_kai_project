...
とする。十分大きい $N$ と一様な `calcKey` を用い、$M$ の増加に応じて表を拡張して再ハッシュすることで、$\alpha$ を一定以下に保つ。このとき各 `key` に属する要素数の期待値が一定に抑えられ、文字列長を一定とみなせば、検索は期待計算量 $O(1)$、格納は再ハッシュを含めて期待償却計算量 $O(1)$ となる。文字列長を $L$ として数える場合、ハッシュ計算や文字列比較には一般に $O(L)$ の時間が必要である。
