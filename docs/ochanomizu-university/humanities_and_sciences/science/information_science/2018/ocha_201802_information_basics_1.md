...
命題論理の恒真性は、有限個の命題変数の真理値表を作れば有限ステップで判定できる。一方、一階述語論理の恒真性について全ての論理式を有限ステップで判定する一様なアルゴリズムは存在しないが、これは Church の決定不能性定理に関する事実である。不完全性定理は、十分に強く効果的に公理化された無矛盾な形式体系では証明も反証もできない命題が存在するという別の定理である。
