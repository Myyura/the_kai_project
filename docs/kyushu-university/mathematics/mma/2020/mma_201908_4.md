---
sidebar_label: "2019年8月実施 [4]"
tags:
  - Kyushu-University
  - Probability-Statistics.Probability-Basics.Joint-Distribution
  - Probability-Statistics.Probability-Distributions-and-Asymptotics.Random-Variable-Transformation
  - Probability-Statistics.Probability-Basics.Convolution-of-Independent-Random-Variables
  - Mathematics.Calculus.Change-of-Variables-and-Jacobian
---
# 九州大学 数理学府 MMAコース 2019年8月実施 \[4\]

## **Author**
[Miyake](https://miyake.github.io/exams/index.html), 祭音Myyura

## **Description**

題意の要約。

独立な連続確率変数 $X,Y$ は同じ密度 $f$ を持つ。$Z=X+Y$ の密度を $f_Z$ とする。

1. $\displaystyle f_Z(z)=\int_{-\infty}^{\infty}f(x)f(z-x)\,dx$ を示す。
2. $f(x)=2x$（$0\le x\le1$）、それ以外で $f(x)=0$ の場合に $f_Z$ を求める。

出典：[九州大学 2020年度 MMAコース 数学](https://web.archive.org/web/20220627110535/https://www.math.kyushu-u.ac.jp/etc/files/MMA2019.pdf#page=3)。

### 题目描述

$X,Y$ 相互独立且具有相同概率密度 $f$，令 $Z=X+Y$。

1. 通过变量变换 $Z=X+Y,\ W=X$ 及 Jacobian，证明 $Z$ 的密度为卷积

   $$
   f_Z(z)=\int_{-\infty}^{\infty}f(x)f(z-x)\,dx.
   $$

2. 当

   $$
   f(x)=
   \begin{cases}
   2x,&0\le x\le1,\\
   0,&\text{其他},
   \end{cases}
   $$

   时，求 $Z$ 在全部实数范围内的分段概率密度。

## **Kai**
### (1)
$z=x+y, w=x$ とすると、 $x=w, y=z-w$ であるから、

$$
  \begin{aligned}
  \begin{vmatrix}
  \frac{\partial x}{\partial z} & \frac{\partial x}{\partial w}
  \\
  \frac{\partial y}{\partial z} & \frac{\partial y}{\partial w}
  \end{vmatrix}
  =
  \begin{vmatrix}
  0 & 1 \\ 1 & -1
  \end{vmatrix}
  =
  -1
  \end{aligned}
$$

である。
さらに、
$X,Y$ の同時確率密度関数は $f(x) f(y)$ であるから、
$Z=X+Y, W=X$ の同時確率密度関数 $f_{ZW}(z,w)$ は、

$$
  \begin{aligned}
  f_{ZW}(z,w)
  &=
  f(x) f(y) |-1|
  \\
  &=
  f(w) f(z-w)
  \end{aligned}
$$

である。
よって、

$$
  \begin{aligned}
  f_Z(z)
  &=
  \int_{- \infty}^\infty f_{ZW}(z,w) dw
  \\
  &=
  \int_{- \infty}^\infty f(w) f(z-w) dw
  \\
  &=
  \int_{- \infty}^\infty f(x) f(z-x) dx
  \end{aligned}
$$

を得る。

### (2)
(1) で示した通り、

$$
\begin{aligned}
f_Z(z)
&=
\int_{- \infty}^\infty f(x) f(z-x) dx
\end{aligned}
$$

であるが、今の場合、
$0 \leq x \leq 1$ かつ $0 \leq z-x \leq 1$
の場合以外は被積分関数は $0$ である。
それをふまえて、
$0 \leq z \leq 1$ のときは、

$$
\begin{aligned}
f_Z(z)
&=
4 \int_0^z x (z-x) dx
\\
&=
\frac{2}{3} z^3
\end{aligned}
$$

であり、
$1 \leq z \leq 2$ のときは、

$$
\begin{aligned}
f_Z(z)
&=
4 \int_{z-1}^1 x (z-x) dx
\\
&=
\frac{2}{3} ( - z^3 + 6z - 4 )
\end{aligned}
$$

であり、
それ以外のときは、

$$
\begin{aligned}
f_Z(z)
&=
0
\end{aligned}
$$

である。
