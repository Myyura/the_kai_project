---
sidebar_label: "2022年8月実施 ② 関数論"
tags:
  - Kyushu-University
  - Mathematics.Complex-Analysis.Harmonic-Functions-and-Harmonic-Conjugates
  - Mathematics.Complex-Analysis.Cauchy-Riemann-Equations
  - Mathematics.Complex-Analysis.Residue-Theorem
---
# 九州大学 工学府 船舶海洋工学専攻 2022年8月実施 ② 関数論

## **Author**
[Miyake](https://miyake.github.io/exams/index.html), 祭音Myyura

## **Description**

題意の要約。$z=x+iy$（$x,y\in\mathbb R$）とする。

### 問題 1

$f(z)=u(x,y)+iv(x,y)$ に対し、次を求める。

1. $u(x,y)=e^x\cos y$ が調和関数であることを示す。
2. $f$ の Cauchy–Riemann の関係式を書く。
3. (1) の $u$ を実部とする正則関数を求める。

### 問題 2

円 $C:|z-1|=\sqrt2$ を正の向きに一周するとき、$\displaystyle\int_C\frac{dz}{z^2+2iz-4}$ を求める。

出典：[九州大学 令和5年度 船舶海洋工学専攻](https://web.archive.org/web/20230606200839/https://www.nams.kyushu-u.ac.jp/wp/wp-content/uploads/2023/01/68053349a28a6688e5925110885acbe2.pdf)。

### 题目描述

**问题 1**：设

$$
u(x,y)=e^x\cos y.
$$

1. 证明 $u$ 是调和函数。
2. 写出复函数 $f(z)=u(x,y)+iv(x,y)$ 满足的柯西–黎曼方程。
3. 求以 $u$ 为实部的全部全纯函数 $f$。

**问题 2**：设 $C$ 为沿正方向（逆时针）绕行一周的圆

$$
|z-1|=\sqrt2,
$$

计算

$$
\int_C\frac{dz}{z^2+2iz-4}.
$$

## **Kai**
### 問題 1
#### (1)

$$
\begin{aligned}
\frac{\partial^2 u}{\partial x^2}
+ \frac{\partial^2 u}{\partial y^2}
&= e^x \cos y - e^x \cos y
\\
&= 0
\end{aligned}
$$

なので、 $u(x,y)$ は調和関数である。

#### (2)
(Eq.1) の複素関数における Cauchy-Riemann の関係式は

$$
\begin{align}
\frac{\partial u}{\partial x}
&= \frac{\partial v}{\partial y}
\tag{a}
\\
\frac{\partial u}{\partial y}
&= - \frac{\partial v}{\partial x}
\tag{b}
\end{align}
$$

である。

#### (3)
まず、($a$) より

$$
\begin{aligned}
\frac{\partial v}{\partial y}
&= \frac{\partial u}{\partial x}
\\
&= e^x \cos y
\\
\therefore \ \ 
v(x,y) &= e^x \sin y + g(x)
\ \ \ \ \ \ \ \ ( g(x) \text{ は } x \text{ の実数値関数 } )
\end{aligned}
$$

であり、さらに ($b$) より、

$$
\begin{aligned}
- e^x \sin y &= - e^x \sin y - \frac{dg(x)}{dx}
\\
\therefore \ \ 
\frac{dg(x)}{dx} &= 0
\\
\therefore \ \ 
g(x) &= C
\ \ \ \ \ \ \ \ ( C \text{ は実数定数} )
\end{aligned}
$$

を得る。
よって、求める正則関数は

$$
\begin{aligned}
f(z)
&= e^x \cos y + i \left( e^x \sin y + C \right)
\ \ \ \ \ \ \ \ ( C \text{ は実数定数} )
\end{aligned}
$$

である。

### 問題 2

$C$ を正の向きにとると、

$$
\begin{aligned}
\int_C f(z) dz
&= \int_C \frac{dz}{(z-(\sqrt{3}-i))(z-(-\sqrt{3}-i))}
\\
&= 2 \pi i \lim_{z \to \sqrt{3}-i} \frac{1}{z-(-\sqrt{3}-i)}
\\
&= \frac{\pi i}{\sqrt{3}}
\end{aligned}
$$

ここで、

$$
\begin{aligned}
\left| \sqrt{3} - i - 1 \right|^2
&= \left( \sqrt{3} - 1 - i \right) \left( \sqrt{3} - 1 + i \right)
\\
&= 5 - 2 \sqrt{3}
\\
&\lt 2
\\
\left| - \sqrt{3} - i - 1 \right|^2
&= \left( - \sqrt{3} - 1 - i \right)
\left( - \sqrt{3} - 1 + i \right)
\\
&= 5 + 2 \sqrt{3}
\\
&\gt 2
\end{aligned}
$$

であるから、 $f(z)$ の極 $z=\sqrt{3}-i$ は $C$ の内部にあり、
$f(z)$ の極 $z=-\sqrt{3}-i$ は $C$ の外部にあることを使った。
