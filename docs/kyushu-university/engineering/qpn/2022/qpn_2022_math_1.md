...
本文件原 Description 为空，解答中的第 [3] 题也完全空白，因而该大题无法恢复。可确认题意如下。
...
1. 求 $A$（或等价地 $B$）的特征值和特征向量。
2. 求可逆矩阵 $P$，将 $A$ 对角化为 $P^{-1}AP$。
...
- **矩阵对角化与矩阵极限**：通过特征分解把 $A^n$ 化为特征值幂，并判断绝对值小于 1 与等于 1 的谱分量在极限中的贡献。
- **二重积分与极坐标**：利用单位圆盘的径向对称性计算含 $x^2+y^2$ 的积分。
- **三重积分**：按平面 $x+y+z=t$ 对第一卦限四面体切片，或直接设置累次积分计算线性函数的体积分。
