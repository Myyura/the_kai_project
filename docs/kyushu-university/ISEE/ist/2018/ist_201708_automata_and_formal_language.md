...
#### (4)
部分集合構成により、到達可能な状態だけを列挙する。原図では $\delta(q_3,a)=\{q_2,q_4\}$、$\delta(q_4,a)=\{q_2\}$、$\delta(q_4,b)=\emptyset$ である。
...
|受理|$\{q_2,q_4\}$|$\{q_1,q_2\}$|$\{q_3\}$|
|受理|$\{q_1,q_2\}$|$\{q_1\}$|$\{q_3\}$|
| |$\emptyset$|$\emptyset$|$\emptyset$|

三つの受理状態は同値なので一つの状態 $A$ にまとめられる。
...
受理状態と他の状態は空語で区別できる。$q_0$ と $q_3$ は接尾語 $ba$ で、これらと $\emptyset$ は接尾語 $a$ で区別できる。したがって四状態が最小である。以下の図では $A$ を $q_1$ と表記している。
