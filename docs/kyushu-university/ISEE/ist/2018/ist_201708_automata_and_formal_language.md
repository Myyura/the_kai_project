...
- **非确定性有限自动机**：依据非确定性自动机的多路径转移枚举定长接受串，并概括其接受语言。
- **正则表达式与语言判定**：从自动机结构提取重复块 $\alpha,\beta$，并用子串条件检验语言的充要关系。
- **确定性有限自动机最小化**：将非确定性自动机识别的语言转化为确定性自动机，并合并等价状态得到最少状态数。
- **上下文无关文法及并封闭性**：完成具体推导、用形式化规律描述文法语言，并通过新增分支产生式构造两个上下文无关语言的并集文法。
