...
##### ($c$)
生成されない。$N$ は少なくとも一桁の数字を生成し、$S$ は少なくとも一つの $N$ を含む式を生成するため、空文字列は得られない。

##### (d)
生成されない。$S$ が生成する式の先頭は必ず $N$ が生成する数字であり、単項の負号を置く規則はない。
...
この解答の $M\to NM\mid DM\mid\varepsilon$ は冗長だが、$M$ が生成する言語は数字列全体である。$N\to0\mid DM$ により、数は単独の $0$ または非零の数字で始まる数に限られる。空欄 (ii) を $0M$ としても正しい。
