...
## **Description**
(1) $\mathbb{{R}}$ 上の関数 $f(x) = \cos x$ の $k$ 階導関数を $f^{(k)}(x)$ で表す。ただし, $\mathbb{R}$ は実数全体の集合である。以下の各問いに答えよ。
...
とするとき, 全ての $k \ge 0$ に関する $a_k$ を求めよ。
...
(3) 閉曲線 $C$ に沿った複素積分 $\oint_{C} \frac{1}{z(z^2 - 1)}\,dz$ を求めよ。ただし, $C$ は円 $|z| = r,r > 0$ かつ $r \neq 1$ とする。

### 题目描述

1. 对实函数 $f(x)=\cos x$，以 $f^{(k)}(x)$ 表示 $k$ 阶导数。
   - 求所有 $k\ge1$ 的 $f^{(k)}(0)$；
