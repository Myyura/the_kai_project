...
1. 求一个满足 $Ax=-2x$ 的非零向量 $x=(x_1,x_2,x_3)^\top$。
2. 求一个数 $d\ne-2$ 以及一个满足 $Ay=dy$ 的非零向量 $y=(y_1,y_2,y_3)^\top$。
3. 求一组满足 $AP=PD$ 的可逆矩阵 $P$ 与对角矩阵
   $$
   D=\begin{pmatrix}
   d_1&0&0\\0&d_2&0\\0&0&x_3
   \end{pmatrix}.
   $$
4. 求 $P^{-1}$。
5. 求 $A^{10}$。
...
- **特征值与特征向量**：根据给定特征值和特征方程求非零特征向量，并寻找其余特征对。
- **矩阵对角化**：用一组线性无关的特征向量组成 $P$，构造满足 $AP=PD$ 的相似对角化。
- **逆矩阵与矩阵幂**：计算对角化矩阵的逆，并利用 $A=PDP^{-1}$ 高效求 $A^{10}$。
