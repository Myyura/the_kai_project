...
在直角坐标系中，$\mathbf i,\mathbf j,\mathbf k$ 分别为 $x,y,z$ 轴方向的单位向量。给定向量场
...
1. 求 $S$ 上每一点 $(x,y,z)$ 处的外向单位法向量 $\mathbf n$。
2. 证明在 $S$ 上
   $$
   \mathbf F\cdot\mathbf n
   =\sqrt{4x^2+9y^2+36z^2}.
...
- **曲面外法向量**：由椭球隐式方程的梯度求法向方向并单位化。
- **通量曲面积分**：把标量曲面积分识别为向量场 $\mathbf F$ 的外向通量。
- **高斯散度定理**：计算 $\nabla\cdot\mathbf F$ 及椭球体积，从体积分得到封闭曲面的通量。
