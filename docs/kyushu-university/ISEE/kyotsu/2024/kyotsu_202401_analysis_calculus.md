...
1. 在约束曲线
   ...
   f(x,y)=x^2+2xy+2y^2
   $$
   的最大值和最小值，并求出所有达到这些值的点。
2. 在 $x\ge0$ 上求初值问题
   $$
   \frac{dy}{dx}+y=\sin x,\qquad y(0)=0
   $$
   的解。
3. 设 $z$ 为复数，求幂级数
   $$
...
- **约束极值**：用拉格朗日乘子或广义 Rayleigh 商求二次型在二次约束上的最值及取值点。
- **一阶线性微分方程初值问题**：用积分因子求通解，再代入 $y(0)=0$ 确定常数。
- **幂级数收敛半径**：对系数使用根值或比值判别，注意展开中心为 $z=-1$。
