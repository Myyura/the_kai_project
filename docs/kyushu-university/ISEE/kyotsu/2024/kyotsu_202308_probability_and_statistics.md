...
$$
P(A) = \frac{\binom{N_1}{2}}{\binom{N_1+N_2}{2}} = \frac{N_1(N_1 - 1)}{(N_1 + N_2)(N_1 + N_2 - 1)}
$$
...
Only the plus sign gives a positive $N_1$. For $k=0$ it gives $N_1=3$, while for $k\geq1$ it is greater than $(4k+3)/2\geq7/2$. Hence the minimum is $3$.
...
For an admissible solution $k\geq1$, and only the plus sign gives a positive $N_1$. For $k=1,2$, the discriminants $33,129$ are not squares; for $k=3$, it is $17^2$ and $N_1=15$. If $k\geq4$, then $N_1>4k+1/2>15$. Hence the minimum is $15$.

### (3)
By (i) and (ii), an integer solution requires $8N_2^2+1$ to be a perfect square.

Let $8N_2^2 + 1 = K^2$, we have $8N_2^2 = (K - 1)(K + 1)$, which implies that $K$ is odd.

Let $K = 2p + 1$. Then we have $2N_2^2 = p(p+1)$.
The positive solutions of $K^2-8N_2^2=1$ are generated by

$$
K+\sqrt{8}N_2=(3+\sqrt{8})^m \qquad (m\geq 1).
$$

The first three are $(K,N_2)=(3,1),(17,6),(99,35)$. Since

$$
N_1=\frac{2N_2+1+K}{2},
$$

the corresponding values of $N=N_1+N_2$ are $4,21,120$. Both coordinates increase with $m$, so these are the three smallest values.
